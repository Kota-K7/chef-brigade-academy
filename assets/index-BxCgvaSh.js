(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const m of o.addedNodes)m.tagName==="LINK"&&m.rel==="modulepreload"&&a(m)}).observe(document,{childList:!0,subtree:!0});function t(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(n){if(n.ep)return;n.ep=!0;const o=t(n);fetch(n.href,o)}})();let Xe=null,et="",tt=0,De=null,ye=null;function te(e,r=1){ct(e,r)}function ct(e,r=1,t=null,a=null,n=0){if(!("speechSynthesis"in window)){console.warn("Speech synthesis not supported on this browser.");return}window.speechSynthesis.cancel();const o=e.replace(/["'➔]/g,"").trim();et=o,tt=n,De=t,ye=a;const m=o.substring(n);if(!m.trim()){ye&&ye();return}const i=new SpeechSynthesisUtterance(m);i.lang="fr-FR",i.rate=r;const d=window.speechSynthesis.getVoices().find(p=>p.lang==="fr-FR"||p.lang.startsWith("fr"));d&&(i.voice=d),i.onboundary=p=>{if(p.name==="word"&&De){const y=tt+p.charIndex;De(y,et.length)}},i.onend=()=>{Xe===i&&ye&&ye()},i.onerror=p=>{console.warn("SpeechSynthesis error:",p),ye&&ye()},Xe=i,window.speechSynthesis.speak(i)}function qt(){"speechSynthesis"in window&&window.speechSynthesis.pause()}function At(){"speechSynthesis"in window&&window.speechSynthesis.resume()}function Ne(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}"speechSynthesis"in window&&(window.speechSynthesis.getVoices(),window.speechSynthesis.onvoiceschanged=()=>{window.speechSynthesis.getVoices()});function Pt(){var L,P,c,s,b,f,x;const e=document.createElement("div"),r=document.createElement("h2");r.className="section-title",r.innerText="Tableau de Bord",e.appendChild(r);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Welcome back, Commis. Here is your kitchen curriculum status.",e.appendChild(t);const a=new Date().toISOString().split("T")[0],n=Object.values(k.srs),o=n.filter(C=>C.dueDate<=a).length,m=n.length,i=document.createElement("div");i.className="dashboard-grid";const u=document.createElement("div");u.style.display="flex",u.style.flexDirection="column",u.style.gap="1.5rem";const d=document.createElement("div");d.className="card",d.style.padding="1.5rem",d.innerHTML=`
  ...
  `,u.appendChild(d);const p=document.createElement("div");p.className="card",p.style.padding="1.5rem";const y=((L=k.meta)==null?void 0:L.featured)||null;p.innerHTML=`
    ...
    `:"<p>No data loaded.</p>"}
  `,y&&p.querySelectorAll(".audio-btn").forEach(C=>{C.addEventListener("click",w=>{w.stopPropagation();const j=w.target.closest(".audio-btn").getAttribute("data-text");te(j)})}),u.appendChild(p),i.appendChild(u);const l=document.createElement("div");l.style.display="flex",l.style.flexDirection="column",l.style.gap="1.5rem";const h=document.createElement("div");h.className="streak-card",h.innerHTML=`
  ...
  `,l.appendChild(h);const _=document.createElement("div");_.className="card",_.style.padding="1.5rem";const g=((c=(P=k.meta)==null?void 0:P.counts)==null?void 0:c.vocabulary)||0,v=((b=(s=k.meta)==null?void 0:s.counts)==null?void 0:b.grammar)||0,A=((x=(f=k.meta)==null?void 0:f.counts)==null?void 0:x.cuisine)||0;return _.innerHTML=`
  ...
  `,l.appendChild(_),i.appendChild(l),e.appendChild(i),e}function Ie(e){const r=Mt(e);return M[r]||""}function Mt(e){return e?e.trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]/g," ").replace(/\s+/g," ").trim():""}const U=(e,r,t)=>`
...
`;function _e(e,r){const t=["L","M","M","J","V","S","D"];let a="";for(let n=0;n<7;n++){const o=24+n*28,m=n===e,i=m?"#e76f51":"#ffffff",u=m?"#ffffff":"#1c2833";a+=`
    ...
    ${a}
  `,r)}function ze(e,r){const t=["Avant-hier","Hier","Aujourd'hui","Demain","Après-demain"],a=["一昨日","昨日","今日","明日","明後日"];let n="";for(let m=0;m<5;m++){const i=25+m*47,u=m===e,d=u?"#e76f51":"#1c2833",p=u?7:4,y=u?8:7,l=u?"#e76f51":"#7f8c8d",h=u?"bold":"normal";n+=`
      <circle cx="${i}" cy="65" r="${p}" fill="${d}" />
      <text x="${i}" y="51" text-anchor="middle" font-size="${y}" font-family="-apple-system, BlinkMacSystemFont, sans-serif" font-weight="${h}" fill="${l}">${t[m]}</text>
      <text x="${i}" y="81" text-anchor="middle" font-size="7" font-family="-apple-system, BlinkMacSystemFont, sans-serif" font-weight="${h}" fill="${l}">${a[m]}</text>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
      ...
      <div>
        ...
        <select id="vocab-tag-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${m.map(v=>`<option value="${v}" ${v===d?"selected":""}>${v==="ALL"?"ALL TAGS":"#"+v}</option>`).join("")}
        </select>
      </div>
      <div>
        ...
        <select id="vocab-level-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${i.map(v=>`<option value="${v}" ${v===p?"selected":""}>${v==="ALL"?"ALL LEVELS":v}</option>`).join("")}
        </select>
      </div>
    </div>
  `,e.appendChild(y);const l=document.createElement("div");l.className="card-grid",e.appendChild(l);function h(){var P;l.innerHTML="";const L=(((P=k.db)==null?void 0:P.knowledge)||[]).filter(c=>c.french&&c.japanese).filter(c=>t||c.is_professional).filter(c=>{const s=u==="ALL"||c.category===u,b=d==="ALL"||c.tags&&c.tags.includes(d),f=p==="ALL"||c.level===p;return s&&b&&f});if(L.length===0){l.innerHTML='<p style="color: var(--color-text-muted); grid-column: 1 / -1; text-align: center; padding: 2rem;">Aucun terme trouvé correspondant à vos critères de filtrage.</p>';return}L.forEach(c=>{const s=document.createElement("div");s.className="card";const b=fe(c.id),f=k.srs[c.id],x=!!f,C=c.examples&&c.examples[0]?c.examples[0].fr:"",w=c.examples&&c.examples[0]?c.examples[0].ja:"",j=Ie(c.french),T=j?`<div class="preposition-illustration-container" style="height: 130px; background-color: #fcfbfa; border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; margin: 0.8rem 0; overflow: hidden; border: 1px solid rgba(0,0,0,0.03);">${j}</div>`:"";let S="";if(c.gender){const E=c.gender==="m"?"Masculin":c.gender==="f"?"Féminin":c.gender==="m/f"?"M/F":c.gender;S=`<span style="font-size: 0.72rem; font-weight: 600; padding: 0.1rem 0.45rem; border-radius: 4px; display: inline-block; ${c.gender==="m"?"background-color: rgba(0, 0, 91, 0.05); color: var(--color-primary); border: 1px solid rgba(0, 0, 91, 0.12);":c.gender==="f"?"background-color: rgba(220, 38, 38, 0.05); color: var(--color-secondary); border: 1px solid rgba(220, 38, 38, 0.12);":"background-color: rgba(90, 106, 128, 0.05); color: var(--color-text-muted); border: 1px solid rgba(90, 106, 128, 0.12);"}">${E}</span>`}let $="";if(c.verb_group){let E="";c.verb_group===1||c.verb_group==="1"?E="1er groupe (-er)":c.verb_group===2||c.verb_group==="2"?E="2e groupe (-ir)":c.verb_group===3||c.verb_group==="3"?E="3e groupe (Irrégulier)":E=`${c.verb_group}e groupe`,$=`<span style="font-size: 0.72rem; font-weight: 600; padding: 0.1rem 0.45rem; border-radius: 4px; display: inline-block; background-color: rgba(107, 156, 104, 0.07); color: var(--color-success); border: 1px solid rgba(107, 156, 104, 0.15);">${E}</span>`}s.innerHTML=`
        <div style="display: flex; flex-direction: column; height: 100%; justify-content: space-between; gap: 1.5rem;">
          <div>
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
              <span class="card-category" style="margin:0;">${c.category}</span>
              <span class="grammar-badge" style="background-color: var(--color-primary);">${c.level}</span>
            </div>
            ...
            <div class="term-header">
              <div style="display: flex; flex-direction: column; gap: 0.25rem; align-items: flex-start;">
                <div style="display: flex; align-items: center; gap: 0.5rem;">
                  ...
                  <button class="audio-btn" data-text="${c.french}" title="Listen pronunciation" style="background: none; border: none; font-size: 1.15rem; cursor: pointer; color: var(--color-accent); transition: var(--transition); padding: 0.2rem;">🔊</button>
                </div>
                ${S||$?`
              ...
              <button class="fav-btn ${b?"active":""}" data-id="${c.id}">
                ${b?"★":"☆"}
              </button>
            </div>
            ...
            <div class="term-translations" style="margin-top: 0.6rem; display: flex; flex-direction: column; gap: 0.4rem;">
              <div class="flip-translation-container">
                <div class="flip-translation-card">
                  <div class="flip-front" style="display: flex; align-items: center; gap: 0.4rem; justify-content: flex-start; padding: 0.4rem 0.6rem; text-align: left;">
                    <button class="audio-btn" data-text="${c.definition_fr}" title="Listen definition" style="background: none; border: none; font-size: 0.9rem; cursor: pointer; color: var(--color-accent); padding: 0.1rem; line-height: 1; flex-shrink: 0;">🔊</button>
                    <span style="font-size: 0.76rem; line-height: 1.25; color: var(--color-text-main); font-weight: 500;">${c.definition_fr||"No definition loaded."}</span>
                  </div>
              ...
              <div class="trans-ja" style="margin-top: 0.2rem; font-weight: 500;">${c.japanese}</div>
            </div>
            ...
            <div class="term-context" style="margin-top: 1rem;">
              <div class="context-fr" style="display: flex; align-items: flex-start; gap: 0.4rem; font-style: italic; color: var(--color-secondary);">
                ...
                <button class="audio-btn" data-text="${C}" title="Listen context sentence" style="background: none; border: none; font-size: 0.9rem; cursor: pointer; color: var(--color-text-muted); padding: 0.15rem; margin-top: 0.15rem;">🔊</button>
              </div>
              <div class="context-ja">${w}</div>
            </div>
            
            ${c.tags&&c.tags.length>0?`
              <div style="margin-top: 1rem; display: flex; flex-wrap: wrap; gap: 0.3rem;">
                ${c.tags.map(E=>`<span class="tag-badge" style="background-color: rgba(197, 168, 128, 0.08); color: var(--color-accent); font-size: 0.75rem; padding: 0.1rem 0.5rem; border-radius: 12px;">#${E}</span>`).join("")}
              </div>
            `:""}
          </div>
          ...
          <div style="border-top: 1px solid rgba(0,0,0,0.05); padding-top: 1rem; font-size: 0.8rem; display: flex; flex-direction: column; gap: 0.5rem;">
            ${x?`
              ...
              <div style="display: flex; gap: 0.5rem;">
                <button class="next-btn srs-action-btn" data-id="${c.id}" data-action="memorized" style="background-color: var(--color-success); font-size: 0.75rem; padding: 0.3rem 0.6rem; flex: 1;">
                  ✓ Memorized
                </button>
                <button class="next-btn srs-action-btn" data-id="${c.id}" data-action="reset" style="background-color: transparent; border: 1px solid var(--color-error); color: var(--color-error); font-size: 0.75rem; padding: 0.3rem 0.6rem;">
                  Forget
                </button>
              </div>
            `:`
              <div style="color: var(--color-text-muted); font-style: italic; margin-bottom: 0.2rem;">Not yet added to SRS memory deck.</div>
              <button class="next-btn srs-action-btn" data-id="${c.id}" data-action="start" style="font-size: 0.75rem; padding: 0.4rem 0.8rem; width: 100%;">
                Start Memorizing (Add to SRS)
              </button>
            `}
          </div>
        </div>
      `;const q=s.querySelector(".flip-translation-container");q.addEventListener("click",E=>{E.stopPropagation(),q.querySelector(".flip-translation-card").classList.toggle("flipped")}),s.querySelector(".fav-btn").addEventListener("click",E=>{E.stopPropagation(),be(c.id);const z=E.target,F=fe(c.id);z.classList.toggle("active",F),z.innerText=F?"★":"☆"}),s.querySelectorAll(".audio-btn").forEach(E=>{E.addEventListener("click",z=>{z.stopPropagation();const F=z.target.closest(".audio-btn").getAttribute("data-text");te(F)})}),s.querySelectorAll(".srs-action-btn").forEach(E=>{E.addEventListener("click",z=>{z.stopPropagation();const F=z.target.getAttribute("data-action"),H=z.target.getAttribute("data-id");F==="start"?He(H,4):F==="memorized"?He(H,5):F==="reset"&&He(H,0),h()})}),l.appendChild(s)})}y.querySelector("#vocab-category-select").addEventListener("change",v=>{u=v.target.value,h()}),y.querySelector("#vocab-tag-select").addEventListener("change",v=>{d=v.target.value,h()}),y.querySelector("#vocab-level-select").addEventListener("change",async v=>{var C;p=v.target.value,l.innerHTML=`<div style="grid-column: 1 / -1; text-align: center; padding: 2rem; color: var(--color-text-muted);">Chargement (Loading level ${p})...</div>`,await oe("knowledge",p);const L=(((C=k.db)==null?void 0:C.knowledge)||[]).filter(w=>w.french&&w.japanese).filter(w=>t||w.is_professional),P=["ALL",...new Set(L.map(w=>w.category))],c=["ALL",...new Set(L.flatMap(w=>w.tags||[]))],s=y.querySelector("#vocab-category-select"),b=s.value;s.innerHTML=P.map(w=>`<option value="${w}" ${w===b?"selected":""}>${w}</option>`).join(""),u=s.value;const f=y.querySelector("#vocab-tag-select"),x=f.value;f.innerHTML=c.map(w=>`<option value="${w}" ${w===x?"selected":""}>${w==="ALL"?"ALL TAGS":"#"+w}</option>`).join(""),d=f.value,h()}),h()}function Ht(){var o;const e=document.createElement("div"),r=document.createElement("h2");r.className="section-title",r.innerText="Grammaire de la Cuisine",e.appendChild(r);const t=document.createElement("p");t.className="section-subtitle",t.innerText="French grammar concepts framed through recipe instructions and professional dialogues.",e.appendChild(t);const a=document.createElement("div");a.className="loading-placeholder",a.innerText="Chargement de la grammaire... (Loading grammar...)",e.appendChild(a);const n=((o=k.settings)==null?void 0:o.targetLevel)||"ALL";return oe("knowledge",n).then(()=>{a.remove(),Rt(e,n)}),e}function Rt(e,r){var n;const a=(((n=k.db)==null?void 0:n.knowledge)||[]).filter(o=>o.grammar).map(o=>({id:o.id,category:o.category,french:o.french,level:o.level,topic:o.grammar.topic,explanation_en:o.grammar.explanation_en,explanation_ja:o.grammar.explanation_ja,examples:o.examples||[]})).filter(o=>r==="ALL"||o.level===r);if(a.length===0){const o=document.createElement("p");o.style.color="var(--color-text-muted)",o.innerText="Aucune leçon de grammaire chargée pour ce niveau.",e.appendChild(o);return}a.forEach(o=>{const m=document.createElement("div");m.className="grammar-card";const i=fe(o.id),u=o.topic.match(/\(([^)]+)\)/),d=u?u[1]:o.topic,p=Ie(o.french||""),y=p?`<div class="preposition-illustration-container" style="height: 130px; background-color: #fcfbfa; border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; margin: 0.8rem 0; overflow: hidden; border: 1px solid rgba(0,0,0,0.03);">${p}</div>`:"";m.innerHTML=`
    ...
    `;const l=m.querySelector(".grammar-header"),h=m.querySelector(".grammar-body"),_=m.querySelector(".toggle-icon");l.addEventListener("click",g=>{if(g.target.classList.contains("fav-btn")||g.target.closest(".audio-btn"))return;const v=h.classList.toggle("open");_.innerText=v?"▲":"▼"}),m.querySelector(".fav-btn").addEventListener("click",g=>{g.stopPropagation(),be(o.id);const v=g.target,A=fe(o.id);v.classList.toggle("active",A),v.innerText=A?"★":"☆"}),m.querySelectorAll(".audio-btn").forEach(g=>{g.addEventListener("click",v=>{v.stopPropagation();const A=v.target.closest(".audio-btn").getAttribute("data-text");te(A)})}),e.appendChild(m)})}function It(){const e=document.createElement("div");e.className="reference-container-view";const r=document.createElement("h2");r.className="section-title",r.innerText="Référence Grammaticale (Grammar Reference)",e.appendChild(r);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Explore structured conjugations, classifications, and auxiliary usage patterns in culinary French.",e.appendChild(t);const a=document.createElement("div");return a.className="loading-placeholder",a.innerText="Chargement des références... (Loading references...)",e.appendChild(a),fetch("data/grammar_reference.json").then(n=>{if(!n.ok)throw new Error("Could not load reference data");return n.json()}).then(n=>{a.remove(),Dt(e,n)}).catch(n=>{a.innerText="Erreur lors du chargement des données. (Error loading reference data.)",console.error(n)}),e}function Dt(e,r){const t=document.createElement("div");t.className="ref-layout";const a=document.createElement("div");a.className="ref-sidebar";const n=document.createElement("div");n.className="ref-content-panel",r.forEach((o,m)=>{const i=document.createElement("button");i.className=`ref-menu-btn ${m===0?"active":""}`,i.innerHTML=`
  ...
  `,e.appendChild(a),r.sections.forEach(n=>{const o=document.createElement("div");if(o.className="ref-section-container",n.type==="table"){const m=document.createElement("h4");m.className="ref-sec-title",m.innerText=n.title,o.appendChild(m);const i=document.createElement("div");i.className="ref-table-wrapper";const u=document.createElement("table");u.className="ref-table";const d=[],p=document.createElement("tr");if(n.headers.forEach((y,l)=>{const h=document.createElement("th");y==="発音"?(d.push(l),h.innerText="音声",h.style.textAlign="center"):h.innerText=y,p.appendChild(h)}),u.appendChild(p),n.rows.forEach(y=>{const l=document.createElement("tr");y.forEach((h,_)=>{const g=document.createElement("td");if(d.includes(_)){const v=y[_-1]?y[_-1].split("(")[0].trim():"";g.innerHTML=`<button class="ref-table-audio-btn" data-speak="${v}" title="Listen pronunciation">🔊</button>`,g.style.textAlign="center"}else g.innerText=h,_===1&&(n.title.includes("Conjugaison")||n.title.includes("Présent")||n.title.includes("Exemple"))&&(g.style.position="relative",g.innerHTML=`
              ...
              `);l.appendChild(g)}),u.appendChild(l)}),i.appendChild(u),o.appendChild(i),n.example){const y=document.createElement("div");y.className="ref-table-attached-ex",y.innerHTML=`
        ...
        `,o.appendChild(y)}}else if(n.type==="examples"){const m=document.createElement("h4");m.className="ref-sec-title",m.innerText=n.title,o.appendChild(m);const i=document.createElement("div");i.className="ref-examples-list",n.examples.forEach(u=>{const d=document.createElement("div");d.className="ref-example-item",d.innerHTML=`
          <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem;">
            <span class="ref-ex-fr">➔ ${u.fr}</span>
            <button class="audio-btn" data-speak="${u.fr}">🔊</button>
          </div>
          <div class="ref-ex-ja">${u.ja}</div>
        `,i.appendChild(d)}),o.appendChild(i)}else if(n.type==="illustrations_grid"){const m=document.createElement("h4");m.className="ref-sec-title",m.innerText=n.title,o.appendChild(m);const i=document.createElement("div");i.className="ref-illustrations-grid",i.style.display="grid",i.style.gridTemplateColumns="repeat(auto-fill, minmax(180px, 1fr))",i.style.gap="1rem",i.style.marginTop="1rem",n.items.forEach(u=>{const d=document.createElement("div");d.className="ref-illustration-card",d.style.border="1px solid rgba(0,0,0,0.06)",d.style.borderRadius="8px",d.style.padding="0.8rem 0.5rem",d.style.backgroundColor="#fcfbfa",d.style.display="flex",d.style.flexDirection="column",d.style.alignItems="center",d.style.transition="transform 0.2s ease, box-shadow 0.2s ease",d.addEventListener("mouseenter",()=>{d.style.transform="translateY(-2px)",d.style.boxShadow="0 4px 12px rgba(0,0,0,0.05)"}),d.addEventListener("mouseleave",()=>{d.style.transform="none",d.style.boxShadow="none"});const p=Ie(u.word);d.innerHTML=`
        ...
        `,i.appendChild(d)}),o.appendChild(i)}else if(n.type==="info"){const m=document.createElement("div");m.className="ref-info-box",m.innerHTML=`
        <h4 class="ref-info-title">💡 ${n.title}</h4>
        <p class="ref-info-fr">${n.content_fr}</p>
        <p class="ref-info-ja">${n.content_ja}</p>
      `,o.appendChild(m)}e.appendChild(o)}),e.querySelectorAll(".audio-btn, .ref-table-audio-btn").forEach(n=>{n.addEventListener("click",o=>{o.stopPropagation();const m=n.getAttribute("data-speak");te(m),n.style.transform="scale(1.2)",setTimeout(()=>n.style.transform="none",150)})})}function Nt(e){var t;e.innerHTML='<div style="text-align: center; padding: 2rem; color: var(--color-text-muted);">Chargement de la théorie... (Loading theory...)</div>';const r=((t=k.settings)==null?void 0:t.targetLevel)||"ALL";oe("knowledge",r).then(()=>{var m;e.innerHTML="";const n=(((m=k.db)==null?void 0:m.knowledge)||[]).filter(i=>i.cuisine).map(i=>({id:i.id,level:i.level,category:i.category||"Theory",topic:i.cuisine.topic,content_fr:i.cuisine.content_fr,content_en:i.cuisine.content_en,content_ja:i.cuisine.content_ja})).filter(i=>r==="ALL"||i.level===r);if(n.length===0){e.innerHTML='<p style="color: var(--color-text-muted);">Aucun document de théorie culinaire disponible pour ce niveau.</p>';return}const o=document.createElement("div");o.style.display="flex",o.style.flexDirection="column",o.style.gap="2rem",n.forEach(i=>{const u=document.createElement("div");u.className="card",u.style.display="block",u.style.padding="2rem";const d=fe(i.id),p=i.topic.match(/^([^(]+)/),y=p?p[1].trim():i.topic;u.innerHTML=`
        ...
        <div class="term-header" style="border-bottom: 1px solid rgba(197, 168, 128, 0.15); padding-bottom: 0.8rem; margin-bottom: 1.5rem;">
          ...
          <button class="fav-btn ${d?"active":""}" data-id="${i.id}" style="font-size: 1.3rem;">
            ${d?"★":"☆"}
          </button>
        </div>
  ...
  `,e.appendChild(r);let t=document.getElementById("map-floating-tooltip");t||(t=document.createElement("div"),t.id="map-floating-tooltip",t.style.position="fixed",t.style.pointerEvents="none",t.style.zIndex="9999",t.style.padding="0.5rem 0.8rem",t.style.background="rgba(10, 25, 49, 0.95)",t.style.border="1px solid var(--color-accent)",t.style.borderRadius="var(--radius-sm)",t.style.color="#fff",t.style.fontSize="0.75rem",t.style.boxShadow="0 4px 12px rgba(0, 0, 0, 0.15)",t.style.display="none",t.style.fontFamily="var(--font-main)",t.style.lineHeight="1.3",document.body.appendChild(t));const a=r.querySelectorAll(".segment-btn");a.forEach(d=>{d.addEventListener("click",p=>{a.forEach(y=>y.classList.remove("active")),p.target.classList.add("active"),I.activeMap=p.target.getAttribute("data-map"),I.selectedRegionId=null,I.selectedItemId=null,m()})});const n=r.querySelectorAll(".theme-tab-btn");n.forEach(d=>{d.addEventListener("click",p=>{n.forEach(y=>y.classList.remove("active")),p.target.classList.add("active"),I.activeTheme=p.target.getAttribute("data-theme"),I.selectedItemId=null,m()})}),o().then(()=>{m()});async function o(){try{const d=["data/regions_administrative.json","data/regions_cultural.json","data/dishes.json","data/cheeses.json","data/wines.json"],p=await Promise.all(d.map(y=>fetch(y)));for(const y of p)if(!y.ok)throw new Error(`Failed to load ${y.url}`);ve=await p[0].json(),xe=await p[1].json(),Ae=await p[2].json(),Pe=await p[3].json(),Me=await p[4].json(),console.log("Map engine data loaded successfully.")}catch(d){console.error("Failed to load map JSON datasets:",d),ve=[],xe=[]}}function m(){if(!(ve.length>0&&xe.length>0)){r.querySelector("#regions-list-group").innerHTML='<p style="font-size:0.8rem;color:var(--color-text-muted);padding:1rem;">読み込み中...</p>';return}const p=r.querySelector("#legacy-map-illustration"),y=r.querySelector("#classic-map-decorations"),l=r.querySelector("#interactive-france-svg");I.activeMap==="cultural"?(p.style.display="block",y.style.display="none",l.setAttribute("viewBox","0 0 600 600")):(p.style.display="none",y.style.display="block",l.setAttribute("viewBox","0 0 1000 1000"));const h=I.activeMap==="admin"?ve:xe,_=r.querySelector("#map-paths-group"),g=I.activeMap==="cultural";_.innerHTML=h.map(c=>{const s=I.selectedRegionId===c.region_id,b=Zt[c.region_id]||"rgba(197, 168, 128, 0.1)",f=g?"none":b,x=g?"none":"rgba(92, 64, 51, 0.65)",C=g?0:1,w=s?"rgba(197, 168, 128, 0.45)":f,j=s?"var(--color-primary)":x,T=s?3.2:g?0:2.2;return`
        <path 
          class="region-path ${s?"active":""}" 
          d="${c.geometry}" 
          id="${c.region_id}" 
          data-id="${c.region_id}"
          style="fill: ${w}; fill-opacity: ${s?.65:C}; stroke: ${j}; stroke-width: ${T}; cursor: pointer; transition: fill 0.25s, fill-opacity 0.25s, stroke 0.25s;"
        />
      `}).join("");const v=r.querySelector("#map-labels-group");g?v.innerHTML="":v.innerHTML=h.map(c=>{const s=c.french_name,b=s.length>20?11.5:s.length>12?12.5:14.5,f=b*.65,x=Math.max(90,s.length*f+20),C=b+8,w=c.label_y-C/2-1,j=c.label_y+b/3+.5;return`
        ...
        `}).join("");const A=r.querySelector("#map-pins-group");if(A.innerHTML="",I.activeTheme!=="overview"){let c=[],s="var(--color-accent)";I.activeTheme==="cheeses"?(c=Pe,s="#EAA812"):I.activeTheme==="wines"?(c=Me,s="#A91B3E"):I.activeTheme==="dishes"&&(c=Ae,s="#2F80ED");const b=h.map(f=>f.region_id);A.innerHTML=c.map(f=>{const x=Oe(f.region,I.activeMap);if(!x||!b.includes(x))return"";const C=h.find($=>$.region_id===x);if(!C)return"";const w=I.activeMap==="cultural"?6:10;let j=f.pin.x*w,T=f.pin.y*w;if(I.activeMap==="admin"){j=j*.15+C.label_x*.85,T=T*.15+C.label_y*.85;const $=f.id.length*5%12-6;j+=$,T+=f.id.length*11%12-6}else{const $=f.id.length*3%6-3;j+=$,T+=f.id.length*5%6-3}const S=I.selectedItemId===f.id;return`
          <g class="map-pin-marker" data-item-id="${f.id}" data-region-id="${x}" style="cursor:pointer; z-index: 10;">
            <circle cx="${j}" cy="${T}" r="${S?"9":"6"}" fill="${s}" stroke="#fff" stroke-width="1.5" />
            ${S?`<circle cx="${j}" cy="${T}" r="14" fill="none" stroke="${s}" stroke-width="1.5" stroke-dasharray="2,2" opacity="0.8" />`:""}
          </g>
        `}).join("")}const L=r.querySelector("#sidebar-list-title");if(I.activeTheme==="overview")L.innerText="🗺️ 地方リスト (Régions)";else{const c={dishes:"🍽️ 郷土料理",cheeses:"🧀 特産チーズ",wines:"🍷 地方ワイン"};L.innerText=`${c[I.activeTheme]} リスト`}const P=r.querySelector("#regions-list-group");if(P.innerHTML="",I.activeTheme==="overview")P.innerHTML=h.map(c=>{const s=I.selectedRegionId===c.region_id,b=c.code||"";return`
          <button class="list-item-btn ${s?"active":""}" data-id="${c.region_id}">
            <span>
            ...
            <span style="font-size: 0.75rem; color: var(--color-text-muted); font-style: italic;">${c.japanese_name}</span>
          </button>
        `}).join("");else{let c=[];I.activeTheme==="cheeses"?c=Pe:I.activeTheme==="wines"?c=Me:I.activeTheme==="dishes"&&(c=Ae);const s=h.map(f=>f.region_id),b=c.filter(f=>{const x=Oe(f.region,I.activeMap);return x&&s.includes(x)});P.innerHTML=b.map(f=>{const x=I.selectedItemId===f.id,C=Oe(f.region,I.activeMap),w=h.find(T=>T.region_id===C),j=w?w.japanese_name.replace("地域圏","").replace("地方",""):"";return`
          <button class="list-item-btn item-btn ${x?"active":""}" data-item-id="${f.id}" data-region-id="${C}">
            <span>
        ...
        `}).join("")}u(),i()}function i(){r.querySelectorAll(".region-path").forEach(l=>{l.addEventListener("mouseenter",h=>{const _=h.target.getAttribute("data-id"),v=(I.activeMap==="admin"?ve:xe).find(A=>A.region_id===_);v&&(t.innerHTML=`
          ...
          `,t.style.display="block")}),l.addEventListener("mousemove",h=>{t.style.left=h.clientX+15+"px",t.style.top=h.clientY+15+"px"}),l.addEventListener("mouseleave",()=>{t.style.display="none"}),l.addEventListener("click",h=>{const _=h.target.getAttribute("data-id");I.selectedRegionId=_,I.selectedItemId=null,m()})}),r.querySelectorAll(".list-item-btn").forEach(l=>{l.addEventListener("click",h=>{const _=h.target.closest(".list-item-btn");_.classList.contains("item-btn")?(I.selectedItemId=_.getAttribute("data-item-id"),I.selectedRegionId=_.getAttribute("data-region-id")):(I.selectedRegionId=_.getAttribute("data-id"),I.selectedItemId=null),m()})}),r.querySelectorAll(".map-pin-marker").forEach(l=>{l.addEventListener("click",h=>{const _=h.target.closest(".map-pin-marker");I.selectedItemId=_.getAttribute("data-item-id"),I.selectedRegionId=_.getAttribute("data-region-id"),m()})})}function u(){const d=r.querySelector("#map-detail-drawer"),p=r.querySelector("#map-region-title"),y=r.querySelector("#map-region-sub"),l=r.querySelector("#drawer-main-content"),h=r.querySelector("#map-audio-title-btn");if(h.style.display="none",I.selectedItemId){let _=null,g="";if(I.activeTheme==="cheeses"?(_=Pe.find(v=>v.id===I.selectedItemId),g="特産チーズ (Fromage AOP)"):I.activeTheme==="wines"?(_=Me.find(v=>v.id===I.selectedItemId),g="地方ワイン (AOC Wine)"):I.activeTheme==="dishes"&&(_=Ae.find(v=>v.id===I.selectedItemId),g="郷土料理 (Plat Régional)"),_){p.innerText=`${_.name_fr} (${_.name_ja})`,y.innerText=`${g} • ${_.region}`;let v="";if(_.properties){v+='<div style="display:flex; flex-wrap:wrap; gap:0.6rem; margin-bottom:0.5rem;">';for(const[L,P]of Object.entries(_.properties)){let c=L;L==="sweetness"?c="甘み":L==="alcohol"?c="アルコール":L==="body"?c="ボディ":L==="saltiness"?c="塩気":L==="aroma"?c="香り":L==="rarity"?c="希少性":L==="type"&&(c="タイプ"),v+=`
              <span style="font-size:0.75rem; background:rgba(10, 25, 49, 0.05); border:1px solid rgba(197, 168, 128, 0.25); padding:0.2rem 0.5rem; border-radius:3px; font-weight:600;">
                ${c}: <span style="color:var(--color-accent-hover);">${P}</span>
              </span>
            `}v+="</div>"}l.innerHTML=`
          ${v}
          ...
          `:""}
        `,h.style.display="inline-block",h.onclick=()=>te(_.name_fr);const A=r.querySelector("#map-audio-desc-btn");A&&(A.onclick=()=>te(_.science||_.desc||"")),d.style.display="block";return}}if(I.selectedRegionId){const g=(I.activeMap==="admin"?ve:xe).find(v=>v.region_id===I.selectedRegionId);if(g){p.innerText=`${g.french_name} (${g.japanese_name})`,y.innerText=`${g.english_name} Region`;let v=en(g.region_id);l.innerHTML=`
          <div>
            <div style="display:flex; align-items:center; gap:0.5rem; margin-bottom:0.3rem;">
              <h4 style="font-size:0.75rem; text-transform:uppercase; color:var(--color-accent); font-weight:600;">Français</h4>
              <button class="audio-btn" id="map-audio-desc-btn" style="background:none; border:none; font-size:0.95rem; cursor:pointer; color:var(--color-accent);">🔊</button>
  ...
  `,document.head.appendChild(e)}function an(){var m;const e=[],r=((m=k.db)==null?void 0:m.knowledge)||[],t=r.filter(i=>i.french&&i.japanese),a=r.filter(i=>i.grammar),n=r.filter(i=>i.cuisine);function o(i,u,d,p){const l=i.filter(_=>_.id!==d).map(p).sort(()=>.5-Math.random()),h=[...new Set(l)].filter(Boolean).slice(0,u);for(;h.length<u;)h.push("Autre option "+(h.length+1));return h}return t.forEach(i=>{var l,h,_,g,v;let u="vocabulary";(l=i.tags)!=null&&l.includes("meat")?u="meat":(h=i.tags)!=null&&h.includes("sauces")||(_=i.tags)!=null&&_.includes("sauce")?u="sauces":(g=i.tags)!=null&&g.includes("cutting")?u="cuts":(v=i.tags)!=null&&v.includes("science")&&(u="science");const d=o(t,3,i.id,A=>A.japanese),p=i.examples&&i.examples[0]?i.examples[0].fr:"";e.push({id:`dyn_vocab_fr_ja_${i.id}`,type:"choice",category:u,question:`Que signifie le mot français "${i.french}" ? / What does the French word "${i.french}" mean?`,question_fr:`Que signifie le mot français "${i.french}" ?`,question_en:`What does the French word "${i.french}" mean?`,context:i.definition_fr||`Usage: ${p}`,options:de([i.japanese,...d]),answer:i.japanese});const y=o(t,3,i.id,A=>A.french);e.push({id:`dyn_vocab_ja_fr_${i.id}`,type:"choice",category:u,question:`Quel est le mot français pour "${i.japanese}" ? / What is the French word for "${i.japanese}"?`,question_fr:`Quel est le mot français pour "${i.japanese}" ?`,question_en:`What is the French word for "${i.japanese}"?`,context:i.definition_fr||`Usage: ${p}`,options:de([i.french,...y]),answer:i.french})}),a.forEach(i=>{if(!i.grammar.topic||!i.examples||i.examples.length===0)return;const u=o(a,3,i.id,d=>d.grammar.topic);e.push({id:`dyn_gram_topic_${i.id}`,type:"choice",category:"grammar",question:`De quel concept de grammaire s'agit-il : "${i.grammar.explanation_ja}" ? / Which grammar concept is this: "${i.grammar.explanation_en}"?`,question_fr:"De quel concept de grammaire s'agit-il ?",question_en:`Which grammar concept is this: "${i.grammar.explanation_en}"?`,context:`Niveau : ${i.level}. Indispensable pour la cuisine.`,options:de([i.grammar.topic,...u]),answer:i.grammar.topic}),i.examples.forEach((d,p)=>{const l=a.flatMap(h=>h.examples||[]).filter(h=>h.fr!==d.fr).sort(()=>.5-Math.random()).map(h=>h.ja).slice(0,3);for(;l.length<3;)l.push("Option de traduction "+(l.length+1));e.push({id:`dyn_gram_ex_${i.id}_${p}`,type:"choice",category:"grammar",question:`Traduisez la phrase : "${d.fr}" / Translate the sentence: "${d.fr}"`,question_fr:`Traduisez la phrase : "${d.fr}"`,question_en:`Translate the sentence: "${d.fr}"`,context:`Grammaire: ${i.grammar.topic} (${i.level})`,options:de([d.ja,...l]),answer:d.ja})})}),n.forEach(i=>{var p,y,l,h,_,g;if(!i.cuisine.topic||!i.cuisine.content_ja)return;const u=o(n,3,i.id,v=>v.cuisine.topic);let d="sauces";(p=i.tags)!=null&&p.includes("knife-cuts")||(y=i.tags)!=null&&y.includes("cuts")?d="cuts":(l=i.tags)!=null&&l.includes("meat")?d="meat":((h=i.tags)!=null&&h.includes("molecular")||(_=i.tags)!=null&&_.includes("chemistry")||(g=i.tags)!=null&&g.includes("science"))&&(d="science"),e.push({id:`dyn_cuis_topic_${i.id}`,type:"choice",category:d,question:`De quel concept culinaire s'agit-il : "${i.cuisine.content_ja.substring(0,120)}..." ? / Which culinary concept is described: "${i.cuisine.content_en.substring(0,120)}..."?`,question_fr:"De quel concept culinaire s'agit-il ?",question_en:"Which culinary concept is described here?",context:`Niveau : ${i.level}. Mots-clés : ${i.tags.join(", ")}`,options:de([i.cuisine.topic,...u]),answer:i.cuisine.topic})}),e}function rn(){var n;const e=document.createElement("div"),r=document.createElement("h2");r.className="section-title",r.innerText="Vérification des Connaissances",e.appendChild(r);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Test your professional French vocabulary, kitchen commands, and classical cuisine theory.",e.appendChild(t);const a=document.createElement("div");return a.className="loading-placeholder",a.innerText="Chargement du quiz... (Loading quiz...)",e.appendChild(a),nn(),(n=k.settings)!=null&&n.targetLevel,Promise.all([Je(),oe("vocabulary","ALL"),oe("grammar","ALL"),oe("cuisine","ALL"),fetch("rpg/questions_db.json").then(o=>o.json()).then(o=>{k.questionsDb=o}).catch(o=>{console.error("Failed to load questions_db in quiz:",o),k.questionsDb=[]})]).then(()=>{a.remove(),on(e)}),e}function on(e){let r="multiple",t="ALL";const a=document.createElement("div");a.className="quiz-mode-selector",a.innerHTML=`
  ...
  `,e.appendChild(n),n.querySelector("#quiz-cat-select").addEventListener("change",l=>{t=l.target.value,i()});const m=document.createElement("div");m.className="quiz-game-wrapper",e.appendChild(m),a.querySelectorAll(".mode-tab-btn").forEach(l=>{l.addEventListener("click",h=>{a.querySelectorAll(".mode-tab-btn").forEach(_=>_.classList.remove("active")),h.target.classList.add("active"),r=h.target.getAttribute("data-mode"),i()})});function i(){m.innerHTML="",r==="multiple"?u():r==="association"?d():r==="spelling"&&y()}function u(){var c;const l=((c=k.db)==null?void 0:c.quizzes)||[],h=an();let _=[...l,...h];if(t!=="ALL"&&(_=_.filter(s=>s.category===t)),_.length===0){m.innerHTML=`
      ...
      `;return}const g=de(_).slice(0,10);let v=0,A=0,L=!1;function P(){if(m.innerHTML="",L=!1,v>=g.length){const j=Math.round(A/g.length*100);let T="Apprenti (Apprentice)";j>=90?T="Chef de Partie (Station Chef)":j>=70&&(T="Commis de Cuisine (Line Cook)"),m.innerHTML=`
          <div class="quiz-card" style="text-align: center;">
            ...
            <p style="font-size: 1.1rem; margin-bottom: 1.5rem;">Your Score: <strong>${A} / ${g.length}</strong> (${j}%)</p>
        ...
        `,m.querySelector("#restart-choice-btn").addEventListener("click",()=>{u()});return}const s=g[v],b=document.createElement("div");b.className="quiz-card",b.innerHTML=`
        <div class="quiz-meta">
          <span>Question ${v+1} of ${g.length}</span>
        ...
        <div class="quiz-options">
          ${s.options.map((j,T)=>`
            <button class="quiz-btn" data-index="${T}">${j}</button>
          `).join("")}
        </div>
      ...
      `;const f=b.querySelector(".quiz-hint-flip-container");f&&f.addEventListener("click",()=>{const j=f.querySelector(".q-en-hint-text"),T=j.style.display==="none";j.style.display=T?"block":"none",f.querySelector(".q-en-card").innerText=T?"💡 Hide English Hint":"💡 Translate (Show English Hint)"});const x=b.querySelectorAll(".quiz-btn"),C=b.querySelector(".quiz-feedback"),w=b.querySelector("#next-q-btn");x.forEach(j=>{j.addEventListener("click",T=>{if(L)return;L=!0;const $=T.target.innerText===s.answer;x.forEach(q=>{q.disabled=!0,q.innerText===s.answer&&q.classList.add("correct")}),$?A++:(T.target.classList.add("incorrect"),st(s.id)),C.style.display="block",w.style.display="block"})}),w.addEventListener("click",()=>{v++,P()}),m.appendChild(b)}P()}function d(){var v,A;let l=[];if(k.questionsDb&&k.questionsDb.length>0){const L=k.questionsDb.filter(P=>P.type==="matching");l=ot(L,t)}if(l.length===0){const L=((v=k.settings)==null?void 0:v.includeGeneral)||!1;let c=(((A=k.db)==null?void 0:A.vocabulary)||[]).filter(s=>L||s.is_professional);if(t!=="ALL"&&(t==="meat"?c=c.filter(s=>{var b;return((b=s.tags)==null?void 0:b.includes("meat"))||/viande|boeuf/i.test(s.french)}):t==="sauces"?c=c.filter(s=>{var b;return((b=s.tags)==null?void 0:b.includes("sauce"))||/sauce|fond/i.test(s.french)}):t==="cuts"?c=c.filter(s=>{var b;return((b=s.tags)==null?void 0:b.includes("cutting"))||/coupe|tailler/i.test(s.french)}):t==="science"?c=c.filter(s=>{var b;return((b=s.tags)==null?void 0:b.includes("science"))||/réaction|émulsion/i.test(s.french)}):t==="map"&&(c=c.filter(s=>{var b;return((b=s.tags)==null?void 0:b.includes("map"))||/région|ville/i.test(s.french)}))),c.length>=4){const s=de(c).slice(0,4);l=[{id:"dyn_match_vocab",tags:["vocabulary"],text:"Associez les paires correctes. (正しいペアを結びつけてください。)",pairs:s.map(b=>({left:b.french,right:b.japanese})),explanation:"単語のペアを正しくマッチさせました。"}]}}if(l.length===0){m.innerHTML=`
        <div class="quiz-card" style="text-align: center; padding: 2rem;">
      ...
      `;return}const h=de(l).slice(0,5);let _=0;function g(){if(m.innerHTML="",_>=h.length){m.innerHTML=`
          <div class="quiz-card" style="text-align: center;">
            <h3 style="font-family: var(--font-serif); font-size: 2rem; color: var(--color-primary); margin-bottom: 1rem;">Session Terminée !</h3>
        ...
        `,m.querySelector("#restart-assoc-btn").addEventListener("click",()=>{d()});return}const L=h[_],P=de(L.pairs.map(f=>f.left)),c=de(L.pairs.map(f=>f.right)),s=L.tags&&L.tags.length>0?L.tags[0]:"association",b=document.createElement("div");b.className="quiz-card",b.innerHTML=`
        <div class="quiz-meta" style="margin-bottom: 1rem;">
          <span>Question ${_+1} of ${h.length}</span>
        ...
        <div class="matching-board">
          <div class="matching-column" id="left-column">
            ${P.map((f,x)=>`
              <div class="drag-card" draggable="true" data-val="${f}" id="drag-${_}-${x}">
                <span>${f}</span>
          ...
          <div class="matching-column" id="right-column">
            ${c.map((f,x)=>`
              <div class="drop-zone" data-val="${f}">
                ${f}
              </div>
            `).join("")}
          </div>
      ...
      `,m.appendChild(b),p(b,L,()=>{const f=b.querySelector(".quiz-feedback"),x=b.querySelector("#matching-completion-panel");f.style.display="block",x.style.display="block"}),b.querySelector("#next-assoc-btn").addEventListener("click",()=>{_++,g()})}g()}function p(l,h,_){let g=null,v=null,A=0;const L=h.pairs.length,P=l.querySelectorAll(".drag-card"),c=l.querySelectorAll(".drop-zone");P.forEach(f=>{f.addEventListener("dragstart",x=>{g=x.target.closest(".drag-card").getAttribute("data-val"),x.target.closest(".drag-card").classList.add("dragging")}),f.addEventListener("dragend",x=>{x.target.closest(".drag-card").classList.remove("dragging")}),f.addEventListener("click",x=>{const C=x.target.closest(".drag-card");C.classList.contains("matched")||(P.forEach(w=>w.style.borderColor="rgba(10,25,49,0.08)"),v=C.getAttribute("data-val"),C.style.borderColor="var(--color-accent)")})}),c.forEach(f=>{f.addEventListener("dragover",x=>{x.preventDefault(),f.classList.contains("matched")||f.classList.add("hovered")}),f.addEventListener("dragleave",()=>{f.classList.remove("hovered")}),f.addEventListener("drop",x=>{x.preventDefault(),f.classList.remove("hovered");const C=f.getAttribute("data-val");h.pairs.find(j=>j.left===g&&j.right===C)?b(g,f):s(g)}),f.addEventListener("click",()=>{if(f.classList.contains("matched")||!v)return;const x=f.getAttribute("data-val");h.pairs.find(w=>w.left===v&&w.right===x)?(b(v,f),v=null):(s(v),v=null,P.forEach(w=>w.style.borderColor="rgba(10,25,49,0.08)"))})});function s(f){const x=Array.from(P).find(C=>C.getAttribute("data-val")===f);x&&(x.style.animation="shake-anim 0.4s ease-in-out",setTimeout(()=>x.style.animation="",400))}function b(f,x){const C=Array.from(P).find(w=>w.getAttribute("data-val")===f);C.classList.add("matched"),C.style.borderColor="var(--color-success)",C.draggable=!1,x.classList.add("matched"),A++,A===L&&_()}}function y(){var L,P;let l=[];if(k.questionsDb&&k.questionsDb.length>0){const c=k.questionsDb.filter(s=>s.type==="typing");l=ot(c,t)}if(l.length===0){const c=((L=k.settings)==null?void 0:L.includeGeneral)||!1;let b=(((P=k.db)==null?void 0:P.vocabulary)||[]).filter(f=>c||f.is_professional);t!=="ALL"&&(t==="meat"?b=b.filter(f=>{var x;return((x=f.tags)==null?void 0:x.includes("meat"))||/viande|boeuf/i.test(f.french)}):t==="sauces"?b=b.filter(f=>{var x;return((x=f.tags)==null?void 0:x.includes("sauce"))||/sauce|fond/i.test(f.french)}):t==="cuts"?b=b.filter(f=>{var x;return((x=f.tags)==null?void 0:x.includes("cutting"))||/coupe|tailler/i.test(f.french)}):t==="science"?b=b.filter(f=>{var x;return((x=f.tags)==null?void 0:x.includes("science"))||/réaction|émulsion/i.test(f.french)}):t==="map"&&(b=b.filter(f=>{var x;return((x=f.tags)==null?void 0:x.includes("map"))||/région|ville/i.test(f.french)}))),l=b.map(f=>({id:f.id,text:`${f.definition_fr}
(Hint Japanese: ${f.japanese})`,acceptedAnswers:[f.french,f.french.toLowerCase()],explanation:`${f.french}: ${f.japanese}`,tags:[f.category]}))}if(l.length===0){m.innerHTML=`
        <div class="quiz-card" style="text-align: center; padding: 2rem;">
      ...
      `;return}const h=de(l).slice(0,10);let _=0,g=0,v=!1;function A(){if(m.innerHTML="",v=!1,_>=h.length){const S=Math.round(g/h.length*100);let $="Apprenti (Apprentice)";S>=90?$="Chef de Partie (Station Chef)":S>=70&&($="Commis de Cuisine (Line Cook)"),m.innerHTML=`
          <div class="quiz-card" style="text-align: center;">
            <h3 style="font-family: var(--font-serif); font-size: 2rem; color: var(--color-primary); margin-bottom: 1rem;">Session Terminée !</h3>
            <p style="font-size: 1.1rem; margin-bottom: 1.5rem;">Your Score: <strong>${g} / ${h.length}</strong> (${S}%)</p>
            <div style="background-color: rgba(197, 168, 128, 0.1); border: 1px solid var(--color-accent); padding: 1.5rem; border-radius: var(--radius-md); margin-bottom: 2rem;">
              <div style="font-size: 0.8rem; text-transform: uppercase; color: var(--color-text-muted); letter-spacing: 1px;">Assigned Rank</div>
        ...
        `,m.querySelector("#restart-spelling-btn").addEventListener("click",()=>{y()});return}const c=h[_],s=document.createElement("div");s.className="quiz-card";const b=c.tags&&c.tags.length>0?c.tags[0]:"orthographe";s.innerHTML=`
        <div class="quiz-meta" style="margin-bottom: 1.2rem;">
          <span>Question ${_+1} of ${h.length}</span>
          <span class="grammar-badge" style="background-color: var(--color-secondary);">${b}</span>
        </div>
        ...
        <div class="spelling-box" style="margin-bottom: 1.5rem; background-color: rgba(10,25,49,0.02); padding: 1.2rem; border-radius: var(--radius-sm); border-left: 3px solid var(--color-accent);">
          <p style="font-size: 1.05rem; color: var(--color-primary); line-height: 1.45; font-family: var(--font-serif); white-space: pre-line;">${c.text}</p>
        </div>
      ...
      `,m.appendChild(s);const f=s.querySelector("#spelling-input-field"),x=s.querySelector("#spelling-submit-btn"),C=s.querySelector("#spelling-next-btn"),w=s.querySelector("#spelling-feedback-panel"),j=s.querySelector("#spelling-feedback-title"),T=s.querySelector("#spelling-feedback-msg");setTimeout(()=>f.focus(),150),f.addEventListener("keydown",S=>{S.key==="Enter"&&x.click()}),x.addEventListener("click",()=>{if(v)return;v=!0;const S=f.value,$=it(S),q=c.acceptedAnswers.some(z=>it(z)===$),E=c.acceptedAnswers[0];f.disabled=!0,x.style.display="none",C.style.display="block",w.style.display="block",q?(g++,f.style.borderColor="var(--color-success)",f.style.backgroundColor="#E8F5E9",w.style.backgroundColor="#E8F5E9",w.style.color="var(--color-success)",j.innerText="✓ Félicitations ! (Correct)",T.innerText=`You correctly spelled: "${E}"`):(f.style.borderColor="var(--color-error)",f.style.backgroundColor="#FFEBEE",w.style.backgroundColor="#FFEBEE",w.style.color="var(--color-error)",j.innerText="✗ Incorrect",T.innerHTML=`Correct spelling is: <strong>${E}</strong>.<br><em style="font-size:0.85rem;">You typed: "${S}"</em>`,st(c.id))}),C.addEventListener("click",()=>{_++,A()})}A()}i()}function $e(){const e=document.createElement("div"),r=document.createElement("h2");r.className="section-title",r.innerText="Coups de Cœur (Favorites)",e.appendChild(r);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Your bookmarked vocabulary terms, grammar guides, and culinary theories.",e.appendChild(t);const a=Array.from(k.favorites);if(a.length===0)return e.innerHTML+=`
      <div class="alert alert-info">
        <p>No favorites saved yet. Browse Vocabulary, Grammar, or Cuisine and click the star (☆) button to save items here.</p>
      </div>
    `,e;const n=document.createElement("div");return n.className="loading-placeholder",n.innerText="Chargement des favoris... (Loading favorites...)",e.appendChild(n),Ye().then(()=>{n.remove(),sn(e,a)}),e}function sn(e,r){var i,u,d;const t=(((i=k.db)==null?void 0:i.vocabulary)||[]).filter(p=>r.includes(p.id)),a=(((u=k.db)==null?void 0:u.grammar)||[]).filter(p=>r.includes(p.id)),n=(((d=k.db)==null?void 0:d.cuisine)||[]).filter(p=>r.includes(p.id)),o=document.createElement("div");o.className="card-grid",e.appendChild(o);function m(){o.innerHTML="",t.forEach(p=>{const y=document.createElement("div");y.className="card",y.innerHTML=`
        <div>
      ...
      `,y.querySelector(".fav-btn").addEventListener("click",l=>{be(p.id),$e(),e.innerHTML="",e.appendChild($e())}),o.appendChild(y)}),a.forEach(p=>{var l,h;const y=document.createElement("div");y.className="card",y.innerHTML=`
        <div>
        ...
        <div class="term-context" style="margin-top: 0.5rem;">
          <div class="context-fr">"${((l=p.examples[0])==null?void 0:l.fr)||""}"</div>
          <div class="context-ja">${((h=p.examples[0])==null?void 0:h.ja)||""}</div>
        </div>
      `,y.querySelector(".fav-btn").addEventListener("click",_=>{be(p.id),e.innerHTML="",e.appendChild($e())}),o.appendChild(y)}),n.forEach(p=>{const y=document.createElement("div");y.className="card",y.innerHTML=`
        <div>
      ...
      `,y.querySelector(".fav-btn").addEventListener("click",l=>{be(p.id),e.innerHTML="",e.appendChild($e())}),o.appendChild(y)})}m()}function ln(){const e=document.createElement("div"),r=document.createElement("h2");r.className="section-title",r.innerText="Pont d'Études (SRS Review Deck)",e.appendChild(r);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Review and grade yourself on items scheduled for active recall today.",e.appendChild(t);const a=document.createElement("div");return a.className="loading-placeholder",a.innerText="Chargement des révisions... (Loading reviews...)",e.appendChild(a),Ye().then(()=>{a.remove(),cn(e)}),e}function cn(e){var u,d,p,y;const r=new Date().toISOString().split("T")[0],a=[...(((u=k.db)==null?void 0:u.vocabulary)||[]).map(l=>({...l,type:"vocabulary",front:l.french})),...(((d=k.db)==null?void 0:d.grammar)||[]).map(l=>({...l,type:"grammar",front:l.topic})),...(((p=k.db)==null?void 0:p.cuisine)||[]).map(l=>({...l,type:"cuisine",front:l.topic})),...(((y=k.db)==null?void 0:y.quizzes)||[]).map(l=>({...l,type:"quiz",front:l.question_fr||l.question||""}))].filter(l=>{const h=k.srs[l.id];return k.wrongAnswers.includes(l.id)||h&&h.dueDate<=r}),n=a.length;let o=0;const m=document.createElement("div");m.className="srs-review-container",e.appendChild(m);function i(){if(m.innerHTML="",a.length===0||o>=a.length){m.innerHTML=`
      ...
      `;return}const l=a[o];k.srs[l.id];const h=k.wrongAnswers.includes(l.id),_=document.createElement("div");_.className="card srs-flip-card",_.style.padding="2rem",_.style.minHeight="300px",_.style.display="flex",_.style.flexDirection="column",_.style.justifyContent="space-between";let g=l.category||l.level||"Theory";l.type==="quiz"&&(g="Quiz Mistake");let v=`
      <div>
    ...
    `;_.innerHTML=v;let A="";l.type==="vocabulary"?A=`
  ...
  `,Ye().then(()=>{n.disabled=!1,n.placeholder="Rechercher... (e.g. sauce, roux, cut, culer, 刻む)";function m(i){var _;if(o.innerHTML="",!i.trim()){o.innerHTML=`
        ...
        `;return}const u=i.toLowerCase().trim(),d=((_=k.db)==null?void 0:_.knowledge)||[],p=d.filter(g=>g.french&&g.japanese&&!g.grammar&&!g.cuisine),y=d.filter(g=>g.grammar),l=d.filter(g=>g.cuisine),h=[];if(p.forEach(g=>{`${g.french} ${g.english} ${g.japanese} ${g.category} ${g.context_fr} ${g.context_en} ${g.context_ja} ${(g.tags||[]).join(" ")}`.toLowerCase().includes(u)&&h.push({...g,type:"vocabulary",title:g.french,subtitle:`${g.category} • Vocabulary`})}),y.forEach(g=>{`${g.topic} ${g.explanation_en} ${g.explanation_ja} ${g.level} ${(g.tags||[]).join(" ")} ${g.examples.map(A=>`${A.fr} ${A.en} ${A.ja}`).join(" ")}`.toLowerCase().includes(u)&&h.push({...g,type:"grammar",title:g.topic,subtitle:`${g.level} • Grammar Lesson`})}),l.forEach(g=>{const v=g.cuisine;`${v.topic} ${g.category} ${v.content_fr} ${v.content_en} ${v.content_ja} ${(g.tags||[]).join(" ")}`.toLowerCase().includes(u)&&h.push({...g,type:"cuisine",title:v.topic,subtitle:`${g.category} • Culinary Theory`})}),h.length===0){o.innerHTML=`
          <div style="text-align: center; padding: 3rem; color: var(--color-text-muted);">
        ...
        `;return}h.forEach(g=>{const v=document.createElement("div");v.className="card search-result-card",v.style.marginBottom="1.2rem",v.style.borderLeft=`4px solid ${g.type==="vocabulary"?"var(--color-primary)":g.type==="grammar"?"var(--color-secondary)":"var(--color-accent)"}`;const A=fe(g.id);let L="";g.type==="vocabulary"?L=`
          ...
          `:g.type==="grammar"?L=`
            ...
            <div class="examples-list" style="margin-top: 1rem; border-top: 1px solid rgba(0,0,0,0.05); padding-top: 0.8rem;">
              ${g.examples.slice(0,2).map(b=>`
                <div class="example-item" style="margin-bottom: 0.5rem;">
                  <div class="example-fr" style="font-weight: 500;">➔ ${b.fr}</div>
                  <div class="example-ja" style="font-size: 0.8rem; color: var(--color-text-muted);">${b.ja}</div>
                </div>
              `).join("")}
            </div>
          `:g.type==="cuisine"&&(L=`
          ...
          `);const P=(g.tags||[]).map(b=>`<span class="tag-badge" style="background-color: rgba(197, 168, 128, 0.12); color: var(--color-accent); font-size: 0.75rem; padding: 0.2rem 0.6rem; border-radius: 20px; font-weight: 500;">#${b}</span>`).join(" "),c=g.type==="vocabulary"?Ie(g.french):"",s=c?`<div class="preposition-illustration-container" style="height: 130px; background-color: #fcfbfa; border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; margin: 0.8rem 0; overflow: hidden; border: 1px solid rgba(0,0,0,0.03);">${c}</div>`:"";v.innerHTML=`
        ...
        `,v.querySelector(".fav-btn").addEventListener("click",b=>{b.stopPropagation(),be(g.id);const f=b.target,x=fe(g.id);f.classList.toggle("active",x),f.innerText=x?"★":"☆"}),o.appendChild(v)})}n.addEventListener("input",i=>{m(i.target.value)}),m("")}),e}function mn(){const e=document.createElement("div"),r=document.createElement("h2");r.className="section-title",r.innerText="Configuration de l'Académie",e.appendChild(r);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Customize your learning goals, target CEFR levels, and database options.",e.appendChild(t);const a=document.createElement("div");a.className="card",a.style.padding="2rem";const n=k.settings;a.innerHTML=`
  ...
  `;const o=a.querySelector("#target-level-select"),m=a.querySelector("#new-cards-goal"),i=a.querySelector("#max-reviews-goal"),u=a.querySelector("#include-general-checkbox"),d=a.querySelector("#settings-status");function p(){const y=o.value,l=parseInt(m.value)||5,h=parseInt(i.value)||20,_=u.checked;Ln({targetLevel:y,newCardsPerDay:l,maxReviewsPerDay:h,includeGeneral:_}),d.style.display="block",setTimeout(()=>{d.style.display="none"},3e3)}return o.addEventListener("change",p),m.addEventListener("input",p),i.addEventListener("input",p),u.addEventListener("change",p),a.querySelector("#reset-srs-btn").addEventListener("click",()=>{confirm("Voulez-vous vraiment réinitialiser toutes vos données de progression SRS ? Cette action est irréversible.")&&(localStorage.removeItem("cba_srs"),k.srs={},alert("Spaced Repetition System progress has been reset."),window.location.reload())}),a.querySelector("#clear-favs-btn").addEventListener("click",()=>{confirm("Voulez-vous vraiment supprimer tous vos favoris ?")&&(localStorage.removeItem("cba_favorites"),k.favorites=new Set,alert("All favorites have been cleared."),window.location.reload())}),a.querySelector("#share-app-btn").addEventListener("click",()=>{const y={title:"Académie de la Brigade",text:"フランス料理・厨房フランス語の統合学習PWAアプリ「Académie de la Brigade」で一緒に料理と語学を学びましょう！",url:window.location.origin+window.location.pathname};navigator.share?navigator.share(y).then(()=>console.log("Shared successfully")).catch(l=>console.log("Error sharing:",l)):navigator.clipboard.writeText(y.url).then(()=>{alert("App link copied to clipboard! Share it with your friends.")}).catch(l=>{console.error("Failed to copy link:",l)})}),e.appendChild(a),e}function un(){var o;const e=document.createElement("div"),r=document.createElement("h2");r.className="section-title",r.innerText="Dictée de Cuisine (Culinary Dictations)",e.appendChild(r);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Listen to French kitchen instruction sentences, type what you hear, and master French spelling.",e.appendChild(t);const a=document.createElement("div");a.className="loading-placeholder",a.innerText="Chargement de la dictée... (Loading dictation...)",e.appendChild(a);const n=((o=k.settings)==null?void 0:o.targetLevel)||"ALL";return oe("vocabulary",n).then(()=>{a.remove(),pn(e)}),e}function pn(e){var v,A;const r=((v=k.settings)==null?void 0:v.includeGeneral)||!1,n=(((A=k.db)==null?void 0:A.vocabulary)||[]).filter(L=>r||L.is_professional).filter(L=>L.context_fr);if(n.length===0){e.innerHTML+='<p style="color: var(--color-text-muted);">Aucun exercice disponible.</p>';return}const o=[...n].sort(()=>.5-Math.random()).slice(0,5);let m=0,i=!1,u=!1,d=!1,p=0,y=1;const l=document.createElement("div");l.className="dictation-container",l.style.maxWidth="600px",l.style.margin="1.5rem auto",e.appendChild(l);function h(L){return L.toLowerCase().trim().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()'"?]/g,"").replace(/\s+/g," ")}function _(L){return L.normalize("NFD").replace(/[\u0300-\u036f]/g,"")}function g(){if(l.innerHTML="",i=!1,Ne(),u=!1,d=!1,p=0,m>=o.length){const z=document.createElement("div");z.className="card",z.style.padding="2.5rem",z.style.textAlign="center",z.innerHTML=`
        <h3 style="font-family: var(--font-serif); font-size: 2rem; color: var(--color-primary); margin-bottom: 1rem;">Session Terminée !</h3>
      ...
      `,z.querySelector("#restart-dictation-btn").addEventListener("click",()=>{m=0,o.length=0,o.push(...[...n].sort(()=>.5-Math.random()).slice(0,5)),g()}),l.appendChild(z);return}const L=o[m],P=L.context_fr,c=document.createElement("div");c.className="card",c.style.padding="2rem",c.innerHTML=`
    ...
    `;const s=c.querySelector("#play-pause-btn"),b=c.querySelector("#stop-btn"),f=c.querySelector("#speed-normal-btn"),x=c.querySelector("#speed-slow-btn"),C=c.querySelector("#dictation-seekbar"),w=c.querySelector("#dictation-input"),j=c.querySelector("#check-btn"),T=c.querySelector("#next-dictation-btn"),S=c.querySelector("#dictation-feedback");function $(z,F){if(!i){const H=z/F*100;C.value=Math.round(H),p=z}}function q(){u=!1,d=!1,p=0,s.innerText="▶ Play",C.value=0}function E(){ct(P,y,$,q,p)}s.addEventListener("click",()=>{i||(u?d?(d=!1,s.innerText="⏸ Pause",At()):(d=!0,s.innerText="▶ Play",qt()):(u=!0,d=!1,s.innerText="⏸ Pause",E()))}),b.addEventListener("click",()=>{Ne(),q()}),C.addEventListener("change",z=>{if(i)return;const F=parseInt(z.target.value),H=P.replace(/["'➔]/g,"").trim();p=Math.floor(F/100*H.length);const ee=H.indexOf(" ",p);ee!==-1&&ee-p<5&&(p=ee+1),u=!0,d=!1,s.innerText="⏸ Pause",E()}),f.addEventListener("click",()=>{y!==1&&(y=1,f.classList.add("active"),f.style.backgroundColor="var(--color-primary)",f.style.color="#FFFFFF",x.classList.remove("active"),x.style.backgroundColor="transparent",x.style.color="var(--color-accent)",u&&!d&&E())}),x.addEventListener("click",()=>{y!==.75&&(y=.75,x.classList.add("active"),x.style.backgroundColor="var(--color-accent)",x.style.color="#FFFFFF",f.classList.remove("active"),f.style.backgroundColor="transparent",f.style.color="var(--color-accent)",u&&!d&&E())}),j.addEventListener("click",()=>{if(i)return;Ne(),q();const z=w.value,F=h(z),H=h(P),ee=_(F),Y=_(H);let ne=!1,le=!1;F===H?ne=!0:ee===Y&&(le=!0),S.style.display="block",w.disabled=!0,j.style.display="none",T.style.display="block",i=!0,ne?(S.style.backgroundColor="#E8F5E9",S.style.borderLeft="4px solid var(--color-success)",S.style.color="var(--color-success)",S.innerHTML=`
        ...
        `):le?(S.style.backgroundColor="#FFF3E0",S.style.borderLeft="4px solid var(--color-accent)",S.style.color="#E65100",S.innerHTML=`
  ...
  `,e.querySelectorAll(".play-episode-btn").forEach(n=>{n.addEventListener("click",o=>{const m=o.target.getAttribute("data-chapter"),i=o.target.getAttribute("data-episode");zt(e,m,i)})});const r=e.querySelectorAll(".mode-tab-btn"),t=e.querySelector("#rpg-card-history"),a=e.querySelector("#rpg-card-story");r.forEach(n=>{n.addEventListener("click",()=>{r.forEach(m=>{m.classList.remove("active-tab"),m.style.background="none",m.style.color="var(--color-text-muted)",m.style.boxShadow="none"}),n.classList.add("active-tab"),n.style.background="var(--color-primary)",n.style.color="white",n.style.boxShadow="var(--shadow-sm)",n.getAttribute("data-mode")==="history"?(t.style.display="flex",a.style.display="none"):(t.style.display="none",a.style.display="flex")})})}async function zt(e,r,t){try{e.innerHTML='<div class="story-loader"><div class="spinner"></div><p>物語を読み込んでいます...</p></div>';const a=r==="career_0"?"rpg/story/chapter_career_0.json":`rpg/history/chapter_${r}.json`,[n,o,m]=await Promise.all([fetch(a),fetch("rpg/questions_db.json"),fetch("data/grammar_reference.json").catch(()=>null)]);if(!n.ok)throw new Error("Story file could not be loaded");if(!o.ok)throw new Error("Questions database could not be loaded");const i=await n.json(),u=await o.json();let d=[];m&&m.ok&&(d=await m.json()),k.questionsDb=u,k.grammarRefs=d;const p=i.episodes.find(y=>y.episodeId===t);if(!p)throw new Error("Episode data not found in chapter file");bn(e,p,r,i)}catch(a){e.innerHTML=`
  ...
  `;const m=e.querySelector("#rpg-viewport"),i=e.querySelector("#rpg-dialog-pane"),u=e.querySelector("#rpg-battle-pane"),d=e.querySelector("#rpg-reward-pane"),p=e.querySelector("#rpg-tutorial-overlay"),y=e.querySelector("#rpg-sidebar");let l=null,h=!1,_="";function g(){if(h){clearInterval(l);const b=e.querySelector("#dialog-text");b.innerText=_,h=!1;return}if(n>=o.length){Et(e);return}const s=o[n];s.type==="tutorial"?A(s):s.type==="dialog"?L(s):s.type==="fixedBattle"||s.type==="randomBattle"?P(s):s.type==="reward"&&c(s),v(s)}function v(s){let b=m.querySelector(".ai-policy-notice");s&&s.background==="bg_thief_caught.png"||m.style.background&&m.style.background.includes("bg_thief_caught.png")?b||(b=document.createElement("div"),b.className="ai-policy-notice",b.innerText="※AIポリシーの都合により、カミーユはうさぎの縫いぐるみに差し替えられました",m.appendChild(b)):b&&b.remove()}m.addEventListener("click",s=>{s.target.closest("button")||s.target.closest(".battle-question-box")||i.style.display==="none"||g()}),g();function A(s){i.style.display="none",u.style.display="none",d.style.display="none",y.style.opacity="0",y.style.pointerEvents="none";const b=e.querySelector("#rpg-character-layer");b&&(b.innerHTML=""),p.style.display="flex",p.style.flexDirection="column";function f(S,$){const q=$.findIndex(E=>E.includes("フランス語")||E.includes("Pronom")||E.includes("Exemple")||E.includes("活用形"));if(q!==-1){if($[q].includes("活用形")){const E=$.findIndex(z=>z.includes("人称")||z.includes("主語"));if(E!==-1){const z=S[E].split("(")[0].split("（")[0].trim(),F=S[q].trim();return`${z} ${F}`}}return S[q].split("(")[0].split("（")[0].trim()}return S[0].split("(")[0].split("（")[0].trim()}function x(S){if(S)if(typeof te=="function")te(S);else{const $=new SpeechSynthesisUtterance(S);$.lang="fr-FR",window.speechSynthesis.speak($)}}function C(S,$){const q=[...S],E=S.findIndex(z=>z.includes("発音")||z.includes("Pronunciation"));return E!==-1&&(q[E]="音声"),`
        <table class="tutorial-table" style="width: 100%; border-collapse: collapse; margin-top: 0.4rem; margin-bottom: 0.6rem; font-size: 0.8rem; box-shadow: var(--shadow-sm); border-radius: var(--radius-sm); overflow: hidden; border: 1px solid rgba(197, 168, 128, 0.2);">
           <thead>
             <tr style="border-bottom: 2px solid rgba(197, 168, 128, 0.3); background: rgba(197, 168, 128, 0.08); color: var(--color-primary);">
               ${q.map(z=>`<th style="padding: 0.4rem; text-align: left; font-weight: 600;">${z}</th>`).join("")}
             </tr>
           </thead>
           <tbody>
             ${$.map(z=>`<tr style="border-bottom: 1px solid rgba(197, 168, 128, 0.1); background: rgba(255,255,255,0.02);">${z.map((H,ee)=>ee===E?`<td style="padding: 0.4rem; text-align: center;"><button class="play-audio-btn" data-french="${f(z,S)}" style="background: none; border: none; cursor: pointer; font-size: 1.15rem; padding: 2px; line-height: 1; transition: transform 0.1s;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'">🔊</button></td>`:`<td style="padding: 0.4rem; line-height: 1.4;">${H}</td>`).join("")}</tr>`).join("")}
           </tbody>
         </table>
      `}function w(S){if(!S)return"";let $="";return S.title&&($+=`<div class="ref-sec-title" style="font-weight: bold; margin-top: 0.6rem; margin-bottom: 0.3rem; color: var(--color-primary); font-size: 0.85rem;">${S.title}</div>`),S.text&&($+=`<p style="line-height: 1.4; margin-bottom: 0.4rem; font-size: 0.8rem; color: var(--color-text-main);">${S.text}</p>`),S.type==="table"&&S.headers&&S.rows?$+=C(S.headers,S.rows):S.type==="info"&&($+=`
        ...
        `),$}function j(S){S.querySelectorAll(".play-audio-btn").forEach($=>{$.addEventListener("click",q=>{q.stopPropagation();const E=$.getAttribute("data-french");x(E)})})}if(s.targets&&s.targets.length>0)p.innerHTML=`
      ...
      `,p.querySelector(".start-tut-btn").addEventListener("click",()=>{p.style.display="none",n++,g()});else if(s.pages&&s.pages.length>0){let z=function(F){const H=s.pages[F],ee=p.querySelector(".tutorial-page-content");let Y=`<h4 style="color: var(--color-accent); font-family: var(--font-serif); font-size: 0.95rem; margin-top: 0; margin-bottom: 0.3rem; border-bottom: 1px solid rgba(197, 168, 128, 0.15); padding-bottom: 0.15rem; line-height: 1.25;">${H.title}</h4>`;if(H.type==="custom")H.text&&(Y+=`<p style="white-space: pre-line; line-height: 1.4; font-size: 0.8rem; margin: 0 0 0.4rem 0; color: var(--color-text-main);">${H.text}</p>`),H.headers&&H.rows&&(Y+=C(H.headers,H.rows));else if(H.referenceTopicId){const ne=k.grammarRefs?k.grammarRefs.find(le=>le.id===H.referenceTopicId):null;ne?(ne.definition_ja&&(Y+=`<p style="font-size: 0.75rem; color: var(--color-text-main); font-weight: 500; margin: 0 0 0.4rem 0; line-height: 1.4;">${ne.definition_ja}</p>`),H.sectionIndices&&H.sectionIndices.length>0?H.sectionIndices.forEach(le=>{const Se=ne.sections[le];Se&&(Y+=w(Se))}):ne.sections&&ne.sections.forEach(le=>{Y+=w(le)})):Y+=`<p style="font-size: 0.8rem; color: var(--color-error);">解説データが見つかりませんでした。(${H.referenceTopicId})</p>`}ee.innerHTML=Y,p.querySelector("#tut-page-curr").innerText=F+1,j(ee),S.style.visibility=F===0?"hidden":"visible",$.style.visibility=F===s.pages.length-1?"hidden":"visible",F===s.pages.length-1?q.style.display="block":q.style.display="none",ee.scrollTop=0};var T=z;p.innerHTML=`
      ...
      `;const S=p.querySelector(".prev-page-btn"),$=p.querySelector(".next-page-btn"),q=p.querySelector(".start-battle-btn");let E=0;S.addEventListener("click",F=>{F.stopPropagation(),E>0&&(E--,z(E))}),$.addEventListener("click",F=>{F.stopPropagation(),E<s.pages.length-1&&(E++,z(E))}),q.addEventListener("click",F=>{F.stopPropagation(),p.style.display="none",n++,g()}),z(0)}else p.innerHTML=`
      ...
      `,p.querySelector(".start-tut-btn").addEventListener("click",()=>{p.style.display="none",n++,g()})}function L(s){p.style.display="none",u.style.display="none",d.style.display="none",i.style.display="block";const b=r.backgrounds[s.background]||"#000000";m.style.background=b,s.shake&&(m.classList.add("shake-vfx"),setTimeout(()=>m.classList.remove("shake-vfx"),400)),b.includes("url(")?(m.style.backgroundSize="cover",m.style.backgroundPosition="center",m.style.backgroundRepeat="no-repeat"):(m.style.backgroundSize="",m.style.backgroundPosition="",m.style.backgroundRepeat="");const f=e.querySelector("#rpg-character-layer");if(f){f.innerHTML="";const S=b.toLowerCase(),$=S.includes("bg_camille_cry.webp")||S.includes("bg_after_battle.webp")||S.includes("bg_father.webp")||S.includes("bg_room.webp");let q=[];$||(s.characters&&Array.isArray(s.characters)?q=s.characters:s.character&&(q=[{id:s.character,expression:s.expression||"default",position:s.position||"center"}])),q.forEach(E=>{const z=r.characters[E.id],F=E.expression||"default";let H=E.position||"center";q.length===1&&E.id==="proprietress"&&(H="center");let ee=null;if(z&&z.images)if(z.images[F])ee=z.images[F];else if(F==="default"&&z.images.normal)ee=z.images.normal;else{const Y=Object.keys(z.images);Y.length>0&&(ee=z.images[Y[0]])}if(ee){const Y=document.createElement("img");Y.src=ee,Y.className=`rpg-character-sprite pos-${H} sprite-${E.id}`;const ne=gn[E.id]||1;Y.style.setProperty("--char-scale",ne),f.appendChild(Y),setTimeout(()=>{Y.classList.add("active")},50)}})}const x=s.character?r.characters[s.character]:null,C=e.querySelector("#dialog-name"),w=e.querySelector("#dialog-text");C.style.display="none",s.learningPoint?(e.querySelector("#lp-title").innerText=s.learningPoint.title,e.querySelector("#lp-content").innerText=s.learningPoint.text,y.style.opacity="1",y.style.pointerEvents="auto"):(y.style.opacity="0",y.style.pointerEvents="none");const j=x?`${x.name}：${s.text}`:s.text;w.innerText="",_=j,h=!0;let T=0;clearInterval(l),l=setInterval(()=>{T<j.length?(w.innerText+=j[T],T++,T%2===0&&hn()):(clearInterval(l),h=!1)},30),n++}async function P(s){var le,Se;i.style.display="none",p.style.display="none",d.style.display="none",y.style.opacity="0",y.style.pointerEvents="none",u.style.display="flex";const b=e.querySelector("#rpg-character-layer");b&&(b.innerHTML=""),u.innerHTML=`
      <div style="margin: auto; text-align: center;">
        <div class="spinner" style="margin: 0 auto 1rem;"></div>
        <p>試練を読み込んでいます...</p>
      </div>
    `;function f(B,O){const K=k.questionsDb||[];let R=[];if(k.recentQuestionIds||(k.recentQuestionIds=JSON.parse(localStorage.getItem("cba_recent_questions")||"[]")),B&&B.length>0){B.forEach(N=>{const V=N.tag,G=N.count||1;let W=K.filter(X=>X.tags&&X.tags.includes(V)),Q=W.filter(X=>!k.recentQuestionIds.includes(X.id));Q.length===0&&(Q=W),Q=q(Q);const J=Q.slice(0,G);R=R.concat(J),J.forEach(X=>{k.recentQuestionIds.push(X.id),k.recentQuestionIds.length>20&&k.recentQuestionIds.shift()})});const D=B.reduce((N,V)=>N+(V.count||1),0);if(R.length<D){const N=D-R.length,V=q(K.filter(G=>!R.some(W=>W.id===G.id)));R=R.concat(V.slice(0,N))}localStorage.setItem("cba_recent_questions",JSON.stringify(k.recentQuestionIds))}else O&&O.length>0&&(R=q(O));return R}let x=[];if(s.type==="randomBattle")try{await Je();const B=((le=s.conditions)==null?void 0:le.level)||"ALL";B!=="ALL"&&await oe("vocabulary",B);let O=[...k.db.quizzes];const K=(Se=s.conditions)==null?void 0:Se.category;K&&(O=O.filter(D=>D.category&&D.category.toLowerCase()===K.toLowerCase())),O=q(O);const R=s.enemyHp||3;x=O.slice(0,R).map(D=>{const N=D.options.indexOf(D.answer);return{id:D.id,text:D.question,options:D.options,answerIndex:N!==-1?N:0,explanation:D.context||`正解は「${D.answer}」です。`,acceptedAnswers:[D.answer,D.answer.toLowerCase()]}})}catch(B){console.error("Failed to generate random questions:",B),x=[{id:"fb_err_1",text:"プロヴァンス地方の代表的なスープ料理は？",options:["Bouillabaisse","Bœuf bourguignon","Cassoulet","Choucroute"],answerIndex:0,acceptedAnswers:["Bouillabaisse","bouillabaisse"],explanation:"ブイヤベースはプロヴァンス地方（マルセイユ）の名物です。"}]}else x=f(s.criteria,s.questions);let C=x.map(B=>{let O=B.type||"choice";if((O==="choice"||O==="typing")&&(O=Math.random()>.15?"typing":"choice"),O==="typing"&&!B.acceptedAnswers&&B.options&&B.answerIndex!==void 0){const R=B.options[B.answerIndex].split("(")[0].split("（")[0].trim();return{...B,type:"typing",acceptedAnswers:[R,R.toLowerCase()]}}return{...B,type:O}}),w=s.enemyHp||C.length;const j=w;let T=10;const S=10;let $=0;function q(B){const O=[...B];for(let K=O.length-1;K>0;K--){const R=Math.floor(Math.random()*(K+1));[O[K],O[R]]=[O[R],O[K]]}return O}let E=null;async function z(){if(E)return E;try{return E=await(await fetch("data/grammar_reference.json")).json(),E}catch(B){return console.error("Failed to load battle reference:",B),[]}}async function F(B){if(y.style.opacity==="1")y.style.opacity="0",y.style.pointerEvents="none",B.innerText="📖 参考資料を表示";else{let N=function(G){const W=K[G];if(!W)return;let Q=`<h4 style="color: var(--color-accent); border-bottom: 1px solid rgba(197,168,128,0.2); padding-bottom: 0.3rem; margin-bottom: 0.6rem;">${W.title_ja} (${W.title_fr})</h4>`;W.sections.forEach(J=>{if(Q+='<div style="margin-bottom: 1rem;">',J.title&&(Q+=`<div style="font-weight: bold; font-size: 0.85rem; color: var(--color-primary); margin-bottom: 0.3rem;">${J.title}</div>`),J.text&&(Q+=`<p style="white-space: pre-line; margin-bottom: 0.5rem; line-height: 1.4;">${J.text}</p>`),J.headers&&J.rows){const X=[],ae=J.headers.map((re,Le)=>re==="発音"?(X.push(Le),"音声"):re);Q+=`
        ...
        `;const R=y.querySelector(".ref-topic-selector"),D=y.querySelector(".ref-topic-content");let V=0;if(r.episodeId==="career_ep_0_1"){const G=K.findIndex(W=>W.id==="ref_essential_irregular_verbs");G!==-1&&(V=G)}else if(r.episodeId==="career_ep_0_2"){const G=K.findIndex(W=>W.id==="ref_time_expressions");G!==-1&&(V=G)}R.value=V,N(V),R.addEventListener("change",G=>{N(parseInt(G.target.value))})}}function H(){if(w<=0){je(),y.style.opacity="0",y.style.pointerEvents="none",u.innerHTML=`
        ...
        `,u.querySelector(".retry-battle-btn").addEventListener("click",()=>{P(s)}),u.querySelector(".exit-battle-btn").addEventListener("click",()=>{var D;const R=(D=s.conditions)==null?void 0:D.category;Ee(R==="grammar"?"grammar":"vocabulary")});return}if($>=C.length){w>0&&(w=0,H());return}const B=C[$],O=`
        <div class="battle-hud">
          <div class="hud-bar-container">
            <span class="hud-label">VOUS (あなた)</span>
            <div class="hud-hp-track">
              <div class="hud-hp-fill player-hp" style="width: ${T/S*100}%"></div>
            </div>
            <span class="hud-value">${T} / ${S}</span>
          </div>
          
          <div class="hud-bar-container">
            <span class="hud-label">${s.enemyName}</span>
            <div class="hud-hp-track">
              <div class="hud-hp-fill enemy-hp" style="width: ${w/j*100}%"></div>
            </div>
            <span class="hud-value">${w} / ${j}</span>
          </div>
        </div>
      `;if(B.type==="typing"){u.innerHTML=`
        ...
        `;const R=u.querySelector(".battle-input-field"),D=u.querySelector(".submit-typing-btn"),N=u.querySelector(".toggle-battle-ref-btn");setTimeout(()=>R.focus(),150),R.addEventListener("keydown",V=>{V.key==="Enter"&&!D.disabled&&D.click()}),D.addEventListener("click",()=>{const V=R.value;Y(V,B,R,D)}),N.addEventListener("click",()=>{F(N)})}else if(B.type==="scramble"){let R=[];const D=q(B.words);u.innerHTML=`
          ...
          <div class="battle-question-box">
            ...
            <div class="scramble-words-pool" style="display: flex; gap: 0.4rem; flex-wrap: wrap; margin-bottom: 0.8rem; justify-content: center;">
              ${D.map((Q,J)=>`<button class="scramble-word-btn" data-word="${Q}" style="padding: 0.35rem 0.7rem; font-size: 0.85rem; border-radius: 20px; border: 1px solid var(--color-accent); background: none; color: white; cursor: pointer;">${Q}</button>`).join("")}
            </div>
        ...
        `;const V=u.querySelector(".toggle-battle-ref-btn");if(V.addEventListener("click",()=>F(V)),N){const G=u.querySelectorAll(".battle-opt-btn");G.forEach(W=>{W.addEventListener("click",Q=>{const J=parseInt(Q.target.getAttribute("data-idx"));ee(J,B,G)})})}else{const G=u.querySelector(".battle-input-field"),W=u.querySelector(".submit-typing-btn");setTimeout(()=>G.focus(),150),G.addEventListener("keydown",Q=>{Q.key==="Enter"&&W.click()}),W.addEventListener("click",()=>{const Q=G.value;Y(Q,B,G,W)})}}else if(B.type==="matching"){let he=function(){if(!X||!ae)return;const Z=X.getAttribute("data-val"),ce=ae.getAttribute("data-val");if(B.pairs.find(pe=>pe.left===Z&&pe.right===ce))Ce(440,"sine",.1,.1),X.style.background="#2E7D32",X.style.borderColor="#A5D6A7",X.disabled=!0,ae.style.background="#2E7D32",ae.style.borderColor="#A5D6A7",ae.disabled=!0,re++,X=null,ae=null,re===Le&&(je(),w=Math.max(0,w-1),W.innerText="✅ すべてマッチしました！ (Très bien)",W.className="feedback-title text-success",Q.innerText=B.explanation,G.style.display="block",J.innerText="次の試練へ",J.onclick=()=>{$++,H()});else{Te(),T=Math.max(0,T-1);const pe=X,qe=ae;if(pe.style.background="#C62828",pe.style.borderColor="#EF9A9A",qe.style.background="#C62828",qe.style.borderColor="#EF9A9A",X=null,ae=null,m.classList.add("shake-vfx"),setTimeout(()=>m.classList.remove("shake-vfx"),400),T<=0){H();return}setTimeout(()=>{if(pe.disabled)return;pe.style.background="rgba(255,255,255,0.05)",pe.style.borderColor="rgba(255,255,255,0.15)",qe.style.background="rgba(255,255,255,0.05)",qe.style.borderColor="rgba(255,255,255,0.15)";const Ke=u.querySelector(".player-hp"),Ze=u.querySelectorAll(".hud-value")[0];Ke&&(Ke.style.width=`${T/S*100}%`),Ze&&(Ze.innerText=`${T} / ${S}`)},800)}};var K=he;const R=q(B.pairs.map(Z=>Z.left)),D=q(B.pairs.map(Z=>Z.right));u.innerHTML=`
          ${O}
          <div class="battle-question-box">
            ...
            <div class="matching-columns-container" style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.6rem; max-height: 160px; overflow-y: auto; padding: 0.2rem;">
              <div class="left-match-col" style="display: flex; flex-direction: column; gap: 0.4rem;">
                ${R.map((Z,ce)=>`<button class="match-card-btn left-card" data-val="${Z}" style="padding: 0.45rem; font-size: 0.8rem; border: 1px solid rgba(255,255,255,0.15); background: rgba(255,255,255,0.05); color: white; border-radius: var(--radius-sm); cursor: pointer; text-align: center; outline: none;">${Z}</button>`).join("")}
              </div>
              <div class="right-match-col" style="display: flex; flex-direction: column; gap: 0.4rem;">
                ${D.map((Z,ce)=>`<button class="match-card-btn right-card" data-val="${Z}" style="padding: 0.45rem; font-size: 0.8rem; border: 1px solid rgba(255,255,255,0.15); background: rgba(255,255,255,0.05); color: white; border-radius: var(--radius-sm); cursor: pointer; text-align: center; outline: none;">${Z}</button>`).join("")}
              </div>
            </div>
        ...
        `;const N=u.querySelectorAll(".left-card"),V=u.querySelectorAll(".right-card"),G=u.querySelector("#battle-feedback"),W=u.querySelector("#fb-title"),Q=u.querySelector("#fb-desc"),J=u.querySelector("#next-q-btn");let X=null,ae=null,re=0;const Le=B.pairs.length;N.forEach(Z=>{Z.addEventListener("click",()=>{Z.disabled||(N.forEach(ce=>{ce.disabled||(ce.style.background="rgba(255,255,255,0.05)",ce.style.borderColor="rgba(255,255,255,0.15)")}),Z.style.background="var(--color-accent)",Z.style.borderColor="white",X=Z,he())})}),V.forEach(Z=>{Z.addEventListener("click",()=>{Z.disabled||(V.forEach(ce=>{ce.disabled||(ce.style.background="rgba(255,255,255,0.05)",ce.style.borderColor="rgba(255,255,255,0.15)")}),Z.style.background="var(--color-accent)",Z.style.borderColor="white",ae=Z,he())})})}else{u.innerHTML=`
          ${O}
          <div class="battle-question-box">
        ...
        `;const R=u.querySelectorAll(".battle-opt-btn"),D=u.querySelector(".toggle-battle-ref-btn");R.forEach(N=>{N.addEventListener("click",V=>{const G=parseInt(V.target.getAttribute("data-idx"));ee(G,B,R)})}),D.addEventListener("click",()=>{F(D)})}}function ee(B,O,K){K.forEach(W=>W.disabled=!0);const R=u.querySelector("#battle-feedback"),D=u.querySelector("#fb-title"),N=u.querySelector("#fb-desc"),V=u.querySelector("#next-q-btn");if(B===O.answerIndex){je(),w=Math.max(0,w-1),D.innerText="✅ 正解！ (Très bien)",D.className="feedback-title text-success",V.innerText="次の試練へ";const W=u.querySelector(".enemy-hp");W&&W.classList.add("flash-white"),V.onclick=()=>{$++,H()}}else Te(),Be(),T=Math.max(0,T-s.enemyDamage),D.innerText="❌ 不正解！",D.className="feedback-title text-error",V.innerText="もう一度挑戦する",m.classList.add("shake-vfx"),setTimeout(()=>m.classList.remove("shake-vfx"),400),V.onclick=()=>{H()};N.innerText=O.explanation,R.style.display="block"}function Y(B,O,K,R){K.disabled=!0,R.disabled=!0;const D=u.querySelector("#battle-feedback"),N=u.querySelector("#fb-title"),V=u.querySelector("#fb-desc"),G=u.querySelector("#next-q-btn"),W=B.toLowerCase().replace(/[.,!?;:・]/g,"").trim();if(O.acceptedAnswers.some(J=>W===J.toLowerCase().replace(/[.,!?;:・]/g,"").trim())){je(),w=Math.max(0,w-1),N.innerText="✅ 正解！ (Très bien)",N.className="feedback-title text-success",G.innerText="次の試練へ";const J=u.querySelector(".enemy-hp");J&&J.classList.add("flash-white"),G.onclick=()=>{$++,H()}}else Te(),Be(),T=Math.max(0,T-2),N.innerText=`❌ 不正解！ (正解: ${O.acceptedAnswers[0]})`,N.className="feedback-title text-error",G.innerText="もう一度挑戦する",m.classList.add("shake-vfx"),setTimeout(()=>m.classList.remove("shake-vfx"),400),G.onclick=()=>{H()};V.innerText=O.explanation,D.style.display="block"}function ne(B,O){const K=u.querySelector("#battle-feedback"),R=u.querySelector("#fb-title"),D=u.querySelector("#fb-desc"),N=u.querySelector("#next-q-btn");if(B){je(),w=Math.max(0,w-1),R.innerText="✅ 正解！ (Très bien)",R.className="feedback-title text-success",N.innerText="次の試練へ";const V=u.querySelector(".enemy-hp");V&&V.classList.add("flash-white"),N.onclick=()=>{$++,H()}}else Te(),Be(),T=Math.max(0,T-2),R.innerText=`❌ 不正解！ (正解: ${O.answer})`,R.className="feedback-title text-error",N.innerText="もう一度挑戦する",m.classList.add("shake-vfx"),setTimeout(()=>m.classList.remove("shake-vfx"),400),N.onclick=()=>{H()};D.innerText=O.explanation,K.style.display="block"}H()}function c(s){i.style.display="none",u.style.display="none",p.style.display="none",y.style.opacity="0",y.style.pointerEvents="none",d.style.display="block";const b=e.querySelector("#rpg-character-layer");b&&(b.innerHTML=""),localStorage.setItem(`cba_story_${r.episodeId}_cleared`,"true");const x=r.episodeId.startsWith("career_")?"assets/story/career_story/group_photo.webp":"assets/story/chapter_0/bg_after_battle.webp";d.innerHTML=`
    ...
    `,d.querySelector(".claim-reward-btn").addEventListener("click",()=>{C()});function C(){d.innerHTML=`
      ...
      `;const T=d.querySelector(".stamp-ink-overlay");setTimeout(()=>{T.style.display="flex",T.classList.add("animate-stamp-stomp"),Be(),m.classList.add("shake-vfx"),setTimeout(()=>m.classList.remove("shake-vfx"),450)},700),setTimeout(()=>{j()},2300)}const w=function(T){let S=document.querySelector(".cba-toast");S&&S.remove(),S=document.createElement("div"),S.className="cba-toast",S.innerText=T,document.body.appendChild(S),setTimeout(()=>{S.classList.add("fade-out"),setTimeout(()=>S.remove(),400)},2500)};function j(){const T=a&&a.episodes?a.episodes.findIndex(z=>z.episodeId===r.episodeId):-1,S=T!==-1&&a.episodes[T+1]?a.episodes[T+1]:null;d.innerHTML=`
  ...
  `,e.appendChild(d);const p=new Map,y=new Map;r.sessions.forEach(C=>{C.tokens.forEach(w=>{if(w.vocab_id){const j=k.db.knowledge.find(T=>T.id===w.vocab_id);j&&p.set(w.vocab_id,j.french)}if(w.gram_id){const j=k.db.knowledge.find(T=>T.id===w.gram_id);j&&j.grammar&&y.set(w.gram_id,j.grammar.topic)}})});const l=document.createElement("div");l.className="card",l.style.backgroundColor="#f7f9fc",l.style.border="1px solid rgba(0,0,91,0.06)",l.style.padding="1.5rem",l.style.borderRadius="10px",l.style.marginBottom="2rem";let h="";p.forEach((C,w)=>{h+=`<button class="learning-badge" data-id="${w}" style="margin: 0.2rem; padding: 0.25rem 0.6rem; font-size: 0.78rem; border-radius: 4px; border: 1px solid rgba(0,0,91,0.1); background-color: #ffffff; color: var(--color-primary); font-weight: 600; cursor: pointer; transition: all 0.2s;">${C}</button>`});let _="";y.forEach((C,w)=>{_+=`<button class="learning-badge" data-id="${w}" style="margin: 0.2rem; padding: 0.25rem 0.6rem; font-size: 0.78rem; border-radius: 4px; border: 1px solid rgba(107,156,104,0.15); background-color: #ffffff; color: var(--color-success); font-weight: 600; cursor: pointer; transition: all 0.2s;">${C}</button>`}),l.innerHTML=`
  ...
  `,l.querySelectorAll(".learning-badge").forEach(C=>{C.addEventListener("click",w=>{w.stopPropagation();const j=C.getAttribute("data-id"),T=j.startsWith("gram_")?{gram_id:j}:{vocab_id:j};Ve(C,T,"")})}),e.appendChild(l);const g=document.createElement("div");g.style.textAlign="center",g.style.marginTop="3.5rem",g.style.marginBottom="3.5rem";const v=document.createElement("div");v.style.display="inline-flex",v.style.alignItems="center",v.style.justifyContent="center",v.style.gap="1.2rem",v.style.flexWrap="wrap";const A=document.createElement("button");A.className="start-practice-btn",A.innerText="🎓 Commencer l'entraînement (問題を解くモードに入る)",A.addEventListener("click",()=>{ue="practice",ie=0,ge(e,r)});const L=document.createElement("div");L.style.display="inline-flex",L.style.alignItems="center",L.style.gap="0.5rem";const P=document.createElement("span");P.style.fontSize="0.8rem",P.style.fontWeight="bold",P.style.color="var(--color-text-muted)",P.innerText="Niveau (Level) :";const c=document.createElement("select");c.style.padding="0.6rem 1.2rem",c.style.fontSize="0.9rem",c.style.borderRadius="50px",c.style.border="1px solid rgba(0,0,0,0.15)",c.style.backgroundColor="#FFFFFF",c.style.cursor="pointer",c.style.fontFamily="var(--font-sans)",c.style.fontWeight="600",c.style.color="var(--color-primary)",c.style.outline="none",[{val:"débutant",label:"Débutant (初級)"},{val:"intermédiaire",label:"Intermédiaire (中級)"},{val:"avancé",label:"Avancé (上級)"}].forEach(C=>{const w=document.createElement("option");w.value=C.val,w.text=C.label,C.val===a&&(w.selected=!0),c.appendChild(w)}),c.addEventListener("change",C=>{k.settings.targetLevel=C.target.value,ge(e,r)}),L.appendChild(P),L.appendChild(c),v.appendChild(A),v.appendChild(L),g.appendChild(v),e.appendChild(g);const b=document.createElement("div");b.className="magazine-body-container",b.style.maxWidth="720px",b.style.margin="0 auto",b.style.display="flex",b.style.flexDirection="column",b.style.gap="2.2rem",r.sessions.forEach((C,w)=>{const j=document.createElement("div");j.className="ref-session-card",j.style.borderLeft="4px solid var(--color-accent)",j.style.padding="1.8rem",j.style.backgroundColor="#ffffff";const T=`
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.6rem;">
        <span style="font-size: 0.72rem; text-transform: uppercase; font-weight: bold; letter-spacing: 1px; color: var(--color-text-muted);">Section ${w+1}: ${C.title}</span>
        <button class="section-audio-btn" data-text="${C.text_fr.replace(/"/g,"&quot;")}" style="background: none; border: none; font-size: 1.15rem; cursor: pointer; color: var(--color-accent); padding: 0.2rem; line-height: 1;" title="Lire la section">🔊</button>
      </div>
    `,S=document.createElement("div");S.className="ref-para-fr",S.style.fontSize="1.18rem",S.style.lineHeight="1.8",S.style.fontFamily="var(--font-serif)",S.style.marginBottom="1rem",S.style.color="var(--color-primary)",w===0&&S.classList.add("editorial-dropcap"),C.tokens.forEach((q,E)=>{const z=document.createElement("span");z.className=`anatomy-token token-pos-${q.pos} has-definition`,z.innerText=q.word+" ";const F=C.tokens.slice(E).map(H=>H.word).join(" ");(q.vocab_id||q.gram_id)&&(z.setAttribute("data-has-db","true"),n?q.pos==="noun"?z.style.borderBottom="2px solid rgba(0, 0, 91, 0.25)":q.pos==="verb"?z.style.borderBottom="2px solid rgba(107, 156, 104, 0.35)":q.pos==="article_partitive"?z.style.borderBottom="2px dashed rgba(220, 38, 38, 0.35)":q.pos==="preposition"&&(z.style.borderBottom="2px dotted rgba(197, 168, 128, 0.5)"):o?z.style.borderBottom="1px solid rgba(0, 0, 0, 0.12)":m&&(z.style.borderBottom="none")),z.addEventListener("click",H=>{H.stopPropagation(),Ve(z,q,F)}),S.appendChild(z)});const $=document.createElement("div");if($.className="ref-para-ja",$.style.fontSize="0.94rem",$.style.lineHeight="1.6",$.style.color="var(--color-text-muted)",$.style.paddingTop="0.8rem",$.style.borderTop="1px dashed #eaeaea",$.innerHTML=C.text_ja.split(`
`).join("<br>"),j.appendChild(document.createRange().createContextualFragment(T)),j.appendChild(S),n)j.appendChild($);else{const q=document.createElement("a");q.href="#",q.className="toggle-translation-link",q.style.fontSize="0.78rem",q.style.color="var(--color-accent)",q.style.textDecoration="underline",q.style.display="block",q.style.marginTop="0.5rem",q.innerText="Afficher la traduction (日本語訳を表示)",$.style.display="none",q.addEventListener("click",E=>{E.preventDefault(),$.style.display==="none"?($.style.display="block",q.innerText="Masquer la traduction (翻訳を非表示)"):($.style.display="none",q.innerText="Afficher la traduction (日本語訳を表示)")}),j.appendChild(q),j.appendChild($)}b.appendChild(j)}),b.querySelectorAll(".section-audio-btn").forEach(C=>{C.addEventListener("click",w=>{w.stopPropagation(),te(C.getAttribute("data-text"))})}),(()=>{d.querySelector("#helper-toggle-outlines").addEventListener("change",C=>{k.settings.readingHelper.outlines=C.target.checked,C.target.checked&&(k.settings.readingHelper.pureText=!1,d.querySelector("#helper-toggle-pure").checked=!1),me(b)}),d.querySelector("#helper-toggle-translations").addEventListener("change",C=>{k.settings.readingHelper.translations=C.target.checked,C.target.checked&&(k.settings.readingHelper.pureText=!1,d.querySelector("#helper-toggle-pure").checked=!1),me(b)}),d.querySelector("#helper-toggle-target").addEventListener("change",C=>{k.settings.readingHelper.targetOnly=C.target.checked,me(b)}),d.querySelector("#helper-toggle-pure").addEventListener("change",C=>{k.settings.readingHelper.pureText=C.target.checked,C.target.checked&&(k.settings.readingHelper.outlines=!1,k.settings.readingHelper.translations=!1,d.querySelector("#helper-toggle-outlines").checked=!1,d.querySelector("#helper-toggle-translations").checked=!1),me(b)})})(),me(b),e.appendChild(b)}function kn(e,r){var l;const t=((l=k.settings)==null?void 0:l.targetLevel)||"ALL";let a="débutant";["B1","B2","intermédiaire"].includes(t)?a="intermédiaire":["C1","C2","avancé"].includes(t)&&(a="avancé");const n=a==="débutant",o=a==="intermédiaire",m=a==="avancé",i=document.createElement("button");i.className="next-btn",i.style.backgroundColor="transparent",i.style.border="1px solid var(--color-secondary)",i.style.color="var(--color-secondary)",i.style.padding="0.4rem 1rem",i.style.marginBottom="1.5rem",i.innerText="← Quitter l'entraînement (戻る)",i.addEventListener("click",()=>{ue="preview",ge(e,r)}),e.appendChild(i);const u=document.createElement("div");u.className="magazine-header",u.style.textAlign="center",u.style.marginBottom="2.5rem",u.innerHTML=`
  ...
  `,e.appendChild(d);const p=document.createElement("div");p.className="magazine-body-container",p.style.maxWidth="720px",p.style.margin="0 auto",p.style.display="flex",p.style.flexDirection="column",p.style.gap="2rem",r.sessions.forEach((h,_)=>{const g=document.createElement("div");g.id=`session-card-${_}`;let v="ref-locked-session";_===ie?v="ref-active-session":_<ie&&(v="ref-completed-session"),g.className=`ref-session-card ${v}`;const A=document.createElement("div");A.style.display="flex",A.style.justifyContent="space-between",A.style.alignItems="center",A.style.marginBottom="0.8rem",A.innerHTML=`
    ...
    `,g.appendChild(A);const L=document.createElement("div");if(L.className="ref-text-block",_<ie)L.innerHTML=`
...
`).join("<br>")}
      ...
      `;else if(_===ie){const P=document.createElement("div");P.className="ref-para-fr",P.style.fontSize="1.15rem",P.style.lineHeight="1.8",P.style.fontFamily="var(--font-serif)",P.style.marginBottom="1.5rem",_===0&&P.classList.add("editorial-dropcap"),h.tokens.forEach((b,f)=>{const x=document.createElement("span");x.className=`anatomy-token token-pos-${b.pos} has-definition`,x.innerText=b.word+" ";const C=h.tokens.slice(f).map(w=>w.word).join(" ");(b.vocab_id||b.gram_id)&&(x.setAttribute("data-has-db","true"),n?b.pos==="noun"?x.style.borderBottom="2px solid var(--color-primary)":b.pos==="verb"?x.style.borderBottom="2px solid var(--color-success)":b.pos==="article_partitive"?x.style.borderBottom="2px dashed var(--color-secondary)":b.pos==="preposition"&&(x.style.borderBottom="2px dotted var(--color-accent)"):o?x.style.borderBottom="1px solid rgba(0,0,0,0.12)":m&&(x.style.borderBottom="none")),x.addEventListener("click",w=>{w.stopPropagation(),Ve(x,b,C)}),P.appendChild(x)}),L.appendChild(P);const c=document.createElement("div");c.className="ref-quiz-container",c.style.marginTop="1.5rem",c.style.padding="1.2rem",c.style.backgroundColor="#fdfcf7",c.style.border="1px solid var(--color-accent)",c.style.borderRadius="8px";const s=h.quizzes.find(b=>b.target_level===a)||h.quizzes[0];if(s){let b="";if(s.type==="choice"||s.type==="structure_analysis"){const f=m&&s.options_fr,x=f?s.options_fr:s.options,C=f?s.answer_fr:s.answer;b=`
          ...
          `,c.innerHTML=`
            ...
            ${m?"":`<p style="font-size: 0.85rem; color: var(--color-text-muted); margin-bottom: 0.5rem;">${s.question_ja}</p>`}
            ${b}
          ...
          `;const w=c.querySelector(".quiz-submit-btn"),j=c.querySelector(".quiz-feedback-msg");w.addEventListener("click",()=>{const T=c.querySelector(`input[name="quiz-${s.id}"]:checked`);T&&T.value===C?(j.style.color="var(--color-success)",j.innerText="✓ Correct ! Excellent !",w.style.display="none",setTimeout(()=>{ke(e,r,_)},1e3)):(j.style.color="var(--color-error)",j.innerText="❌ Incorrect. Réessayez !")})}else if(s.type==="preposition"){b=`
            <div style="margin-top: 0.8rem;">
              <select id="preposition-select-${s.id}" style="padding: 0.5rem 1rem; font-size: 0.9rem; border-radius: 4px; border: 1px solid rgba(0,0,91,0.15); background-color: white; outline: none; width: 100%; max-width: 250px; font-weight: 600;">
                ...
                ${s.options.map(C=>`<option value="${C}">${C}</option>`).join("")}
              </select>
            </div>
          `,c.innerHTML=`
          ...
          `;const f=c.querySelector(".quiz-submit-btn"),x=c.querySelector(".quiz-feedback-msg");f.addEventListener("click",()=>{c.querySelector(`#preposition-select-${s.id}`).value===s.answer?(x.style.color="var(--color-success)",x.innerText="✓ Correct ! Très bien !",f.style.display="none",setTimeout(()=>{ke(e,r,_)},1e3)):(x.style.color="var(--color-error)",x.innerText="❌ Incorrect. Réessayez !")})}else if(s.type==="word_order"){c.innerHTML=`
          ...
          `;const f=c.querySelector(".selected-chips-area"),x=c.querySelector(".shuffled-chips-area"),C=c.querySelector(".quiz-submit-btn"),w=c.querySelector(".quiz-feedback-msg");let j=[],T=[...s.options].sort(()=>Math.random()-.5);const S=()=>{f.innerHTML="",x.innerHTML="",j.length===0&&(f.innerHTML='<span style="font-size: 0.8rem; color: #888; font-style: italic; align-self: center;">Cliquez sur les mots ci-dessous...</span>'),j.forEach(($,q)=>{const E=document.createElement("span");E.className="word-chip selected",E.innerText=$,E.addEventListener("click",()=>{j.splice(q,1),T.push($),S()}),f.appendChild(E)}),T.forEach(($,q)=>{const E=document.createElement("span");E.className="word-chip",E.innerText=$,E.addEventListener("click",()=>{T.splice(q,1),j.push($),S()}),x.appendChild(E)})};S(),C.addEventListener("click",()=>{const $=j.join(" "),q=s.answer_words.join(" ");$===q?(w.style.color="var(--color-success)",w.innerText="✓ Correct ! Formidable !",C.style.display="none",f.style.pointerEvents="none",x.style.pointerEvents="none",setTimeout(()=>{ke(e,r,_)},1e3)):(w.style.color="var(--color-error)",w.innerText="❌ Ordre incorrect. Réessayez !")})}else if(s.type==="kitchen_situation"){b=`
          ...
          `;const f=c.querySelector(".quiz-submit-btn"),x=c.querySelector(".quiz-feedback-msg");f.addEventListener("click",()=>{c.querySelector(`#quiz-input-${s.id}`).value.trim().toLowerCase()===s.blank_word.toLowerCase()?(x.style.color="var(--color-success)",x.innerText="✓ Oui, Chef ! Correct !",f.style.display="none",setTimeout(()=>{ke(e,r,_)},1e3)):(x.style.color="var(--color-error)",x.innerText="❌ Ce n'est pas tout à fait ça. Réessayez !")})}else if(s.type==="input"||s.type==="conjugation"){b=`
            <div style="margin-top: 0.8rem;">
          ...
          `;const f=c.querySelector(".quiz-submit-btn"),x=c.querySelector(".quiz-feedback-msg");let C=0;f.addEventListener("click",()=>{if(c.querySelector(`#quiz-input-${s.id}`).value.trim().toLowerCase()===s.blank_word.toLowerCase())x.style.color="var(--color-success)",x.innerText="✓ Correct ! Excellent !",f.style.display="none",setTimeout(()=>{ke(e,r,_)},1e3);else if(C++,x.style.color="var(--color-error)",n&&C>=2){const j=s.blank_word[0].toUpperCase();x.innerText=`❌ Incorrect. Indice : commence par "${j}"`}else x.innerText="❌ Incorrect. Réessayez !"})}}else c.innerHTML=`
        ...
        `,c.querySelector(".validate-session-btn").addEventListener("click",()=>{ke(e,r,_)});L.appendChild(c)}else L.innerHTML=`
...
`).join("<br>")}
      ...
      `;g.appendChild(L),p.appendChild(g)}),p.querySelectorAll(".section-audio-btn").forEach(h=>{h.addEventListener("click",_=>{_.stopPropagation(),te(h.getAttribute("data-text"))})}),(()=>{d.querySelector("#helper-toggle-outlines").addEventListener("change",h=>{k.settings.readingHelper.outlines=h.target.checked,h.target.checked&&(k.settings.readingHelper.pureText=!1,d.querySelector("#helper-toggle-pure").checked=!1),me(p)}),d.querySelector("#helper-toggle-translations").addEventListener("change",h=>{k.settings.readingHelper.translations=h.target.checked,h.target.checked&&(k.settings.readingHelper.pureText=!1,d.querySelector("#helper-toggle-pure").checked=!1),me(p)}),d.querySelector("#helper-toggle-target").addEventListener("change",h=>{k.settings.readingHelper.targetOnly=h.target.checked,me(p)}),d.querySelector("#helper-toggle-pure").addEventListener("change",h=>{k.settings.readingHelper.pureText=h.target.checked,h.target.checked&&(k.settings.readingHelper.outlines=!1,k.settings.readingHelper.translations=!1,d.querySelector("#helper-toggle-outlines").checked=!1,d.querySelector("#helper-toggle-translations").checked=!1),me(p)})})(),me(p),e.appendChild(p)}function ke(e,r,t){ie=t+1,ie>=r.sessions.length?(ue="complete",ge(e,r)):(ge(e,r),setTimeout(()=>{const a=document.getElementById(`session-card-${ie}`);a&&a.scrollIntoView({behavior:"smooth",block:"center"})},100))}function wn(e,r){const t=document.createElement("div");t.className="magazine-header",t.style.textAlign="center",t.style.marginBottom="2.5rem",t.innerHTML=`
    <div style="font-family: var(--font-serif); font-size: 0.85rem; letter-spacing: 2px; text-transform: uppercase; color: var(--color-text-muted); margin-bottom: 0.5rem; border-bottom: 2px double rgba(0,0,91,0.1); padding-bottom: 0.4rem; font-weight: bold;">
  ...
  `,e.appendChild(t);const a=document.createElement("div");a.className="newspaper-columns",a.style.fontSize="1.18rem",a.style.lineHeight="1.8",a.style.fontFamily="var(--font-serif)",a.style.color="var(--color-primary)",a.style.marginBottom="2.5rem";let n="";r.sessions.forEach((u,d)=>{n+=`
      <p class="${d===0?"editorial-dropcap":""}" style="margin-bottom: 1.5rem; text-align: justify; text-justify: inter-word;">
        ${u.text_fr.split(`
`).join("<br>")}
      </p>
    `}),a.innerHTML=n,e.appendChild(a);const o=document.createElement("div");o.style.display="flex",o.style.justifyContent="center",o.style.gap="1rem",o.style.marginTop="2rem";const m=document.createElement("button");m.className="next-btn",m.innerText="← Retour à l'aperçu (予習モードに戻る)",m.addEventListener("click",()=>{ue="preview",ie=0,ge(e,r)});const i=document.createElement("button");i.className="next-btn",i.style.backgroundColor="transparent",i.style.border="1px solid var(--color-primary)",i.style.color="var(--color-primary)",i.innerText="📚 Liste des articles (目次へ戻る)",i.addEventListener("click",()=>{Ue(e,[r])}),o.appendChild(m),o.appendChild(i),e.appendChild(o)}function Ve(e,r,t){we&&we.remove();const a=document.createElement("div");a.className="anatomy-tooltip card",a.style.position="absolute",a.style.zIndex="1000",a.style.padding="1rem",a.style.width="290px",a.style.boxShadow="var(--shadow-lg)",a.style.border="1px solid rgba(0, 0, 91, 0.15)",a.style.borderRadius="8px",a.style.backgroundColor="#FFFFFF";const n=r.vocab_id||r.gram_id,o=k.db.knowledge.find(_=>_.id===n),i={noun:"Nom (名詞)",verb:"Verbe (動詞)",preposition:"Préposition (前置詞)",article_partitive:"Article partitif (部分冠詞)",article:"Article (冠詞)",pronoun_subject:"Pronom (代名詞)",conjunction:"Conjonction (接続詞)",other:"Autre (その他)"}[r.pos]||r.pos||"Autre (その他)";let u="";if(r.analysis&&(u=`
    ...
    `),o){const _=!!o.french,g=`<span style="font-size: 0.85rem; font-weight: bold; color: var(--color-secondary);">${o.level}</span>`;if(_){const v=o.gender?o.gender==="m"?"Masculin":o.gender==="f"?"Féminin":"M/F":"",A=v?`<span style="font-size: 0.7rem; font-weight: 600; padding: 0.05rem 0.3rem; border-radius: 3px; background-color: #f0f4f8; color: var(--color-primary); border: 1px solid rgba(0,0,91,0.1);">${v}</span>`:"",L=o.verb_group?o.verb_group===1?"1er groupe":o.verb_group===2?"2e groupe":"3e groupe":"",P=L?`<span style="font-size: 0.7rem; font-weight: 600; padding: 0.05rem 0.3rem; border-radius: 3px; background-color: rgba(107,156,104,0.08); color: var(--color-success); border: 1px solid rgba(107,156,104,0.15);">${L}</span>`:"";a.innerHTML=`
    ...
    `;document.body.appendChild(a),we=a;const d=e.getBoundingClientRect(),p=d.bottom+window.scrollY+8,y=Math.max(10,Math.min(window.innerWidth-300,d.left+window.scrollX-100));a.style.top=`${p}px`,a.style.left=`${y}px`;const l=a.querySelector(".play-here-btn");l&&l.addEventListener("click",_=>{_.stopPropagation();const g=r.word.replace(/^[^\wÀ-ÿ]+|[^\wÀ-ÿ]+$/g,"");te(g)});const h=a.querySelector(".play-from-here-btn");h&&t&&h.addEventListener("click",_=>{_.stopPropagation(),te(t)})}const k={meta:null,db:{knowledge:[],quizzes:[]},loaded:{knowledge:new Set,quizzes:!1},favorites:new Set(JSON.parse(localStorage.getItem("cba_favorites")||"[]")),wrongAnswers:JSON.parse(localStorage.getItem("cba_wrong")||"[]"),streak:parseInt(localStorage.getItem("cba_streak")||"0"),lastStudyDate:localStorage.getItem("cba_last_study")||"",srs:JSON.parse(localStorage.getItem("cba_srs")||"{}"),settings:JSON.parse(localStorage.getItem("cba_settings")||JSON.stringify({targetLevel:"ALL",newCardsPerDay:5,maxReviewsPerDay:20,includeGeneral:!1}))};function be(e){k.favorites.has(e)?k.favorites.delete(e):k.favorites.add(e),localStorage.setItem("cba_favorites",JSON.stringify(Array.from(k.favorites)))}function fe(e){return k.favorites.has(e)}function st(e){k.wrongAnswers.includes(e)||(k.wrongAnswers.push(e),localStorage.setItem("cba_wrong",JSON.stringify(k.wrongAnswers)))}function Cn(e){k.wrongAnswers=k.wrongAnswers.filter(r=>r!==e),localStorage.setItem("cba_wrong",JSON.stringify(k.wrongAnswers))}function Sn(){const e=new Date().toISOString().split("T")[0];if(k.lastStudyDate!==e){if(k.lastStudyDate){const r=new Date(k.lastStudyDate),t=new Date(e),a=Math.abs(t-r),n=Math.ceil(a/(1e3*60*60*24));n===1?k.streak+=1:n>1&&(k.streak=1)}else k.streak=1;k.lastStudyDate=e,localStorage.setItem("cba_streak",k.streak.toString()),localStorage.setItem("cba_last_study",e)}}function He(e,r){const t=new Date().toISOString().split("T")[0],a=k.srs[e]||{easiness:2.5,interval:0,repetitions:0,dueDate:t};let n=Math.max(0,Math.min(5,r));n>=3?(a.repetitions===0?a.interval=1:a.repetitions===1?a.interval=6:a.interval=Math.round(a.interval*a.easiness),a.repetitions++):(a.repetitions=0,a.interval=1),a.easiness=a.easiness+(.1-(5-n)*(.08+(5-n)*.02)),a.easiness<1.3&&(a.easiness=1.3);const o=new Date;o.setDate(o.getDate()+a.interval),a.dueDate=o.toISOString().split("T")[0],a.lastRated=t,k.srs[e]=a,localStorage.setItem("cba_srs",JSON.stringify(k.srs))}function Ln(e){k.settings={...k.settings,...e},localStorage.setItem("cba_settings",JSON.stringify(k.settings))}async function oe(e,r){const t=r==="ALL"?["A1","A2","B1","B2","C1","C2"]:[r];for(const a of t)if(!k.loaded.knowledge.has(a))try{const n=await fetch(`data/knowledge_${a}.json`);if(!n.ok){console.warn(`Could not load data/knowledge_${a}.json`);continue}const o=await n.json(),m=new Set(k.db.knowledge.map(i=>i.id));for(const i of o)m.has(i.id)||(!i.context_fr&&i.examples&&i.examples.length>0&&(i.context_fr=i.examples[0].fr,i.context_en=i.examples[0].en,i.context_ja=i.examples[0].ja),k.db.knowledge.push(i));k.loaded.knowledge.add(a)}catch(n){console.error(`Failed to load knowledge level ${a}:`,n)}}async function Je(){if(!k.loaded.quizzes)try{const e=await fetch("data/quizzes.json");if(!e.ok)throw new Error("Network response was not ok");k.db.quizzes=await e.json(),k.loaded.quizzes=!0}catch(e){console.error("Failed to load quizzes:",e)}}async function Ye(){await Promise.all([oe("vocabulary","ALL"),oe("grammar","ALL"),oe("cuisine","ALL"),Je()])}const lt={home:Pt,vocabulary:Bt,grammar:Ht,reference:It,cuisine:tn,quiz:rn,favorites:$e,review:ln,search:dn,settings:mn,dictation:un,story:yn,reading:_n};function Ee(e){const r=document.getElementById("main-content");if(lt[e]){Sn(),r.innerHTML="";const t=lt[e]();t instanceof HTMLElement?r.appendChild(t):r.innerHTML=t,document.querySelectorAll(".nav-link").forEach(a=>{a.getAttribute("data-tab")===e?a.classList.add("active"):a.classList.remove("active")})}}async function jn(){if(!window.__app_initialized){window.__app_initialized=!0;try{const e=await fetch("data/meta.json");k.meta=await e.json();const r=document.getElementById("hamburger-menu-toggle"),t=document.querySelector(".nav-band");r&&t&&(r.addEventListener("click",a=>{a.stopPropagation(),t.classList.toggle("mobile-open"),r.classList.toggle("active")}),document.addEventListener("click",a=>{!a.target.closest(".nav-band")&&!a.target.closest("#hamburger-menu-toggle")&&(t.classList.remove("mobile-open"),r.classList.remove("active"))})),document.querySelectorAll(".nav-link").forEach(a=>{a.addEventListener("click",n=>{const o=n.target.closest(".nav-link").getAttribute("data-tab");Ee(o),t&&t.classList.contains("mobile-open")&&(t.classList.remove("mobile-open"),r&&r.classList.remove("active"))})}),Ee("home"),Tn()}catch(e){console.error("Failed to load database metadata:",e),document.getElementById("main-content").innerHTML=`
      <div class="alert alert-info" style="border-left-color: var(--color-error); background-color: #FFEBEE; color: var(--color-error)">
