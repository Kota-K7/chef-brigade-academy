(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const o of i)if(o.type==="childList")for(const m of o.addedNodes)m.tagName==="LINK"&&m.rel==="modulepreload"&&n(m)}).observe(document,{childList:!0,subtree:!0});function t(i){const o={};return i.integrity&&(o.integrity=i.integrity),i.referrerPolicy&&(o.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?o.credentials="include":i.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(i){if(i.ep)return;i.ep=!0;const o=t(i);fetch(i.href,o)}})();let Ve=null,Ge="",We=0,ze=null,ce=null;function U(e,a=1){Xe(e,a)}function Xe(e,a=1,t=null,n=null,i=0){if(!("speechSynthesis"in window)){console.warn("Speech synthesis not supported on this browser.");return}window.speechSynthesis.cancel();const o=e.replace(/["'➔]/g,"").trim();Ge=o,We=i,ze=t,ce=n;const m=o.substring(i);if(!m.trim()){ce&&ce();return}const r=new SpeechSynthesisUtterance(m);r.lang="fr-FR",r.rate=a;const s=window.speechSynthesis.getVoices().find(u=>u.lang==="fr-FR"||u.lang.startsWith("fr"));s&&(r.voice=s),r.onboundary=u=>{if(u.name==="word"&&ze){const h=We+u.charIndex;ze(h,Ge.length)}},r.onend=()=>{Ve===r&&ce&&ce()},r.onerror=u=>{console.warn("SpeechSynthesis error:",u),ce&&ce()},Ve=r,window.speechSynthesis.speak(r)}function kt(){"speechSynthesis"in window&&window.speechSynthesis.pause()}function wt(){"speechSynthesis"in window&&window.speechSynthesis.resume()}function qe(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}"speechSynthesis"in window&&(window.speechSynthesis.getVoices(),window.speechSynthesis.onvoiceschanged=()=>{window.speechSynthesis.getVoices()});function Ct(){var S,E,d,p,y,k,v;const e=document.createElement("div"),a=document.createElement("h2");a.className="section-title",a.innerText="Tableau de Bord",e.appendChild(a);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Welcome back, Commis. Here is your kitchen curriculum status.",e.appendChild(t);const n=new Date().toISOString().split("T")[0],i=Object.values(C.srs),o=i.filter(w=>w.dueDate<=n).length,m=i.length,r=document.createElement("div");r.className="dashboard-grid";const g=document.createElement("div");g.style.display="flex",g.style.flexDirection="column",g.style.gap="1.5rem";const s=document.createElement("div");s.className="card",s.style.padding="1.5rem",s.innerHTML=`
  ...
  `,g.appendChild(s);const u=document.createElement("div");u.className="card",u.style.padding="1.5rem";const h=((S=C.meta)==null?void 0:S.featured)||null;u.innerHTML=`
    ...
    ${h?`
      <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem;">
        <div style="font-size: 1.25rem; font-weight: 600; color: var(--color-accent);">${h.french}</div>
        <button class="audio-btn" data-text="${h.french}" title="Listen pronunciation" style="background: none; border: none; font-size: 1.1rem; cursor: pointer; color: var(--color-accent); padding: 0.1rem; line-height: 1;">🔊</button>
      </div>
      <div style="font-size: 0.9rem; color: var(--color-text-main); font-weight: 500; margin-bottom: 0.5rem;">${h.english} / ${h.japanese}</div>
      <div style="background-color: rgba(197, 168, 128, 0.08); border-left: 2px solid var(--color-accent); padding: 0.8rem; font-size: 0.85rem; border-radius: 0 var(--radius-sm) var(--radius-sm) 0;">
        <div style="display: flex; align-items: flex-start; gap: 0.4rem; font-style: italic; color: var(--color-secondary); font-weight: 500;">
          <span style="flex: 1;">"${h.context_fr}"</span>
          <button class="audio-btn" data-text="${h.context_fr}" title="Listen context sentence" style="background: none; border: none; font-size: 0.9rem; cursor: pointer; color: var(--color-text-muted); padding: 0.15rem; margin-top: 0.1rem; line-height: 1;">🔊</button>
        </div>
        <div style="color: var(--color-text-muted); margin-top: 0.2rem;">${h.context_ja}</div>
      </div>
    `:"<p>No data loaded.</p>"}
  `,h&&u.querySelectorAll(".audio-btn").forEach(w=>{w.addEventListener("click",x=>{x.stopPropagation();const L=x.target.closest(".audio-btn").getAttribute("data-text");U(L)})}),g.appendChild(u),r.appendChild(g);const l=document.createElement("div");l.style.display="flex",l.style.flexDirection="column",l.style.gap="1.5rem";const f=document.createElement("div");f.className="streak-card",f.innerHTML=`
  ...
  `,l.appendChild(f);const b=document.createElement("div");b.className="card",b.style.padding="1.5rem";const c=((d=(E=C.meta)==null?void 0:E.counts)==null?void 0:d.vocabulary)||0,_=((y=(p=C.meta)==null?void 0:p.counts)==null?void 0:y.grammar)||0,j=((v=(k=C.meta)==null?void 0:k.counts)==null?void 0:v.cuisine)||0;return b.innerHTML=`
  ...
  `,l.appendChild(b),r.appendChild(l),e.appendChild(r),e}function Te(e){const a=Lt(e);return A[a]||""}function Lt(e){return e?e.trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]/g," ").replace(/\s+/g," ").trim():""}const F=(e,a,t)=>`
...
`;function me(e,a){const t=["L","M","M","J","V","S","D"];let n="";for(let i=0;i<7;i++){const o=24+i*28,m=i===e,r=m?"#e76f51":"#ffffff",g=m?"#ffffff":"#1c2833";n+=`
      <rect x="${o}" y="50" width="24" height="24" fill="${r}" ${m?'stroke="#e76f51" stroke-width="2"':'stroke="#1c2833" stroke-width="1.5"'} rx="4" />
      <text x="${o+12}" y="66" text-anchor="middle" font-size="10" font-family="-apple-system, BlinkMacSystemFont, sans-serif" font-weight="bold" fill="${g}">${t[i]}</text>
    `}return F(`calendar_${e}`,`
    ...
    ${n}
  `,a)}function ve(e,a){const t=["Avant-hier","Hier","Aujourd'hui","Demain","Après-demain"],n=["一昨日","昨日","今日","明日","明後日"];let i="";for(let m=0;m<5;m++){const r=25+m*47,g=m===e,s=g?"#e76f51":"#1c2833",u=g?7:4,h=g?8:7,l=g?"#e76f51":"#7f8c8d",f=g?"bold":"normal";i+=`
      <circle cx="${r}" cy="65" r="${u}" fill="${s}" />
      <text x="${r}" y="51" text-anchor="middle" font-size="${h}" font-family="-apple-system, BlinkMacSystemFont, sans-serif" font-weight="${f}" fill="${l}">${t[m]}</text>
      <text x="${r}" y="81" text-anchor="middle" font-size="7" font-family="-apple-system, BlinkMacSystemFont, sans-serif" font-weight="${f}" fill="${l}">${n[m]}</text>
    `}return F(`timeline_${e}`,`
    <!-- Timeline axis -->
  ...
  `,"UNDER / BENEATH (sous)"),dans:F("dans",`
    <!-- Bowl body -->
  ...
  `,"FROM / EXTRACT (de)"),devant:F("devant",`
  ...
  `,"IN FRONT OF (devant)"),derriere:F("derriere",`
  ...
  `,"AT THE SHOP / PLACE OF (chez)"),entre:F("entre",`
    <!-- Top Sheet -->
  ...
  `,"AGAINST / PRESSED (contre)"),depuis:F("depuis",`
    <!-- Horizontal timeline -->
  ...
  `,"SINCE / CONTINUING (depuis)"),pendant:F("pendant",`
    <!-- Time bar -->
  ...
  `,"FOR A DURATION (pendant)"),selon:F("selon",`
    <!-- Slider -->
  ...
  `,"ACCORDING TO / DEPENDING ON (selon)"),a_cote_de:F("a_cote_de",`
    <!-- Water Glass (left) -->
  ...
  `,"NEXT TO / BESIDE (à côté de)"),en_face_de:F("en_face_de",`
    <!-- Station 1 (left) -->
  ...
  `,"FACING / OPPOSITE (en face de)"),au_dessus_de:F("au_dessus_de",`
    <!-- Bowl at bottom -->
  ...
  `,"ABOVE / OVER (au-dessus de)"),au_dessous_de:F("au_dessous_de",`
    <!-- Limit line -->
  ...
  `,"BELOW / UNDER (au-dessous de)"),autour_de:F("autour_de",`
    <!-- Main Dish (center) -->
  ...
  `,"AROUND (autour de)"),au_milieu_de:F("au_milieu_de",`
    <!-- Plate -->
  ...
  `,"IN THE MIDDLE OF (au milieu de)"),pres_de:F("pres_de",`
    <!-- Hot stove (left) -->
  ...
  `,"NEAR / CLOSE TO (près de)"),loin_de:F("loin_de",`
    <!-- Food shelf (left) -->
  ...
  `,"FAR FROM (loin de)"),jusqu_a:F("jusqu_a",`
    <!-- Thermometer -->
  ...
  `,"UNTIL / UP TO LIMIT (jusqu'à)"),matin:F("matin",`
    <!-- Horizon line -->
  ...
  `,"NOON (midi)"),"apres midi":F("apres_midi",`
  ...
  `,"AFTERNOON (l'après-midi)"),soir:F("soir",`
    <!-- Horizon line -->
    <line x1="30" y1="85" x2="210" y2="85" stroke="#1c2833" stroke-width="2"/>
    <!-- Sun setting -->
  ...
  `,"NIGHT (la nuit)")};A.a=A.a;A.à=A.a;A.contre=A.contre;A.derriere=A.derriere;A.derrière=A.derriere;A["a cote de"]=A.a_cote_de;A["a coté de"]=A.a_cote_de;A["à côté de"]=A.a_cote_de;A["en face de"]=A.en_face_de;A["au dessus de"]=A.au_dessus_de;A["au-dessus de"]=A.au_dessus_de;A["au dessous de"]=A.au_dessous_de;A["au-dessous de"]=A.au_dessous_de;A["autour de"]=A.autour_de;A["au milieu de"]=A.au_milieu_de;A["pres de"]=A.pres_de;A["près de"]=A.pres_de;A["loin de"]=A.loin_de;A["jusqu a"]=A.jusqu_a;A["jusqu à"]=A.jusqu_a;A["jusqu'a"]=A.jusqu_a;A["jusqu'à"]=A.jusqu_a;A.lundi=me(0,"MONDAY (lundi)");A.mardi=me(1,"TUESDAY (mardi)");A.mercredi=me(2,"WEDNESDAY (mercredi)");A.jeudi=me(3,"THURSDAY (jeudi)");A.vendredi=me(4,"FRIDAY (vendredi)");A.samedi=me(5,"SATURDAY (samedi)");A.dimanche=me(6,"SUNDAY (dimanche)");A["avant hier"]=ve(0,"DAY BEFORE YESTERDAY (avant-hier)");A.hier=ve(1,"YESTERDAY (hier)");A["aujourd hui"]=ve(2,"TODAY (aujourd'hui)");A.demain=ve(3,"TOMORROW (demain)");A["apres demain"]=ve(4,"DAY AFTER TOMORROW (après-demain)");A["l apres midi"]=A["apres midi"];function St(){var o;const e=document.createElement("div"),a=document.createElement("h2");a.className="section-title",a.innerText="Vocabulaire Professionnel",e.appendChild(a);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Essential culinary terminology for kitchen brigade communications. Filter by category, tag, or difficulty level.",e.appendChild(t);const n=document.createElement("div");n.className="loading-placeholder",n.innerText="Chargement du vocabulaire... (Loading vocabulary...)",e.appendChild(n);const i=((o=C.settings)==null?void 0:o.targetLevel)||"ALL";return ee("knowledge",i).then(()=>{n.remove(),jt(e,i)}),e}function jt(e,a){var b,c;const t=((b=C.settings)==null?void 0:b.includeGeneral)||!1,i=(((c=C.db)==null?void 0:c.knowledge)||[]).filter(_=>_.french&&_.japanese).filter(_=>t||_.is_professional),o=["ALL",...new Set(i.map(_=>_.category))],m=["ALL",...new Set(i.flatMap(_=>_.tags||[]))],r=["ALL","A1","A2","B1","B2","C1","C2"];let g="ALL",s="ALL",u=a;const h=document.createElement("div");h.className="card",h.style.padding="1.2rem",h.style.marginBottom="2rem",h.style.display="flex",h.style.flexDirection="column",h.style.gap="1rem",h.innerHTML=`
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
      <div>
        ...
        <select id="vocab-category-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${o.map(_=>`<option value="${_}" ${_===g?"selected":""}>${_}</option>`).join("")}
        </select>
      </div>
      <div>
        ...
        <select id="vocab-tag-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${m.map(_=>`<option value="${_}" ${_===s?"selected":""}>${_==="ALL"?"ALL TAGS":"#"+_}</option>`).join("")}
        </select>
      </div>
      <div>
        ...
        <select id="vocab-level-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${r.map(_=>`<option value="${_}" ${_===u?"selected":""}>${_==="ALL"?"ALL LEVELS":_}</option>`).join("")}
        </select>
      </div>
    </div>
  `,e.appendChild(h);const l=document.createElement("div");l.className="card-grid",e.appendChild(l);function f(){var E;l.innerHTML="";const S=(((E=C.db)==null?void 0:E.knowledge)||[]).filter(d=>d.french&&d.japanese).filter(d=>t||d.is_professional).filter(d=>{const p=g==="ALL"||d.category===g,y=s==="ALL"||d.tags&&d.tags.includes(s),k=u==="ALL"||d.level===u;return p&&y&&k});if(S.length===0){l.innerHTML='<p style="color: var(--color-text-muted); grid-column: 1 / -1; text-align: center; padding: 2rem;">Aucun terme trouvé correspondant à vos critères de filtrage.</p>';return}S.forEach(d=>{const p=document.createElement("div");p.className="card";const y=se(d.id),k=C.srs[d.id],v=!!k,w=d.examples&&d.examples[0]?d.examples[0].fr:"",x=d.examples&&d.examples[0]?d.examples[0].ja:"",L=Te(d.french),T=L?`<div class="preposition-illustration-container" style="height: 130px; background-color: #fcfbfa; border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; margin: 0.8rem 0; overflow: hidden; border: 1px solid rgba(0,0,0,0.03);">${L}</div>`:"";let $="";if(d.gender){const q=d.gender==="m"?"Masculin":d.gender==="f"?"Féminin":d.gender==="m/f"?"M/F":d.gender;$=`<span style="font-size: 0.72rem; font-weight: 600; padding: 0.1rem 0.45rem; border-radius: 4px; display: inline-block; ${d.gender==="m"?"background-color: rgba(0, 0, 91, 0.05); color: var(--color-primary); border: 1px solid rgba(0, 0, 91, 0.12);":d.gender==="f"?"background-color: rgba(220, 38, 38, 0.05); color: var(--color-secondary); border: 1px solid rgba(220, 38, 38, 0.12);":"background-color: rgba(90, 106, 128, 0.05); color: var(--color-text-muted); border: 1px solid rgba(90, 106, 128, 0.12);"}">${q}</span>`}let P="";if(d.verb_group){let q="";d.verb_group===1||d.verb_group==="1"?q="1er groupe (-er)":d.verb_group===2||d.verb_group==="2"?q="2e groupe (-ir)":d.verb_group===3||d.verb_group==="3"?q="3e groupe (Irrégulier)":q=`${d.verb_group}e groupe`,P=`<span style="font-size: 0.72rem; font-weight: 600; padding: 0.1rem 0.45rem; border-radius: 4px; display: inline-block; background-color: rgba(107, 156, 104, 0.07); color: var(--color-success); border: 1px solid rgba(107, 156, 104, 0.15);">${q}</span>`}p.innerHTML=`
          ...
          <div>
            ...
            ${d.tags&&d.tags.length>0?`
              <div style="margin-top: 1rem; display: flex; flex-wrap: wrap; gap: 0.3rem;">
                ${d.tags.map(q=>`<span class="tag-badge" style="background-color: rgba(197, 168, 128, 0.08); color: var(--color-accent); font-size: 0.75rem; padding: 0.1rem 0.5rem; border-radius: 12px;">#${q}</span>`).join("")}
              </div>
            `:""}
          </div>
      ...
      `;const M=p.querySelector(".flip-translation-container");M.addEventListener("click",q=>{q.stopPropagation(),M.querySelector(".flip-translation-card").classList.toggle("flipped")}),p.querySelector(".fav-btn").addEventListener("click",q=>{q.stopPropagation(),de(d.id);const z=q.target,H=se(d.id);z.classList.toggle("active",H),z.innerText=H?"★":"☆"}),p.querySelectorAll(".audio-btn").forEach(q=>{q.addEventListener("click",z=>{z.stopPropagation();const H=z.target.closest(".audio-btn").getAttribute("data-text");U(H)})}),p.querySelectorAll(".srs-action-btn").forEach(q=>{q.addEventListener("click",z=>{z.stopPropagation();const H=z.target.getAttribute("data-action"),V=z.target.getAttribute("data-id");H==="start"?Se(V,4):H==="memorized"?Se(V,5):H==="reset"&&Se(V,0),f()})}),l.appendChild(p)})}h.querySelector("#vocab-category-select").addEventListener("change",_=>{g=_.target.value,f()}),h.querySelector("#vocab-tag-select").addEventListener("change",_=>{s=_.target.value,f()}),h.querySelector("#vocab-level-select").addEventListener("change",async _=>{var w;u=_.target.value,l.innerHTML=`<div style="grid-column: 1 / -1; text-align: center; padding: 2rem; color: var(--color-text-muted);">Chargement (Loading level ${u})...</div>`,await ee("knowledge",u);const S=(((w=C.db)==null?void 0:w.knowledge)||[]).filter(x=>x.french&&x.japanese).filter(x=>t||x.is_professional),E=["ALL",...new Set(S.map(x=>x.category))],d=["ALL",...new Set(S.flatMap(x=>x.tags||[]))],p=h.querySelector("#vocab-category-select"),y=p.value;p.innerHTML=E.map(x=>`<option value="${x}" ${x===y?"selected":""}>${x}</option>`).join(""),g=p.value;const k=h.querySelector("#vocab-tag-select"),v=k.value;k.innerHTML=d.map(x=>`<option value="${x}" ${x===v?"selected":""}>${x==="ALL"?"ALL TAGS":"#"+x}</option>`).join(""),s=k.value,f()}),f()}function Tt(){var o;const e=document.createElement("div"),a=document.createElement("h2");a.className="section-title",a.innerText="Grammaire de la Cuisine",e.appendChild(a);const t=document.createElement("p");t.className="section-subtitle",t.innerText="French grammar concepts framed through recipe instructions and professional dialogues.",e.appendChild(t);const n=document.createElement("div");n.className="loading-placeholder",n.innerText="Chargement de la grammaire... (Loading grammar...)",e.appendChild(n);const i=((o=C.settings)==null?void 0:o.targetLevel)||"ALL";return ee("knowledge",i).then(()=>{n.remove(),$t(e,i)}),e}function $t(e,a){var i;const n=(((i=C.db)==null?void 0:i.knowledge)||[]).filter(o=>o.grammar).map(o=>({id:o.id,category:o.category,french:o.french,level:o.level,topic:o.grammar.topic,explanation_en:o.grammar.explanation_en,explanation_ja:o.grammar.explanation_ja,examples:o.examples||[]})).filter(o=>a==="ALL"||o.level===a);if(n.length===0){const o=document.createElement("p");o.style.color="var(--color-text-muted)",o.innerText="Aucune leçon de grammaire chargée pour ce niveau.",e.appendChild(o);return}n.forEach(o=>{const m=document.createElement("div");m.className="grammar-card";const r=se(o.id),g=o.topic.match(/\(([^)]+)\)/),s=g?g[1]:o.topic,u=Te(o.french||""),h=u?`<div class="preposition-illustration-container" style="height: 130px; background-color: #fcfbfa; border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; margin: 0.8rem 0; overflow: hidden; border: 1px solid rgba(0,0,0,0.03);">${u}</div>`:"";m.innerHTML=`
            ...
            ${o.examples.map(c=>`
              <div class="example-item" style="margin-bottom: 0.8rem;">
                <div class="example-fr" style="display: flex; align-items: center; gap: 0.4rem;">
                  <span style="flex: 1;">➔ ${c.fr}</span>
                  <button class="audio-btn" data-text="${c.fr}" title="Listen pronunciation" style="background: none; border: none; font-size: 0.9rem; cursor: pointer; color: var(--color-accent); padding: 0.15rem; line-height: 1;">🔊</button>
                </div>
                <div class="example-en">${c.en}</div>
                <div class="example-ja">${c.ja}</div>
              </div>
            `).join("")}
          </div>
        </div>
      </div>
    `;const l=m.querySelector(".grammar-header"),f=m.querySelector(".grammar-body"),b=m.querySelector(".toggle-icon");l.addEventListener("click",c=>{if(c.target.classList.contains("fav-btn")||c.target.closest(".audio-btn"))return;const _=f.classList.toggle("open");b.innerText=_?"▲":"▼"}),m.querySelector(".fav-btn").addEventListener("click",c=>{c.stopPropagation(),de(o.id);const _=c.target,j=se(o.id);_.classList.toggle("active",j),_.innerText=j?"★":"☆"}),m.querySelectorAll(".audio-btn").forEach(c=>{c.addEventListener("click",_=>{_.stopPropagation();const j=_.target.closest(".audio-btn").getAttribute("data-text");U(j)})}),e.appendChild(m)})}function Et(){const e=document.createElement("div");e.className="reference-container-view";const a=document.createElement("h2");a.className="section-title",a.innerText="Référence Grammaticale (Grammar Reference)",e.appendChild(a);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Explore structured conjugations, classifications, and auxiliary usage patterns in culinary French.",e.appendChild(t);const n=document.createElement("div");return n.className="loading-placeholder",n.innerText="Chargement des références... (Loading references...)",e.appendChild(n),fetch("data/grammar_reference.json").then(i=>{if(!i.ok)throw new Error("Could not load reference data");return i.json()}).then(i=>{n.remove(),zt(e,i)}).catch(i=>{n.innerText="Erreur lors du chargement des données. (Error loading reference data.)",console.error(i)}),e}function zt(e,a){const t=document.createElement("div");t.className="ref-layout";const n=document.createElement("div");n.className="ref-sidebar";const i=document.createElement("div");i.className="ref-content-panel",a.forEach((o,m)=>{const r=document.createElement("button");r.className=`ref-menu-btn ${m===0?"active":""}`,r.innerHTML=`
    ...
    `,r.addEventListener("click",()=>{document.querySelectorAll(".ref-menu-btn").forEach(g=>g.classList.remove("active")),r.classList.add("active"),Qe(i,o),window.innerWidth<=768&&i.scrollIntoView({behavior:"smooth"})}),n.appendChild(r)}),t.appendChild(n),t.appendChild(i),e.appendChild(t),a.length>0&&Qe(i,a[0])}function Qe(e,a){e.innerHTML="",e.style.animation="fadeIn 0.3s ease";const t=document.createElement("div");t.className="ref-topic-header",t.innerHTML=`
  ...
  `,e.appendChild(n),a.sections.forEach(i=>{const o=document.createElement("div");if(o.className="ref-section-container",i.type==="table"){const m=document.createElement("h4");m.className="ref-sec-title",m.innerText=i.title,o.appendChild(m);const r=document.createElement("div");r.className="ref-table-wrapper";const g=document.createElement("table");g.className="ref-table";const s=[],u=document.createElement("tr");if(i.headers.forEach((h,l)=>{const f=document.createElement("th");h==="発音"?(s.push(l),f.innerText="音声",f.style.textAlign="center"):f.innerText=h,u.appendChild(f)}),g.appendChild(u),i.rows.forEach(h=>{const l=document.createElement("tr");h.forEach((f,b)=>{const c=document.createElement("td");if(s.includes(b)){const _=h[b-1]?h[b-1].split("(")[0].trim():"";c.innerHTML=`<button class="ref-table-audio-btn" data-speak="${_}" title="Listen pronunciation">🔊</button>`,c.style.textAlign="center"}else c.innerText=f,b===1&&(i.title.includes("Conjugaison")||i.title.includes("Présent")||i.title.includes("Exemple"))&&(c.style.position="relative",c.innerHTML=`
                <span style="margin-right: 1.5rem;">${f}</span>
                <button class="ref-table-audio-btn" data-speak="${f.split("(")[0].trim()}" title="Listen pronunciation">🔊</button>
              `);l.appendChild(c)}),g.appendChild(l)}),r.appendChild(g),o.appendChild(r),i.example){const h=document.createElement("div");h.className="ref-table-attached-ex",h.innerHTML=`
          <div style="display: flex; align-items: center; justify-content: space-between;">
            <div class="ref-ex-fr">➔ ${i.example.fr}</div>
            <button class="audio-btn" data-speak="${i.example.fr}">🔊</button>
          </div>
          <div class="ref-ex-ja">${i.example.ja}</div>
        `,o.appendChild(h)}}else if(i.type==="examples"){const m=document.createElement("h4");m.className="ref-sec-title",m.innerText=i.title,o.appendChild(m);const r=document.createElement("div");r.className="ref-examples-list",i.examples.forEach(g=>{const s=document.createElement("div");s.className="ref-example-item",s.innerHTML=`
          <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem;">
            <span class="ref-ex-fr">➔ ${g.fr}</span>
            <button class="audio-btn" data-speak="${g.fr}">🔊</button>
          </div>
          <div class="ref-ex-ja">${g.ja}</div>
        `,r.appendChild(s)}),o.appendChild(r)}else if(i.type==="illustrations_grid"){const m=document.createElement("h4");m.className="ref-sec-title",m.innerText=i.title,o.appendChild(m);const r=document.createElement("div");r.className="ref-illustrations-grid",r.style.display="grid",r.style.gridTemplateColumns="repeat(auto-fill, minmax(180px, 1fr))",r.style.gap="1rem",r.style.marginTop="1rem",i.items.forEach(g=>{const s=document.createElement("div");s.className="ref-illustration-card",s.style.border="1px solid rgba(0,0,0,0.06)",s.style.borderRadius="8px",s.style.padding="0.8rem 0.5rem",s.style.backgroundColor="#fcfbfa",s.style.display="flex",s.style.flexDirection="column",s.style.alignItems="center",s.style.transition="transform 0.2s ease, box-shadow 0.2s ease",s.addEventListener("mouseenter",()=>{s.style.transform="translateY(-2px)",s.style.boxShadow="0 4px 12px rgba(0,0,0,0.05)"}),s.addEventListener("mouseleave",()=>{s.style.transform="none",s.style.boxShadow="none"});const u=Te(g.word);s.innerHTML=`
        ...
        `,r.appendChild(s)}),o.appendChild(r)}else if(i.type==="info"){const m=document.createElement("div");m.className="ref-info-box",m.innerHTML=`
        <h4 class="ref-info-title">💡 ${i.title}</h4>
        <p class="ref-info-fr">${i.content_fr}</p>
        <p class="ref-info-ja">${i.content_ja}</p>
      `,o.appendChild(m)}e.appendChild(o)}),e.querySelectorAll(".audio-btn, .ref-table-audio-btn").forEach(i=>{i.addEventListener("click",o=>{o.stopPropagation();const m=i.getAttribute("data-speak");U(m),i.style.transform="scale(1.2)",setTimeout(()=>i.style.transform="none",150)})})}function qt(e){var t;e.innerHTML='<div style="text-align: center; padding: 2rem; color: var(--color-text-muted);">Chargement de la théorie... (Loading theory...)</div>';const a=((t=C.settings)==null?void 0:t.targetLevel)||"ALL";ee("knowledge",a).then(()=>{var m;e.innerHTML="";const i=(((m=C.db)==null?void 0:m.knowledge)||[]).filter(r=>r.cuisine).map(r=>({id:r.id,level:r.level,category:r.category||"Theory",topic:r.cuisine.topic,content_fr:r.cuisine.content_fr,content_en:r.cuisine.content_en,content_ja:r.cuisine.content_ja})).filter(r=>a==="ALL"||r.level===a);if(i.length===0){e.innerHTML='<p style="color: var(--color-text-muted);">Aucun document de théorie culinaire disponible pour ce niveau.</p>';return}const o=document.createElement("div");o.style.display="flex",o.style.flexDirection="column",o.style.gap="2rem",i.forEach(r=>{const g=document.createElement("div");g.className="card",g.style.display="block",g.style.padding="2rem";const s=se(r.id),u=r.topic.match(/^([^(]+)/),h=u?u[1].trim():r.topic;g.innerHTML=`
        ...
        <div class="term-header" style="border-bottom: 1px solid rgba(197, 168, 128, 0.15); padding-bottom: 0.8rem; margin-bottom: 1.5rem;">
          <div style="display: flex; align-items: center; gap: 0.6rem; flex-wrap: wrap;">
            ...
            <button class="audio-btn" data-text="${h}" title="Listen topic pronunciation" style="background: none; border: none; font-size: 1.1rem; cursor: pointer; color: var(--color-accent); padding: 0.15rem; line-height: 1;">🔊</button>
          </div>
          <button class="fav-btn ${s?"active":""}" data-id="${r.id}" style="font-size: 1.3rem;">
            ${s?"★":"☆"}
          </button>
        </div>
  ...
  `,a.appendChild(t);const n=document.createElement("div");n.className="meat-type-tabs",n.style.marginTop="0.5rem",n.style.borderBottom="none",a.appendChild(n);const i=document.createElement("div");i.className="meat-display-area",a.appendChild(i),e.appendChild(a);const o={primary:{title:"主要食肉 (Primary Meats)",subCategories:[{key:"beef",label:"🐂 Bœuf (牛)",cuts:et,img:"assets/beef_cuts.png",placeholder:"Coupe+de+Boeuf",pdf:"beef_maff_guide.pdf",pdfText:"📄 日本農水省 牛肉部位基準 (PDF)"},{key:"porc",label:"🐖 Porc (豚)",cuts:nt,img:"assets/porc_cuts.png",placeholder:"Coupe+de+Porc",pdf:"pork_maff_guide.pdf",pdfText:"📄 日本農水省 豚肉部位基準 (PDF)",quote:'"Tout est bon dans le cochon" (豚はすべてが使える食材である)'},{key:"volaille",label:"🐓 Volaille (鶏)",cuts:tt,img:"assets/poultry_cuts.png",placeholder:"Coupe+de+Volaille",pdf:"poultry_maff_guide.pdf",pdfText:"📄 日本農水省 鶏肉部位基準 (PDF)",quote:'"La volaille est la reine des cuisines et la directrice des banquets" (鶏肉は厨房の女王であり、宴の演出家である)'}]},livestock:{title:"その他の家畜肉 (Other Livestock)",subCategories:[{key:"agneau",label:"🐏 Agneau (羊)",cuts:at,img:"assets/agneau_cuts.png",placeholder:"Agneau"},{key:"veau",label:"🐂 Veau (仔牛)",cuts:it,img:"assets/veal_cuts.png",placeholder:"Veau"},{key:"canard",label:"🦆 Canard (鴨)",cuts:rt,img:"assets/canard_cuts.png",placeholder:"Canard"}]},game:{title:"ジビエ (Game Meats)",subCategories:[{key:"chevreuil",label:"🦌 Chevreuil (鹿)",cuts:ot,img:"assets/game_cuts.png",placeholder:"Chevreuil"},{key:"sanglier",label:"🐗 Sanglier (猪)",cuts:st,img:"assets/game_cuts.png",placeholder:"Sanglier"},{key:"pigeon",label:"🕊️ Pigeon (鳩)",cuts:lt,img:"assets/game_cuts.png",placeholder:"Pigeon"},{key:"lievre",label:"🐇 Lièvre (野ウサギ)",cuts:ct,img:"assets/game_cuts.png",placeholder:"Lievre"}]},seafood:{title:"魚介類 (Seafood)",subCategories:[{key:"poisson",label:"🐟 Poisson (魚)",cuts:dt,img:"assets/seafood_cuts.png",placeholder:"Poisson"},{key:"crustaces",label:"🦞 Crustacés (甲殻類)",cuts:mt,img:"assets/seafood_cuts.png",placeholder:"Crustaces"},{key:"coquillages",label:"🐚 Coquillages (貝類)",cuts:ut,img:"assets/seafood_cuts.png",placeholder:"Coquillages"},{key:"mollusques",label:"🐙 Mollusques (軟体類)",cuts:pt,img:"assets/seafood_cuts.png",placeholder:"Mollusques"}]},cheese_wine:{title:"チーズ・ワイン (Cheese & Wine)",subCategories:[{key:"fromages_fr",label:"🇫🇷 France (仏チーズ)",cuts:Re,img:"assets/cheese_wine.png",placeholder:"Fromages"},{key:"fromages_world",label:"🌐 Monde (世界チーズ)",cuts:He,img:"assets/cheese_wine.png",placeholder:"Fromages",hidePins:!0},{key:"fromages_classif",label:"🔬 Classification (製法分類)",cuts:gt,img:"assets/cheese_wine.png",placeholder:"Fromages",hidePins:!0},{key:"vins",label:"🍷 Vins (ワイン)",cuts:Ie,img:"assets/cheese_wine.png",placeholder:"Vins"}]},vegetables:{title:"野菜 (Vegetables)",subCategories:[{key:"legumes",label:"🥦 Légumes (野菜全般)",cuts:ft,img:"assets/vegetables.png",placeholder:"Legumes"}]},herbs_spices:{title:"ハーブ・スパイス (Herbs & Spices)",subCategories:[{key:"herbes",label:"🌿 Herbes (香草)",cuts:ht,img:"assets/herbs_spices.png",placeholder:"Herbes"},{key:"epices",label:"🌶️ Épices (香辛料)",cuts:yt,img:"assets/herbs_spices.png",placeholder:"Epices"}]},fruits_mushrooms:{title:"果物・きのこ (Fruits & Mushrooms)",subCategories:[{key:"fruits",label:"🍎 Fruits (果物)",cuts:bt,img:"assets/fruits_mushrooms.png",placeholder:"Fruits"},{key:"champignons",label:"🍄 Champignons (きのこ)",cuts:_t,img:"assets/fruits_mushrooms.png",placeholder:"Champignons"}]}};function m(r){const g=o[r];n.innerHTML="",g.subCategories.forEach((s,u)=>{const h=document.createElement("button");h.className=`meat-tab-btn ${u===0?"active":""}`,h.setAttribute("data-sub",s.key),h.innerText=s.label,h.style.fontSize="0.8rem",h.style.padding="0.3rem 0.8rem",h.addEventListener("click",l=>{n.querySelectorAll(".meat-tab-btn").forEach(f=>f.classList.remove("active")),h.classList.add("active"),Ue(s,i)}),n.appendChild(h)}),g.subCategories.length<=1?n.style.display="none":n.style.display="flex",Ue(g.subCategories[0],i)}t.querySelectorAll(".meat-tab-btn").forEach(r=>{r.addEventListener("click",g=>{t.querySelectorAll(".meat-tab-btn").forEach(u=>u.classList.remove("active")),r.classList.add("active");const s=r.getAttribute("data-category");m(s)})}),m("primary")}function Ue(e,a){a.innerHTML="";const t=document.createElement("div");t.style.display="flex",t.style.flexDirection="column",t.style.gap="1.5rem";let n="";e.quote&&(n+=`
  ...
  `,e.appendChild(a);let t=document.getElementById("map-floating-tooltip");t||(t=document.createElement("div"),t.id="map-floating-tooltip",t.style.position="fixed",t.style.pointerEvents="none",t.style.zIndex="9999",t.style.padding="0.5rem 0.8rem",t.style.background="rgba(10, 25, 49, 0.95)",t.style.border="1px solid var(--color-accent)",t.style.borderRadius="var(--radius-sm)",t.style.color="#fff",t.style.fontSize="0.75rem",t.style.boxShadow="0 4px 12px rgba(0, 0, 0, 0.15)",t.style.display="none",t.style.fontFamily="var(--font-main)",t.style.lineHeight="1.3",document.body.appendChild(t));const n=a.querySelectorAll(".segment-btn");n.forEach(s=>{s.addEventListener("click",u=>{n.forEach(h=>h.classList.remove("active")),u.target.classList.add("active"),B.activeMap=u.target.getAttribute("data-map"),B.selectedRegionId=null,B.selectedItemId=null,m()})});const i=a.querySelectorAll(".theme-tab-btn");i.forEach(s=>{s.addEventListener("click",u=>{i.forEach(h=>h.classList.remove("active")),u.target.classList.add("active"),B.activeTheme=u.target.getAttribute("data-theme"),B.selectedItemId=null,m()})}),o().then(()=>{m()});async function o(){try{const s=["data/regions_administrative.json","data/regions_cultural.json","data/dishes.json","data/cheeses.json","data/wines.json"],u=await Promise.all(s.map(h=>fetch(h)));for(const h of u)if(!h.ok)throw new Error(`Failed to load ${h.url}`);pe=await u[0].json(),ge=await u[1].json(),ke=await u[2].json(),we=await u[3].json(),Ce=await u[4].json(),console.log("Map engine data loaded successfully.")}catch(s){console.error("Failed to load map JSON datasets:",s),pe=[],ge=[]}}function m(){if(!(pe.length>0&&ge.length>0)){a.querySelector("#regions-list-group").innerHTML='<p style="font-size:0.8rem;color:var(--color-text-muted);padding:1rem;">読み込み中...</p>';return}const u=a.querySelector("#legacy-map-illustration"),h=a.querySelector("#classic-map-decorations"),l=a.querySelector("#interactive-france-svg");B.activeMap==="cultural"?(u.style.display="block",h.style.display="none",l.setAttribute("viewBox","0 0 600 600")):(u.style.display="none",h.style.display="block",l.setAttribute("viewBox","0 0 1000 1000"));const f=B.activeMap==="admin"?pe:ge,b=a.querySelector("#map-paths-group"),c=B.activeMap==="cultural";b.innerHTML=f.map(d=>{const p=B.selectedRegionId===d.region_id,y=Ot[d.region_id]||"rgba(197, 168, 128, 0.1)",k=c?"none":y,v=c?"none":"rgba(92, 64, 51, 0.65)",w=c?0:1,x=p?"rgba(197, 168, 128, 0.45)":k,L=p?"var(--color-primary)":v,T=p?3.2:c?0:2.2;return`
        <path 
          class="region-path ${p?"active":""}" 
          d="${d.geometry}" 
          id="${d.region_id}" 
          data-id="${d.region_id}"
          style="fill: ${x}; fill-opacity: ${p?.65:w}; stroke: ${L}; stroke-width: ${T}; cursor: pointer; transition: fill 0.25s, fill-opacity 0.25s, stroke 0.25s;"
        />
      `}).join("");const _=a.querySelector("#map-labels-group");c?_.innerHTML="":_.innerHTML=f.map(d=>{const p=d.french_name,y=p.length>20?11.5:p.length>12?12.5:14.5,k=y*.65,v=Math.max(90,p.length*k+20),w=y+8,x=d.label_y-w/2-1,L=d.label_y+y/3+.5;return`
        ...
        `}).join("");const j=a.querySelector("#map-pins-group");if(j.innerHTML="",B.activeTheme!=="overview"){let d=[],p="var(--color-accent)";B.activeTheme==="cheeses"?(d=we,p="#EAA812"):B.activeTheme==="wines"?(d=Ce,p="#A91B3E"):B.activeTheme==="dishes"&&(d=ke,p="#2F80ED");const y=f.map(k=>k.region_id);j.innerHTML=d.map(k=>{const v=Ae(k.region,B.activeMap);if(!v||!y.includes(v))return"";const w=f.find(P=>P.region_id===v);if(!w)return"";const x=B.activeMap==="cultural"?6:10;let L=k.pin.x*x,T=k.pin.y*x;if(B.activeMap==="admin"){L=L*.15+w.label_x*.85,T=T*.15+w.label_y*.85;const P=k.id.length*5%12-6;L+=P,T+=k.id.length*11%12-6}else{const P=k.id.length*3%6-3;L+=P,T+=k.id.length*5%6-3}const $=B.selectedItemId===k.id;return`
          <g class="map-pin-marker" data-item-id="${k.id}" data-region-id="${v}" style="cursor:pointer; z-index: 10;">
            <circle cx="${L}" cy="${T}" r="${$?"9":"6"}" fill="${p}" stroke="#fff" stroke-width="1.5" />
            ${$?`<circle cx="${L}" cy="${T}" r="14" fill="none" stroke="${p}" stroke-width="1.5" stroke-dasharray="2,2" opacity="0.8" />`:""}
          </g>
        `}).join("")}const S=a.querySelector("#sidebar-list-title");if(B.activeTheme==="overview")S.innerText="🗺️ 地方リスト (Régions)";else{const d={dishes:"🍽️ 郷土料理",cheeses:"🧀 特産チーズ",wines:"🍷 地方ワイン"};S.innerText=`${d[B.activeTheme]} リスト`}const E=a.querySelector("#regions-list-group");if(E.innerHTML="",B.activeTheme==="overview")E.innerHTML=f.map(d=>{const p=B.selectedRegionId===d.region_id,y=d.code||"";return`
        ...
        `}).join("");else{let d=[];B.activeTheme==="cheeses"?d=we:B.activeTheme==="wines"?d=Ce:B.activeTheme==="dishes"&&(d=ke);const p=f.map(k=>k.region_id),y=d.filter(k=>{const v=Ae(k.region,B.activeMap);return v&&p.includes(v)});E.innerHTML=y.map(k=>{const v=B.selectedItemId===k.id,w=Ae(k.region,B.activeMap),x=f.find(T=>T.region_id===w),L=x?x.japanese_name.replace("地域圏","").replace("地方",""):"";return`
          <button class="list-item-btn item-btn ${v?"active":""}" data-item-id="${k.id}" data-region-id="${w}">
            <span>
        ...
        `}).join("")}g(),r()}function r(){a.querySelectorAll(".region-path").forEach(l=>{l.addEventListener("mouseenter",f=>{const b=f.target.getAttribute("data-id"),_=(B.activeMap==="admin"?pe:ge).find(j=>j.region_id===b);_&&(t.innerHTML=`
            <div style="font-weight:700; border-bottom:1px solid rgba(255,255,255,0.3); padding-bottom:2px; margin-bottom:2px;">
              ${_.french_name}
            </div>
            <div style="font-size:0.7rem; opacity:0.85;">${_.japanese_name}</div>
            <div style="font-size:0.65rem; opacity:0.7; font-style:italic;">${_.english_name}</div>
          `,t.style.display="block")}),l.addEventListener("mousemove",f=>{t.style.left=f.clientX+15+"px",t.style.top=f.clientY+15+"px"}),l.addEventListener("mouseleave",()=>{t.style.display="none"}),l.addEventListener("click",f=>{const b=f.target.getAttribute("data-id");B.selectedRegionId=b,B.selectedItemId=null,m()})}),a.querySelectorAll(".list-item-btn").forEach(l=>{l.addEventListener("click",f=>{const b=f.target.closest(".list-item-btn");b.classList.contains("item-btn")?(B.selectedItemId=b.getAttribute("data-item-id"),B.selectedRegionId=b.getAttribute("data-region-id")):(B.selectedRegionId=b.getAttribute("data-id"),B.selectedItemId=null),m()})}),a.querySelectorAll(".map-pin-marker").forEach(l=>{l.addEventListener("click",f=>{const b=f.target.closest(".map-pin-marker");B.selectedItemId=b.getAttribute("data-item-id"),B.selectedRegionId=b.getAttribute("data-region-id"),m()})})}function g(){const s=a.querySelector("#map-detail-drawer"),u=a.querySelector("#map-region-title"),h=a.querySelector("#map-region-sub"),l=a.querySelector("#drawer-main-content"),f=a.querySelector("#map-audio-title-btn");if(f.style.display="none",B.selectedItemId){let b=null,c="";if(B.activeTheme==="cheeses"?(b=we.find(_=>_.id===B.selectedItemId),c="特産チーズ (Fromage AOP)"):B.activeTheme==="wines"?(b=Ce.find(_=>_.id===B.selectedItemId),c="地方ワイン (AOC Wine)"):B.activeTheme==="dishes"&&(b=ke.find(_=>_.id===B.selectedItemId),c="郷土料理 (Plat Régional)"),b){u.innerText=`${b.name_fr} (${b.name_ja})`,h.innerText=`${c} • ${b.region}`;let _="";if(b.properties){_+='<div style="display:flex; flex-wrap:wrap; gap:0.6rem; margin-bottom:0.5rem;">';for(const[S,E]of Object.entries(b.properties)){let d=S;S==="sweetness"?d="甘み":S==="alcohol"?d="アルコール":S==="body"?d="ボディ":S==="saltiness"?d="塩気":S==="aroma"?d="香り":S==="rarity"?d="希少性":S==="type"&&(d="タイプ"),_+=`
              <span style="font-size:0.75rem; background:rgba(10, 25, 49, 0.05); border:1px solid rgba(197, 168, 128, 0.25); padding:0.2rem 0.5rem; border-radius:3px; font-weight:600;">
                ${d}: <span style="color:var(--color-accent-hover);">${E}</span>
              </span>
            `}_+="</div>"}l.innerHTML=`
          ${_}
          ...
          ${b.classification?`
            <div>
              ...
              <p style="font-size:0.9rem; color:var(--color-text-main); font-weight:600;">${b.classification}</p>
            </div>
          `:""}
          ...
          <div>
            ...
            <p style="font-size:0.9rem; color:var(--color-text-main); line-height:1.5; text-align:justify;" id="map-desc-fr">${b.science||b.desc||""}</p>
          </div>
          ...
          ${b.cooking?`
            <div>
              ...
              <p style="font-size:0.9rem; color:var(--color-text-main); line-height:1.5;">${b.cooking}</p>
            </div>
          `:""}
          ...
          ${b.chef_note?`
            <div style="background:rgba(197, 168, 128, 0.08); border-left:3px solid var(--border-color); padding:0.6rem 0.8rem; border-radius:0 var(--radius-sm) var(--radius-sm) 0;">
              ...
              <p style="font-size:0.85rem; color:var(--color-primary); font-style:italic; line-height:1.4;">${b.chef_note}</p>
            </div>
          `:""}
        `,f.style.display="inline-block",f.onclick=()=>U(b.name_fr);const j=a.querySelector("#map-audio-desc-btn");j&&(j.onclick=()=>U(b.science||b.desc||"")),s.style.display="block";return}}if(B.selectedRegionId){const c=(B.activeMap==="admin"?pe:ge).find(_=>_.region_id===B.selectedRegionId);if(c){u.innerText=`${c.french_name} (${c.japanese_name})`,h.innerText=`${c.english_name} Region`;let _=Vt(c.region_id);l.innerHTML=`
          <div>
            <div style="display:flex; align-items:center; gap:0.5rem; margin-bottom:0.3rem;">
              <h4 style="font-size:0.75rem; text-transform:uppercase; color:var(--color-accent); font-weight:600;">Français</h4>
              <button class="audio-btn" id="map-audio-desc-btn" style="background:none; border:none; font-size:0.95rem; cursor:pointer; color:var(--color-accent);">🔊</button>
            </div>
            <p style="font-size:0.95rem; color:var(--color-primary); font-style:italic; line-height:1.5; text-align:justify;" id="map-desc-fr">${_.desc_fr}</p>
          </div>
          <div>
            ...
            <p style="font-size:0.9rem; color:var(--color-text-main); line-height:1.5; text-align:justify;">${_.desc_en}</p>
          </div>
          <div>
            <h4 style="font-size:0.75rem; text-transform:uppercase; color:var(--color-accent); font-weight:600; margin-bottom:0.3rem;">日本語解説</h4>
            <p style="font-size:0.9rem; color:var(--color-text-main); line-height:1.5; text-align:justify;">${_.desc_ja}</p>
          </div>
  ...
  `,e.appendChild(n);const i=document.createElement("div");i.className="cuisine-content-wrapper",e.appendChild(i),n.querySelectorAll(".cuisine-tab-btn").forEach(m=>{m.addEventListener("click",r=>{n.querySelectorAll(".cuisine-tab-btn").forEach(s=>s.classList.remove("active")),r.target.classList.add("active");const g=r.target.getAttribute("data-tab");o(g)})});function o(m){i.innerHTML="",m==="theory"?qt(i):m==="ingredients"?Nt(i):m==="map"&&Dt(i)}return o("theory"),e}function ne(e){const a=[...e];for(let t=a.length-1;t>0;t--){const n=Math.floor(Math.random()*(t+1));[a[t],a[n]]=[a[n],a[t]]}return a}function Ye(e){return e.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().trim().replace(/[-\s]+/g," ")}function Wt(){if(document.getElementById("quiz-dynamic-styles"))return;const e=document.createElement("style");e.id="quiz-dynamic-styles",e.innerHTML=`
    .quiz-mode-selector {
      display: flex;
  ...
  `,document.head.appendChild(e)}function Qt(){var m;const e=[],a=((m=C.db)==null?void 0:m.knowledge)||[],t=a.filter(r=>r.french&&r.japanese),n=a.filter(r=>r.grammar),i=a.filter(r=>r.cuisine);function o(r,g,s,u){const l=r.filter(b=>b.id!==s).map(u).sort(()=>.5-Math.random()),f=[...new Set(l)].filter(Boolean).slice(0,g);for(;f.length<g;)f.push("Autre option "+(f.length+1));return f}return t.forEach(r=>{var l,f,b,c,_;let g="vocabulary";(l=r.tags)!=null&&l.includes("meat")?g="meat":(f=r.tags)!=null&&f.includes("sauces")||(b=r.tags)!=null&&b.includes("sauce")?g="sauces":(c=r.tags)!=null&&c.includes("cutting")?g="cuts":(_=r.tags)!=null&&_.includes("science")&&(g="science");const s=o(t,3,r.id,j=>j.japanese),u=r.examples&&r.examples[0]?r.examples[0].fr:"";e.push({id:`dyn_vocab_fr_ja_${r.id}`,type:"choice",category:g,question:`Que signifie le mot français "${r.french}" ? / What does the French word "${r.french}" mean?`,question_fr:`Que signifie le mot français "${r.french}" ?`,question_en:`What does the French word "${r.french}" mean?`,context:r.definition_fr||`Usage: ${u}`,options:ne([r.japanese,...s]),answer:r.japanese});const h=o(t,3,r.id,j=>j.french);e.push({id:`dyn_vocab_ja_fr_${r.id}`,type:"choice",category:g,question:`Quel est le mot français pour "${r.japanese}" ? / What is the French word for "${r.japanese}"?`,question_fr:`Quel est le mot français pour "${r.japanese}" ?`,question_en:`What is the French word for "${r.japanese}"?`,context:r.definition_fr||`Usage: ${u}`,options:ne([r.french,...h]),answer:r.french})}),n.forEach(r=>{if(!r.grammar.topic||!r.examples||r.examples.length===0)return;const g=o(n,3,r.id,s=>s.grammar.topic);e.push({id:`dyn_gram_topic_${r.id}`,type:"choice",category:"grammar",question:`De quel concept de grammaire s'agit-il : "${r.grammar.explanation_ja}" ? / Which grammar concept is this: "${r.grammar.explanation_en}"?`,question_fr:"De quel concept de grammaire s'agit-il ?",question_en:`Which grammar concept is this: "${r.grammar.explanation_en}"?`,context:`Niveau : ${r.level}. Indispensable pour la cuisine.`,options:ne([r.grammar.topic,...g]),answer:r.grammar.topic}),r.examples.forEach((s,u)=>{const l=n.flatMap(f=>f.examples||[]).filter(f=>f.fr!==s.fr).sort(()=>.5-Math.random()).map(f=>f.ja).slice(0,3);for(;l.length<3;)l.push("Option de traduction "+(l.length+1));e.push({id:`dyn_gram_ex_${r.id}_${u}`,type:"choice",category:"grammar",question:`Traduisez la phrase : "${s.fr}" / Translate the sentence: "${s.fr}"`,question_fr:`Traduisez la phrase : "${s.fr}"`,question_en:`Translate the sentence: "${s.fr}"`,context:`Grammaire: ${r.grammar.topic} (${r.level})`,options:ne([s.ja,...l]),answer:s.ja})})}),i.forEach(r=>{var u,h,l,f,b,c;if(!r.cuisine.topic||!r.cuisine.content_ja)return;const g=o(i,3,r.id,_=>_.cuisine.topic);let s="sauces";(u=r.tags)!=null&&u.includes("knife-cuts")||(h=r.tags)!=null&&h.includes("cuts")?s="cuts":(l=r.tags)!=null&&l.includes("meat")?s="meat":((f=r.tags)!=null&&f.includes("molecular")||(b=r.tags)!=null&&b.includes("chemistry")||(c=r.tags)!=null&&c.includes("science"))&&(s="science"),e.push({id:`dyn_cuis_topic_${r.id}`,type:"choice",category:s,question:`De quel concept culinaire s'agit-il : "${r.cuisine.content_ja.substring(0,120)}..." ? / Which culinary concept is described: "${r.cuisine.content_en.substring(0,120)}..."?`,question_fr:"De quel concept culinaire s'agit-il ?",question_en:"Which culinary concept is described here?",context:`Niveau : ${r.level}. Mots-clés : ${r.tags.join(", ")}`,options:ne([r.cuisine.topic,...g]),answer:r.cuisine.topic})}),e}function Ut(){var o;const e=[],a=((o=C.db)==null?void 0:o.knowledge)||[],t=[];a.forEach(m=>{(m.examples||[]).forEach(r=>{t.push({fr:r.fr,ja:r.ja})})});const n=t.sort(()=>.5-Math.random());for(const m of n){if(e.length>=4)break;const r=m.fr.replace(/^➔\s*/,"").trim();if(r.includes(" - ")){const g=r.split(" - ");e.push({id:`pair_${e.length}`,left:`🗣️ ${g[0].trim()}`,right:`💬 ${g[1].trim()}`,context:m.ja})}else if(r.includes(",")){const g=r.split(",");e.push({id:`pair_${e.length}`,left:`${g[0].trim()} ,`,right:`... ${g[1].trim()}`,context:m.ja})}else{const g=r.split(" ");if(g.length>=4){const s=Math.floor(g.length/2),u=g.slice(0,s).join(" "),h=g.slice(s).join(" ");e.push({id:`pair_${e.length}`,left:`${u} ...`,right:`... ${h}`,context:m.ja})}}}const i=[{left:"🗣️ Chaud devant !",right:"💬 Oui, chef !",context:"お通りです！ / はい、シェフ！"},{left:"🗣️ Entrée prête ?",right:"💬 Oui, j'envoie.",context:"前菜はできていますか？ / はい、送ります。"},{left:"🗣️ Combien de couverts ?",right:"💬 Vingt couverts.",context:"何名様（何席）ですか？ / 20席です。"},{left:"🗣️ Envoyez la sauce !",right:"💬 Tout de suite !",context:"ソースを出して！ / ただちに！"}];for(;e.length<4;){const m=i[e.length%i.length];e.push({id:`default_${e.length}`,left:m.left,right:m.right,context:m.context})}return e}function Jt(){var i;const e=document.createElement("div"),a=document.createElement("h2");a.className="section-title",a.innerText="Vérification des Connaissances",e.appendChild(a);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Test your professional French vocabulary, kitchen commands, and classical cuisine theory.",e.appendChild(t);const n=document.createElement("div");return n.className="loading-placeholder",n.innerText="Chargement du quiz... (Loading quiz...)",e.appendChild(n),Wt(),(i=C.settings)!=null&&i.targetLevel,Promise.all([Oe(),ee("vocabulary","ALL"),ee("grammar","ALL"),ee("cuisine","ALL")]).then(()=>{n.remove(),Yt(e)}),e}function Yt(e){let a="multiple",t="ALL";const n=document.createElement("div");n.className="quiz-mode-selector",n.innerHTML=`
  ...
  `,e.appendChild(n);const i=document.createElement("div");i.className="quiz-category-filter-wrapper",i.style.margin="1rem auto 1.5rem auto",i.style.display="flex",i.style.justifyContent="center",i.style.alignItems="center",i.style.gap="0.8rem",i.innerHTML=`
  ...
  `,e.appendChild(i);const o=i.querySelector("#quiz-cat-select");o.addEventListener("change",f=>{t=f.target.value,r()});const m=document.createElement("div");m.className="quiz-game-wrapper",e.appendChild(m),n.querySelectorAll(".mode-tab-btn").forEach(f=>{f.addEventListener("click",b=>{n.querySelectorAll(".mode-tab-btn").forEach(c=>c.classList.remove("active")),b.target.classList.add("active"),a=b.target.getAttribute("data-mode"),a==="matching_taking"?(o.disabled=!0,o.style.opacity="0.5"):(o.disabled=!1,o.style.opacity="1.0"),r()})});function r(){m.innerHTML="",a==="multiple"?g():a==="matching_vocab"?s():a==="matching_taking"?u():a==="spelling"&&l()}function g(){var p;const f=((p=C.db)==null?void 0:p.quizzes)||[],b=Qt();let c=[...f,...b];if(t!=="ALL"&&(c=c.filter(y=>y.category===t)),c.length===0){m.innerHTML=`
      ...
      `;return}const _=ne(c).slice(0,10);let j=0,S=0,E=!1;function d(){if(m.innerHTML="",E=!1,j>=_.length){const T=Math.round(S/_.length*100);let $="Apprenti (Apprentice)";T>=90?$="Chef de Partie (Station Chef)":T>=70&&($="Commis de Cuisine (Line Cook)"),m.innerHTML=`
          <div class="quiz-card" style="text-align: center;">
            ...
            <p style="font-size: 1.1rem; margin-bottom: 1.5rem;">Your Score: <strong>${S} / ${_.length}</strong> (${T}%)</p>
        ...
        `,m.querySelector("#restart-choice-btn").addEventListener("click",()=>{g()});return}const y=_[j],k=document.createElement("div");k.className="quiz-card",k.innerHTML=`
        <div class="quiz-meta">
          <span>Question ${j+1} of ${_.length}</span>
        ...
        <div class="quiz-options">
          ${y.options.map((T,$)=>`
            <button class="quiz-btn" data-index="${$}">${T}</button>
          `).join("")}
        </div>
      ...
      `;const v=k.querySelector(".quiz-hint-flip-container");v&&v.addEventListener("click",()=>{const T=v.querySelector(".q-en-hint-text"),$=T.style.display==="none";T.style.display=$?"block":"none",v.querySelector(".q-en-card").innerText=$?"💡 Hide English Hint":"💡 Translate (Show English Hint)"});const w=k.querySelectorAll(".quiz-btn"),x=k.querySelector(".quiz-feedback"),L=k.querySelector("#next-q-btn");w.forEach(T=>{T.addEventListener("click",$=>{if(E)return;E=!0;const M=$.target.innerText===y.answer;w.forEach(q=>{q.disabled=!0,q.innerText===y.answer&&q.classList.add("correct")}),M?S++:($.target.classList.add("incorrect"),Ke(y.id)),x.style.display="block",L.style.display="block"})}),L.addEventListener("click",()=>{j++,d()}),m.appendChild(k)}d()}function s(){var d,p;const f=((d=C.settings)==null?void 0:d.includeGeneral)||!1;let c=(((p=C.db)==null?void 0:p.vocabulary)||[]).filter(y=>f||y.is_professional);if(t!=="ALL"&&(t==="meat"?c=c.filter(y=>{var k,v,w,x;return((k=y.tags)==null?void 0:k.includes("meat"))||((v=y.tags)==null?void 0:v.includes("beef"))||((w=y.tags)==null?void 0:w.includes("pork"))||((x=y.tags)==null?void 0:x.includes("poultry"))||/viande|boeuf|porc|poulet/i.test(y.french)}):t==="sauces"?c=c.filter(y=>{var k,v,w;return((k=y.tags)==null?void 0:k.includes("sauce"))||((v=y.tags)==null?void 0:v.includes("sauces"))||((w=y.tags)==null?void 0:w.includes("stocks"))||/sauce|fond|jus|bouillon/i.test(y.french)}):t==="cuts"?c=c.filter(y=>{var k,v;return((k=y.tags)==null?void 0:k.includes("cutting"))||((v=y.tags)==null?void 0:v.includes("vegetables"))||/coupe|tailler|ciseler|mincer|brunoise|julienne/i.test(y.french)}):t==="science"?c=c.filter(y=>{var k;return((k=y.tags)==null?void 0:k.includes("science"))||/réaction|émulsion|liaison/i.test(y.french)}):t==="grammar"&&(c=[])),c.length<4){m.innerHTML=`
        <div class="quiz-card" style="text-align: center; padding: 2rem;">
      ...
      `;return}const _=ne(c).slice(0,4),j=ne(_),S=ne(_),E=document.createElement("div");E.className="quiz-card",E.innerHTML=`
    ...
    `,m.appendChild(E),h(E,4,s)}function u(){const f=Ut(),b=document.createElement("div");b.className="quiz-card",b.innerHTML=`
      <div class="quiz-meta" style="margin-bottom: 1rem;">
      ...
      <div class="matching-board">
        <div class="matching-column" id="left-column">
          ${ne(f).map(c=>`
            <div class="drag-card" draggable="true" data-id="${c.id}" id="drag-${c.id}" style="font-size: 0.9rem; padding: 0.8rem;">
              <span>${c.left}</span>
              <span style="font-size: 1rem; opacity: 0.3;">☰</span>
            </div>
          `).join("")}
        ...
        <div class="matching-column" id="right-column">
          ${ne(f).map(c=>`
            <div class="drop-zone" data-id="${c.id}" style="font-size: 0.9rem; padding: 0.8rem; min-height: 48px;">
              ${c.right}
            </div>
          `).join("")}
        </div>
    ...
    `,m.appendChild(b),h(b,4,u)}function h(f,b,c){let _=null,j=null,S=0;const E=f.querySelectorAll(".drag-card"),d=f.querySelectorAll(".drop-zone"),p=f.querySelector("#matching-completion-panel");E.forEach(v=>{v.addEventListener("dragstart",w=>{_=w.target.closest(".drag-card").getAttribute("data-id"),w.target.closest(".drag-card").classList.add("dragging")}),v.addEventListener("dragend",w=>{w.target.closest(".drag-card").classList.remove("dragging")}),v.addEventListener("click",w=>{const x=w.target.closest(".drag-card");x.classList.contains("matched")||(E.forEach(L=>L.style.borderColor="rgba(10,25,49,0.08)"),j=x.getAttribute("data-id"),x.style.borderColor="var(--color-accent)")})}),d.forEach(v=>{v.addEventListener("dragover",w=>{w.preventDefault(),v.classList.contains("matched")||v.classList.add("hovered")}),v.addEventListener("dragleave",()=>{v.classList.remove("hovered")}),v.addEventListener("drop",w=>{w.preventDefault(),v.classList.remove("hovered");const x=v.getAttribute("data-id");_===x?k(_,v):y(_)}),v.addEventListener("click",()=>{if(v.classList.contains("matched")||!j)return;const w=v.getAttribute("data-id");j===w?(k(j,v),j=null):(y(j),j=null,E.forEach(x=>x.style.borderColor="rgba(10,25,49,0.08)"))})});function y(v){const w=f.querySelector(`#drag-${v}`);w&&(w.style.animation="shake-anim 0.4s ease-in-out",setTimeout(()=>w.style.animation="",400))}function k(v,w){const x=f.querySelector(`#drag-${v}`);x.classList.add("matched"),x.style.borderColor="var(--color-success)",w.classList.add("matched"),S++,S===b&&(p.style.display="block")}f.querySelector("#restart-match-btn").addEventListener("click",()=>{c()})}function l(){var v,w;const f=((v=C.settings)==null?void 0:v.includeGeneral)||!1;let c=(((w=C.db)==null?void 0:w.vocabulary)||[]).filter(x=>f||x.is_professional);if(t!=="ALL"&&(t==="meat"?c=c.filter(x=>{var L,T,$,P;return((L=x.tags)==null?void 0:L.includes("meat"))||((T=x.tags)==null?void 0:T.includes("beef"))||(($=x.tags)==null?void 0:$.includes("pork"))||((P=x.tags)==null?void 0:P.includes("poultry"))||/viande|boeuf|porc|poulet/i.test(x.french)}):t==="sauces"?c=c.filter(x=>{var L,T,$;return((L=x.tags)==null?void 0:L.includes("sauce"))||((T=x.tags)==null?void 0:T.includes("sauces"))||(($=x.tags)==null?void 0:$.includes("stocks"))||/sauce|fond|jus|bouillon/i.test(x.french)}):t==="cuts"?c=c.filter(x=>{var L,T;return((L=x.tags)==null?void 0:L.includes("cutting"))||((T=x.tags)==null?void 0:T.includes("vegetables"))||/coupe|tailler|ciseler|mincer|brunoise|julienne/i.test(x.french)}):t==="science"?c=c.filter(x=>{var L;return((L=x.tags)==null?void 0:L.includes("science"))||/réaction|émulsion|liaison/i.test(x.french)}):t==="grammar"&&(c=[])),c.length===0){m.innerHTML=`
        <div class="quiz-card" style="text-align: center; padding: 2rem;">
      ...
      `;return}let _=ne(c)[0];const j=document.createElement("div");j.className="quiz-card",j.innerHTML=`
      <div class="quiz-meta" style="margin-bottom: 1.2rem;">
        <span>📖 Orthographe de Cuisine</span>
        <span class="grammar-badge" style="background-color: var(--color-secondary);">${_.category}</span>
      </div>
      ...
      <div class="spelling-box" style="margin-bottom: 1.5rem; background-color: rgba(10,25,49,0.02); padding: 1.2rem; border-radius: var(--radius-sm); border-left: 3px solid var(--color-accent);">
        ...
        <p style="font-size: 1.05rem; font-style: italic; color: var(--color-primary); line-height: 1.4; font-family: var(--font-serif);">${_.definition_fr}</p>
        ...
        <div style="margin-top: 1rem; border-top: 1px solid rgba(0,0,0,0.05); padding-top: 0.8rem; font-size: 0.85rem; color: var(--color-text-muted);">
          <strong>Hint (Japanese):</strong> ${_.japanese}
        </div>
      </div>
    ...
    `,m.appendChild(j);const S=j.querySelector("#spelling-input-field"),E=j.querySelector("#spelling-submit-btn"),d=j.querySelector("#spelling-next-btn"),p=j.querySelector("#spelling-feedback-panel"),y=j.querySelector("#spelling-feedback-title"),k=j.querySelector("#spelling-feedback-msg");S.addEventListener("keydown",x=>{x.key==="Enter"&&E.click()}),E.addEventListener("click",()=>{const x=S.value,L=_.french,T=Ye(x),$=Ye(L),P=T===$;S.disabled=!0,E.style.display="none",d.style.display="block",p.style.display="block",P?(S.style.borderColor="var(--color-success)",S.style.backgroundColor="#E8F5E9",p.style.backgroundColor="#E8F5E9",p.style.color="var(--color-success)",y.innerText="✓ Félicitations ! (Correct)",k.innerText=`You correctly spelled: "${L}"`):(S.style.borderColor="var(--color-error)",S.style.backgroundColor="#FFEBEE",p.style.backgroundColor="#FFEBEE",p.style.color="var(--color-error)",y.innerText="✗ Incorrect",k.innerHTML=`Correct spelling is: <strong>${L}</strong>.<br><em style="font-size:0.85rem;">You typed: "${x}"</em>`,Ke(_.id))}),d.addEventListener("click",()=>{l()})}r()}function ye(){const e=document.createElement("div"),a=document.createElement("h2");a.className="section-title",a.innerText="Coups de Cœur (Favorites)",e.appendChild(a);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Your bookmarked vocabulary terms, grammar guides, and culinary theories.",e.appendChild(t);const n=Array.from(C.favorites);if(n.length===0)return e.innerHTML+=`
      <div class="alert alert-info">
        <p>No favorites saved yet. Browse Vocabulary, Grammar, or Cuisine and click the star (☆) button to save items here.</p>
      </div>
    `,e;const i=document.createElement("div");return i.className="loading-placeholder",i.innerText="Chargement des favoris... (Loading favorites...)",e.appendChild(i),De().then(()=>{i.remove(),Kt(e,n)}),e}function Kt(e,a){var r,g,s;const t=(((r=C.db)==null?void 0:r.vocabulary)||[]).filter(u=>a.includes(u.id)),n=(((g=C.db)==null?void 0:g.grammar)||[]).filter(u=>a.includes(u.id)),i=(((s=C.db)==null?void 0:s.cuisine)||[]).filter(u=>a.includes(u.id)),o=document.createElement("div");o.className="card-grid",e.appendChild(o);function m(){o.innerHTML="",t.forEach(u=>{const h=document.createElement("div");h.className="card",h.innerHTML=`
        <div>
      ...
      `,h.querySelector(".fav-btn").addEventListener("click",l=>{de(u.id),ye(),e.innerHTML="",e.appendChild(ye())}),o.appendChild(h)}),n.forEach(u=>{var l,f;const h=document.createElement("div");h.className="card",h.innerHTML=`
        <div>
        ...
        <div class="term-context" style="margin-top: 0.5rem;">
          ...
          <div class="context-ja">${((f=u.examples[0])==null?void 0:f.ja)||""}</div>
        </div>
      `,h.querySelector(".fav-btn").addEventListener("click",b=>{de(u.id),e.innerHTML="",e.appendChild(ye())}),o.appendChild(h)}),i.forEach(u=>{const h=document.createElement("div");h.className="card",h.innerHTML=`
        <div>
      ...
      `,h.querySelector(".fav-btn").addEventListener("click",l=>{de(u.id),e.innerHTML="",e.appendChild(ye())}),o.appendChild(h)})}m()}function Zt(){const e=document.createElement("div"),a=document.createElement("h2");a.className="section-title",a.innerText="Pont d'Études (SRS Review Deck)",e.appendChild(a);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Review and grade yourself on items scheduled for active recall today.",e.appendChild(t);const n=document.createElement("div");return n.className="loading-placeholder",n.innerText="Chargement des révisions... (Loading reviews...)",e.appendChild(n),De().then(()=>{n.remove(),Xt(e)}),e}function Xt(e){var g,s,u,h;const a=new Date().toISOString().split("T")[0],n=[...(((g=C.db)==null?void 0:g.vocabulary)||[]).map(l=>({...l,type:"vocabulary",front:l.french})),...(((s=C.db)==null?void 0:s.grammar)||[]).map(l=>({...l,type:"grammar",front:l.topic})),...(((u=C.db)==null?void 0:u.cuisine)||[]).map(l=>({...l,type:"cuisine",front:l.topic})),...(((h=C.db)==null?void 0:h.quizzes)||[]).map(l=>({...l,type:"quiz",front:l.question_fr||l.question||""}))].filter(l=>{const f=C.srs[l.id];return C.wrongAnswers.includes(l.id)||f&&f.dueDate<=a}),i=n.length;let o=0;const m=document.createElement("div");m.className="srs-review-container",e.appendChild(m);function r(){if(m.innerHTML="",n.length===0||o>=n.length){m.innerHTML=`
      ...
      `;return}const l=n[o];C.srs[l.id];const f=C.wrongAnswers.includes(l.id),b=document.createElement("div");b.className="card srs-flip-card",b.style.padding="2rem",b.style.minHeight="300px",b.style.display="flex",b.style.flexDirection="column",b.style.justifyContent="space-between";let c=l.category||l.level||"Theory";l.type==="quiz"&&(c="Quiz Mistake");let _=`
      <div>
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
          <span class="card-category">${c} • ${l.type.toUpperCase()}</span>
    ...
    `;b.innerHTML=_;let j="";l.type==="vocabulary"?j=`
      ...
      `),b.querySelector("#reveal-btn").addEventListener("click",()=>{b.innerHTML=`
        <div>
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
            <span class="card-category">${c} • ${l.type.toUpperCase()}</span>
            <span style="font-size: 0.8rem; color: var(--color-text-muted);">Card ${o+1} of ${i}</span>
          </div>
      ...
      `;const S=b.querySelector(".flip-translation-container");S&&S.addEventListener("click",E=>{E.stopPropagation(),S.querySelector(".flip-translation-card").classList.toggle("flipped")}),b.querySelectorAll(".audio-btn").forEach(E=>{E.addEventListener("click",d=>{d.stopPropagation();const p=d.target.closest(".audio-btn").getAttribute("data-text");U(p)})}),b.querySelectorAll(".srs-score-btn").forEach(E=>{E.addEventListener("click",d=>{const p=parseInt(d.target.getAttribute("data-score"));Se(l.id,p),p>=4&&f&&fn(l.id),o++,r()})})}),m.appendChild(b)}r()}function en(){const e=document.createElement("div"),a=document.createElement("h2");a.className="section-title",a.innerText="Recherche Globale",e.appendChild(a);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Search terms across the entire curriculum: Vocabulary, Grammar, and Culinary Theory.",e.appendChild(t);const n=document.createElement("div");n.className="search-box-wrapper",n.style.marginBottom="2rem",n.style.position="relative";const i=document.createElement("input");i.type="text",i.placeholder="Chargement de la base de données... (Loading database...)",i.className="search-input",i.style.width="100%",i.style.padding="1rem 1.5rem",i.style.fontSize="1.1rem",i.style.borderRadius="var(--radius-md)",i.style.border="2px solid rgba(197, 168, 128, 0.2)",i.style.backgroundColor="var(--color-bg)",i.style.fontFamily="var(--font-sans)",i.style.transition="var(--transition)",i.style.outline="none",i.disabled=!0,i.addEventListener("focus",()=>{i.style.borderColor="var(--color-accent)",i.style.boxShadow="0 0 10px rgba(212, 175, 55, 0.15)"}),i.addEventListener("blur",()=>{i.style.borderColor="rgba(197, 168, 128, 0.2)",i.style.boxShadow="none"}),n.appendChild(i),e.appendChild(n);const o=document.createElement("div");return o.className="search-results",e.appendChild(o),o.innerHTML=`
    <div style="text-align: center; padding: 3rem; color: var(--color-text-muted);">
      <p>Préparation de la recherche... (Preparing search database...)</p>
    </div>
  `,De().then(()=>{i.disabled=!1,i.placeholder="Rechercher... (e.g. sauce, roux, cut, culer, 刻む)";function m(r){var b;if(o.innerHTML="",!r.trim()){o.innerHTML=`
          <div style="text-align: center; padding: 3rem; color: var(--color-text-muted);">
            <span style="font-size: 3rem;">🔍</span>
            <p style="margin-top: 1rem;">Tapez un mot-clé pour commencer votre recherche.</p>
          </div>
        `;return}const g=r.toLowerCase().trim(),s=((b=C.db)==null?void 0:b.knowledge)||[],u=s.filter(c=>c.french&&c.japanese&&!c.grammar&&!c.cuisine),h=s.filter(c=>c.grammar),l=s.filter(c=>c.cuisine),f=[];if(u.forEach(c=>{`${c.french} ${c.english} ${c.japanese} ${c.category} ${c.context_fr} ${c.context_en} ${c.context_ja} ${(c.tags||[]).join(" ")}`.toLowerCase().includes(g)&&f.push({...c,type:"vocabulary",title:c.french,subtitle:`${c.category} • Vocabulary`})}),h.forEach(c=>{`${c.topic} ${c.explanation_en} ${c.explanation_ja} ${c.level} ${(c.tags||[]).join(" ")} ${c.examples.map(j=>`${j.fr} ${j.en} ${j.ja}`).join(" ")}`.toLowerCase().includes(g)&&f.push({...c,type:"grammar",title:c.topic,subtitle:`${c.level} • Grammar Lesson`})}),l.forEach(c=>{const _=c.cuisine;`${_.topic} ${c.category} ${_.content_fr} ${_.content_en} ${_.content_ja} ${(c.tags||[]).join(" ")}`.toLowerCase().includes(g)&&f.push({...c,type:"cuisine",title:_.topic,subtitle:`${c.category} • Culinary Theory`})}),f.length===0){o.innerHTML=`
          <div style="text-align: center; padding: 3rem; color: var(--color-text-muted);">
        ...
        `;return}f.forEach(c=>{const _=document.createElement("div");_.className="card search-result-card",_.style.marginBottom="1.2rem",_.style.borderLeft=`4px solid ${c.type==="vocabulary"?"var(--color-primary)":c.type==="grammar"?"var(--color-secondary)":"var(--color-accent)"}`;const j=se(c.id);let S="";c.type==="vocabulary"?S=`
          ...
          `:c.type==="cuisine"&&(S=`
            <p style="font-size: 0.9rem; margin-top: 0.5rem; font-style: italic; color: var(--color-primary);">${(c.cuisine.content_fr||"").substring(0,150)}...</p>
            <p style="font-size: 0.85rem; margin-top: 0.5rem; color: var(--color-text-main);">${(c.cuisine.content_ja||"").substring(0,120)}...</p>
          `);const E=(c.tags||[]).map(y=>`<span class="tag-badge" style="background-color: rgba(197, 168, 128, 0.12); color: var(--color-accent); font-size: 0.75rem; padding: 0.2rem 0.6rem; border-radius: 20px; font-weight: 500;">#${y}</span>`).join(" "),d=c.type==="vocabulary"?Te(c.french):"",p=d?`<div class="preposition-illustration-container" style="height: 130px; background-color: #fcfbfa; border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; margin: 0.8rem 0; overflow: hidden; border: 1px solid rgba(0,0,0,0.03);">${d}</div>`:"";_.innerHTML=`
          <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem;">
            ...
            <button class="fav-btn ${j?"active":""}" data-id="${c.id}">
              ${j?"★":"☆"}
            </button>
          </div>
          ${p}
          ${S}
          ${c.tags&&c.tags.length>0?`<div style="margin-top: 1rem; display: flex; flex-wrap: wrap; gap: 0.4rem;">${E}</div>`:""}
        `,_.querySelector(".fav-btn").addEventListener("click",y=>{y.stopPropagation(),de(c.id);const k=y.target,v=se(c.id);k.classList.toggle("active",v),k.innerText=v?"★":"☆"}),o.appendChild(_)})}i.addEventListener("input",r=>{m(r.target.value)}),m("")}),e}function tn(){const e=document.createElement("div"),a=document.createElement("h2");a.className="section-title",a.innerText="Configuration de l'Académie",e.appendChild(a);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Customize your learning goals, target CEFR levels, and database options.",e.appendChild(t);const n=document.createElement("div");n.className="card",n.style.padding="2rem";const i=C.settings;n.innerHTML=`
    ...
    <div style="display: flex; flex-direction: column; gap: 1.8rem;">
      ...
      <div>
        ...
        <select id="target-level-select" style="padding: 0.6rem 1rem; font-size: 0.95rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.15); width: 100%; max-width: 300px; background-color: var(--color-bg); cursor: pointer;">
          <option value="ALL" ${i.targetLevel==="ALL"?"selected":""}>ALL LEVELS (Tout)</option>
          <option value="A1" ${i.targetLevel==="A1"?"selected":""}>A1 - Beginner (Apprenti)</option>
          <option value="A2" ${i.targetLevel==="A2"?"selected":""}>A2 - Intermediate (Commis)</option>
          <option value="B1" ${i.targetLevel==="B1"?"selected":""}>B1 - Advanced (Chef de Partie)</option>
          <option value="B2" ${i.targetLevel==="B2"?"selected":""}>B2 - Upper Intermediate (Sous Chef)</option>
          <option value="C1" ${i.targetLevel==="C1"?"selected":""}>C1 - Expert (Chef de Cuisine)</option>
          <option value="C2" ${i.targetLevel==="C2"?"selected":""}>C2 - Master (Directeur de Cuisine)</option>
        </select>
      </div>
      ...
      <div>
        ...
        <input type="number" id="new-cards-goal" min="1" max="50" value="${i.newCardsPerDay}" style="padding: 0.6rem 1rem; font-size: 0.95rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.15); width: 100%; max-width: 120px;">
      </div>
      ...
      <div>
        ...
        <input type="number" id="max-reviews-goal" min="5" max="200" value="${i.maxReviewsPerDay}" style="padding: 0.6rem 1rem; font-size: 0.95rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.15); width: 100%; max-width: 120px;">
      </div>
      ...
      <div>
        ...
        <div style="display: flex; align-items: center; gap: 0.8rem; margin-bottom: 0.5rem;">
          <input type="checkbox" id="include-general-checkbox" style="transform: scale(1.3); cursor: pointer;" ${i.includeGeneral?"checked":""}>
  ...
  `;const o=n.querySelector("#target-level-select"),m=n.querySelector("#new-cards-goal"),r=n.querySelector("#max-reviews-goal"),g=n.querySelector("#include-general-checkbox"),s=n.querySelector("#settings-status");function u(){const h=o.value,l=parseInt(m.value)||5,f=parseInt(r.value)||20,b=g.checked;yn({targetLevel:h,newCardsPerDay:l,maxReviewsPerDay:f,includeGeneral:b}),s.style.display="block",setTimeout(()=>{s.style.display="none"},3e3)}return o.addEventListener("change",u),m.addEventListener("input",u),r.addEventListener("input",u),g.addEventListener("change",u),n.querySelector("#reset-srs-btn").addEventListener("click",()=>{confirm("Voulez-vous vraiment réinitialiser toutes vos données de progression SRS ? Cette action est irréversible.")&&(localStorage.removeItem("cba_srs"),C.srs={},alert("Spaced Repetition System progress has been reset."),window.location.reload())}),n.querySelector("#clear-favs-btn").addEventListener("click",()=>{confirm("Voulez-vous vraiment supprimer tous vos favoris ?")&&(localStorage.removeItem("cba_favorites"),C.favorites=new Set,alert("All favorites have been cleared."),window.location.reload())}),n.querySelector("#share-app-btn").addEventListener("click",()=>{const h={title:"Académie de la Brigade",text:"フランス料理・厨房フランス語の統合学習PWAアプリ「Académie de la Brigade」で一緒に料理と語学を学びましょう！",url:window.location.origin+window.location.pathname};navigator.share?navigator.share(h).then(()=>console.log("Shared successfully")).catch(l=>console.log("Error sharing:",l)):navigator.clipboard.writeText(h.url).then(()=>{alert("App link copied to clipboard! Share it with your friends.")}).catch(l=>{console.error("Failed to copy link:",l)})}),e.appendChild(n),e}function nn(){var o;const e=document.createElement("div"),a=document.createElement("h2");a.className="section-title",a.innerText="Dictée de Cuisine (Culinary Dictations)",e.appendChild(a);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Listen to French kitchen instruction sentences, type what you hear, and master French spelling.",e.appendChild(t);const n=document.createElement("div");n.className="loading-placeholder",n.innerText="Chargement de la dictée... (Loading dictation...)",e.appendChild(n);const i=((o=C.settings)==null?void 0:o.targetLevel)||"ALL";return ee("vocabulary",i).then(()=>{n.remove(),an(e)}),e}function an(e){var _,j;const a=((_=C.settings)==null?void 0:_.includeGeneral)||!1,i=(((j=C.db)==null?void 0:j.vocabulary)||[]).filter(S=>a||S.is_professional).filter(S=>S.context_fr);if(i.length===0){e.innerHTML+='<p style="color: var(--color-text-muted);">Aucun exercice disponible.</p>';return}const o=[...i].sort(()=>.5-Math.random()).slice(0,5);let m=0,r=!1,g=!1,s=!1,u=0,h=1;const l=document.createElement("div");l.className="dictation-container",l.style.maxWidth="600px",l.style.margin="1.5rem auto",e.appendChild(l);function f(S){return S.toLowerCase().trim().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()'"?]/g,"").replace(/\s+/g," ")}function b(S){return S.normalize("NFD").replace(/[\u0300-\u036f]/g,"")}function c(){if(l.innerHTML="",r=!1,qe(),g=!1,s=!1,u=0,m>=o.length){const z=document.createElement("div");z.className="card",z.style.padding="2.5rem",z.style.textAlign="center",z.innerHTML=`
        <h3 style="font-family: var(--font-serif); font-size: 2rem; color: var(--color-primary); margin-bottom: 1rem;">Session Terminée !</h3>
      ...
      `,z.querySelector("#restart-dictation-btn").addEventListener("click",()=>{m=0,o.length=0,o.push(...[...i].sort(()=>.5-Math.random()).slice(0,5)),c()}),l.appendChild(z);return}const S=o[m],E=S.context_fr,d=document.createElement("div");d.className="card",d.style.padding="2rem",d.innerHTML=`
    ...
    `;const p=d.querySelector("#play-pause-btn"),y=d.querySelector("#stop-btn"),k=d.querySelector("#speed-normal-btn"),v=d.querySelector("#speed-slow-btn"),w=d.querySelector("#dictation-seekbar"),x=d.querySelector("#dictation-input"),L=d.querySelector("#check-btn"),T=d.querySelector("#next-dictation-btn"),$=d.querySelector("#dictation-feedback");function P(z,H){if(!r){const V=z/H*100;w.value=Math.round(V),u=z}}function M(){g=!1,s=!1,u=0,p.innerText="▶ Play",w.value=0}function q(){Xe(E,h,P,M,u)}p.addEventListener("click",()=>{r||(g?s?(s=!1,p.innerText="⏸ Pause",wt()):(s=!0,p.innerText="▶ Play",kt()):(g=!0,s=!1,p.innerText="⏸ Pause",q()))}),y.addEventListener("click",()=>{qe(),M()}),w.addEventListener("change",z=>{if(r)return;const H=parseInt(z.target.value),V=E.replace(/["'➔]/g,"").trim();u=Math.floor(H/100*V.length);const J=V.indexOf(" ",u);J!==-1&&J-u<5&&(u=J+1),g=!0,s=!1,p.innerText="⏸ Pause",q()}),k.addEventListener("click",()=>{h!==1&&(h=1,k.classList.add("active"),k.style.backgroundColor="var(--color-primary)",k.style.color="#FFFFFF",v.classList.remove("active"),v.style.backgroundColor="transparent",v.style.color="var(--color-accent)",g&&!s&&q())}),v.addEventListener("click",()=>{h!==.75&&(h=.75,v.classList.add("active"),v.style.backgroundColor="var(--color-accent)",v.style.color="#FFFFFF",k.classList.remove("active"),k.style.backgroundColor="transparent",k.style.color="var(--color-accent)",g&&!s&&q())}),L.addEventListener("click",()=>{if(r)return;qe(),M();const z=x.value,H=f(z),V=f(E),J=b(H),Y=b(V);let R=!1,O=!1;H===V?R=!0:J===Y&&(O=!0),$.style.display="block",x.disabled=!0,L.style.display="none",T.style.display="block",r=!0,R?($.style.backgroundColor="#E8F5E9",$.style.borderLeft="4px solid var(--color-success)",$.style.color="var(--color-success)",$.innerHTML=`
        ...
        `):O?($.style.backgroundColor="#FFF3E0",$.style.borderLeft="4px solid var(--color-accent)",$.style.color="#E65100",$.innerHTML=`
          ...
          <div style="margin: 0.8rem 0; padding: 0.6rem; background: rgba(0,0,0,0.02); font-size: 0.9rem; border-radius: var(--radius-sm);">
            <div><strong>Expected:</strong> <span style="font-family: monospace; font-weight: 600; color: var(--color-success);">${E}</span></div>
        ...
        `)}),T.addEventListener("click",()=>{m++,c()}),l.appendChild(d),x.focus()}c()}const rn={proprietress:1.5,kanetake:1.45,saeki:1.45,elodie:1.45,gael:1.45,jean_pierre:1.45,ピエール:1.45};let te=null;function on(){te||(te=new(window.AudioContext||window.webkitAudioContext))}function _e(e,a,t,n=.06){try{on(),(!te||te.state==="suspended")&&te.resume();const i=te.createOscillator(),o=te.createGain();i.type=a||"sine",i.frequency.setValueAtTime(e,te.currentTime),o.gain.setValueAtTime(n,te.currentTime),o.gain.exponentialRampToValueAtTime(.001,te.currentTime+t),i.connect(o),o.connect(te.destination),i.start(),i.stop(te.currentTime+t)}catch{}}function sn(){_e(780,"sine",.03,.04)}function Pe(){_e(523.25,"sine",.08,.08),setTimeout(()=>_e(659.25,"sine",.15,.08),80)}function Me(){_e(180,"triangle",.3,.12)}function Be(){_e(110,"sawtooth",.2,.1)}function ln(){const e=document.createElement("div");return e.className="story-mode-container",vt(e),e}async function vt(e){e.innerHTML=`
  ...
  `,e.querySelectorAll(".play-episode-btn").forEach(a=>{a.addEventListener("click",t=>{const n=t.target.getAttribute("data-chapter"),i=t.target.getAttribute("data-episode");xt(e,n,i)})})}async function xt(e,a,t){try{e.innerHTML='<div class="story-loader"><div class="spinner"></div><p>物語を読み込んでいます...</p></div>';const n=a==="career_0"?"chapter_career_0.json":`chapter_${a}.json`,i=await fetch(`data/story/${n}`);if(!i.ok)throw new Error("Story file could not be loaded");const o=await i.json(),m=o.episodes.find(r=>r.episodeId===t);if(!m)throw new Error("Episode data not found in chapter file");cn(e,m,a,o)}catch(n){e.innerHTML=`
    ...
    `}}function cn(e,a,t,n){let i=0;const o=a.sequence;e.innerHTML=`
  ...
  `;const m=e.querySelector("#rpg-viewport"),r=e.querySelector("#rpg-dialog-pane"),g=e.querySelector("#rpg-battle-pane"),s=e.querySelector("#rpg-reward-pane"),u=e.querySelector("#rpg-tutorial-overlay"),h=e.querySelector("#rpg-sidebar");let l=null,f=!1,b="";function c(){if(f){clearInterval(l);const y=e.querySelector("#dialog-text");y.innerText=b,f=!1;return}if(i>=o.length){vt(e);return}const p=o[i];p.type==="tutorial"?j(p):p.type==="dialog"?S(p):p.type==="fixedBattle"||p.type==="randomBattle"?E(p):p.type==="reward"&&d(p),_(p)}function _(p){let y=m.querySelector(".ai-policy-notice");p&&p.background==="bg_thief_caught.png"||m.style.background&&m.style.background.includes("bg_thief_caught.png")?y||(y=document.createElement("div"),y.className="ai-policy-notice",y.innerText="※AIポリシーの都合により、カミーユはうさぎの縫いぐるみに差し替えられました",m.appendChild(y)):y&&y.remove()}m.addEventListener("click",p=>{p.target.closest("button")||p.target.closest(".battle-question-box")||r.style.display==="none"||c()}),c();function j(p){r.style.display="none",g.style.display="none",s.style.display="none",h.style.opacity="0",h.style.pointerEvents="none";const y=e.querySelector("#rpg-character-layer");y&&(y.innerHTML=""),u.innerHTML=`
    ...
    `,u.style.display="flex",u.querySelector(".start-tut-btn").addEventListener("click",()=>{u.style.display="none",i++,c()})}function S(p){u.style.display="none",g.style.display="none",s.style.display="none",r.style.display="block";const y=a.backgrounds[p.background]||"#000000";m.style.background=y,p.shake&&(m.classList.add("shake-vfx"),setTimeout(()=>m.classList.remove("shake-vfx"),400)),y.includes("url(")?(m.style.backgroundSize="cover",m.style.backgroundPosition="center",m.style.backgroundRepeat="no-repeat"):(m.style.backgroundSize="",m.style.backgroundPosition="",m.style.backgroundRepeat="");const k=e.querySelector("#rpg-character-layer");if(k){k.innerHTML="";const $=y.toLowerCase(),P=$.includes("bg_camille_cry.webp")||$.includes("bg_after_battle.webp")||$.includes("bg_father.webp")||$.includes("bg_room.webp");let M=[];P||(p.characters&&Array.isArray(p.characters)?M=p.characters:p.character&&(M=[{id:p.character,expression:p.expression||"default",position:p.position||"center"}])),M.forEach(q=>{const z=a.characters[q.id],H=q.expression||"default";let V=q.position||"center";M.length===1&&q.id==="proprietress"&&(V="center");let J=null;if(z&&z.images)if(z.images[H])J=z.images[H];else if(H==="default"&&z.images.normal)J=z.images.normal;else{const Y=Object.keys(z.images);Y.length>0&&(J=z.images[Y[0]])}if(J){const Y=document.createElement("img");Y.src=J,Y.className=`rpg-character-sprite pos-${V} sprite-${q.id}`;const R=rn[q.id]||1;Y.style.setProperty("--char-scale",R),k.appendChild(Y),setTimeout(()=>{Y.classList.add("active")},50)}})}const v=p.character?a.characters[p.character]:null,w=e.querySelector("#dialog-name"),x=e.querySelector("#dialog-text");w.style.display="none",p.learningPoint?(e.querySelector("#lp-title").innerText=p.learningPoint.title,e.querySelector("#lp-content").innerText=p.learningPoint.text,h.style.opacity="1",h.style.pointerEvents="auto"):(h.style.opacity="0",h.style.pointerEvents="none");const L=v?`${v.name}：${p.text}`:p.text;x.innerText="",b=L,f=!0;let T=0;clearInterval(l),l=setInterval(()=>{T<L.length?(x.innerText+=L[T],T++,T%2===0&&sn()):(clearInterval(l),f=!1)},30),i++}async function E(p){var J,Y;r.style.display="none",u.style.display="none",s.style.display="none",h.style.opacity="0",h.style.pointerEvents="none",g.style.display="flex";const y=e.querySelector("#rpg-character-layer");y&&(y.innerHTML=""),g.innerHTML=`
      <div style="margin: auto; text-align: center;">
        <div class="spinner" style="margin: 0 auto 1rem;"></div>
        <p>試練を読み込んでいます...</p>
      </div>
    `;let k=[];if(p.type==="randomBattle")try{await Oe();const R=((J=p.conditions)==null?void 0:J.level)||"ALL";R!=="ALL"&&await ee("vocabulary",R);let O=[...C.db.quizzes];const N=(Y=p.conditions)==null?void 0:Y.category;N&&(O=O.filter(I=>I.category&&I.category.toLowerCase()===N.toLowerCase())),O=$(O);const D=p.enemyHp||3;if(k=O.slice(0,D),k.length<D){const I=$(C.db.quizzes.filter(G=>!k.some(Z=>Z.id===G.id)));k=k.concat(I.slice(0,D-k.length))}k=k.map(I=>{const G=I.options.indexOf(I.answer);return{questionId:I.id,text:I.question,options:I.options,answerIndex:G!==-1?G:0,explanation:I.context||`正解は「${I.answer}」です。`}})}catch(R){console.error("Failed to generate random questions:",R),k=[{questionId:"fb_err_1",text:"プロヴァンス地方の代表的なスープ料理は？",options:["Bouillabaisse","Bœuf bourguignon","Cassoulet","Choucroute"],answerIndex:0,explanation:"ブイヤベースはプロヴァンス地方（マルセイユ）の名物です。"}]}else k=p.questions;let v=p.enemyHp||k.length;const w=v;let x=10;const L=10;let T=0;function $(R){const O=[...R];for(let N=O.length-1;N>0;N--){const D=Math.floor(Math.random()*(N+1));[O[N],O[D]]=[O[D],O[N]]}return O}let P=null;async function M(){if(P)return P;try{return P=await(await fetch("data/grammar_reference.json")).json(),P}catch(R){return console.error("Failed to load battle reference:",R),[]}}async function q(R){if(h.style.opacity==="1")h.style.opacity="0",h.style.pointerEvents="none",R.innerText="📖 参考資料を表示";else{let G=function(W){const Q=N[W];if(!Q)return;let le=`<h4 style="color: var(--color-accent); border-bottom: 1px solid rgba(197,168,128,0.2); padding-bottom: 0.3rem; margin-bottom: 0.6rem;">${Q.title_ja} (${Q.title_fr})</h4>`;Q.sections.forEach(K=>{if(le+='<div style="margin-bottom: 1rem;">',K.title&&(le+=`<div style="font-weight: bold; font-size: 0.85rem; color: var(--color-primary); margin-bottom: 0.3rem;">${K.title}</div>`),K.text&&(le+=`<p style="white-space: pre-line; margin-bottom: 0.5rem; line-height: 1.4;">${K.text}</p>`),K.headers&&K.rows){const xe=[],ue=K.headers.map((ae,$e)=>ae==="発音"?(xe.push($e),"音声"):ae);le+=`
                <table style="width: 100%; border-collapse: collapse; margin: 0.5rem 0; font-size: 0.75rem;">
                  <thead>
                    <tr style="border-bottom: 2px solid rgba(197,168,128,0.3);">
                      ${ue.map(ae=>`<th style="padding: 0.3rem; text-align: left; background: rgba(197,168,128,0.05);">${ae}</th>`).join("")}
                    </tr>
                  </thead>
                  <tbody>
                    ${K.rows.map(ae=>`
                      <tr style="border-bottom: 1px solid rgba(0,0,0,0.05);">
                        ${ae.map(($e,Ee)=>xe.includes(Ee)?`<td style="padding: 0.3rem; text-align: center;"><button class="ref-table-audio-btn" data-speak="${ae[Ee-1]?ae[Ee-1].split("(")[0].trim():""}" style="background: none; border: none; cursor: pointer; padding: 0; font-size: 0.85rem;">🔊</button></td>`:`<td style="padding: 0.3rem;">${$e}</td>`).join("")}
                      </tr>
                    `).join("")}
                  </tbody>
                </table>
              `}le+="</div>"}),I.innerHTML=le,I.querySelectorAll(".ref-table-audio-btn").forEach(K=>{K.addEventListener("click",xe=>{xe.stopPropagation();const ue=K.getAttribute("data-speak");if(ue&&typeof U=="function")U(ue);else if(ue){const ae=new SpeechSynthesisUtterance(ue);ae.lang="fr-FR",window.speechSynthesis.speak(ae)}})})};var O=G;h.style.opacity="1",h.style.pointerEvents="auto",R.innerText="📖 参考資料を隠す",h.innerHTML=`
        ...
        `;const N=await M();h.innerHTML=`
          ...
          <select class="battle-ref-topic-select" style="width: 100%; padding: 0.5rem; border-radius: var(--radius-sm); border: 1px solid rgba(197,168,128,0.3); background: #ffffff; color: #1e293b; font-size: 0.85rem; margin-bottom: 1rem;">
            ${N.map((W,Q)=>`
              <option value="${Q}">${W.title_ja} (${W.title_fr})</option>
            `).join("")}
          </select>
          
          <div class="battle-ref-content" style="max-height: 420px; overflow-y: auto; font-size: 0.8rem;"></div>
        `;const D=h.querySelector(".battle-ref-topic-select"),I=h.querySelector(".battle-ref-content");let Z=0;if(a.episodeId==="career_ep_0_1"){const W=N.findIndex(Q=>Q.id==="ref_essential_irregular_verbs");W!==-1&&(Z=W)}else if(a.episodeId==="career_ep_0_2"){const W=N.findIndex(Q=>Q.id==="ref_time_expressions");W!==-1&&(Z=W)}D.value=Z,G(Z),D.addEventListener("change",W=>{G(parseInt(W.target.value))})}}function z(){if(v<=0){Pe(),h.style.opacity="0",h.style.pointerEvents="none",g.innerHTML=`
        ...
        `,g.querySelector(".proceed-battle-btn").addEventListener("click",()=>{i++,c()});return}if(x<=0){Me(),h.style.opacity="0",h.style.pointerEvents="none",g.innerHTML=`
        ...
        `,g.querySelector(".retry-battle-btn").addEventListener("click",()=>{E(p)}),g.querySelector(".exit-battle-btn").addEventListener("click",()=>{var D;const N=(D=p.conditions)==null?void 0:D.category;be(N==="grammar"?"grammar":"vocabulary")});return}if(T>=k.length){v>0&&(v=0,z());return}const R=k[T],O=`
        <div class="battle-hud">
          <div class="hud-bar-container">
            <span class="hud-label">VOUS (あなた)</span>
            <div class="hud-hp-track">
              <div class="hud-hp-fill player-hp" style="width: ${x/L*100}%"></div>
            </div>
            <span class="hud-value">${x} / ${L}</span>
          </div>
          
          <div class="hud-bar-container">
            <span class="hud-label">${p.enemyName}</span>
            <div class="hud-hp-track">
              <div class="hud-hp-fill enemy-hp" style="width: ${v/w*100}%"></div>
            </div>
            <span class="hud-value">${v} / ${w}</span>
          </div>
        </div>
      `;if(R.type==="typing"){g.innerHTML=`
          ${O}
          <div class="battle-question-box">
            <div class="q-header">Question ${T+1} (スペル入力)</div>
            <div class="q-body" style="white-space: pre-line; line-height: 1.4; font-size: 0.95rem;">${R.text}</div>
        ...
        `;const N=g.querySelector(".battle-input-field"),D=g.querySelector(".submit-typing-btn"),I=g.querySelector(".toggle-battle-ref-btn");setTimeout(()=>N.focus(),150),N.addEventListener("keydown",G=>{G.key==="Enter"&&!D.disabled&&D.click()}),D.addEventListener("click",()=>{const G=N.value;V(G,R,N,D)}),I.addEventListener("click",()=>{q(I)})}else{g.innerHTML=`
          ${O}
          <div class="battle-question-box">
            <div class="q-header">Question ${T+1}</div>
            <div class="q-body" style="white-space: pre-line;">${R.text}</div>
            
            <div class="battle-options-list">
              ${R.options.map((I,G)=>`
                <button class="battle-opt-btn" data-idx="${G}">${I}</button>
              `).join("")}
            </div>
        ...
        `;const N=g.querySelectorAll(".battle-opt-btn"),D=g.querySelector(".toggle-battle-ref-btn");N.forEach(I=>{I.addEventListener("click",G=>{const Z=parseInt(G.target.getAttribute("data-idx"));H(Z,R,N)})}),D.addEventListener("click",()=>{q(D)})}}function H(R,O,N){N.forEach(Q=>Q.disabled=!0);const D=g.querySelector("#battle-feedback"),I=g.querySelector("#fb-title"),G=g.querySelector("#fb-desc"),Z=g.querySelector("#next-q-btn");if(R===O.answerIndex){Pe(),v=Math.max(0,v-1),I.innerText="✅ 正解！ (Très bien)",I.className="feedback-title text-success",Z.innerText="次の試練へ";const Q=g.querySelector(".enemy-hp");Q&&Q.classList.add("flash-white"),Z.onclick=()=>{T++,z()}}else Me(),Be(),x=Math.max(0,x-p.enemyDamage),I.innerText="❌ 不正解！",I.className="feedback-title text-error",Z.innerText="もう一度挑戦する",m.classList.add("shake-vfx"),setTimeout(()=>m.classList.remove("shake-vfx"),400),Z.onclick=()=>{z()};G.innerText=O.explanation,D.style.display="block"}function V(R,O,N,D){N.disabled=!0,D.disabled=!0;const I=g.querySelector("#battle-feedback"),G=g.querySelector("#fb-title"),Z=g.querySelector("#fb-desc"),W=g.querySelector("#next-q-btn"),Q=R.toLowerCase().replace(/[.,!?;:・]/g,"").trim();if(O.acceptedAnswers.some(K=>Q===K.toLowerCase().replace(/[.,!?;:・]/g,"").trim())){Pe(),v=Math.max(0,v-1),G.innerText="✅ 正解！ (Très bien)",G.className="feedback-title text-success",W.innerText="次の試練へ";const K=g.querySelector(".enemy-hp");K&&K.classList.add("flash-white"),W.onclick=()=>{T++,z()}}else Me(),Be(),x=Math.max(0,x-2),G.innerText=`❌ 不正解！ (正解: ${O.acceptedAnswers[0]})`,G.className="feedback-title text-error",W.innerText="もう一度挑戦する",m.classList.add("shake-vfx"),setTimeout(()=>m.classList.remove("shake-vfx"),400),W.onclick=()=>{z()};Z.innerText=O.explanation,I.style.display="block"}z()}function d(p){r.style.display="none",g.style.display="none",u.style.display="none",h.style.opacity="0",h.style.pointerEvents="none",s.style.display="block";const y=e.querySelector("#rpg-character-layer");y&&(y.innerHTML=""),localStorage.setItem(`cba_story_${a.episodeId}_cleared`,"true");const v=a.episodeId.startsWith("career_")?"assets/story/career_story/group_photo.webp":"assets/story/chapter_0/bg_after_battle.webp";s.innerHTML=`
      <div class="reward-card animate-fade-in">
        ...
        <div class="reward-photo-frame" style="width: 100%; border-radius: var(--radius-md); overflow: hidden; border: 2px solid var(--color-accent); margin-bottom: 1rem; box-shadow: 0 4px 10px rgba(0,0,0,0.15);">
          <img src="${v}" alt="Reward Image" style="width: 100%; height: auto; display: block;" />
        </div>
    ...
    `,s.querySelector(".claim-reward-btn").addEventListener("click",()=>{w()});function w(){s.innerHTML=`
      ...
      `;const T=s.querySelector(".stamp-ink-overlay");setTimeout(()=>{T.style.display="flex",T.classList.add("animate-stamp-stomp"),Be(),m.classList.add("shake-vfx"),setTimeout(()=>m.classList.remove("shake-vfx"),450)},700),setTimeout(()=>{L()},2300)}const x=function(T){let $=document.querySelector(".cba-toast");$&&$.remove(),$=document.createElement("div"),$.className="cba-toast",$.innerText=T,document.body.appendChild($),setTimeout(()=>{$.classList.add("fade-out"),setTimeout(()=>$.remove(),400)},2500)};function L(){const T=n&&n.episodes?n.episodes.findIndex(z=>z.episodeId===a.episodeId):-1,$=T!==-1&&n.episodes[T+1]?n.episodes[T+1]:null;s.innerHTML=`
      ...
      `;const P=s.querySelector(".home-btn"),M=s.querySelector(".share-btn"),q=s.querySelector(".next-ep-btn");P.addEventListener("click",()=>{be("home")}),$&&q&&q.addEventListener("click",()=>{xt(e,t,$.episodeId)}),M.addEventListener("click",()=>{const z=`CHEF BRIGADE ACADEMYでフランス語と料理の修行中！「${a.episodeTitle}」をクリアしました！みんなも一緒に楽しく学習しよう！ #ChefBrigadeAcademy`;navigator.share?navigator.share({title:"Chef Brigade Academy",text:z,url:window.location.href}).catch(H=>{console.log("Error sharing:",H)}):navigator.clipboard.writeText(z).then(()=>{x("シェア用テキストをクリップボードにコピーしました！")}).catch(H=>{console.error("Could not copy text: ",H)})})}}}let je=null,re="preview",X=0,he=null;function dn(){const e=document.createElement("div");e.className="reading-container-view",C.settings||(C.settings={}),C.settings.readingHelper||(C.settings.readingHelper={outlines:!0,translations:!0,targetOnly:!1,pureText:!1});const a=document.createElement("h2");a.className="section-title",a.innerText="Lecture de Cuisine (Culinary Reading)",e.appendChild(a);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Immerse yourself in authentic French culinary articles. Decode grammar and vocabulary interactively as you read.",e.appendChild(t);const n=document.createElement("div");return n.className="loading-placeholder",n.innerText="Chargement des articles... (Loading articles...)",e.appendChild(n),ee("knowledge","ALL").then(()=>{fetch("data/articles.json").then(i=>{if(!i.ok)throw new Error("Could not load articles");return i.json()}).then(i=>{if(C.db.articles=i,n.remove(),je){const o=i.find(m=>m.id===je.id)||i[0];oe(e,o)}else Ne(e,i)}).catch(i=>{n.innerText="Erreur de chargement. (Loading error.)",console.error(i)})}),document.addEventListener("click",mn),e}function mn(e){he&&!e.target.closest(".anatomy-tooltip")&&!e.target.closest(".anatomy-token")&&!e.target.closest(".learning-badge")&&(he.remove(),he=null)}function Ne(e,a){je=null,re="preview",X=0;const t=document.createElement("div");t.className="magazine-grid",t.style.display="grid",t.style.gridTemplateColumns="repeat(auto-fit, minmax(320px, 1fr))",t.style.gap="2rem",t.style.marginTop="2rem",a.forEach(n=>{const i=document.createElement("div");i.className="magazine-card",i.style.cursor="pointer";const o=n.image?`<img src="${n.image}" class="magazine-cover" style="width: 100%; height: 200px; object-fit: cover; border-radius: var(--radius-md) var(--radius-md) 0 0;" onerror="this.src='assets/cheese_wine.png'">`:'<div class="magazine-cover-placeholder" style="width: 100%; height: 200px; background-color: var(--color-primary); border-radius: var(--radius-md) var(--radius-md) 0 0; display: flex; align-items: center; justify-content: center; color: white; font-family: var(--font-serif); font-size: 1.5rem; font-weight: bold;">Cuisine</div>';i.innerHTML=`
      ${o}
    ...
    `,i.addEventListener("click",()=>{je=n,re="preview",X=0,oe(e,n)}),t.appendChild(i)}),e.appendChild(t)}function oe(e,a){e.innerHTML="",re==="preview"?un(e,a):re==="practice"?pn(e,a):re==="complete"&&gn(e,a)}function ie(e){const a=C.settings.readingHelper;e.classList.toggle("hide-outlines",!a.outlines||a.pureText),e.classList.toggle("hide-translations",!a.translations||a.pureText),e.classList.toggle("highlight-target-only",a.targetOnly),e.classList.toggle("pure-text-mode",a.pureText)}function un(e,a){var v;const t=((v=C.settings)==null?void 0:v.targetLevel)||"ALL";let n="débutant";["B1","B2","intermédiaire"].includes(t)?n="intermédiaire":["C1","C2","avancé"].includes(t)&&(n="avancé");const i=n==="débutant",o=n==="intermédiaire",m=n==="avancé",r=document.createElement("button");r.className="next-btn",r.style.backgroundColor="transparent",r.style.border="1px solid var(--color-primary)",r.style.color="var(--color-primary)",r.style.padding="0.4rem 1rem",r.style.marginBottom="1.5rem",r.innerText="← Retour aux articles (Back)",r.addEventListener("click",()=>{Ne(e,[a])}),e.appendChild(r);const g=document.createElement("div");g.className="magazine-header",g.style.textAlign="center",g.style.marginBottom="2.5rem",g.innerHTML=`
  ...
  `,e.appendChild(g);const s=document.createElement("div");s.className="reading-helper-panel",s.innerHTML=`
  ...
  `,e.appendChild(s);const u=new Map,h=new Map;a.sessions.forEach(w=>{w.tokens.forEach(x=>{if(x.vocab_id){const L=C.db.knowledge.find(T=>T.id===x.vocab_id);L&&u.set(x.vocab_id,L.french)}if(x.gram_id){const L=C.db.knowledge.find(T=>T.id===x.gram_id);L&&L.grammar&&h.set(x.gram_id,L.grammar.topic)}})});const l=document.createElement("div");l.className="card",l.style.backgroundColor="#f7f9fc",l.style.border="1px solid rgba(0,0,91,0.06)",l.style.padding="1.5rem",l.style.borderRadius="10px",l.style.marginBottom="2rem";let f="";u.forEach((w,x)=>{f+=`<button class="learning-badge" data-id="${x}" style="margin: 0.2rem; padding: 0.25rem 0.6rem; font-size: 0.78rem; border-radius: 4px; border: 1px solid rgba(0,0,91,0.1); background-color: #ffffff; color: var(--color-primary); font-weight: 600; cursor: pointer; transition: all 0.2s;">${w}</button>`});let b="";h.forEach((w,x)=>{b+=`<button class="learning-badge" data-id="${x}" style="margin: 0.2rem; padding: 0.25rem 0.6rem; font-size: 0.78rem; border-radius: 4px; border: 1px solid rgba(107,156,104,0.15); background-color: #ffffff; color: var(--color-success); font-weight: 600; cursor: pointer; transition: all 0.2s;">${w}</button>`}),l.innerHTML=`
    ...
    <div style="display: flex; flex-direction: column; gap: 1rem;">
      <div>
        ...
        <div style="display: flex; flex-wrap: wrap;">${f||'<span style="font-size:0.8rem; color:#888;">Aucun vocabulaire</span>'}</div>
      </div>
      <div>
        <p style="font-size: 0.85rem; font-weight: bold; color: var(--color-text-muted); margin-bottom: 0.3rem;">📕 Points de grammaire (文法ポイント):</p>
        <div style="display: flex; flex-wrap: wrap;">${b||'<span style="font-size:0.8rem; color:#888;">Aucune grammaire</span>'}</div>
      </div>
    </div>
  `,l.querySelectorAll(".learning-badge").forEach(w=>{w.addEventListener("click",x=>{x.stopPropagation();const L=w.getAttribute("data-id"),T=L.startsWith("gram_")?{gram_id:L}:{vocab_id:L};Fe(w,T,"")})}),e.appendChild(l);const c=document.createElement("div");c.style.textAlign="center",c.style.marginTop="3.5rem",c.style.marginBottom="3.5rem";const _=document.createElement("div");_.style.display="inline-flex",_.style.alignItems="center",_.style.justifyContent="center",_.style.gap="1.2rem",_.style.flexWrap="wrap";const j=document.createElement("button");j.className="start-practice-btn",j.innerText="🎓 Commencer l'entraînement (問題を解くモードに入る)",j.addEventListener("click",()=>{re="practice",X=0,oe(e,a)});const S=document.createElement("div");S.style.display="inline-flex",S.style.alignItems="center",S.style.gap="0.5rem";const E=document.createElement("span");E.style.fontSize="0.8rem",E.style.fontWeight="bold",E.style.color="var(--color-text-muted)",E.innerText="Niveau (Level) :";const d=document.createElement("select");d.style.padding="0.6rem 1.2rem",d.style.fontSize="0.9rem",d.style.borderRadius="50px",d.style.border="1px solid rgba(0,0,0,0.15)",d.style.backgroundColor="#FFFFFF",d.style.cursor="pointer",d.style.fontFamily="var(--font-sans)",d.style.fontWeight="600",d.style.color="var(--color-primary)",d.style.outline="none",[{val:"débutant",label:"Débutant (初級)"},{val:"intermédiaire",label:"Intermédiaire (中級)"},{val:"avancé",label:"Avancé (上級)"}].forEach(w=>{const x=document.createElement("option");x.value=w.val,x.text=w.label,w.val===n&&(x.selected=!0),d.appendChild(x)}),d.addEventListener("change",w=>{C.settings.targetLevel=w.target.value,oe(e,a)}),S.appendChild(E),S.appendChild(d),_.appendChild(j),_.appendChild(S),c.appendChild(_),e.appendChild(c);const y=document.createElement("div");y.className="magazine-body-container",y.style.maxWidth="720px",y.style.margin="0 auto",y.style.display="flex",y.style.flexDirection="column",y.style.gap="2.2rem",a.sessions.forEach((w,x)=>{const L=document.createElement("div");L.className="ref-session-card",L.style.borderLeft="4px solid var(--color-accent)",L.style.padding="1.8rem",L.style.backgroundColor="#ffffff";const T=`
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.6rem;">
        <span style="font-size: 0.72rem; text-transform: uppercase; font-weight: bold; letter-spacing: 1px; color: var(--color-text-muted);">Section ${x+1}: ${w.title}</span>
    ...
    `,$=document.createElement("div");$.className="ref-para-fr",$.style.fontSize="1.18rem",$.style.lineHeight="1.8",$.style.fontFamily="var(--font-serif)",$.style.marginBottom="1rem",$.style.color="var(--color-primary)",x===0&&$.classList.add("editorial-dropcap"),w.tokens.forEach((M,q)=>{const z=document.createElement("span");z.className=`anatomy-token token-pos-${M.pos} has-definition`,z.innerText=M.word+" ";const H=w.tokens.slice(q).map(V=>V.word).join(" ");(M.vocab_id||M.gram_id)&&(z.setAttribute("data-has-db","true"),i?M.pos==="noun"?z.style.borderBottom="2px solid rgba(0, 0, 91, 0.25)":M.pos==="verb"?z.style.borderBottom="2px solid rgba(107, 156, 104, 0.35)":M.pos==="article_partitive"?z.style.borderBottom="2px dashed rgba(220, 38, 38, 0.35)":M.pos==="preposition"&&(z.style.borderBottom="2px dotted rgba(197, 168, 128, 0.5)"):o?z.style.borderBottom="1px solid rgba(0, 0, 0, 0.12)":m&&(z.style.borderBottom="none")),z.addEventListener("click",V=>{V.stopPropagation(),Fe(z,M,H)}),$.appendChild(z)});const P=document.createElement("div");if(P.className="ref-para-ja",P.style.fontSize="0.94rem",P.style.lineHeight="1.6",P.style.color="var(--color-text-muted)",P.style.paddingTop="0.8rem",P.style.borderTop="1px dashed #eaeaea",P.innerHTML=w.text_ja.split(`
`).join("<br>"),L.appendChild(document.createRange().createContextualFragment(T)),L.appendChild($),i)L.appendChild(P);else{const M=document.createElement("a");M.href="#",M.className="toggle-translation-link",M.style.fontSize="0.78rem",M.style.color="var(--color-accent)",M.style.textDecoration="underline",M.style.display="block",M.style.marginTop="0.5rem",M.innerText="Afficher la traduction (日本語訳を表示)",P.style.display="none",M.addEventListener("click",q=>{q.preventDefault(),P.style.display==="none"?(P.style.display="block",M.innerText="Masquer la traduction (翻訳を非表示)"):(P.style.display="none",M.innerText="Afficher la traduction (日本語訳を表示)")}),L.appendChild(M),L.appendChild(P)}y.appendChild(L)}),y.querySelectorAll(".section-audio-btn").forEach(w=>{w.addEventListener("click",x=>{x.stopPropagation(),U(w.getAttribute("data-text"))})}),(()=>{s.querySelector("#helper-toggle-outlines").addEventListener("change",w=>{C.settings.readingHelper.outlines=w.target.checked,w.target.checked&&(C.settings.readingHelper.pureText=!1,s.querySelector("#helper-toggle-pure").checked=!1),ie(y)}),s.querySelector("#helper-toggle-translations").addEventListener("change",w=>{C.settings.readingHelper.translations=w.target.checked,w.target.checked&&(C.settings.readingHelper.pureText=!1,s.querySelector("#helper-toggle-pure").checked=!1),ie(y)}),s.querySelector("#helper-toggle-target").addEventListener("change",w=>{C.settings.readingHelper.targetOnly=w.target.checked,ie(y)}),s.querySelector("#helper-toggle-pure").addEventListener("change",w=>{C.settings.readingHelper.pureText=w.target.checked,w.target.checked&&(C.settings.readingHelper.outlines=!1,C.settings.readingHelper.translations=!1,s.querySelector("#helper-toggle-outlines").checked=!1,s.querySelector("#helper-toggle-translations").checked=!1),ie(y)})})(),ie(y),e.appendChild(y)}function pn(e,a){var l;const t=((l=C.settings)==null?void 0:l.targetLevel)||"ALL";let n="débutant";["B1","B2","intermédiaire"].includes(t)?n="intermédiaire":["C1","C2","avancé"].includes(t)&&(n="avancé");const i=n==="débutant",o=n==="intermédiaire",m=n==="avancé",r=document.createElement("button");r.className="next-btn",r.style.backgroundColor="transparent",r.style.border="1px solid var(--color-secondary)",r.style.color="var(--color-secondary)",r.style.padding="0.4rem 1rem",r.style.marginBottom="1.5rem",r.innerText="← Quitter l'entraînement (戻る)",r.addEventListener("click",()=>{re="preview",oe(e,a)}),e.appendChild(r);const g=document.createElement("div");g.className="magazine-header",g.style.textAlign="center",g.style.marginBottom="2.5rem",g.innerHTML=`
  ...
  `,e.appendChild(s);const u=document.createElement("div");u.className="magazine-body-container",u.style.maxWidth="720px",u.style.margin="0 auto",u.style.display="flex",u.style.flexDirection="column",u.style.gap="2rem",a.sessions.forEach((f,b)=>{const c=document.createElement("div");c.id=`session-card-${b}`;let _="ref-locked-session";b===X?_="ref-active-session":b<X&&(_="ref-completed-session"),c.className=`ref-session-card ${_}`;const j=document.createElement("div");j.style.display="flex",j.style.justifyContent="space-between",j.style.alignItems="center",j.style.marginBottom="0.8rem",j.innerHTML=`
      <div style="display: flex; align-items: center; gap: 0.6rem;">
        <span style="font-size: 0.75rem; text-transform: uppercase; font-weight: bold; letter-spacing: 1px; color: var(--color-text-muted);">Section ${b+1}: ${f.title}</span>
        ${b<=X?`<button class="section-audio-btn" data-text="${f.text_fr.replace(/"/g,"&quot;")}" style="background: none; border: none; font-size: 1rem; cursor: pointer; color: var(--color-accent); padding: 0.1rem; line-height: 1;" title="Lire la section">🔊</button>`:""}
      </div>
      ${b<X?'<span style="color: var(--color-success); font-size: 0.8rem; font-weight: 700;">✓ Complétée</span>':""}
    `,c.appendChild(j);const S=document.createElement("div");if(S.className="ref-text-block",b<X)S.innerHTML=`
...
`).join("<br>")}
      ...
      `;else if(b===X){const E=document.createElement("div");E.className="ref-para-fr",E.style.fontSize="1.15rem",E.style.lineHeight="1.8",E.style.fontFamily="var(--font-serif)",E.style.marginBottom="1.5rem",b===0&&E.classList.add("editorial-dropcap"),f.tokens.forEach((y,k)=>{const v=document.createElement("span");v.className=`anatomy-token token-pos-${y.pos} has-definition`,v.innerText=y.word+" ";const w=f.tokens.slice(k).map(x=>x.word).join(" ");(y.vocab_id||y.gram_id)&&(v.setAttribute("data-has-db","true"),i?y.pos==="noun"?v.style.borderBottom="2px solid var(--color-primary)":y.pos==="verb"?v.style.borderBottom="2px solid var(--color-success)":y.pos==="article_partitive"?v.style.borderBottom="2px dashed var(--color-secondary)":y.pos==="preposition"&&(v.style.borderBottom="2px dotted var(--color-accent)"):o?v.style.borderBottom="1px solid rgba(0,0,0,0.12)":m&&(v.style.borderBottom="none")),v.addEventListener("click",x=>{x.stopPropagation(),Fe(v,y,w)}),E.appendChild(v)}),S.appendChild(E);const d=document.createElement("div");d.className="ref-quiz-container",d.style.marginTop="1.5rem",d.style.padding="1.2rem",d.style.backgroundColor="#fdfcf7",d.style.border="1px solid var(--color-accent)",d.style.borderRadius="8px";const p=f.quizzes.find(y=>y.target_level===n)||f.quizzes[0];if(p){let y="";if(p.type==="choice"||p.type==="structure_analysis"){const k=m&&p.options_fr,v=k?p.options_fr:p.options,w=k?p.answer_fr:p.answer;y=`
            <div style="display: flex; flex-direction: column; gap: 0.5rem; margin-top: 0.8rem;">
              ${v.map(T=>`
                <label style="display: flex; align-items: center; gap: 0.6rem; font-size: 0.9rem; cursor: pointer;">
                  <input type="radio" name="quiz-${p.id}" value="${T}">
                  <span>${T}</span>
                </label>
              `).join("")}
            </div>
          ...
          `;const x=d.querySelector(".quiz-submit-btn"),L=d.querySelector(".quiz-feedback-msg");x.addEventListener("click",()=>{const T=d.querySelector(`input[name="quiz-${p.id}"]:checked`);T&&T.value===w?(L.style.color="var(--color-success)",L.innerText="✓ Correct ! Excellent !",x.style.display="none",setTimeout(()=>{fe(e,a,b)},1e3)):(L.style.color="var(--color-error)",L.innerText="❌ Incorrect. Réessayez !")})}else if(p.type==="preposition"){y=`
          ...
          `;const k=d.querySelector(".quiz-submit-btn"),v=d.querySelector(".quiz-feedback-msg");k.addEventListener("click",()=>{d.querySelector(`#preposition-select-${p.id}`).value===p.answer?(v.style.color="var(--color-success)",v.innerText="✓ Correct ! Très bien !",k.style.display="none",setTimeout(()=>{fe(e,a,b)},1e3)):(v.style.color="var(--color-error)",v.innerText="❌ Incorrect. Réessayez !")})}else if(p.type==="word_order"){d.innerHTML=`
          ...
          `;const k=d.querySelector(".selected-chips-area"),v=d.querySelector(".shuffled-chips-area"),w=d.querySelector(".quiz-submit-btn"),x=d.querySelector(".quiz-feedback-msg");let L=[],T=[...p.options].sort(()=>Math.random()-.5);const $=()=>{k.innerHTML="",v.innerHTML="",L.length===0&&(k.innerHTML='<span style="font-size: 0.8rem; color: #888; font-style: italic; align-self: center;">Cliquez sur les mots ci-dessous...</span>'),L.forEach((P,M)=>{const q=document.createElement("span");q.className="word-chip selected",q.innerText=P,q.addEventListener("click",()=>{L.splice(M,1),T.push(P),$()}),k.appendChild(q)}),T.forEach((P,M)=>{const q=document.createElement("span");q.className="word-chip",q.innerText=P,q.addEventListener("click",()=>{T.splice(M,1),L.push(P),$()}),v.appendChild(q)})};$(),w.addEventListener("click",()=>{const P=L.join(" "),M=p.answer_words.join(" ");P===M?(x.style.color="var(--color-success)",x.innerText="✓ Correct ! Formidable !",w.style.display="none",k.style.pointerEvents="none",v.style.pointerEvents="none",setTimeout(()=>{fe(e,a,b)},1e3)):(x.style.color="var(--color-error)",x.innerText="❌ Ordre incorrect. Réessayez !")})}else if(p.type==="kitchen_situation"){y=`
          ...
          `;const k=d.querySelector(".quiz-submit-btn"),v=d.querySelector(".quiz-feedback-msg");k.addEventListener("click",()=>{d.querySelector(`#quiz-input-${p.id}`).value.trim().toLowerCase()===p.blank_word.toLowerCase()?(v.style.color="var(--color-success)",v.innerText="✓ Oui, Chef ! Correct !",k.style.display="none",setTimeout(()=>{fe(e,a,b)},1e3)):(v.style.color="var(--color-error)",v.innerText="❌ Ce n'est pas tout à fait ça. Réessayez !")})}else if(p.type==="input"||p.type==="conjugation"){y=`
            <div style="margin-top: 0.8rem;">
          ...
          `;const k=d.querySelector(".quiz-submit-btn"),v=d.querySelector(".quiz-feedback-msg");let w=0;k.addEventListener("click",()=>{if(d.querySelector(`#quiz-input-${p.id}`).value.trim().toLowerCase()===p.blank_word.toLowerCase())v.style.color="var(--color-success)",v.innerText="✓ Correct ! Excellent !",k.style.display="none",setTimeout(()=>{fe(e,a,b)},1e3);else if(w++,v.style.color="var(--color-error)",i&&w>=2){const L=p.blank_word[0].toUpperCase();v.innerText=`❌ Incorrect. Indice : commence par "${L}"`}else v.innerText="❌ Incorrect. Réessayez !"})}}else d.innerHTML=`
        ...
        `,d.querySelector(".validate-session-btn").addEventListener("click",()=>{fe(e,a,b)});S.appendChild(d)}else S.innerHTML=`
...
`).join("<br>")}
      ...
      `;c.appendChild(S),u.appendChild(c)}),u.querySelectorAll(".section-audio-btn").forEach(f=>{f.addEventListener("click",b=>{b.stopPropagation(),U(f.getAttribute("data-text"))})}),(()=>{s.querySelector("#helper-toggle-outlines").addEventListener("change",f=>{C.settings.readingHelper.outlines=f.target.checked,f.target.checked&&(C.settings.readingHelper.pureText=!1,s.querySelector("#helper-toggle-pure").checked=!1),ie(u)}),s.querySelector("#helper-toggle-translations").addEventListener("change",f=>{C.settings.readingHelper.translations=f.target.checked,f.target.checked&&(C.settings.readingHelper.pureText=!1,s.querySelector("#helper-toggle-pure").checked=!1),ie(u)}),s.querySelector("#helper-toggle-target").addEventListener("change",f=>{C.settings.readingHelper.targetOnly=f.target.checked,ie(u)}),s.querySelector("#helper-toggle-pure").addEventListener("change",f=>{C.settings.readingHelper.pureText=f.target.checked,f.target.checked&&(C.settings.readingHelper.outlines=!1,C.settings.readingHelper.translations=!1,s.querySelector("#helper-toggle-outlines").checked=!1,s.querySelector("#helper-toggle-translations").checked=!1),ie(u)})})(),ie(u),e.appendChild(u)}function fe(e,a,t){X=t+1,X>=a.sessions.length?(re="complete",oe(e,a)):(oe(e,a),setTimeout(()=>{const n=document.getElementById(`session-card-${X}`);n&&n.scrollIntoView({behavior:"smooth",block:"center"})},100))}function gn(e,a){const t=document.createElement("div");t.className="magazine-header",t.style.textAlign="center",t.style.marginBottom="2.5rem",t.innerHTML=`
    <div style="font-family: var(--font-serif); font-size: 0.85rem; letter-spacing: 2px; text-transform: uppercase; color: var(--color-text-muted); margin-bottom: 0.5rem; border-bottom: 2px double rgba(0,0,91,0.1); padding-bottom: 0.4rem; font-weight: bold;">
  ...
  `,e.appendChild(t);const n=document.createElement("div");n.className="newspaper-columns",n.style.fontSize="1.18rem",n.style.lineHeight="1.8",n.style.fontFamily="var(--font-serif)",n.style.color="var(--color-primary)",n.style.marginBottom="2.5rem";let i="";a.sessions.forEach((g,s)=>{i+=`
      <p class="${s===0?"editorial-dropcap":""}" style="margin-bottom: 1.5rem; text-align: justify; text-justify: inter-word;">
        ${g.text_fr.split(`
`).join("<br>")}
      </p>
    `}),n.innerHTML=i,e.appendChild(n);const o=document.createElement("div");o.style.display="flex",o.style.justifyContent="center",o.style.gap="1rem",o.style.marginTop="2rem";const m=document.createElement("button");m.className="next-btn",m.innerText="← Retour à l'aperçu (予習モードに戻る)",m.addEventListener("click",()=>{re="preview",X=0,oe(e,a)});const r=document.createElement("button");r.className="next-btn",r.style.backgroundColor="transparent",r.style.border="1px solid var(--color-primary)",r.style.color="var(--color-primary)",r.innerText="📚 Liste des articles (目次へ戻る)",r.addEventListener("click",()=>{Ne(e,[a])}),o.appendChild(m),o.appendChild(r),e.appendChild(o)}function Fe(e,a,t){he&&he.remove();const n=document.createElement("div");n.className="anatomy-tooltip card",n.style.position="absolute",n.style.zIndex="1000",n.style.padding="1rem",n.style.width="290px",n.style.boxShadow="var(--shadow-lg)",n.style.border="1px solid rgba(0, 0, 91, 0.15)",n.style.borderRadius="8px",n.style.backgroundColor="#FFFFFF";const i=a.vocab_id||a.gram_id,o=C.db.knowledge.find(b=>b.id===i),r={noun:"Nom (名詞)",verb:"Verbe (動詞)",preposition:"Préposition (前置詞)",article_partitive:"Article partitif (部分冠詞)",article:"Article (冠詞)",pronoun_subject:"Pronom (代名詞)",conjunction:"Conjonction (接続詞)",other:"Autre (その他)"}[a.pos]||a.pos||"Autre (その他)";let g="";if(a.analysis&&(g=`
    ...
    `),o){const b=!!o.french,c=`<span style="font-size: 0.85rem; font-weight: bold; color: var(--color-secondary);">${o.level}</span>`;if(b){const _=o.gender?o.gender==="m"?"Masculin":o.gender==="f"?"Féminin":"M/F":"",j=_?`<span style="font-size: 0.7rem; font-weight: 600; padding: 0.05rem 0.3rem; border-radius: 3px; background-color: #f0f4f8; color: var(--color-primary); border: 1px solid rgba(0,0,91,0.1);">${_}</span>`:"",S=o.verb_group?o.verb_group===1?"1er groupe":o.verb_group===2?"2e groupe":"3e groupe":"",E=S?`<span style="font-size: 0.7rem; font-weight: 600; padding: 0.05rem 0.3rem; border-radius: 3px; background-color: rgba(107,156,104,0.08); color: var(--color-success); border: 1px solid rgba(107,156,104,0.15);">${S}</span>`:"";n.innerHTML=`
    ...
    `;document.body.appendChild(n),he=n;const s=e.getBoundingClientRect(),u=s.bottom+window.scrollY+8,h=Math.max(10,Math.min(window.innerWidth-300,s.left+window.scrollX-100));n.style.top=`${u}px`,n.style.left=`${h}px`;const l=n.querySelector(".play-here-btn");l&&l.addEventListener("click",b=>{b.stopPropagation();const c=a.word.replace(/^[^\wÀ-ÿ]+|[^\wÀ-ÿ]+$/g,"");U(c)});const f=n.querySelector(".play-from-here-btn");f&&t&&f.addEventListener("click",b=>{b.stopPropagation(),U(t)})}const C={meta:null,db:{knowledge:[],quizzes:[]},loaded:{knowledge:new Set,quizzes:!1},favorites:new Set(JSON.parse(localStorage.getItem("cba_favorites")||"[]")),wrongAnswers:JSON.parse(localStorage.getItem("cba_wrong")||"[]"),streak:parseInt(localStorage.getItem("cba_streak")||"0"),lastStudyDate:localStorage.getItem("cba_last_study")||"",srs:JSON.parse(localStorage.getItem("cba_srs")||"{}"),settings:JSON.parse(localStorage.getItem("cba_settings")||JSON.stringify({targetLevel:"ALL",newCardsPerDay:5,maxReviewsPerDay:20,includeGeneral:!1}))};function de(e){C.favorites.has(e)?C.favorites.delete(e):C.favorites.add(e),localStorage.setItem("cba_favorites",JSON.stringify(Array.from(C.favorites)))}function se(e){return C.favorites.has(e)}function Ke(e){C.wrongAnswers.includes(e)||(C.wrongAnswers.push(e),localStorage.setItem("cba_wrong",JSON.stringify(C.wrongAnswers)))}function fn(e){C.wrongAnswers=C.wrongAnswers.filter(a=>a!==e),localStorage.setItem("cba_wrong",JSON.stringify(C.wrongAnswers))}function hn(){const e=new Date().toISOString().split("T")[0];if(C.lastStudyDate!==e){if(C.lastStudyDate){const a=new Date(C.lastStudyDate),t=new Date(e),n=Math.abs(t-a),i=Math.ceil(n/(1e3*60*60*24));i===1?C.streak+=1:i>1&&(C.streak=1)}else C.streak=1;C.lastStudyDate=e,localStorage.setItem("cba_streak",C.streak.toString()),localStorage.setItem("cba_last_study",e)}}function Se(e,a){const t=new Date().toISOString().split("T")[0],n=C.srs[e]||{easiness:2.5,interval:0,repetitions:0,dueDate:t};let i=Math.max(0,Math.min(5,a));i>=3?(n.repetitions===0?n.interval=1:n.repetitions===1?n.interval=6:n.interval=Math.round(n.interval*n.easiness),n.repetitions++):(n.repetitions=0,n.interval=1),n.easiness=n.easiness+(.1-(5-i)*(.08+(5-i)*.02)),n.easiness<1.3&&(n.easiness=1.3);const o=new Date;o.setDate(o.getDate()+n.interval),n.dueDate=o.toISOString().split("T")[0],n.lastRated=t,C.srs[e]=n,localStorage.setItem("cba_srs",JSON.stringify(C.srs))}function yn(e){C.settings={...C.settings,...e},localStorage.setItem("cba_settings",JSON.stringify(C.settings))}async function ee(e,a){const t=a==="ALL"?["A1","A2","B1","B2","C1","C2"]:[a];for(const n of t)if(!C.loaded.knowledge.has(n))try{const i=await fetch(`data/knowledge_${n}.json`);if(!i.ok){console.warn(`Could not load data/knowledge_${n}.json`);continue}const o=await i.json(),m=new Set(C.db.knowledge.map(r=>r.id));for(const r of o)m.has(r.id)||(!r.context_fr&&r.examples&&r.examples.length>0&&(r.context_fr=r.examples[0].fr,r.context_en=r.examples[0].en,r.context_ja=r.examples[0].ja),C.db.knowledge.push(r));C.loaded.knowledge.add(n)}catch(i){console.error(`Failed to load knowledge level ${n}:`,i)}}async function Oe(){if(!C.loaded.quizzes)try{const e=await fetch("data/quizzes.json");if(!e.ok)throw new Error("Network response was not ok");C.db.quizzes=await e.json(),C.loaded.quizzes=!0}catch(e){console.error("Failed to load quizzes:",e)}}async function De(){await Promise.all([ee("vocabulary","ALL"),ee("grammar","ALL"),ee("cuisine","ALL"),Oe()])}const Ze={home:Ct,vocabulary:St,grammar:Tt,reference:Et,cuisine:Gt,quiz:Jt,favorites:ye,review:Zt,search:en,settings:tn,dictation:nn,story:ln,reading:dn};function be(e){const a=document.getElementById("main-content");if(Ze[e]){hn(),a.innerHTML="";const t=Ze[e]();t instanceof HTMLElement?a.appendChild(t):a.innerHTML=t,document.querySelectorAll(".nav-link").forEach(n=>{n.getAttribute("data-tab")===e?n.classList.add("active"):n.classList.remove("active")})}}async function bn(){if(!window.__app_initialized){window.__app_initialized=!0;try{const e=await fetch("data/meta.json");C.meta=await e.json();const a=document.getElementById("hamburger-menu-toggle"),t=document.querySelector(".nav-band");a&&t&&(a.addEventListener("click",n=>{n.stopPropagation(),t.classList.toggle("mobile-open"),a.classList.toggle("active")}),document.addEventListener("click",n=>{!n.target.closest(".nav-band")&&!n.target.closest("#hamburger-menu-toggle")&&(t.classList.remove("mobile-open"),a.classList.remove("active"))})),document.querySelectorAll(".nav-link").forEach(n=>{n.addEventListener("click",i=>{const o=i.target.closest(".nav-link").getAttribute("data-tab");be(o),t&&t.classList.contains("mobile-open")&&(t.classList.remove("mobile-open"),a&&a.classList.remove("active"))})}),be("home"),_n()}catch(e){console.error("Failed to load database metadata:",e),document.getElementById("main-content").innerHTML=`
      <div class="alert alert-info" style="border-left-color: var(--color-error); background-color: #FFEBEE; color: var(--color-error)">
        <h3>Error: Métadonnées inaccessibles</h3>
        <p>Could not load curriculum metadata. Please reload or check your local setup.</p>
      </div>
    `}}}function _n(){const e=document.getElementById("youglish-sidebar"),a=document.getElementById("youglish-sidebar-toggle"),t=document.getElementById("youglish-search-input"),n=document.getElementById("youglish-search-btn"),i=document.getElementById("yg-widget-container");if(!e||!a||!t||!n||!i)return;a.addEventListener("click",r=>{r.stopPropagation(),e.classList.toggle("open"),e.classList.contains("open")&&t.focus()}),document.addEventListener("click",r=>{e.classList.contains("open")&&!e.contains(r.target)&&e.classList.remove("open")});let o=null;function m(){const r=t.value.trim();if(r)if(i.innerHTML='<div id="yg-widget" style="width: 100%;"></div>',window.YG)try{o=new YG("yg-widget",{width:230,components:9,accentColor:"#C5A880",autoStart:1,search:r,language:"french"})}catch(g){console.error("Failed to load YouGlish widget:",g),i.innerHTML=`
        ...
        `}else window.open(`https://youglish.com/pronounce/${encodeURIComponent(r)}/french`,"_blank"),i.innerHTML=`
      ...
      `}n.addEventListener("click",m),t.addEventListener("keypress",r=>{r.key==="Enter"&&m()})}"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("sw.js").then(e=>console.log("ServiceWorker registered:",e.scope)).catch(e=>console.warn("ServiceWorker registration failed:",e))});document.addEventListener("DOMContentLoaded",bn);
