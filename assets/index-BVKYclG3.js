(function(){const u=document.createElement("link").relList;if(u&&u.supports&&u.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))r(n);new MutationObserver(n=>{for(const c of n)if(c.type==="childList")for(const k of c.addedNodes)k.tagName==="LINK"&&k.rel==="modulepreload"&&r(k)}).observe(document,{childList:!0,subtree:!0});function l(n){const c={};return n.integrity&&(c.integrity=n.integrity),n.referrerPolicy&&(c.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?c.credentials="include":n.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function r(n){if(n.ep)return;n.ep=!0;const c=l(n);fetch(n.href,c)}})();let X=null,Z="",ee=0,K=null,D=null;function z(t,u=1){ue(t,u)}function ue(t,u=1,l=null,r=null,n=0){if(!("speechSynthesis"in window)){console.warn("Speech synthesis not supported on this browser.");return}window.speechSynthesis.cancel();const c=t.replace(/["'➔]/g,"").trim();Z=c,ee=n,K=l,D=r;const k=c.substring(n);if(!k.trim()){D&&D();return}const S=new SpeechSynthesisUtterance(k);S.lang="fr-FR",S.rate=u;const C=window.speechSynthesis.getVoices().find(i=>i.lang==="fr-FR"||i.lang.startsWith("fr"));C&&(S.voice=C),S.onboundary=i=>{if(i.name==="word"&&K){const g=ee+i.charIndex;K(g,Z.length)}},S.onend=()=>{X===S&&D&&D()},S.onerror=i=>{console.warn("SpeechSynthesis error:",i),D&&D()},X=S,window.speechSynthesis.speak(S)}function ge(){"speechSynthesis"in window&&window.speechSynthesis.pause()}function fe(){"speechSynthesis"in window&&window.speechSynthesis.resume()}function J(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}"speechSynthesis"in window&&(window.speechSynthesis.getVoices(),window.speechSynthesis.onvoiceschanged=()=>{window.speechSynthesis.getVoices()});function ve(){var o,f,v,_,b,w,L;const t=document.createElement("div"),u=document.createElement("h2");u.className="section-title",u.innerText="Tableau de Bord",t.appendChild(u);const l=document.createElement("p");l.className="section-subtitle",l.innerText="Welcome back, Commis. Here is your kitchen curriculum status.",t.appendChild(l);const r=new Date().toISOString().split("T")[0],n=Object.values(h.srs),c=n.filter(j=>j.dueDate<=r).length,k=n.length,S=document.createElement("div");S.className="dashboard-grid";const s=document.createElement("div");s.style.display="flex",s.style.flexDirection="column",s.style.gap="1.5rem";const C=document.createElement("div");C.className="card",C.style.padding="1.5rem",C.innerHTML=`
    ...
    <ul style="list-style: none; display: flex; flex-direction: column; gap: 0.8rem;">
      <li style="display: flex; align-items: center; gap: 0.8rem; font-size: 0.95rem;">
        <input type="checkbox" id="mission-vocab" style="transform: scale(1.2); cursor: pointer;" ${k>0?"checked":""}>
        <label for="mission-vocab" style="cursor: pointer;">Study new terms in the Vocabulary deck</label>
      </li>
      <li style="display: flex; align-items: center; gap: 0.8rem; font-size: 0.95rem;">
        <input type="checkbox" id="mission-srs" style="transform: scale(1.2); cursor: pointer;" ${c===0&&k>0?"checked":""}>
  ...
  `,s.appendChild(C);const i=document.createElement("div");i.className="card",i.style.padding="1.5rem";const g=((o=h.meta)==null?void 0:o.featured)||null;i.innerHTML=`
    ...
    ${g?`
      <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem;">
        <div style="font-size: 1.25rem; font-weight: 600; color: var(--color-accent);">${g.french}</div>
        <button class="audio-btn" data-text="${g.french}" title="Listen pronunciation" style="background: none; border: none; font-size: 1.1rem; cursor: pointer; color: var(--color-accent); padding: 0.1rem; line-height: 1;">🔊</button>
      </div>
      <div style="font-size: 0.9rem; color: var(--color-text-main); font-weight: 500; margin-bottom: 0.5rem;">${g.english} / ${g.japanese}</div>
      <div style="background-color: rgba(197, 168, 128, 0.08); border-left: 2px solid var(--color-accent); padding: 0.8rem; font-size: 0.85rem; border-radius: 0 var(--radius-sm) var(--radius-sm) 0;">
        <div style="display: flex; align-items: flex-start; gap: 0.4rem; font-style: italic; color: var(--color-secondary); font-weight: 500;">
          ...
          <button class="audio-btn" data-text="${g.context_fr}" title="Listen context sentence" style="background: none; border: none; font-size: 0.9rem; cursor: pointer; color: var(--color-text-muted); padding: 0.15rem; margin-top: 0.1rem; line-height: 1;">🔊</button>
        </div>
        <div style="color: var(--color-text-muted); margin-top: 0.2rem;">${g.context_ja}</div>
      </div>
    `:"<p>No data loaded.</p>"}
  `,g&&i.querySelectorAll(".audio-btn").forEach(j=>{j.addEventListener("click",x=>{x.stopPropagation();const $=x.target.closest(".audio-btn").getAttribute("data-text");z($)})}),s.appendChild(i),S.appendChild(s);const e=document.createElement("div");e.style.display="flex",e.style.flexDirection="column",e.style.gap="1.5rem";const m=document.createElement("div");m.className="streak-card",m.innerHTML=`
  ...
  `,e.appendChild(m);const p=document.createElement("div");p.className="card",p.style.padding="1.5rem";const d=((v=(f=h.meta)==null?void 0:f.counts)==null?void 0:v.vocabulary)||0,a=((b=(_=h.meta)==null?void 0:_.counts)==null?void 0:b.grammar)||0,y=((L=(w=h.meta)==null?void 0:w.counts)==null?void 0:L.cuisine)||0;return p.innerHTML=`
    ...
    <div style="display: flex; flex-direction: column; gap: 0.8rem; font-size: 0.9rem;">
      ...
      <div style="border-top: 1px solid rgba(0,0,0,0.05); padding-top: 0.8rem; display: flex; justify-content: space-between; align-items: center;">
        ...
        <strong style="color: var(--color-accent);">${h.favorites.size} items</strong>
      </div>
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <span>Deck SRS Actif (Active SRS):</span>
        <strong style="color: var(--color-primary);">${k} cards</strong>
      </div>
      <div style="display: flex; justify-content: space-between; align-items: center;">
  ...
  `,e.appendChild(p),S.appendChild(e),t.appendChild(S),t}function he(){var c;const t=document.createElement("div"),u=document.createElement("h2");u.className="section-title",u.innerText="Vocabulaire Professionnel",t.appendChild(u);const l=document.createElement("p");l.className="section-subtitle",l.innerText="Essential culinary terminology for kitchen brigade communications. Filter by category, tag, or difficulty level.",t.appendChild(l);const r=document.createElement("div");r.className="loading-placeholder",r.innerText="Chargement du vocabulaire... (Loading vocabulary...)",t.appendChild(r);const n=((c=h.settings)==null?void 0:c.targetLevel)||"ALL";return M("vocabulary",n).then(()=>{r.remove(),be(t,n)}),t}function be(t,u){var p,d;const l=((p=h.settings)==null?void 0:p.includeGeneral)||!1,n=(((d=h.db)==null?void 0:d.vocabulary)||[]).filter(a=>l||a.is_professional),c=["ALL",...new Set(n.map(a=>a.category))],k=["ALL",...new Set(n.flatMap(a=>a.tags||[]))],S=["ALL","A1","A2","B1","B2","C1","C2"];let s="ALL",C="ALL",i=u;const g=document.createElement("div");g.className="card",g.style.padding="1.2rem",g.style.marginBottom="2rem",g.style.display="flex",g.style.flexDirection="column",g.style.gap="1rem",g.innerHTML=`
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
      <div>
        ...
        <select id="vocab-category-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${c.map(a=>`<option value="${a}" ${a===s?"selected":""}>${a}</option>`).join("")}
        </select>
      </div>
      <div>
        ...
        <select id="vocab-tag-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${k.map(a=>`<option value="${a}" ${a===C?"selected":""}>${a==="ALL"?"ALL TAGS":"#"+a}</option>`).join("")}
        </select>
      </div>
      <div>
        ...
        <select id="vocab-level-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${S.map(a=>`<option value="${a}" ${a===i?"selected":""}>${a==="ALL"?"ALL LEVELS":a}</option>`).join("")}
        </select>
      </div>
    </div>
  `,t.appendChild(g);const e=document.createElement("div");e.className="card-grid",t.appendChild(e);function m(){var f;e.innerHTML="";const o=(((f=h.db)==null?void 0:f.vocabulary)||[]).filter(v=>l||v.is_professional).filter(v=>{const _=s==="ALL"||v.category===s,b=C==="ALL"||v.tags&&v.tags.includes(C),w=i==="ALL"||v.level===i;return _&&b&&w});if(o.length===0){e.innerHTML='<p style="color: var(--color-text-muted); grid-column: 1 / -1; text-align: center; padding: 2rem;">Aucun terme trouvé correspondant à vos critères de filtrage.</p>';return}o.forEach(v=>{const _=document.createElement("div");_.className="card";const b=F(v.id),w=h.srs[v.id],L=!!w;_.innerHTML=`
          ...
          <div>
            ...
            ${v.tags&&v.tags.length>0?`
              <div style="margin-top: 1rem; display: flex; flex-wrap: wrap; gap: 0.3rem;">
                ${v.tags.map(x=>`<span class="tag-badge" style="background-color: rgba(197, 168, 128, 0.08); color: var(--color-accent); font-size: 0.75rem; padding: 0.1rem 0.5rem; border-radius: 12px;">#${x}</span>`).join("")}
              </div>
            `:""}
          </div>
      ...
      `;const j=_.querySelector(".flip-translation-container");j.addEventListener("click",x=>{x.stopPropagation(),j.querySelector(".flip-translation-card").classList.toggle("flipped")}),_.querySelector(".fav-btn").addEventListener("click",x=>{x.stopPropagation(),H(v.id);const $=x.target,T=F(v.id);$.classList.toggle("active",T),$.innerText=T?"★":"☆"}),_.querySelectorAll(".audio-btn").forEach(x=>{x.addEventListener("click",$=>{$.stopPropagation();const T=$.target.closest(".audio-btn").getAttribute("data-text");z(T)})}),_.querySelectorAll(".srs-action-btn").forEach(x=>{x.addEventListener("click",$=>{$.stopPropagation();const T=$.target.getAttribute("data-action"),q=$.target.getAttribute("data-id");T==="start"?W(q,4):T==="memorized"?W(q,5):T==="reset"&&W(q,0),m()})}),e.appendChild(_)})}g.querySelector("#vocab-category-select").addEventListener("change",a=>{s=a.target.value,m()}),g.querySelector("#vocab-tag-select").addEventListener("change",a=>{C=a.target.value,m()}),g.querySelector("#vocab-level-select").addEventListener("change",async a=>{var j;i=a.target.value,e.innerHTML=`<div style="grid-column: 1 / -1; text-align: center; padding: 2rem; color: var(--color-text-muted);">Chargement (Loading level ${i})...</div>`,await M("vocabulary",i);const o=(((j=h.db)==null?void 0:j.vocabulary)||[]).filter(x=>l||x.is_professional),f=["ALL",...new Set(o.map(x=>x.category))],v=["ALL",...new Set(o.flatMap(x=>x.tags||[]))],_=g.querySelector("#vocab-category-select"),b=_.value;_.innerHTML=f.map(x=>`<option value="${x}" ${x===b?"selected":""}>${x}</option>`).join(""),s=_.value;const w=g.querySelector("#vocab-tag-select"),L=w.value;w.innerHTML=v.map(x=>`<option value="${x}" ${x===L?"selected":""}>${x==="ALL"?"ALL TAGS":"#"+x}</option>`).join(""),C=w.value,m()}),m()}function ye(){var c;const t=document.createElement("div"),u=document.createElement("h2");u.className="section-title",u.innerText="Grammaire de la Cuisine",t.appendChild(u);const l=document.createElement("p");l.className="section-subtitle",l.innerText="French grammar concepts framed through recipe instructions and professional dialogues.",t.appendChild(l);const r=document.createElement("div");r.className="loading-placeholder",r.innerText="Chargement de la grammaire... (Loading grammar...)",t.appendChild(r);const n=((c=h.settings)==null?void 0:c.targetLevel)||"ALL";return M("grammar",n).then(()=>{r.remove(),_e(t,n)}),t}function _e(t,u){var n;const r=(((n=h.db)==null?void 0:n.grammar)||[]).filter(c=>u==="ALL"||c.level===u);if(r.length===0){const c=document.createElement("p");c.style.color="var(--color-text-muted)",c.innerText="Aucune leçon de grammaire chargée pour ce niveau.",t.appendChild(c);return}r.forEach(c=>{const k=document.createElement("div");k.className="grammar-card";const S=F(c.id),s=c.topic.match(/\(([^)]+)\)/),C=s?s[1]:c.topic;k.innerHTML=`
      <div class="grammar-header">
        ...
        <div style="display: flex; align-items: center; gap: 1rem;">
          <button class="fav-btn ${S?"active":""}" data-id="${c.id}" style="font-size: 1.1rem; padding: 0.2rem;">
            ${S?"★":"☆"}
          </button>
  ...
  `,t.appendChild(r);const n=document.createElement("div");n.className="cuisine-content-wrapper",t.appendChild(n),r.querySelectorAll(".cuisine-tab-btn").forEach(i=>{i.addEventListener("click",g=>{r.querySelectorAll(".cuisine-tab-btn").forEach(m=>m.classList.remove("active")),g.target.classList.add("active");const e=g.target.getAttribute("data-tab");c(e)})});function c(i){n.innerHTML="",i==="theory"?k():i==="meat"?S():i==="map"&&C()}function k(){var g;n.innerHTML='<div style="text-align: center; padding: 2rem; color: var(--color-text-muted);">Chargement de la théorie... (Loading theory...)</div>';const i=((g=h.settings)==null?void 0:g.targetLevel)||"ALL";M("cuisine",i).then(()=>{var d;n.innerHTML="";const m=(((d=h.db)==null?void 0:d.cuisine)||[]).filter(a=>i==="ALL"||a.level===i);if(m.length===0){n.innerHTML='<p style="color: var(--color-text-muted);">Aucun document de théorie culinaire disponible pour ce niveau.</p>';return}const p=document.createElement("div");p.style.display="flex",p.style.flexDirection="column",p.style.gap="2rem",m.forEach(a=>{const y=document.createElement("div");y.className="card",y.style.display="block",y.style.padding="2rem";const o=F(a.id),f=a.topic.match(/^([^(]+)/),v=f?f[1].trim():a.topic;y.innerHTML=`
          <div class="card-category" style="margin-bottom: 0.5rem;">${a.category}</div>
          <div class="term-header" style="border-bottom: 1px solid rgba(197, 168, 128, 0.15); padding-bottom: 0.8rem; margin-bottom: 1.5rem;">
            <div style="display: flex; align-items: center; gap: 0.6rem; flex-wrap: wrap;">
              <h3 style="font-family: var(--font-serif); font-size: 1.6rem; color: var(--color-primary); margin: 0;">${a.topic}</h3>
            ...
            <button class="fav-btn ${o?"active":""}" data-id="${a.id}" style="font-size: 1.3rem;">
              ${o?"★":"☆"}
            </button>
          </div>
          ...
          <div style="display: flex; flex-direction: column; gap: 1.5rem;">
            <div>
              <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.3rem;">
                ...
                <button class="audio-btn" data-text="${a.content_fr}" title="Listen paragraph" style="background: none; border: none; font-size: 0.95rem; cursor: pointer; color: var(--color-accent); padding: 0.15rem; line-height: 1;">🔊</button>
              </div>
              <p style="font-size: 0.95rem; color: var(--color-primary); font-style: italic; text-align: justify; line-height: 1.6;">${a.content_fr}</p>
            </div>
            <div>
              ...
              <p style="font-size: 0.9rem; color: var(--color-text-main); text-align: justify; line-height: 1.6;">${a.content_en}</p>
            </div>
            <div style="background-color: rgba(10, 25, 49, 0.03); padding: 1.2rem; border-radius: var(--radius-sm); border-left: 3px solid var(--color-secondary);">
              ...
              <p style="font-size: 0.9rem; color: var(--color-text-main); text-align: justify; line-height: 1.6;">${a.content_ja}</p>
            </div>
          </div>
        `,y.querySelector(".fav-btn").addEventListener("click",_=>{_.stopPropagation(),H(a.id);const b=_.target,w=F(a.id);b.classList.toggle("active",w),b.innerText=w?"★":"☆"}),y.querySelectorAll(".audio-btn").forEach(_=>{_.addEventListener("click",b=>{b.stopPropagation();const w=b.target.closest(".audio-btn").getAttribute("data-text");z(w)})}),p.appendChild(y)}),n.appendChild(p)})}function S(){const i=document.createElement("div");i.className="meat-section-container";const g=document.createElement("div");g.className="meat-type-tabs",g.innerHTML=`
    ...
    `,i.appendChild(g);const e=document.createElement("div");e.className="meat-display-area",i.appendChild(e),n.appendChild(i),g.querySelectorAll(".meat-tab-btn").forEach(m=>{m.addEventListener("click",p=>{g.querySelectorAll(".meat-tab-btn").forEach(y=>y.classList.remove("active"));const d=p.target.closest(".meat-tab-btn");d.classList.add("active");const a=d.getAttribute("data-type");s(a,e)})}),s("beef",e)}function s(i,g){if(g.innerHTML="",i==="beef"){const e=document.createElement("div");e.innerHTML=`
      ...
      `,g.appendChild(e);const m=e.querySelector("#beef-detail-drawer"),p=e.querySelectorAll(".interactive-area");p.forEach(d=>{d.addEventListener("click",a=>{p.forEach(f=>f.classList.remove("active")),a.target.classList.add("active");const y=a.target.getAttribute("data-id"),o=te.find(f=>f.id===y);if(o){e.querySelector("#beef-cut-title").innerText=`${o.name_fr} (${o.name_ja})`,e.querySelector("#beef-cut-sub").innerText=`Cut #${o.number} • ${o.name_en}`,e.querySelector("#beef-prop-tenderness").innerText=o.properties.tenderness,e.querySelector("#beef-prop-fat").innerText=o.properties.fat,e.querySelector("#beef-prop-collagen").innerText=o.properties.collagen,e.querySelector("#beef-cooking").innerText=o.cooking,e.querySelector("#beef-classification").innerText=o.classification,e.querySelector("#beef-logic").innerText=o.logic,e.querySelector("#beef-science").innerText=o.science,e.querySelector("#beef-chef-note").innerText=o.chef_note;const f=e.querySelector("#beef-relations-container"),v=e.querySelector("#beef-relations-content"),_=I(y,"cut");_?(v.innerHTML=_,f.style.display="block"):f.style.display="none";const b=e.querySelector("#beef-audio-title-btn");b.style.display="inline-block",b.onclick=()=>z(o.name_fr),m.style.display="block"}})})}else if(i==="porc"){const e=document.createElement("div");e.innerHTML=`
      ...
      `,g.appendChild(e);const m=e.querySelector("#pork-detail-drawer"),p=e.querySelectorAll(".interactive-area");p.forEach(d=>{d.addEventListener("click",a=>{p.forEach(f=>f.classList.remove("active")),a.target.classList.add("active");const y=a.target.getAttribute("data-id"),o=ne.find(f=>f.id===y);if(o){e.querySelector("#pork-cut-title").innerText=`${o.name_fr} (${o.name_ja})`,e.querySelector("#pork-cut-sub").innerText=`Cut #${o.number} • ${o.name_en}`,e.querySelector("#pork-prop-tenderness").innerText=o.properties.tenderness,e.querySelector("#pork-prop-fat").innerText=o.properties.fat,e.querySelector("#pork-prop-collagen").innerText=o.properties.collagen,e.querySelector("#pork-cooking").innerText=o.cooking,e.querySelector("#pork-classification").innerText=o.classification,e.querySelector("#pork-logic").innerText=o.logic,e.querySelector("#pork-science").innerText=o.science,e.querySelector("#pork-chef-note").innerText=o.chef_note;const f=e.querySelector("#pork-relations-container"),v=e.querySelector("#pork-relations-content"),_=I(y,"cut");_?(v.innerHTML=_,f.style.display="block"):f.style.display="none";const b=e.querySelector("#pork-audio-title-btn");b.style.display="inline-block",b.onclick=()=>z(o.name_fr),m.style.display="block"}})})}else if(i==="volaille"){const e=document.createElement("div");e.innerHTML=`
        <div style="display: flex; justify-content: flex-end; margin-bottom: 0.8rem;">
      ...
      `,g.appendChild(e);const m=e.querySelector("#poultry-detail-drawer"),p=e.querySelectorAll(".interactive-area");p.forEach(d=>{d.addEventListener("click",a=>{p.forEach(f=>f.classList.remove("active")),a.target.classList.add("active");const y=a.target.getAttribute("data-id"),o=ae.find(f=>f.id===y);if(o){e.querySelector("#poultry-cut-title").innerText=`${o.name_fr} (${o.name_ja})`,e.querySelector("#poultry-cut-sub").innerText=`Cut #${o.number} • ${o.name_en}`,e.querySelector("#poultry-prop-tenderness").innerText=o.properties.tenderness,e.querySelector("#poultry-prop-fat").innerText=o.properties.fat,e.querySelector("#poultry-prop-collagen").innerText=o.properties.collagen,e.querySelector("#poultry-cooking").innerText=o.cooking,e.querySelector("#poultry-classification").innerText=o.classification,e.querySelector("#poultry-logic").innerText=o.logic,e.querySelector("#poultry-science").innerText=o.science,e.querySelector("#poultry-chef-note").innerText=o.chef_note;const f=e.querySelector("#poultry-relations-container"),v=e.querySelector("#poultry-relations-content"),_=I(y,"cut");_?(v.innerHTML=_,f.style.display="block"):f.style.display="none";const b=e.querySelector("#poultry-audio-title-btn");b.style.display="inline-block",b.onclick=()=>z(o.name_fr),m.style.display="block"}})})}else if(i==="poisson"){const e=document.createElement("div");e.innerHTML=`
        <div class="interactive-canvas-container" style="position: relative;">
      ...
      `,g.appendChild(e);const m=e.querySelector("#fish-detail-drawer"),p=e.querySelectorAll(".interactive-area");p.forEach(d=>{d.addEventListener("click",a=>{p.forEach(f=>f.classList.remove("active")),a.target.classList.add("active");const y=a.target.getAttribute("data-id"),o=re.find(f=>f.id===y);if(o){e.querySelector("#fish-cut-title").innerText=`${o.name_fr} (${o.name_ja})`,e.querySelector("#fish-cut-sub").innerText=`Cut #${o.number} • ${o.name_en}`,e.querySelector("#fish-prop-tenderness").innerText=o.properties.tenderness,e.querySelector("#fish-prop-fat").innerText=o.properties.fat,e.querySelector("#fish-prop-collagen").innerText=o.properties.collagen,e.querySelector("#fish-cooking").innerText=o.cooking,e.querySelector("#fish-classification").innerText=o.classification,e.querySelector("#fish-logic").innerText=o.logic,e.querySelector("#fish-science").innerText=o.science,e.querySelector("#fish-chef-note").innerText=o.chef_note;const f=e.querySelector("#fish-relations-container"),v=e.querySelector("#fish-relations-content"),_=I(y,"cut");_?(v.innerHTML=_,f.style.display="block"):f.style.display="none";const b=e.querySelector("#fish-audio-title-btn");b.style.display="inline-block",b.onclick=()=>z(o.name_fr),m.style.display="block"}})})}else if(i==="autre"){const e=document.createElement("div");e.innerHTML=`
        <div class="interactive-canvas-container" style="position: relative;">
      ...
      `,g.appendChild(e);const m=e.querySelector("#other-detail-drawer"),p=e.querySelectorAll(".interactive-area");p.forEach(d=>{d.addEventListener("click",a=>{p.forEach(f=>f.classList.remove("active")),a.target.classList.add("active");const y=a.target.getAttribute("data-id"),o=ie.find(f=>f.id===y);if(o){e.querySelector("#other-cut-title").innerText=`${o.name_fr} (${o.name_ja})`,e.querySelector("#other-cut-sub").innerText=`Cut #${o.number} • ${o.name_en}`,e.querySelector("#other-prop-tenderness").innerText=o.properties.tenderness,e.querySelector("#other-prop-fat").innerText=o.properties.fat,e.querySelector("#other-prop-collagen").innerText=o.properties.collagen,e.querySelector("#other-cooking").innerText=o.cooking,e.querySelector("#other-classification").innerText=o.classification,e.querySelector("#other-logic").innerText=o.logic,e.querySelector("#other-science").innerText=o.science,e.querySelector("#other-chef-note").innerText=o.chef_note;const f=e.querySelector("#other-relations-container"),v=e.querySelector("#other-relations-content"),_=I(y,"cut");_?(v.innerHTML=_,f.style.display="block"):f.style.display="none";const b=e.querySelector("#other-audio-title-btn");b.style.display="inline-block",b.onclick=()=>z(o.name_fr),m.style.display="block"}})})}}function C(){const i=document.createElement("div");i.innerHTML=`
      <div class="interactive-canvas-container" style="position: relative;">
        ...
        <svg class="interactive-svg-overlay" viewBox="0 0 100 100">
          ${oe.map(m=>`
            <polygon class="interactive-area" points="${m.points}" data-id="${m.id}" />
          `).join("")}
        </svg>
      </div>
    ...
    `,n.appendChild(i);const g=i.querySelector("#map-detail-drawer"),e=i.querySelectorAll(".interactive-area");e.forEach(m=>{m.addEventListener("click",p=>{e.forEach(y=>y.classList.remove("active")),p.target.classList.add("active");const d=p.target.getAttribute("data-id"),a=oe.find(y=>y.id===d);if(a){i.querySelector("#map-region-title").innerText=`${a.name_fr} (${a.name_ja})`,i.querySelector("#map-region-sub").innerText=`${a.name_en} Region`,i.querySelector("#map-desc-fr").innerText=a.desc_fr,i.querySelector("#map-desc-en").innerText=a.desc_en,i.querySelector("#map-desc-ja").innerText=a.desc_ja;const y=i.querySelector("#map-relations-container"),o=i.querySelector("#map-relations-content"),f=I(d,"region");f?(o.innerHTML=f,y.style.display="block"):y.style.display="none";const v=i.querySelector("#map-audio-title-btn");v.style.display="inline-block",v.onclick=()=>z(a.name_fr);const _=i.querySelector("#map-audio-desc-btn");_.style.display="inline-block",_.onclick=()=>z(a.desc_fr),g.style.display="block"}})})}return c("theory"),t}function A(t){const u=[...t];for(let l=u.length-1;l>0;l--){const r=Math.floor(Math.random()*(l+1));[u[l],u[r]]=[u[r],u[l]]}return u}function se(t){return t.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().trim().replace(/[-\s]+/g," ")}function qe(){if(document.getElementById("quiz-dynamic-styles"))return;const t=document.createElement("style");t.id="quiz-dynamic-styles",t.innerHTML=`
    .quiz-mode-selector {
      display: flex;
  ...
  `,document.head.appendChild(t)}function $e(){var c,k,S;const t=[],u=((c=h.db)==null?void 0:c.vocabulary)||[],l=((k=h.db)==null?void 0:k.grammar)||[],r=((S=h.db)==null?void 0:S.cuisine)||[];function n(s,C,i,g="japanese"){const m=s.filter(d=>d.id!==i).map(d=>d[g]).sort(()=>.5-Math.random()),p=[...new Set(m)].filter(Boolean).slice(0,C);for(;p.length<C;)p.push("Autre option "+(p.length+1));return p}return u.forEach(s=>{var e,m,p,d,a;if(!s.french||!s.japanese)return;let C="vocabulary";(e=s.tags)!=null&&e.includes("meat")?C="meat":(m=s.tags)!=null&&m.includes("sauces")||(p=s.tags)!=null&&p.includes("sauce")?C="sauces":(d=s.tags)!=null&&d.includes("cutting")?C="cuts":(a=s.tags)!=null&&a.includes("science")&&(C="science");const i=n(u,3,s.id,"japanese");t.push({id:`dyn_vocab_fr_ja_${s.id}`,type:"choice",category:C,question:`Que signifie le mot français "${s.french}" ? / What does the French word "${s.french}" mean?`,question_fr:`Que signifie le mot français "${s.french}" ?`,question_en:`What does the French word "${s.french}" mean?`,context:s.definition_fr||`Usage: ${s.context_fr}`,options:A([s.japanese,...i]),answer:s.japanese});const g=n(u,3,s.id,"french");t.push({id:`dyn_vocab_ja_fr_${s.id}`,type:"choice",category:C,question:`Quel est le mot français pour "${s.japanese}" ? / What is the French word for "${s.japanese}"?`,question_fr:`Quel est le mot français pour "${s.japanese}" ?`,question_en:`What is the French word for "${s.japanese}"?`,context:s.definition_fr||`Usage: ${s.context_fr}`,options:A([s.french,...g]),answer:s.french})}),l.forEach(s=>{if(!s.topic||!s.examples||s.examples.length===0)return;const C=n(l,3,s.id,"topic");t.push({id:`dyn_gram_topic_${s.id}`,type:"choice",category:"grammar",question:`De quel concept de grammaire s'agit-il : "${s.explanation_ja}" ? / Which grammar concept is this: "${s.explanation_en}"?`,question_fr:"De quel concept de grammaire s'agit-il ?",question_en:`Which grammar concept is this: "${s.explanation_en}"?`,context:`Niveau : ${s.level}. Indispensable pour la cuisine.`,options:A([s.topic,...C]),answer:s.topic}),s.examples.forEach((i,g)=>{const m=l.flatMap(p=>p.examples||[]).filter(p=>p.fr!==i.fr).sort(()=>.5-Math.random()).map(p=>p.ja).slice(0,3);for(;m.length<3;)m.push("Option de traduction "+(m.length+1));t.push({id:`dyn_gram_ex_${s.id}_${g}`,type:"choice",category:"grammar",question:`Traduisez la phrase : "${i.fr}" / Translate the sentence: "${i.fr}"`,question_fr:`Traduisez la phrase : "${i.fr}"`,question_en:`Translate the sentence: "${i.fr}"`,context:`Grammaire: ${s.topic} (${s.level})`,options:A([i.ja,...m]),answer:i.ja})})}),r.forEach(s=>{var g,e,m,p,d,a;if(!s.topic||!s.content_ja)return;const C=n(r,3,s.id,"topic");let i="sauces";(g=s.tags)!=null&&g.includes("knife-cuts")||(e=s.tags)!=null&&e.includes("cuts")?i="cuts":(m=s.tags)!=null&&m.includes("meat")?i="meat":((p=s.tags)!=null&&p.includes("molecular")||(d=s.tags)!=null&&d.includes("chemistry")||(a=s.tags)!=null&&a.includes("science"))&&(i="science"),t.push({id:`dyn_cuis_topic_${s.id}`,type:"choice",category:i,question:`De quel concept culinaire s'agit-il : "${s.content_ja.substring(0,120)}..." ? / Which culinary concept is described: "${s.content_en.substring(0,120)}..."?`,question_fr:"De quel concept culinaire s'agit-il ?",question_en:"Which culinary concept is described here?",context:`Niveau : ${s.level}. Mots-clés : ${s.tags.join(", ")}`,options:A([s.topic,...C]),answer:s.topic})}),t}function Te(){var k,S;const t=[],u=((k=h.db)==null?void 0:k.grammar)||[],l=((S=h.db)==null?void 0:S.vocabulary)||[],r=[];u.forEach(s=>{(s.examples||[]).forEach(C=>{r.push({fr:C.fr,ja:C.ja})})}),l.forEach(s=>{s.context_fr&&!s.context_fr.includes("Exemple avec le mot")&&r.push({fr:s.context_fr,ja:s.context_ja})});const n=r.sort(()=>.5-Math.random());for(const s of n){if(t.length>=4)break;const C=s.fr.replace(/^➔\s*/,"").trim();if(C.includes(" - ")){const i=C.split(" - ");t.push({id:`pair_${t.length}`,left:`🗣️ ${i[0].trim()}`,right:`💬 ${i[1].trim()}`,context:s.ja})}else if(C.includes(",")){const i=C.split(",");t.push({id:`pair_${t.length}`,left:`${i[0].trim()} ,`,right:`... ${i[1].trim()}`,context:s.ja})}else{const i=C.split(" ");if(i.length>=4){const g=Math.floor(i.length/2),e=i.slice(0,g).join(" "),m=i.slice(g).join(" ");t.push({id:`pair_${t.length}`,left:`${e} ...`,right:`... ${m}`,context:s.ja})}}}const c=[{left:"🗣️ Chaud devant !",right:"💬 Oui, chef !",context:"お通りです！ / はい、シェフ！"},{left:"🗣️ Entrée prête ?",right:"💬 Oui, j'envoie.",context:"前菜はできていますか？ / はい、送ります。"},{left:"🗣️ Combien de couverts ?",right:"💬 Vingt couverts.",context:"何名様（何席）ですか？ / 20席です。"},{left:"🗣️ Envoyez la sauce !",right:"💬 Tout de suite !",context:"ソースを出して！ / ただちに！"}];for(;t.length<4;){const s=c[t.length%c.length];t.push({id:`default_${t.length}`,left:s.left,right:s.right,context:s.context})}return t}function Ee(){var n;const t=document.createElement("div"),u=document.createElement("h2");u.className="section-title",u.innerText="Vérification des Connaissances",t.appendChild(u);const l=document.createElement("p");l.className="section-subtitle",l.innerText="Test your professional French vocabulary, kitchen commands, and classical cuisine theory.",t.appendChild(l);const r=document.createElement("div");return r.className="loading-placeholder",r.innerText="Chargement du quiz... (Loading quiz...)",t.appendChild(r),qe(),(n=h.settings)!=null&&n.targetLevel,Promise.all([me(),M("vocabulary","ALL"),M("grammar","ALL"),M("cuisine","ALL")]).then(()=>{r.remove(),ze(t)}),t}function ze(t){let u="multiple",l="ALL";const r=document.createElement("div");r.className="quiz-mode-selector",r.innerHTML=`
  ...
  `,t.appendChild(r);const n=document.createElement("div");n.className="quiz-category-filter-wrapper",n.style.margin="1rem auto 1.5rem auto",n.style.display="flex",n.style.justifyContent="center",n.style.alignItems="center",n.style.gap="0.8rem",n.innerHTML=`
  ...
  `,t.appendChild(n);const c=n.querySelector("#quiz-cat-select");c.addEventListener("change",m=>{l=m.target.value,S()});const k=document.createElement("div");k.className="quiz-game-wrapper",t.appendChild(k),r.querySelectorAll(".mode-tab-btn").forEach(m=>{m.addEventListener("click",p=>{r.querySelectorAll(".mode-tab-btn").forEach(d=>d.classList.remove("active")),p.target.classList.add("active"),u=p.target.getAttribute("data-mode"),u==="matching_taking"?(c.disabled=!0,c.style.opacity="0.5"):(c.disabled=!1,c.style.opacity="1.0"),S()})});function S(){k.innerHTML="",u==="multiple"?s():u==="matching_vocab"?C():u==="matching_taking"?i():u==="spelling"&&e()}function s(){var _;const m=((_=h.db)==null?void 0:_.quizzes)||[],p=$e();let d=[...m,...p];if(l!=="ALL"&&(d=d.filter(b=>b.category===l)),d.length===0){k.innerHTML=`
      ...
      `;return}const a=A(d).slice(0,10);let y=0,o=0,f=!1;function v(){if(k.innerHTML="",f=!1,y>=a.length){const T=Math.round(o/a.length*100);let q="Apprenti (Apprentice)";T>=90?q="Chef de Partie (Station Chef)":T>=70&&(q="Commis de Cuisine (Line Cook)"),k.innerHTML=`
          <div class="quiz-card" style="text-align: center;">
            ...
            <p style="font-size: 1.1rem; margin-bottom: 1.5rem;">Your Score: <strong>${o} / ${a.length}</strong> (${T}%)</p>
        ...
        `,k.querySelector("#restart-choice-btn").addEventListener("click",()=>{s()});return}const b=a[y],w=document.createElement("div");w.className="quiz-card",w.innerHTML=`
        <div class="quiz-meta">
          <span>Question ${y+1} of ${a.length}</span>
        ...
        <div class="quiz-options">
          ${b.options.map((T,q)=>`
            <button class="quiz-btn" data-index="${q}">${T}</button>
          `).join("")}
        </div>
      ...
      `;const L=w.querySelector(".quiz-hint-flip-container");L&&L.addEventListener("click",()=>{const T=L.querySelector(".q-en-hint-text"),q=T.style.display==="none";T.style.display=q?"block":"none",L.querySelector(".q-en-card").innerText=q?"💡 Hide English Hint":"💡 Translate (Show English Hint)"});const j=w.querySelectorAll(".quiz-btn"),x=w.querySelector(".quiz-feedback"),$=w.querySelector("#next-q-btn");j.forEach(T=>{T.addEventListener("click",q=>{if(f)return;f=!0;const G=q.target.innerText===b.answer;j.forEach(B=>{B.disabled=!0,B.innerText===b.answer&&B.classList.add("correct")}),G?o++:(q.target.classList.add("incorrect"),ce(b.id)),x.style.display="block",$.style.display="block"})}),$.addEventListener("click",()=>{y++,v()}),k.appendChild(w)}v()}function C(){var v,_;const m=((v=h.settings)==null?void 0:v.includeGeneral)||!1;let d=(((_=h.db)==null?void 0:_.vocabulary)||[]).filter(b=>m||b.is_professional);if(l!=="ALL"&&(l==="meat"?d=d.filter(b=>{var w,L,j,x;return((w=b.tags)==null?void 0:w.includes("meat"))||((L=b.tags)==null?void 0:L.includes("beef"))||((j=b.tags)==null?void 0:j.includes("pork"))||((x=b.tags)==null?void 0:x.includes("poultry"))||/viande|boeuf|porc|poulet/i.test(b.french)}):l==="sauces"?d=d.filter(b=>{var w,L,j;return((w=b.tags)==null?void 0:w.includes("sauce"))||((L=b.tags)==null?void 0:L.includes("sauces"))||((j=b.tags)==null?void 0:j.includes("stocks"))||/sauce|fond|jus|bouillon/i.test(b.french)}):l==="cuts"?d=d.filter(b=>{var w,L;return((w=b.tags)==null?void 0:w.includes("cutting"))||((L=b.tags)==null?void 0:L.includes("vegetables"))||/coupe|tailler|ciseler|mincer|brunoise|julienne/i.test(b.french)}):l==="science"?d=d.filter(b=>{var w;return((w=b.tags)==null?void 0:w.includes("science"))||/réaction|émulsion|liaison/i.test(b.french)}):l==="grammar"&&(d=[])),d.length<4){k.innerHTML=`
        <div class="quiz-card" style="text-align: center; padding: 2rem;">
      ...
      `;return}const a=A(d).slice(0,4),y=A(a),o=A(a),f=document.createElement("div");f.className="quiz-card",f.innerHTML=`
    ...
    `,k.appendChild(f),g(f,4,C)}function i(){const m=Te(),p=document.createElement("div");p.className="quiz-card",p.innerHTML=`
      <div class="quiz-meta" style="margin-bottom: 1rem;">
    ...
    `,k.appendChild(p),g(p,4,i)}function g(m,p,d){let a=null,y=null,o=0;const f=m.querySelectorAll(".drag-card"),v=m.querySelectorAll(".drop-zone"),_=m.querySelector("#matching-completion-panel");f.forEach(L=>{L.addEventListener("dragstart",j=>{a=j.target.closest(".drag-card").getAttribute("data-id"),j.target.closest(".drag-card").classList.add("dragging")}),L.addEventListener("dragend",j=>{j.target.closest(".drag-card").classList.remove("dragging")}),L.addEventListener("click",j=>{const x=j.target.closest(".drag-card");x.classList.contains("matched")||(f.forEach($=>$.style.borderColor="rgba(10,25,49,0.08)"),y=x.getAttribute("data-id"),x.style.borderColor="var(--color-accent)")})}),v.forEach(L=>{L.addEventListener("dragover",j=>{j.preventDefault(),L.classList.contains("matched")||L.classList.add("hovered")}),L.addEventListener("dragleave",()=>{L.classList.remove("hovered")}),L.addEventListener("drop",j=>{j.preventDefault(),L.classList.remove("hovered");const x=L.getAttribute("data-id");a===x?w(a,L):b(a)}),L.addEventListener("click",()=>{if(L.classList.contains("matched")||!y)return;const j=L.getAttribute("data-id");y===j?(w(y,L),y=null):(b(y),y=null,f.forEach(x=>x.style.borderColor="rgba(10,25,49,0.08)"))})});function b(L){const j=m.querySelector(`#drag-${L}`);j&&(j.style.animation="shake-anim 0.4s ease-in-out",setTimeout(()=>j.style.animation="",400))}function w(L,j){const x=m.querySelector(`#drag-${L}`);x.classList.add("matched"),x.style.borderColor="var(--color-success)",j.classList.add("matched"),o++,o===p&&(_.style.display="block")}m.querySelector("#restart-match-btn").addEventListener("click",()=>{d()})}function e(){var L,j;const m=((L=h.settings)==null?void 0:L.includeGeneral)||!1;let d=(((j=h.db)==null?void 0:j.vocabulary)||[]).filter(x=>m||x.is_professional);if(l!=="ALL"&&(l==="meat"?d=d.filter(x=>{var $,T,q,P;return(($=x.tags)==null?void 0:$.includes("meat"))||((T=x.tags)==null?void 0:T.includes("beef"))||((q=x.tags)==null?void 0:q.includes("pork"))||((P=x.tags)==null?void 0:P.includes("poultry"))||/viande|boeuf|porc|poulet/i.test(x.french)}):l==="sauces"?d=d.filter(x=>{var $,T,q;return(($=x.tags)==null?void 0:$.includes("sauce"))||((T=x.tags)==null?void 0:T.includes("sauces"))||((q=x.tags)==null?void 0:q.includes("stocks"))||/sauce|fond|jus|bouillon/i.test(x.french)}):l==="cuts"?d=d.filter(x=>{var $,T;return(($=x.tags)==null?void 0:$.includes("cutting"))||((T=x.tags)==null?void 0:T.includes("vegetables"))||/coupe|tailler|ciseler|mincer|brunoise|julienne/i.test(x.french)}):l==="science"?d=d.filter(x=>{var $;return(($=x.tags)==null?void 0:$.includes("science"))||/réaction|émulsion|liaison/i.test(x.french)}):l==="grammar"&&(d=[])),d.length===0){k.innerHTML=`
        <div class="quiz-card" style="text-align: center; padding: 2rem;">
      ...
      `;return}let a=A(d)[0];const y=document.createElement("div");y.className="quiz-card",y.innerHTML=`
      <div class="quiz-meta" style="margin-bottom: 1.2rem;">
        <span>📖 Orthographe de Cuisine</span>
        <span class="grammar-badge" style="background-color: var(--color-secondary);">${a.category}</span>
      </div>
      ...
      <div class="spelling-box" style="margin-bottom: 1.5rem; background-color: rgba(10,25,49,0.02); padding: 1.2rem; border-radius: var(--radius-sm); border-left: 3px solid var(--color-accent);">
        ...
        <p style="font-size: 1.05rem; font-style: italic; color: var(--color-primary); line-height: 1.4; font-family: var(--font-serif);">${a.definition_fr}</p>
        ...
        <div style="margin-top: 1rem; border-top: 1px solid rgba(0,0,0,0.05); padding-top: 0.8rem; font-size: 0.85rem; color: var(--color-text-muted);">
          <strong>Hint (Japanese):</strong> ${a.japanese}
        </div>
      </div>
    ...
    `,k.appendChild(y);const o=y.querySelector("#spelling-input-field"),f=y.querySelector("#spelling-submit-btn"),v=y.querySelector("#spelling-next-btn"),_=y.querySelector("#spelling-feedback-panel"),b=y.querySelector("#spelling-feedback-title"),w=y.querySelector("#spelling-feedback-msg");o.addEventListener("keydown",x=>{x.key==="Enter"&&f.click()}),f.addEventListener("click",()=>{const x=o.value,$=a.french,T=se(x),q=se($),P=T===q;o.disabled=!0,f.style.display="none",v.style.display="block",_.style.display="block",P?(o.style.borderColor="var(--color-success)",o.style.backgroundColor="#E8F5E9",_.style.backgroundColor="#E8F5E9",_.style.color="var(--color-success)",b.innerText="✓ Félicitations ! (Correct)",w.innerText=`You correctly spelled: "${$}"`):(o.style.borderColor="var(--color-error)",o.style.backgroundColor="#FFEBEE",_.style.backgroundColor="#FFEBEE",_.style.color="var(--color-error)",b.innerText="✗ Incorrect",w.innerHTML=`Correct spelling is: <strong>${$}</strong>.<br><em style="font-size:0.85rem;">You typed: "${x}"</em>`,ce(a.id))}),v.addEventListener("click",()=>{e()})}S()}function O(){const t=document.createElement("div"),u=document.createElement("h2");u.className="section-title",u.innerText="Coups de Cœur (Favorites)",t.appendChild(u);const l=document.createElement("p");l.className="section-subtitle",l.innerText="Your bookmarked vocabulary terms, grammar guides, and culinary theories.",t.appendChild(l);const r=Array.from(h.favorites);if(r.length===0)return t.innerHTML+=`
      <div class="alert alert-info">
        <p>No favorites saved yet. Browse Vocabulary, Grammar, or Cuisine and click the star (☆) button to save items here.</p>
      </div>
    `,t;const n=document.createElement("div");return n.className="loading-placeholder",n.innerText="Chargement des favoris... (Loading favorites...)",t.appendChild(n),Q().then(()=>{n.remove(),Ae(t,r)}),t}function Ae(t,u){var S,s,C;const l=(((S=h.db)==null?void 0:S.vocabulary)||[]).filter(i=>u.includes(i.id)),r=(((s=h.db)==null?void 0:s.grammar)||[]).filter(i=>u.includes(i.id)),n=(((C=h.db)==null?void 0:C.cuisine)||[]).filter(i=>u.includes(i.id)),c=document.createElement("div");c.className="card-grid",t.appendChild(c);function k(){c.innerHTML="",l.forEach(i=>{const g=document.createElement("div");g.className="card",g.innerHTML=`
        <div>
          <div class="card-category">Vocabulary: ${i.category}</div>
          <div class="term-header">
            <h3 class="term-title">${i.french}</h3>
            <button class="fav-btn active" data-id="${i.id}">★</button>
          </div>
          <div class="term-translations">
            <div class="trans-en">${i.english}</div>
            <div class="trans-ja">${i.japanese}</div>
          </div>
        </div>
        <div class="term-context">
          <div class="context-fr">"${i.context_fr}"</div>
          <div class="context-ja">${i.context_ja}</div>
        </div>
      `,g.querySelector(".fav-btn").addEventListener("click",e=>{H(i.id),O(),t.innerHTML="",t.appendChild(O())}),c.appendChild(g)}),r.forEach(i=>{var e,m;const g=document.createElement("div");g.className="card",g.innerHTML=`
        <div>
          <div class="card-category">Grammar: ${i.level}</div>
          <div class="term-header">
            <h3 class="term-title" style="font-size: 1.25rem;">${i.topic}</h3>
            <button class="fav-btn active" data-id="${i.id}">★</button>
          </div>
          <p style="font-size: 0.85rem; margin-bottom: 1rem; color: var(--color-text-main);">${i.explanation_en.substring(0,100)}...</p>
        </div>
        ...
        <div class="term-context" style="margin-top: 0.5rem;">
          <div class="context-fr">"${((e=i.examples[0])==null?void 0:e.fr)||""}"</div>
          <div class="context-ja">${((m=i.examples[0])==null?void 0:m.ja)||""}</div>
        </div>
      `,g.querySelector(".fav-btn").addEventListener("click",p=>{H(i.id),t.innerHTML="",t.appendChild(O())}),c.appendChild(g)}),n.forEach(i=>{const g=document.createElement("div");g.className="card",g.innerHTML=`
        <div>
          <div class="card-category">Cuisine: ${i.category}</div>
          <div class="term-header">
            <h3 class="term-title" style="font-size: 1.25rem;">${i.topic}</h3>
            <button class="fav-btn active" data-id="${i.id}">★</button>
          </div>
          <p style="font-size: 0.85rem; margin-bottom: 1rem; color: var(--color-text-main);">${i.content_en.substring(0,100)}...</p>
        </div>
      `,g.querySelector(".fav-btn").addEventListener("click",e=>{H(i.id),t.innerHTML="",t.appendChild(O())}),c.appendChild(g)})}k()}function Me(){const t=document.createElement("div"),u=document.createElement("h2");u.className="section-title",u.innerText="Pont d'Études (SRS Review Deck)",t.appendChild(u);const l=document.createElement("p");l.className="section-subtitle",l.innerText="Review and grade yourself on items scheduled for active recall today.",t.appendChild(l);const r=document.createElement("div");return r.className="loading-placeholder",r.innerText="Chargement des révisions... (Loading reviews...)",t.appendChild(r),Q().then(()=>{r.remove(),Be(t)}),t}function Be(t){var s,C,i,g;const u=new Date().toISOString().split("T")[0],r=[...(((s=h.db)==null?void 0:s.vocabulary)||[]).map(e=>({...e,type:"vocabulary",front:e.french})),...(((C=h.db)==null?void 0:C.grammar)||[]).map(e=>({...e,type:"grammar",front:e.topic})),...(((i=h.db)==null?void 0:i.cuisine)||[]).map(e=>({...e,type:"cuisine",front:e.topic})),...(((g=h.db)==null?void 0:g.quizzes)||[]).map(e=>({...e,type:"quiz",front:e.question_fr||e.question||""}))].filter(e=>{const m=h.srs[e.id];return h.wrongAnswers.includes(e.id)||m&&m.dueDate<=u}),n=r.length;let c=0;const k=document.createElement("div");k.className="srs-review-container",t.appendChild(k);function S(){if(k.innerHTML="",r.length===0||c>=r.length){k.innerHTML=`
      ...
      `;return}const e=r[c];h.srs[e.id];const m=h.wrongAnswers.includes(e.id),p=document.createElement("div");p.className="card srs-flip-card",p.style.padding="2rem",p.style.minHeight="300px",p.style.display="flex",p.style.flexDirection="column",p.style.justifyContent="space-between";let d=e.category||e.level||"Theory";e.type==="quiz"&&(d="Quiz Mistake");let a=`
      <div>
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
          <span class="card-category">${d} • ${e.type.toUpperCase()}</span>
          <span style="font-size: 0.8rem; color: var(--color-text-muted);">
            Card ${c+1} of ${n}
            ${m?' <span style="color: var(--color-error); font-weight: bold;">(Wrong Answer)</span>':""}
          </span>
        </div>
    ...
    `;p.innerHTML=a;let y="";e.type==="vocabulary"?y=`
          ...
          <div class="examples-list" style="border-top: 1px solid rgba(0,0,0,0.05); padding-top: 0.8rem;">
            ${e.examples.map(o=>`
              <div class="example-item" style="margin-bottom: 0.5rem;">
                <div class="example-fr">➔ ${o.fr}</div>
                <div class="example-ja" style="font-size: 0.8rem; color: var(--color-text-muted);">${o.ja}</div>
              </div>
            `).join("")}
          </div>
        </div>
      ...
      `),p.querySelector("#reveal-btn").addEventListener("click",()=>{p.innerHTML=`
        <div>
      ...
      `;const o=p.querySelector(".flip-translation-container");o&&o.addEventListener("click",f=>{f.stopPropagation(),o.querySelector(".flip-translation-card").classList.toggle("flipped")}),p.querySelectorAll(".audio-btn").forEach(f=>{f.addEventListener("click",v=>{v.stopPropagation();const _=v.target.closest(".audio-btn").getAttribute("data-text");z(_)})}),p.querySelectorAll(".srs-score-btn").forEach(f=>{f.addEventListener("click",v=>{const _=parseInt(v.target.getAttribute("data-score"));W(e.id,_),_>=4&&m&&He(e.id),c++,S()})})}),k.appendChild(p)}S()}function Fe(){const t=document.createElement("div"),u=document.createElement("h2");u.className="section-title",u.innerText="Recherche Globale",t.appendChild(u);const l=document.createElement("p");l.className="section-subtitle",l.innerText="Search terms across the entire curriculum: Vocabulary, Grammar, and Culinary Theory.",t.appendChild(l);const r=document.createElement("div");r.className="search-box-wrapper",r.style.marginBottom="2rem",r.style.position="relative";const n=document.createElement("input");n.type="text",n.placeholder="Chargement de la base de données... (Loading database...)",n.className="search-input",n.style.width="100%",n.style.padding="1rem 1.5rem",n.style.fontSize="1.1rem",n.style.borderRadius="var(--radius-md)",n.style.border="2px solid rgba(197, 168, 128, 0.2)",n.style.backgroundColor="var(--color-bg)",n.style.fontFamily="var(--font-sans)",n.style.transition="var(--transition)",n.style.outline="none",n.disabled=!0,n.addEventListener("focus",()=>{n.style.borderColor="var(--color-accent)",n.style.boxShadow="0 0 10px rgba(212, 175, 55, 0.15)"}),n.addEventListener("blur",()=>{n.style.borderColor="rgba(197, 168, 128, 0.2)",n.style.boxShadow="none"}),r.appendChild(n),t.appendChild(r);const c=document.createElement("div");return c.className="search-results",t.appendChild(c),c.innerHTML=`
    <div style="text-align: center; padding: 3rem; color: var(--color-text-muted);">
      <p>Préparation de la recherche... (Preparing search database...)</p>
    </div>
  `,Q().then(()=>{n.disabled=!1,n.placeholder="Rechercher... (e.g. sauce, roux, cut, culer, 刻む)";function k(S){var m,p,d;if(c.innerHTML="",!S.trim()){c.innerHTML=`
          <div style="text-align: center; padding: 3rem; color: var(--color-text-muted);">
            <span style="font-size: 3rem;">🔍</span>
            <p style="margin-top: 1rem;">Tapez un mot-clé pour commencer votre recherche.</p>
          </div>
        `;return}const s=S.toLowerCase().trim(),C=((m=h.db)==null?void 0:m.vocabulary)||[],i=((p=h.db)==null?void 0:p.grammar)||[],g=((d=h.db)==null?void 0:d.cuisine)||[],e=[];if(C.forEach(a=>{`${a.french} ${a.english} ${a.japanese} ${a.category} ${a.context_fr} ${a.context_en} ${a.context_ja} ${(a.tags||[]).join(" ")}`.toLowerCase().includes(s)&&e.push({...a,type:"vocabulary",title:a.french,subtitle:`${a.category} • Vocabulary`})}),i.forEach(a=>{`${a.topic} ${a.explanation_en} ${a.explanation_ja} ${a.level} ${(a.tags||[]).join(" ")} ${a.examples.map(o=>`${o.fr} ${o.en} ${o.ja}`).join(" ")}`.toLowerCase().includes(s)&&e.push({...a,type:"grammar",title:a.topic,subtitle:`${a.level} • Grammar Lesson`})}),g.forEach(a=>{`${a.topic} ${a.category} ${a.content_fr} ${a.content_en} ${a.content_ja} ${(a.tags||[]).join(" ")}`.toLowerCase().includes(s)&&e.push({...a,type:"cuisine",title:a.topic,subtitle:`${a.category} • Culinary Theory`})}),e.length===0){c.innerHTML=`
          <div style="text-align: center; padding: 3rem; color: var(--color-text-muted);">
            <p>Aucun résultat trouvé pour "<strong>${S}</strong>".</p>
            <p style="font-size: 0.9rem; margin-top: 0.5rem;">Check your spelling or try another keyword.</p>
          </div>
        `;return}e.forEach(a=>{const y=document.createElement("div");y.className="card search-result-card",y.style.marginBottom="1.2rem",y.style.borderLeft=`4px solid ${a.type==="vocabulary"?"var(--color-primary)":a.type==="grammar"?"var(--color-secondary)":"var(--color-accent)"}`;const o=F(a.id);let f="";a.type==="vocabulary"?f=`
            <div class="term-translations" style="margin-top: 0.5rem;">
              <div class="trans-en">${a.english}</div>
              <div class="trans-ja">${a.japanese}</div>
            </div>
            <div class="term-context" style="margin-top: 1rem;">
              <div class="context-fr">"${a.context_fr}"</div>
              <div class="context-ja">${a.context_ja}</div>
            </div>
          `:a.type==="grammar"?f=`
            <p style="font-size: 0.9rem; margin-top: 0.5rem; color: var(--color-text-main);">${a.explanation_en}</p>
            <p style="font-size: 0.85rem; margin-top: 0.3rem; color: var(--color-text-muted);">${a.explanation_ja}</p>
            <div class="examples-list" style="margin-top: 1rem; border-top: 1px solid rgba(0,0,0,0.05); padding-top: 0.8rem;">
              ${a.examples.slice(0,2).map(_=>`
                <div class="example-item" style="margin-bottom: 0.5rem;">
          ...
          `:a.type==="cuisine"&&(f=`
            <p style="font-size: 0.9rem; margin-top: 0.5rem; font-style: italic; color: var(--color-primary);">${a.content_fr.substring(0,150)}...</p>
            <p style="font-size: 0.85rem; margin-top: 0.5rem; color: var(--color-text-main);">${a.content_ja.substring(0,120)}...</p>
          `);const v=(a.tags||[]).map(_=>`<span class="tag-badge" style="background-color: rgba(197, 168, 128, 0.12); color: var(--color-accent); font-size: 0.75rem; padding: 0.2rem 0.6rem; border-radius: 20px; font-weight: 500;">#${_}</span>`).join(" ");y.innerHTML=`
          <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem;">
            <div>
              <div class="card-category" style="margin: 0; font-size: 0.75rem;">${a.subtitle}</div>
              <h3 class="term-title" style="margin-top: 0.2rem; font-size: 1.4rem;">${a.title}</h3>
            </div>
            <button class="fav-btn ${o?"active":""}" data-id="${a.id}">
              ${o?"★":"☆"}
            </button>
          </div>
          ${f}
          ${a.tags&&a.tags.length>0?`<div style="margin-top: 1rem; display: flex; flex-wrap: wrap; gap: 0.4rem;">${v}</div>`:""}
        `,y.querySelector(".fav-btn").addEventListener("click",_=>{_.stopPropagation(),H(a.id);const b=_.target,w=F(a.id);b.classList.toggle("active",w),b.innerText=w?"★":"☆"}),c.appendChild(y)})}n.addEventListener("input",S=>{k(S.target.value)}),k("")}),t}function Pe(){const t=document.createElement("div"),u=document.createElement("h2");u.className="section-title",u.innerText="Configuration de l'Académie",t.appendChild(u);const l=document.createElement("p");l.className="section-subtitle",l.innerText="Customize your learning goals, target CEFR levels, and database options.",t.appendChild(l);const r=document.createElement("div");r.className="card",r.style.padding="2rem";const n=h.settings;r.innerHTML=`
  ...
  `;const c=r.querySelector("#target-level-select"),k=r.querySelector("#new-cards-goal"),S=r.querySelector("#max-reviews-goal"),s=r.querySelector("#include-general-checkbox"),C=r.querySelector("#settings-status");function i(){const g=c.value,e=parseInt(k.value)||5,m=parseInt(S.value)||20,p=s.checked;Ie({targetLevel:g,newCardsPerDay:e,maxReviewsPerDay:m,includeGeneral:p}),C.style.display="block",setTimeout(()=>{C.style.display="none"},3e3)}return c.addEventListener("change",i),k.addEventListener("input",i),S.addEventListener("input",i),s.addEventListener("change",i),r.querySelector("#reset-srs-btn").addEventListener("click",()=>{confirm("Voulez-vous vraiment réinitialiser toutes vos données de progression SRS ? Cette action est irréversible.")&&(localStorage.removeItem("cba_srs"),h.srs={},alert("Spaced Repetition System progress has been reset."),window.location.reload())}),r.querySelector("#clear-favs-btn").addEventListener("click",()=>{confirm("Voulez-vous vraiment supprimer tous vos favoris ?")&&(localStorage.removeItem("cba_favorites"),h.favorites=new Set,alert("All favorites have been cleared."),window.location.reload())}),r.querySelector("#share-app-btn").addEventListener("click",()=>{const g={title:"Académie de la Brigade",text:"フランス料理・厨房フランス語の統合学習PWAアプリ「Académie de la Brigade」で一緒に料理と語学を学びましょう！",url:window.location.origin+window.location.pathname};navigator.share?navigator.share(g).then(()=>console.log("Shared successfully")).catch(e=>console.log("Error sharing:",e)):navigator.clipboard.writeText(g.url).then(()=>{alert("App link copied to clipboard! Share it with your friends.")}).catch(e=>{console.error("Failed to copy link:",e)})}),t.appendChild(r),t}function Ne(){var c;const t=document.createElement("div"),u=document.createElement("h2");u.className="section-title",u.innerText="Dictée de Cuisine (Culinary Dictations)",t.appendChild(u);const l=document.createElement("p");l.className="section-subtitle",l.innerText="Listen to French kitchen instruction sentences, type what you hear, and master French spelling.",t.appendChild(l);const r=document.createElement("div");r.className="loading-placeholder",r.innerText="Chargement de la dictée... (Loading dictation...)",t.appendChild(r);const n=((c=h.settings)==null?void 0:c.targetLevel)||"ALL";return M("vocabulary",n).then(()=>{r.remove(),De(t)}),t}function De(t){var a,y;const u=((a=h.settings)==null?void 0:a.includeGeneral)||!1,n=(((y=h.db)==null?void 0:y.vocabulary)||[]).filter(o=>u||o.is_professional).filter(o=>o.context_fr);if(n.length===0){t.innerHTML+='<p style="color: var(--color-text-muted);">Aucun exercice disponible.</p>';return}const c=[...n].sort(()=>.5-Math.random()).slice(0,5);let k=0,S=!1,s=!1,C=!1,i=0,g=1;const e=document.createElement("div");e.className="dictation-container",e.style.maxWidth="600px",e.style.margin="1.5rem auto",t.appendChild(e);function m(o){return o.toLowerCase().trim().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()'"?]/g,"").replace(/\s+/g," ")}function p(o){return o.normalize("NFD").replace(/[\u0300-\u036f]/g,"")}function d(){if(e.innerHTML="",S=!1,J(),s=!1,C=!1,i=0,k>=c.length){const E=document.createElement("div");E.className="card",E.style.padding="2.5rem",E.style.textAlign="center",E.innerHTML=`
        <h3 style="font-family: var(--font-serif); font-size: 2rem; color: var(--color-primary); margin-bottom: 1rem;">Session Terminée !</h3>
      ...
      `,E.querySelector("#restart-dictation-btn").addEventListener("click",()=>{k=0,c.length=0,c.push(...[...n].sort(()=>.5-Math.random()).slice(0,5)),d()}),e.appendChild(E);return}const o=c[k],f=o.context_fr,v=document.createElement("div");v.className="card",v.style.padding="2rem",v.innerHTML=`
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; border-bottom: 1px solid rgba(0,0,0,0.05); padding-bottom: 0.8rem;">
        <span class="card-category" style="margin: 0;">Drill ${k+1} of ${c.length}</span>
        <span style="font-size: 0.8rem; color: var(--color-text-muted); font-weight: 500;">Topic: ${o.category}</span>
      </div>
    ...
    `;const _=v.querySelector("#play-pause-btn"),b=v.querySelector("#stop-btn"),w=v.querySelector("#speed-normal-btn"),L=v.querySelector("#speed-slow-btn"),j=v.querySelector("#dictation-seekbar"),x=v.querySelector("#dictation-input"),$=v.querySelector("#check-btn"),T=v.querySelector("#next-dictation-btn"),q=v.querySelector("#dictation-feedback");function P(E,R){if(!S){const N=E/R*100;j.value=Math.round(N),i=E}}function G(){s=!1,C=!1,i=0,_.innerText="▶ Play",j.value=0}function B(){ue(f,g,P,G,i)}_.addEventListener("click",()=>{S||(s?C?(C=!1,_.innerText="⏸ Pause",fe()):(C=!0,_.innerText="▶ Play",ge()):(s=!0,C=!1,_.innerText="⏸ Pause",B()))}),b.addEventListener("click",()=>{J(),G()}),j.addEventListener("change",E=>{if(S)return;const R=parseInt(E.target.value),N=f.replace(/["'➔]/g,"").trim();i=Math.floor(R/100*N.length);const V=N.indexOf(" ",i);V!==-1&&V-i<5&&(i=V+1),s=!0,C=!1,_.innerText="⏸ Pause",B()}),w.addEventListener("click",()=>{g!==1&&(g=1,w.classList.add("active"),w.style.backgroundColor="var(--color-primary)",w.style.color="#FFFFFF",L.classList.remove("active"),L.style.backgroundColor="transparent",L.style.color="var(--color-accent)",s&&!C&&B())}),L.addEventListener("click",()=>{g!==.75&&(g=.75,L.classList.add("active"),L.style.backgroundColor="var(--color-accent)",L.style.color="#FFFFFF",w.classList.remove("active"),w.style.backgroundColor="transparent",w.style.color="var(--color-accent)",s&&!C&&B())}),$.addEventListener("click",()=>{if(S)return;J(),G();const E=x.value,R=m(E),N=m(f),V=p(R),pe=p(N);let U=!1,Y=!1;R===N?U=!0:V===pe&&(Y=!0),q.style.display="block",x.disabled=!0,$.style.display="none",T.style.display="block",S=!0,U?(q.style.backgroundColor="#E8F5E9",q.style.borderLeft="4px solid var(--color-success)",q.style.color="var(--color-success)",q.innerHTML=`
          ...
          <div style="margin-top: 0.8rem; border-top: 1px solid rgba(0,0,0,0.05); padding-top: 0.6rem; font-size: 0.85rem; color: var(--color-text-muted);">
            <strong>Meaning:</strong> ${o.context_ja}
          </div>
        `):Y?(q.style.backgroundColor="#FFF3E0",q.style.borderLeft="4px solid var(--color-accent)",q.style.color="#E65100",q.innerHTML=`
        ...
        `)}),T.addEventListener("click",()=>{k++,d()}),e.appendChild(v),x.focus()}d()}const h={meta:null,db:{vocabulary:[],grammar:[],cuisine:[],quizzes:[]},loaded:{vocabulary:new Set,grammar:new Set,cuisine:new Set,quizzes:!1},favorites:new Set(JSON.parse(localStorage.getItem("cba_favorites")||"[]")),wrongAnswers:JSON.parse(localStorage.getItem("cba_wrong")||"[]"),streak:parseInt(localStorage.getItem("cba_streak")||"0"),lastStudyDate:localStorage.getItem("cba_last_study")||"",srs:JSON.parse(localStorage.getItem("cba_srs")||"{}"),settings:JSON.parse(localStorage.getItem("cba_settings")||JSON.stringify({targetLevel:"ALL",newCardsPerDay:5,maxReviewsPerDay:20,includeGeneral:!1}))};function H(t){h.favorites.has(t)?h.favorites.delete(t):h.favorites.add(t),localStorage.setItem("cba_favorites",JSON.stringify(Array.from(h.favorites)))}function F(t){return h.favorites.has(t)}function ce(t){h.wrongAnswers.includes(t)||(h.wrongAnswers.push(t),localStorage.setItem("cba_wrong",JSON.stringify(h.wrongAnswers)))}function He(t){h.wrongAnswers=h.wrongAnswers.filter(u=>u!==t),localStorage.setItem("cba_wrong",JSON.stringify(h.wrongAnswers))}function Re(){const t=new Date().toISOString().split("T")[0];if(h.lastStudyDate!==t){if(h.lastStudyDate){const u=new Date(h.lastStudyDate),l=new Date(t),r=Math.abs(l-u),n=Math.ceil(r/(1e3*60*60*24));n===1?h.streak+=1:n>1&&(h.streak=1)}else h.streak=1;h.lastStudyDate=t,localStorage.setItem("cba_streak",h.streak.toString()),localStorage.setItem("cba_last_study",t)}}function W(t,u){const l=new Date().toISOString().split("T")[0],r=h.srs[t]||{easiness:2.5,interval:0,repetitions:0,dueDate:l};let n=Math.max(0,Math.min(5,u));n>=3?(r.repetitions===0?r.interval=1:r.repetitions===1?r.interval=6:r.interval=Math.round(r.interval*r.easiness),r.repetitions++):(r.repetitions=0,r.interval=1),r.easiness=r.easiness+(.1-(5-n)*(.08+(5-n)*.02)),r.easiness<1.3&&(r.easiness=1.3);const c=new Date;c.setDate(c.getDate()+r.interval),r.dueDate=c.toISOString().split("T")[0],r.lastRated=l,h.srs[t]=r,localStorage.setItem("cba_srs",JSON.stringify(h.srs))}function Ie(t){h.settings={...h.settings,...t},localStorage.setItem("cba_settings",JSON.stringify(h.settings))}async function M(t,u){const l=u==="ALL"?["A1","A2","B1","B2","C1","C2"]:[u];for(const r of l)if(!h.loaded[t].has(r))try{const n=await fetch(`data/${t}_${r}.json`);if(!n.ok){console.warn(`Could not load data/${t}_${r}.json`);continue}const c=await n.json(),k=new Set(h.db[t].map(S=>S.id));for(const S of c)k.has(S.id)||h.db[t].push(S);h.loaded[t].add(r)}catch(n){console.error(`Failed to load ${t} level ${r}:`,n)}}async function me(){if(!h.loaded.quizzes)try{const t=await fetch("data/quizzes.json");if(!t.ok)throw new Error("Network response was not ok");h.db.quizzes=await t.json(),h.loaded.quizzes=!0}catch(t){console.error("Failed to load quizzes:",t)}}async function Q(){await Promise.all([M("vocabulary","ALL"),M("grammar","ALL"),M("cuisine","ALL"),me()])}const le={home:ve,vocabulary:he,grammar:ye,cuisine:je,quiz:Ee,favorites:O,review:Me,search:Fe,settings:Pe,dictation:Ne};function de(t){const u=document.getElementById("main-content");if(le[t]){Re(),u.innerHTML="";const l=le[t]();l instanceof HTMLElement?u.appendChild(l):u.innerHTML=l,document.querySelectorAll(".nav-link").forEach(r=>{r.getAttribute("data-tab")===t?r.classList.add("active"):r.classList.remove("active")})}}async function Ge(){try{const t=await fetch("data/meta.json");h.meta=await t.json(),document.querySelectorAll(".nav-link").forEach(u=>{u.addEventListener("click",l=>{const r=l.target.closest(".nav-link").getAttribute("data-tab");de(r)})}),de("home")}catch(t){console.error("Failed to load database metadata:",t),document.getElementById("main-content").innerHTML=`
