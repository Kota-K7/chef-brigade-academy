(function(){const c=document.createElement("link").relList;if(c&&c.supports&&c.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const d of i.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&n(d)}).observe(document,{childList:!0,subtree:!0});function r(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(a){if(a.ep)return;a.ep=!0;const i=r(a);fetch(a.href,i)}})();let X=null,Z="",ee=0,W=null,R=null;function B(t,c=1){oe(t,c)}function oe(t,c=1,r=null,n=null,a=0){if(!("speechSynthesis"in window)){console.warn("Speech synthesis not supported on this browser.");return}window.speechSynthesis.cancel();const i=t.replace(/["'➔]/g,"").trim();Z=i,ee=a,W=r,R=n;const d=i.substring(a);if(!d.trim()){R&&R();return}const e=new SpeechSynthesisUtterance(d);e.lang="fr-FR",e.rate=c;const u=window.speechSynthesis.getVoices().find(l=>l.lang==="fr-FR"||l.lang.startsWith("fr"));u&&(e.voice=u),e.onboundary=l=>{if(l.name==="word"&&W){const m=ee+l.charIndex;W(m,Z.length)}},e.onend=()=>{X===e&&R&&R()},e.onerror=l=>{console.warn("SpeechSynthesis error:",l),R&&R()},X=e,window.speechSynthesis.speak(e)}function de(){"speechSynthesis"in window&&window.speechSynthesis.pause()}function ue(){"speechSynthesis"in window&&window.speechSynthesis.resume()}function K(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}"speechSynthesis"in window&&(window.speechSynthesis.getVoices(),window.speechSynthesis.onvoiceschanged=()=>{window.speechSynthesis.getVoices()});function me(){var k,L,h,w,y,S,C;const t=document.createElement("div"),c=document.createElement("h2");c.className="section-title",c.innerText="Tableau de Bord",t.appendChild(c);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Welcome back, Commis. Here is your kitchen curriculum status.",t.appendChild(r);const n=new Date().toISOString().split("T")[0],a=Object.values(f.srs),i=a.filter(j=>j.dueDate<=n).length,d=a.length,e=document.createElement("div");e.className="dashboard-grid";const p=document.createElement("div");p.style.display="flex",p.style.flexDirection="column",p.style.gap="1.5rem";const u=document.createElement("div");u.className="card",u.style.padding="1.5rem",u.innerHTML=`
    ...
    <ul style="list-style: none; display: flex; flex-direction: column; gap: 0.8rem;">
      <li style="display: flex; align-items: center; gap: 0.8rem; font-size: 0.95rem;">
        <input type="checkbox" id="mission-vocab" style="transform: scale(1.2); cursor: pointer;" ${d>0?"checked":""}>
        <label for="mission-vocab" style="cursor: pointer;">Study new terms in the Vocabulary deck</label>
      </li>
      <li style="display: flex; align-items: center; gap: 0.8rem; font-size: 0.95rem;">
        <input type="checkbox" id="mission-srs" style="transform: scale(1.2); cursor: pointer;" ${i===0&&d>0?"checked":""}>
  ...
  `,p.appendChild(u);const l=document.createElement("div");l.className="card",l.style.padding="1.5rem";const m=((k=f.meta)==null?void 0:k.featured)||null;l.innerHTML=`
    ...
    `:"<p>No data loaded.</p>"}
  `,m&&l.querySelectorAll(".audio-btn").forEach(j=>{j.addEventListener("click",v=>{v.stopPropagation();const E=v.target.closest(".audio-btn").getAttribute("data-text");B(E)})}),p.appendChild(l),e.appendChild(p);const o=document.createElement("div");o.style.display="flex",o.style.flexDirection="column",o.style.gap="1.5rem";const g=document.createElement("div");g.className="streak-card",g.innerHTML=`
  ...
  `,o.appendChild(g);const b=document.createElement("div");b.className="card",b.style.padding="1.5rem";const _=((h=(L=f.meta)==null?void 0:L.counts)==null?void 0:h.vocabulary)||0,s=((y=(w=f.meta)==null?void 0:w.counts)==null?void 0:y.grammar)||0,x=((C=(S=f.meta)==null?void 0:S.counts)==null?void 0:C.cuisine)||0;return b.innerHTML=`
    ...
    <div style="display: flex; flex-direction: column; gap: 0.8rem; font-size: 0.9rem;">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <strong style="color: var(--color-primary);">${d} cards</strong>
      </div>
      <div style="display: flex; justify-content: space-between; align-items: center;">
  ...
  `,o.appendChild(b),e.appendChild(o),t.appendChild(e),t}function pe(){var i;const t=document.createElement("div"),c=document.createElement("h2");c.className="section-title",c.innerText="Vocabulaire Professionnel",t.appendChild(c);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Essential culinary terminology for kitchen brigade communications. Filter by category, tag, or difficulty level.",t.appendChild(r);const n=document.createElement("div");n.className="loading-placeholder",n.innerText="Chargement du vocabulaire... (Loading vocabulary...)",t.appendChild(n);const a=((i=f.settings)==null?void 0:i.targetLevel)||"ALL";return M("knowledge",a).then(()=>{n.remove(),ge(t,a)}),t}function ge(t,c){var b,_;const r=((b=f.settings)==null?void 0:b.includeGeneral)||!1,a=(((_=f.db)==null?void 0:_.knowledge)||[]).filter(s=>s.french&&s.japanese).filter(s=>r||s.is_professional),i=["ALL",...new Set(a.map(s=>s.category))],d=["ALL",...new Set(a.flatMap(s=>s.tags||[]))],e=["ALL","A1","A2","B1","B2","C1","C2"];let p="ALL",u="ALL",l=c;const m=document.createElement("div");m.className="card",m.style.padding="1.2rem",m.style.marginBottom="2rem",m.style.display="flex",m.style.flexDirection="column",m.style.gap="1rem",m.innerHTML=`
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
      ...
      <div>
        ...
        <select id="vocab-tag-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${d.map(s=>`<option value="${s}" ${s===u?"selected":""}>${s==="ALL"?"ALL TAGS":"#"+s}</option>`).join("")}
        </select>
      </div>
      <div>
        ...
        <select id="vocab-level-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${e.map(s=>`<option value="${s}" ${s===l?"selected":""}>${s==="ALL"?"ALL LEVELS":s}</option>`).join("")}
        </select>
      </div>
    </div>
  `,t.appendChild(m);const o=document.createElement("div");o.className="card-grid",t.appendChild(o);function g(){var L;o.innerHTML="";const k=(((L=f.db)==null?void 0:L.knowledge)||[]).filter(h=>h.french&&h.japanese).filter(h=>r||h.is_professional).filter(h=>{const w=p==="ALL"||h.category===p,y=u==="ALL"||h.tags&&h.tags.includes(u),S=l==="ALL"||h.level===l;return w&&y&&S});if(k.length===0){o.innerHTML='<p style="color: var(--color-text-muted); grid-column: 1 / -1; text-align: center; padding: 2rem;">Aucun terme trouvé correspondant à vos critères de filtrage.</p>';return}k.forEach(h=>{const w=document.createElement("div");w.className="card";const y=N(h.id),S=f.srs[h.id],C=!!S,j=h.examples&&h.examples[0]?h.examples[0].fr:"",v=h.examples&&h.examples[0]?h.examples[0].ja:"";w.innerHTML=`
      ...
      `;const E=w.querySelector(".flip-translation-container");E.addEventListener("click",$=>{$.stopPropagation(),E.querySelector(".flip-translation-card").classList.toggle("flipped")}),w.querySelector(".fav-btn").addEventListener("click",$=>{$.stopPropagation(),D(h.id);const q=$.target,z=N(h.id);q.classList.toggle("active",z),q.innerText=z?"★":"☆"}),w.querySelectorAll(".audio-btn").forEach($=>{$.addEventListener("click",q=>{q.stopPropagation();const z=q.target.closest(".audio-btn").getAttribute("data-text");B(z)})}),w.querySelectorAll(".srs-action-btn").forEach($=>{$.addEventListener("click",q=>{q.stopPropagation();const z=q.target.getAttribute("data-action"),F=q.target.getAttribute("data-id");z==="start"?O(F,4):z==="memorized"?O(F,5):z==="reset"&&O(F,0),g()})}),o.appendChild(w)})}m.querySelector("#vocab-category-select").addEventListener("change",s=>{p=s.target.value,g()}),m.querySelector("#vocab-tag-select").addEventListener("change",s=>{u=s.target.value,g()}),m.querySelector("#vocab-level-select").addEventListener("change",async s=>{var j;l=s.target.value,o.innerHTML=`<div style="grid-column: 1 / -1; text-align: center; padding: 2rem; color: var(--color-text-muted);">Chargement (Loading level ${l})...</div>`,await M("knowledge",l);const k=(((j=f.db)==null?void 0:j.knowledge)||[]).filter(v=>v.french&&v.japanese).filter(v=>r||v.is_professional),L=["ALL",...new Set(k.map(v=>v.category))],h=["ALL",...new Set(k.flatMap(v=>v.tags||[]))],w=m.querySelector("#vocab-category-select"),y=w.value;w.innerHTML=L.map(v=>`<option value="${v}" ${v===y?"selected":""}>${v}</option>`).join(""),p=w.value;const S=m.querySelector("#vocab-tag-select"),C=S.value;S.innerHTML=h.map(v=>`<option value="${v}" ${v===C?"selected":""}>${v==="ALL"?"ALL TAGS":"#"+v}</option>`).join(""),u=S.value,g()}),g()}function fe(){var i;const t=document.createElement("div"),c=document.createElement("h2");c.className="section-title",c.innerText="Grammaire de la Cuisine",t.appendChild(c);const r=document.createElement("p");r.className="section-subtitle",r.innerText="French grammar concepts framed through recipe instructions and professional dialogues.",t.appendChild(r);const n=document.createElement("div");n.className="loading-placeholder",n.innerText="Chargement de la grammaire... (Loading grammar...)",t.appendChild(n);const a=((i=f.settings)==null?void 0:i.targetLevel)||"ALL";return M("knowledge",a).then(()=>{n.remove(),he(t,a)}),t}function he(t,c){var a;const n=(((a=f.db)==null?void 0:a.knowledge)||[]).filter(i=>i.grammar).map(i=>({id:i.id,level:i.level,topic:i.grammar.topic,explanation_en:i.grammar.explanation_en,explanation_ja:i.grammar.explanation_ja,examples:i.examples||[]})).filter(i=>c==="ALL"||i.level===c);if(n.length===0){const i=document.createElement("p");i.style.color="var(--color-text-muted)",i.innerText="Aucune leçon de grammaire chargée pour ce niveau.",t.appendChild(i);return}n.forEach(i=>{const d=document.createElement("div");d.className="grammar-card";const e=N(i.id),p=i.topic.match(/\(([^)]+)\)/),u=p?p[1]:i.topic;d.innerHTML=`
      <div class="grammar-header">
        ...
        <div style="display: flex; align-items: center; gap: 1rem;">
          <button class="fav-btn ${e?"active":""}" data-id="${i.id}" style="font-size: 1.1rem; padding: 0.2rem;">
            ${e?"★":"☆"}
          </button>
    ...
    `;const l=d.querySelector(".grammar-header"),m=d.querySelector(".grammar-body"),o=d.querySelector(".toggle-icon");l.addEventListener("click",g=>{if(g.target.classList.contains("fav-btn")||g.target.closest(".audio-btn"))return;const b=m.classList.toggle("open");o.innerText=b?"▲":"▼"}),d.querySelector(".fav-btn").addEventListener("click",g=>{g.stopPropagation(),D(i.id);const b=g.target,_=N(i.id);b.classList.toggle("active",_),b.innerText=_?"★":"☆"}),d.querySelectorAll(".audio-btn").forEach(g=>{g.addEventListener("click",b=>{b.stopPropagation();const _=b.target.closest(".audio-btn").getAttribute("data-text");B(_)})}),t.appendChild(d)})}function be(t){var r;t.innerHTML='<div style="text-align: center; padding: 2rem; color: var(--color-text-muted);">Chargement de la théorie... (Loading theory...)</div>';const c=((r=f.settings)==null?void 0:r.targetLevel)||"ALL";M("knowledge",c).then(()=>{var d;t.innerHTML="";const a=(((d=f.db)==null?void 0:d.knowledge)||[]).filter(e=>e.cuisine).map(e=>({id:e.id,level:e.level,category:e.category||"Theory",topic:e.cuisine.topic,content_fr:e.cuisine.content_fr,content_en:e.cuisine.content_en,content_ja:e.cuisine.content_ja})).filter(e=>c==="ALL"||e.level===c);if(a.length===0){t.innerHTML='<p style="color: var(--color-text-muted);">Aucun document de théorie culinaire disponible pour ce niveau.</p>';return}const i=document.createElement("div");i.style.display="flex",i.style.flexDirection="column",i.style.gap="2rem",a.forEach(e=>{const p=document.createElement("div");p.className="card",p.style.display="block",p.style.padding="2rem";const u=N(e.id),l=e.topic.match(/^([^(]+)/),m=l?l[1].trim():e.topic;p.innerHTML=`
        ...
        <div class="term-header" style="border-bottom: 1px solid rgba(197, 168, 128, 0.15); padding-bottom: 0.8rem; margin-bottom: 1.5rem;">
          ...
          <button class="fav-btn ${u?"active":""}" data-id="${e.id}" style="font-size: 1.3rem;">
            ${u?"★":"☆"}
          </button>
        </div>
  ...
  `,c.appendChild(r);const n=document.createElement("div");n.className="meat-type-tabs",n.style.marginTop="0.5rem",n.style.borderBottom="none",c.appendChild(n);const a=document.createElement("div");a.className="meat-display-area",c.appendChild(a),t.appendChild(c);const i={primary:{title:"主要食肉 (Primary Meats)",subCategories:[{key:"beef",label:"🐂 Bœuf (牛)",cuts:we,img:"assets/beef_cuts.png",placeholder:"Coupe+de+Boeuf",pdf:"beef_maff_guide.pdf",pdfText:"📄 日本農水省 牛肉部位基準 (PDF)"},{key:"porc",label:"🐖 Porc (豚)",cuts:Le,img:"assets/porc_cuts.png",placeholder:"Coupe+de+Porc",pdf:"pork_maff_guide.pdf",pdfText:"📄 日本農水省 豚肉部位基準 (PDF)",quote:'"Tout est bon dans le cochon" (豚はすべてが使える食材である)'},{key:"volaille",label:"🐓 Volaille (鶏)",cuts:Se,img:"assets/poultry_cuts.png",placeholder:"Coupe+de+Volaille",pdf:"poultry_maff_guide.pdf",pdfText:"📄 日本農水省 鶏肉部位基準 (PDF)",quote:'"La volaille est la reine des cuisines et la directrice des banquets" (鶏肉は厨房の女王であり、宴の演出家である)'}]},livestock:{title:"その他の家畜肉 (Other Livestock)",subCategories:[{key:"agneau",label:"🐏 Agneau (羊)",cuts:je,img:"assets/agneau_cuts.png",placeholder:"Agneau"},{key:"veau",label:"🐂 Veau (仔牛)",cuts:qe,img:"assets/veal_cuts.png",placeholder:"Veau"},{key:"canard",label:"🦆 Canard (鴨)",cuts:$e,img:"assets/canard_cuts.png",placeholder:"Canard"}]},game:{title:"ジビエ (Game Meats)",subCategories:[{key:"chevreuil",label:"🦌 Chevreuil (鹿)",cuts:Ee,img:"assets/game_cuts.png",placeholder:"Chevreuil"},{key:"sanglier",label:"🐗 Sanglier (猪)",cuts:ze,img:"assets/game_cuts.png",placeholder:"Sanglier"},{key:"pigeon",label:"🕊️ Pigeon (鳩)",cuts:Te,img:"assets/game_cuts.png",placeholder:"Pigeon"},{key:"lievre",label:"🐇 Lièvre (野ウサギ)",cuts:Ae,img:"assets/game_cuts.png",placeholder:"Lievre"}]},seafood:{title:"魚介類 (Seafood)",subCategories:[{key:"poisson",label:"🐟 Poisson (魚)",cuts:Me,img:"assets/seafood_cuts.png",placeholder:"Poisson"},{key:"crustaces",label:"🦞 Crustacés (甲殻類)",cuts:Fe,img:"assets/seafood_cuts.png",placeholder:"Crustaces"},{key:"coquillages",label:"🐚 Coquillages (貝類)",cuts:Pe,img:"assets/seafood_cuts.png",placeholder:"Coquillages"},{key:"mollusques",label:"🐙 Mollusques (軟体類)",cuts:Be,img:"assets/seafood_cuts.png",placeholder:"Mollusques"}]},cheese_wine:{title:"チーズ・ワイン (Cheese & Wine)",subCategories:[{key:"fromages",label:"🧀 Fromages (チーズ)",cuts:Ne,img:"assets/cheese_wine.png",placeholder:"Fromages"},{key:"vins",label:"🍷 Vins (ワイン)",cuts:He,img:"assets/cheese_wine.png",placeholder:"Vins"}]},vegetables:{title:"野菜 (Vegetables)",subCategories:[{key:"legumes",label:"🥦 Légumes (野菜全般)",cuts:Re,img:"assets/vegetables.png",placeholder:"Legumes"}]},herbs_spices:{title:"ハーブ・スパイス (Herbs & Spices)",subCategories:[{key:"herbes",label:"🌿 Herbes (香草)",cuts:De,img:"assets/herbs_spices.png",placeholder:"Herbes"},{key:"epices",label:"🌶️ Épices (香辛料)",cuts:Ge,img:"assets/herbs_spices.png",placeholder:"Epices"}]},fruits_mushrooms:{title:"果物・きのこ (Fruits & Mushrooms)",subCategories:[{key:"fruits",label:"🍎 Fruits (果物)",cuts:Ie,img:"assets/fruits_mushrooms.png",placeholder:"Fruits"},{key:"champignons",label:"🍄 Champignons (きのこ)",cuts:Ve,img:"assets/fruits_mushrooms.png",placeholder:"Champignons"}]}};function d(e){const p=i[e];n.innerHTML="",p.subCategories.forEach((u,l)=>{const m=document.createElement("button");m.className=`meat-tab-btn ${l===0?"active":""}`,m.setAttribute("data-sub",u.key),m.innerText=u.label,m.style.fontSize="0.8rem",m.style.padding="0.3rem 0.8rem",m.addEventListener("click",o=>{n.querySelectorAll(".meat-tab-btn").forEach(g=>g.classList.remove("active")),m.classList.add("active"),te(u,a)}),n.appendChild(m)}),p.subCategories.length<=1?n.style.display="none":n.style.display="flex",te(p.subCategories[0],a)}r.querySelectorAll(".meat-tab-btn").forEach(e=>{e.addEventListener("click",p=>{r.querySelectorAll(".meat-tab-btn").forEach(l=>l.classList.remove("active")),e.classList.add("active");const u=e.getAttribute("data-category");d(u)})}),d("primary")}function te(t,c){c.innerHTML="";const r=document.createElement("div");r.style.display="flex",r.style.flexDirection="column",r.style.gap="1.5rem";let n="";t.quote&&(n+=`
      <div style="background-color: rgba(197, 168, 128, 0.05); padding: 1rem; border-radius: var(--radius-sm); border-left: 3px solid var(--color-accent); margin-bottom: 0.5rem; text-align: center;">
        <span style="font-family: var(--font-serif); font-size: 1.1rem; font-style: italic; color: var(--color-primary);">${t.quote}</span>
      </div>
    `),t.pdf&&(n+=`
      <div style="display: flex; justify-content: flex-end; margin-bottom: 0.5rem;">
        <a href="${t.pdf}" target="_blank" class="next-btn" style="text-decoration: none; display: inline-flex; align-items: center; gap: 0.5rem; font-size: 0.8rem; padding: 0.4rem 0.8rem; background-color: rgba(197, 168, 128, 0.08); color: var(--color-primary); border: 1px solid rgba(197, 168, 128, 0.3); border-radius: var(--radius-sm); font-weight: 600; cursor: pointer; transition: var(--transition);">
          ${t.pdfText}
        </a>
      </div>
    `),n+=`
    <div class="ingredient-grid-layout">
      ...
      <div class="interactive-canvas-container" style="position: relative; width: 100%; margin: 0;">
        <img src="${t.img}" alt="${t.label}" class="interactive-image" onerror="this.src='https://placehold.co/700x450/F4EAD4/0a1931?text=${t.placeholder}'">
        ...
        <svg class="interactive-svg-overlay" viewBox="0 0 100 100">
          ${t.cuts.filter(u=>u.points).map(u=>`
    ...
    <div class="cuisine-detail-drawer" id="ingredient-detail-drawer">
      <div class="detail-drawer-header">
        ...
        <span style="font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px; color: var(--color-text-muted); font-weight: 600;" id="ingredient-cut-sub">${t.label}</span>
      </div>
  ...
  `,t.appendChild(c);const r=c.querySelector("#map-detail-drawer"),n=c.querySelectorAll(".interactive-area"),a=c.querySelectorAll(".list-item-btn");function i(d,e){n.forEach(m=>m.classList.remove("active")),a.forEach(m=>m.classList.remove("active"));const p=c.querySelector(`.interactive-area[data-id="${d}"]`);p&&p.classList.add("active");const u=c.querySelector(`.list-item-btn[data-id="${d}"]`);u&&u.classList.add("active");const l=J.find(m=>m.id===d);if(l){c.querySelector("#map-region-title").innerText=`${l.name_fr} (${l.name_ja})`,c.querySelector("#map-region-sub").innerText=`${l.name_en} Region`,c.querySelector("#map-desc-fr").innerText=l.desc_fr,c.querySelector("#map-desc-en").innerText=l.desc_en,c.querySelector("#map-desc-ja").innerText=l.desc_ja;const m=c.querySelector("#map-relations-container"),o=c.querySelector("#map-relations-content"),g=se(d,"region");g?(o.innerHTML=g,m.style.display="block"):m.style.display="none";const b=c.querySelector("#map-audio-title-btn");b.style.display="inline-block",b.onclick=()=>B(l.name_fr);const _=c.querySelector("#map-audio-desc-btn");_.style.display="inline-block",_.onclick=()=>B(l.desc_fr),r.style.display="block",window.innerWidth<=600&&r.scrollIntoView({behavior:"smooth"})}}n.forEach(d=>{d.addEventListener("click",e=>{const p=e.target.getAttribute("data-id");i(p,e.target)})}),a.forEach(d=>{d.addEventListener("click",e=>{const p=e.target.closest(".list-item-btn").getAttribute("data-id");i(p,e.target.closest(".list-item-btn"))})})}function Ke(){const t=document.createElement("div"),c=document.createElement("h2");c.className="section-title",c.innerText="Théorie de l'Art Culinaire",t.appendChild(c);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Classical French culinary theory, stock making, classical sauces, and interactive gastronomy diagrams.",t.appendChild(r);const n=document.createElement("div");n.className="cuisine-tabs",n.innerHTML=`
  ...
  `,t.appendChild(n);const a=document.createElement("div");a.className="cuisine-content-wrapper",t.appendChild(a),n.querySelectorAll(".cuisine-tab-btn").forEach(d=>{d.addEventListener("click",e=>{n.querySelectorAll(".cuisine-tab-btn").forEach(u=>u.classList.remove("active")),e.target.classList.add("active");const p=e.target.getAttribute("data-tab");i(p)})});function i(d){a.innerHTML="",d==="theory"?be(a):d==="ingredients"?Oe(a):d==="map"&&We(a)}return i("theory"),t}function A(t){const c=[...t];for(let r=c.length-1;r>0;r--){const n=Math.floor(Math.random()*(r+1));[c[r],c[n]]=[c[n],c[r]]}return c}function ne(t){return t.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().trim().replace(/[-\s]+/g," ")}function Je(){if(document.getElementById("quiz-dynamic-styles"))return;const t=document.createElement("style");t.id="quiz-dynamic-styles",t.innerHTML=`
    .quiz-mode-selector {
      display: flex;
  ...
  `,document.head.appendChild(t)}function Qe(){var d;const t=[],c=((d=f.db)==null?void 0:d.knowledge)||[],r=c.filter(e=>e.french&&e.japanese),n=c.filter(e=>e.grammar),a=c.filter(e=>e.cuisine);function i(e,p,u,l){const o=e.filter(b=>b.id!==u).map(l).sort(()=>.5-Math.random()),g=[...new Set(o)].filter(Boolean).slice(0,p);for(;g.length<p;)g.push("Autre option "+(g.length+1));return g}return r.forEach(e=>{var o,g,b,_,s;let p="vocabulary";(o=e.tags)!=null&&o.includes("meat")?p="meat":(g=e.tags)!=null&&g.includes("sauces")||(b=e.tags)!=null&&b.includes("sauce")?p="sauces":(_=e.tags)!=null&&_.includes("cutting")?p="cuts":(s=e.tags)!=null&&s.includes("science")&&(p="science");const u=i(r,3,e.id,x=>x.japanese),l=e.examples&&e.examples[0]?e.examples[0].fr:"";t.push({id:`dyn_vocab_fr_ja_${e.id}`,type:"choice",category:p,question:`Que signifie le mot français "${e.french}" ? / What does the French word "${e.french}" mean?`,question_fr:`Que signifie le mot français "${e.french}" ?`,question_en:`What does the French word "${e.french}" mean?`,context:e.definition_fr||`Usage: ${l}`,options:A([e.japanese,...u]),answer:e.japanese});const m=i(r,3,e.id,x=>x.french);t.push({id:`dyn_vocab_ja_fr_${e.id}`,type:"choice",category:p,question:`Quel est le mot français pour "${e.japanese}" ? / What is the French word for "${e.japanese}"?`,question_fr:`Quel est le mot français pour "${e.japanese}" ?`,question_en:`What is the French word for "${e.japanese}"?`,context:e.definition_fr||`Usage: ${l}`,options:A([e.french,...m]),answer:e.french})}),n.forEach(e=>{if(!e.grammar.topic||!e.examples||e.examples.length===0)return;const p=i(n,3,e.id,u=>u.grammar.topic);t.push({id:`dyn_gram_topic_${e.id}`,type:"choice",category:"grammar",question:`De quel concept de grammaire s'agit-il : "${e.grammar.explanation_ja}" ? / Which grammar concept is this: "${e.grammar.explanation_en}"?`,question_fr:"De quel concept de grammaire s'agit-il ?",question_en:`Which grammar concept is this: "${e.grammar.explanation_en}"?`,context:`Niveau : ${e.level}. Indispensable pour la cuisine.`,options:A([e.grammar.topic,...p]),answer:e.grammar.topic}),e.examples.forEach((u,l)=>{const o=n.flatMap(g=>g.examples||[]).filter(g=>g.fr!==u.fr).sort(()=>.5-Math.random()).map(g=>g.ja).slice(0,3);for(;o.length<3;)o.push("Option de traduction "+(o.length+1));t.push({id:`dyn_gram_ex_${e.id}_${l}`,type:"choice",category:"grammar",question:`Traduisez la phrase : "${u.fr}" / Translate the sentence: "${u.fr}"`,question_fr:`Traduisez la phrase : "${u.fr}"`,question_en:`Translate the sentence: "${u.fr}"`,context:`Grammaire: ${e.grammar.topic} (${e.level})`,options:A([u.ja,...o]),answer:u.ja})})}),a.forEach(e=>{var l,m,o,g,b,_;if(!e.cuisine.topic||!e.cuisine.content_ja)return;const p=i(a,3,e.id,s=>s.cuisine.topic);let u="sauces";(l=e.tags)!=null&&l.includes("knife-cuts")||(m=e.tags)!=null&&m.includes("cuts")?u="cuts":(o=e.tags)!=null&&o.includes("meat")?u="meat":((g=e.tags)!=null&&g.includes("molecular")||(b=e.tags)!=null&&b.includes("chemistry")||(_=e.tags)!=null&&_.includes("science"))&&(u="science"),t.push({id:`dyn_cuis_topic_${e.id}`,type:"choice",category:u,question:`De quel concept culinaire s'agit-il : "${e.cuisine.content_ja.substring(0,120)}..." ? / Which culinary concept is described: "${e.cuisine.content_en.substring(0,120)}..."?`,question_fr:"De quel concept culinaire s'agit-il ?",question_en:"Which culinary concept is described here?",context:`Niveau : ${e.level}. Mots-clés : ${e.tags.join(", ")}`,options:A([e.cuisine.topic,...p]),answer:e.cuisine.topic})}),t}function Ue(){var i;const t=[],c=((i=f.db)==null?void 0:i.knowledge)||[],r=[];c.forEach(d=>{(d.examples||[]).forEach(e=>{r.push({fr:e.fr,ja:e.ja})})});const n=r.sort(()=>.5-Math.random());for(const d of n){if(t.length>=4)break;const e=d.fr.replace(/^➔\s*/,"").trim();if(e.includes(" - ")){const p=e.split(" - ");t.push({id:`pair_${t.length}`,left:`🗣️ ${p[0].trim()}`,right:`💬 ${p[1].trim()}`,context:d.ja})}else if(e.includes(",")){const p=e.split(",");t.push({id:`pair_${t.length}`,left:`${p[0].trim()} ,`,right:`... ${p[1].trim()}`,context:d.ja})}else{const p=e.split(" ");if(p.length>=4){const u=Math.floor(p.length/2),l=p.slice(0,u).join(" "),m=p.slice(u).join(" ");t.push({id:`pair_${t.length}`,left:`${l} ...`,right:`... ${m}`,context:d.ja})}}}const a=[{left:"🗣️ Chaud devant !",right:"💬 Oui, chef !",context:"お通りです！ / はい、シェフ！"},{left:"🗣️ Entrée prête ?",right:"💬 Oui, j'envoie.",context:"前菜はできていますか？ / はい、送ります。"},{left:"🗣️ Combien de couverts ?",right:"💬 Vingt couverts.",context:"何名様（何席）ですか？ / 20席です。"},{left:"🗣️ Envoyez la sauce !",right:"💬 Tout de suite !",context:"ソースを出して！ / ただちに！"}];for(;t.length<4;){const d=a[t.length%a.length];t.push({id:`default_${t.length}`,left:d.left,right:d.right,context:d.context})}return t}function Ye(){var a;const t=document.createElement("div"),c=document.createElement("h2");c.className="section-title",c.innerText="Vérification des Connaissances",t.appendChild(c);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Test your professional French vocabulary, kitchen commands, and classical cuisine theory.",t.appendChild(r);const n=document.createElement("div");return n.className="loading-placeholder",n.innerText="Chargement du quiz... (Loading quiz...)",t.appendChild(n),Je(),(a=f.settings)!=null&&a.targetLevel,Promise.all([ce(),M("vocabulary","ALL"),M("grammar","ALL"),M("cuisine","ALL")]).then(()=>{n.remove(),Xe(t)}),t}function Xe(t){let c="multiple",r="ALL";const n=document.createElement("div");n.className="quiz-mode-selector",n.innerHTML=`
  ...
  `,t.appendChild(a);const i=a.querySelector("#quiz-cat-select");i.addEventListener("change",g=>{r=g.target.value,e()});const d=document.createElement("div");d.className="quiz-game-wrapper",t.appendChild(d),n.querySelectorAll(".mode-tab-btn").forEach(g=>{g.addEventListener("click",b=>{n.querySelectorAll(".mode-tab-btn").forEach(_=>_.classList.remove("active")),b.target.classList.add("active"),c=b.target.getAttribute("data-mode"),c==="matching_taking"?(i.disabled=!0,i.style.opacity="0.5"):(i.disabled=!1,i.style.opacity="1.0"),e()})});function e(){d.innerHTML="",c==="multiple"?p():c==="matching_vocab"?u():c==="matching_taking"?l():c==="spelling"&&o()}function p(){var w;const g=((w=f.db)==null?void 0:w.quizzes)||[],b=Qe();let _=[...g,...b];if(r!=="ALL"&&(_=_.filter(y=>y.category===r)),_.length===0){d.innerHTML=`
      ...
      `;return}const s=A(_).slice(0,10);let x=0,k=0,L=!1;function h(){if(d.innerHTML="",L=!1,x>=s.length){const $=Math.round(k/s.length*100);let q="Apprenti (Apprentice)";$>=90?q="Chef de Partie (Station Chef)":$>=70&&(q="Commis de Cuisine (Line Cook)"),d.innerHTML=`
        ...
        `,d.querySelector("#restart-choice-btn").addEventListener("click",()=>{p()});return}const y=s[x],S=document.createElement("div");S.className="quiz-card",S.innerHTML=`
        <div class="quiz-meta">
      ...
      `;const C=S.querySelector(".quiz-hint-flip-container");C&&C.addEventListener("click",()=>{const $=C.querySelector(".q-en-hint-text"),q=$.style.display==="none";$.style.display=q?"block":"none",C.querySelector(".q-en-card").innerText=q?"💡 Hide English Hint":"💡 Translate (Show English Hint)"});const j=S.querySelectorAll(".quiz-btn"),v=S.querySelector(".quiz-feedback"),E=S.querySelector("#next-q-btn");j.forEach($=>{$.addEventListener("click",q=>{if(L)return;L=!0;const F=q.target.innerText===y.answer;j.forEach(P=>{P.disabled=!0,P.innerText===y.answer&&P.classList.add("correct")}),F?k++:(q.target.classList.add("incorrect"),ae(y.id)),v.style.display="block",E.style.display="block"})}),E.addEventListener("click",()=>{x++,h()}),d.appendChild(S)}h()}function u(){var h,w;const g=((h=f.settings)==null?void 0:h.includeGeneral)||!1;let _=(((w=f.db)==null?void 0:w.vocabulary)||[]).filter(y=>g||y.is_professional);if(r!=="ALL"&&(r==="meat"?_=_.filter(y=>{var S,C,j,v;return((S=y.tags)==null?void 0:S.includes("meat"))||((C=y.tags)==null?void 0:C.includes("beef"))||((j=y.tags)==null?void 0:j.includes("pork"))||((v=y.tags)==null?void 0:v.includes("poultry"))||/viande|boeuf|porc|poulet/i.test(y.french)}):r==="sauces"?_=_.filter(y=>{var S,C,j;return((S=y.tags)==null?void 0:S.includes("sauce"))||((C=y.tags)==null?void 0:C.includes("sauces"))||((j=y.tags)==null?void 0:j.includes("stocks"))||/sauce|fond|jus|bouillon/i.test(y.french)}):r==="cuts"?_=_.filter(y=>{var S,C;return((S=y.tags)==null?void 0:S.includes("cutting"))||((C=y.tags)==null?void 0:C.includes("vegetables"))||/coupe|tailler|ciseler|mincer|brunoise|julienne/i.test(y.french)}):r==="science"?_=_.filter(y=>{var S;return((S=y.tags)==null?void 0:S.includes("science"))||/réaction|émulsion|liaison/i.test(y.french)}):r==="grammar"&&(_=[])),_.length<4){d.innerHTML=`
        <div class="quiz-card" style="text-align: center; padding: 2rem;">
      ...
      `;return}const s=A(_).slice(0,4),x=A(s),k=A(s),L=document.createElement("div");L.className="quiz-card",L.innerHTML=`
    ...
    `,d.appendChild(L),m(L,4,u)}function l(){const g=Ue(),b=document.createElement("div");b.className="quiz-card",b.innerHTML=`
      <div class="quiz-meta" style="margin-bottom: 1rem;">
    ...
    `,d.appendChild(b),m(b,4,l)}function m(g,b,_){let s=null,x=null,k=0;const L=g.querySelectorAll(".drag-card"),h=g.querySelectorAll(".drop-zone"),w=g.querySelector("#matching-completion-panel");L.forEach(C=>{C.addEventListener("dragstart",j=>{s=j.target.closest(".drag-card").getAttribute("data-id"),j.target.closest(".drag-card").classList.add("dragging")}),C.addEventListener("dragend",j=>{j.target.closest(".drag-card").classList.remove("dragging")}),C.addEventListener("click",j=>{const v=j.target.closest(".drag-card");v.classList.contains("matched")||(L.forEach(E=>E.style.borderColor="rgba(10,25,49,0.08)"),x=v.getAttribute("data-id"),v.style.borderColor="var(--color-accent)")})}),h.forEach(C=>{C.addEventListener("dragover",j=>{j.preventDefault(),C.classList.contains("matched")||C.classList.add("hovered")}),C.addEventListener("dragleave",()=>{C.classList.remove("hovered")}),C.addEventListener("drop",j=>{j.preventDefault(),C.classList.remove("hovered");const v=C.getAttribute("data-id");s===v?S(s,C):y(s)}),C.addEventListener("click",()=>{if(C.classList.contains("matched")||!x)return;const j=C.getAttribute("data-id");x===j?(S(x,C),x=null):(y(x),x=null,L.forEach(v=>v.style.borderColor="rgba(10,25,49,0.08)"))})});function y(C){const j=g.querySelector(`#drag-${C}`);j&&(j.style.animation="shake-anim 0.4s ease-in-out",setTimeout(()=>j.style.animation="",400))}function S(C,j){const v=g.querySelector(`#drag-${C}`);v.classList.add("matched"),v.style.borderColor="var(--color-success)",j.classList.add("matched"),k++,k===b&&(w.style.display="block")}g.querySelector("#restart-match-btn").addEventListener("click",()=>{_()})}function o(){var C,j;const g=((C=f.settings)==null?void 0:C.includeGeneral)||!1;let _=(((j=f.db)==null?void 0:j.vocabulary)||[]).filter(v=>g||v.is_professional);if(r!=="ALL"&&(r==="meat"?_=_.filter(v=>{var E,$,q,z;return((E=v.tags)==null?void 0:E.includes("meat"))||(($=v.tags)==null?void 0:$.includes("beef"))||((q=v.tags)==null?void 0:q.includes("pork"))||((z=v.tags)==null?void 0:z.includes("poultry"))||/viande|boeuf|porc|poulet/i.test(v.french)}):r==="sauces"?_=_.filter(v=>{var E,$,q;return((E=v.tags)==null?void 0:E.includes("sauce"))||(($=v.tags)==null?void 0:$.includes("sauces"))||((q=v.tags)==null?void 0:q.includes("stocks"))||/sauce|fond|jus|bouillon/i.test(v.french)}):r==="cuts"?_=_.filter(v=>{var E,$;return((E=v.tags)==null?void 0:E.includes("cutting"))||(($=v.tags)==null?void 0:$.includes("vegetables"))||/coupe|tailler|ciseler|mincer|brunoise|julienne/i.test(v.french)}):r==="science"?_=_.filter(v=>{var E;return((E=v.tags)==null?void 0:E.includes("science"))||/réaction|émulsion|liaison/i.test(v.french)}):r==="grammar"&&(_=[])),_.length===0){d.innerHTML=`
        <div class="quiz-card" style="text-align: center; padding: 2rem;">
    ...
    `,d.appendChild(x);const k=x.querySelector("#spelling-input-field"),L=x.querySelector("#spelling-submit-btn"),h=x.querySelector("#spelling-next-btn"),w=x.querySelector("#spelling-feedback-panel"),y=x.querySelector("#spelling-feedback-title"),S=x.querySelector("#spelling-feedback-msg");k.addEventListener("keydown",v=>{v.key==="Enter"&&L.click()}),L.addEventListener("click",()=>{const v=k.value,E=s.french,$=ne(v),q=ne(E),z=$===q;k.disabled=!0,L.style.display="none",h.style.display="block",w.style.display="block",z?(k.style.borderColor="var(--color-success)",k.style.backgroundColor="#E8F5E9",w.style.backgroundColor="#E8F5E9",w.style.color="var(--color-success)",y.innerText="✓ Félicitations ! (Correct)",S.innerText=`You correctly spelled: "${E}"`):(k.style.borderColor="var(--color-error)",k.style.backgroundColor="#FFEBEE",w.style.backgroundColor="#FFEBEE",w.style.color="var(--color-error)",y.innerText="✗ Incorrect",S.innerHTML=`Correct spelling is: <strong>${E}</strong>.<br><em style="font-size:0.85rem;">You typed: "${v}"</em>`,ae(s.id))}),h.addEventListener("click",()=>{o()})}e()}function V(){const t=document.createElement("div"),c=document.createElement("h2");c.className="section-title",c.innerText="Coups de Cœur (Favorites)",t.appendChild(c);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Your bookmarked vocabulary terms, grammar guides, and culinary theories.",t.appendChild(r);const n=Array.from(f.favorites);if(n.length===0)return t.innerHTML+=`
      <div class="alert alert-info">
        <p>No favorites saved yet. Browse Vocabulary, Grammar, or Cuisine and click the star (☆) button to save items here.</p>
      </div>
    `,t;const a=document.createElement("div");return a.className="loading-placeholder",a.innerText="Chargement des favoris... (Loading favorites...)",t.appendChild(a),Q().then(()=>{a.remove(),Ze(t,n)}),t}function Ze(t,c){var e,p,u;const r=(((e=f.db)==null?void 0:e.vocabulary)||[]).filter(l=>c.includes(l.id)),n=(((p=f.db)==null?void 0:p.grammar)||[]).filter(l=>c.includes(l.id)),a=(((u=f.db)==null?void 0:u.cuisine)||[]).filter(l=>c.includes(l.id)),i=document.createElement("div");i.className="card-grid",t.appendChild(i);function d(){i.innerHTML="",r.forEach(l=>{const m=document.createElement("div");m.className="card",m.innerHTML=`
        <div>
      ...
      `,m.querySelector(".fav-btn").addEventListener("click",o=>{D(l.id),V(),t.innerHTML="",t.appendChild(V())}),i.appendChild(m)}),n.forEach(l=>{var o,g;const m=document.createElement("div");m.className="card",m.innerHTML=`
        <div>
      ...
      `,m.querySelector(".fav-btn").addEventListener("click",b=>{D(l.id),t.innerHTML="",t.appendChild(V())}),i.appendChild(m)}),a.forEach(l=>{const m=document.createElement("div");m.className="card",m.innerHTML=`
        <div>
      ...
      `,m.querySelector(".fav-btn").addEventListener("click",o=>{D(l.id),t.innerHTML="",t.appendChild(V())}),i.appendChild(m)})}d()}function et(){const t=document.createElement("div"),c=document.createElement("h2");c.className="section-title",c.innerText="Pont d'Études (SRS Review Deck)",t.appendChild(c);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Review and grade yourself on items scheduled for active recall today.",t.appendChild(r);const n=document.createElement("div");return n.className="loading-placeholder",n.innerText="Chargement des révisions... (Loading reviews...)",t.appendChild(n),Q().then(()=>{n.remove(),tt(t)}),t}function tt(t){var p,u,l,m;const c=new Date().toISOString().split("T")[0],n=[...(((p=f.db)==null?void 0:p.vocabulary)||[]).map(o=>({...o,type:"vocabulary",front:o.french})),...(((u=f.db)==null?void 0:u.grammar)||[]).map(o=>({...o,type:"grammar",front:o.topic})),...(((l=f.db)==null?void 0:l.cuisine)||[]).map(o=>({...o,type:"cuisine",front:o.topic})),...(((m=f.db)==null?void 0:m.quizzes)||[]).map(o=>({...o,type:"quiz",front:o.question_fr||o.question||""}))].filter(o=>{const g=f.srs[o.id];return f.wrongAnswers.includes(o.id)||g&&g.dueDate<=c}),a=n.length;let i=0;const d=document.createElement("div");d.className="srs-review-container",t.appendChild(d);function e(){if(d.innerHTML="",n.length===0||i>=n.length){d.innerHTML=`
  ...
  `,Q().then(()=>{a.disabled=!1,a.placeholder="Rechercher... (e.g. sauce, roux, cut, culer, 刻む)";function d(e){var g,b,_;if(i.innerHTML="",!e.trim()){i.innerHTML=`
        ...
        `,x.querySelector(".fav-btn").addEventListener("click",w=>{w.stopPropagation(),D(s.id);const y=w.target,S=N(s.id);y.classList.toggle("active",S),y.innerText=S?"★":"☆"}),i.appendChild(x)})}a.addEventListener("input",e=>{d(e.target.value)}),d("")}),t}function at(){const t=document.createElement("div"),c=document.createElement("h2");c.className="section-title",c.innerText="Configuration de l'Académie",t.appendChild(c);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Customize your learning goals, target CEFR levels, and database options.",t.appendChild(r);const n=document.createElement("div");n.className="card",n.style.padding="2rem";const a=f.settings;n.innerHTML=`
  ...
  `;const i=n.querySelector("#target-level-select"),d=n.querySelector("#new-cards-goal"),e=n.querySelector("#max-reviews-goal"),p=n.querySelector("#include-general-checkbox"),u=n.querySelector("#settings-status");function l(){const m=i.value,o=parseInt(d.value)||5,g=parseInt(e.value)||20,b=p.checked;ct({targetLevel:m,newCardsPerDay:o,maxReviewsPerDay:g,includeGeneral:b}),u.style.display="block",setTimeout(()=>{u.style.display="none"},3e3)}return i.addEventListener("change",l),d.addEventListener("input",l),e.addEventListener("input",l),p.addEventListener("change",l),n.querySelector("#reset-srs-btn").addEventListener("click",()=>{confirm("Voulez-vous vraiment réinitialiser toutes vos données de progression SRS ? Cette action est irréversible.")&&(localStorage.removeItem("cba_srs"),f.srs={},alert("Spaced Repetition System progress has been reset."),window.location.reload())}),n.querySelector("#clear-favs-btn").addEventListener("click",()=>{confirm("Voulez-vous vraiment supprimer tous vos favoris ?")&&(localStorage.removeItem("cba_favorites"),f.favorites=new Set,alert("All favorites have been cleared."),window.location.reload())}),n.querySelector("#share-app-btn").addEventListener("click",()=>{const m={title:"Académie de la Brigade",text:"フランス料理・厨房フランス語の統合学習PWAアプリ「Académie de la Brigade」で一緒に料理と語学を学びましょう！",url:window.location.origin+window.location.pathname};navigator.share?navigator.share(m).then(()=>console.log("Shared successfully")).catch(o=>console.log("Error sharing:",o)):navigator.clipboard.writeText(m.url).then(()=>{alert("App link copied to clipboard! Share it with your friends.")}).catch(o=>{console.error("Failed to copy link:",o)})}),t.appendChild(n),t}function rt(){var i;const t=document.createElement("div"),c=document.createElement("h2");c.className="section-title",c.innerText="Dictée de Cuisine (Culinary Dictations)",t.appendChild(c);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Listen to French kitchen instruction sentences, type what you hear, and master French spelling.",t.appendChild(r);const n=document.createElement("div");n.className="loading-placeholder",n.innerText="Chargement de la dictée... (Loading dictation...)",t.appendChild(n);const a=((i=f.settings)==null?void 0:i.targetLevel)||"ALL";return M("vocabulary",a).then(()=>{n.remove(),it(t)}),t}function it(t){var s,x;const c=((s=f.settings)==null?void 0:s.includeGeneral)||!1,a=(((x=f.db)==null?void 0:x.vocabulary)||[]).filter(k=>c||k.is_professional).filter(k=>k.context_fr);if(a.length===0){t.innerHTML+='<p style="color: var(--color-text-muted);">Aucun exercice disponible.</p>';return}const i=[...a].sort(()=>.5-Math.random()).slice(0,5);let d=0,e=!1,p=!1,u=!1,l=0,m=1;const o=document.createElement("div");o.className="dictation-container",o.style.maxWidth="600px",o.style.margin="1.5rem auto",t.appendChild(o);function g(k){return k.toLowerCase().trim().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()'"?]/g,"").replace(/\s+/g," ")}function b(k){return k.normalize("NFD").replace(/[\u0300-\u036f]/g,"")}function _(){if(o.innerHTML="",e=!1,K(),p=!1,u=!1,l=0,d>=i.length){const T=document.createElement("div");T.className="card",T.style.padding="2.5rem",T.style.textAlign="center",T.innerHTML=`
        <h3 style="font-family: var(--font-serif); font-size: 2rem; color: var(--color-primary); margin-bottom: 1rem;">Session Terminée !</h3>
      ...
      `,T.querySelector("#restart-dictation-btn").addEventListener("click",()=>{d=0,i.length=0,i.push(...[...a].sort(()=>.5-Math.random()).slice(0,5)),_()}),o.appendChild(T);return}const k=i[d],L=k.context_fr,h=document.createElement("div");h.className="card",h.style.padding="2rem",h.innerHTML=`
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; border-bottom: 1px solid rgba(0,0,0,0.05); padding-bottom: 0.8rem;">
        <span class="card-category" style="margin: 0;">Drill ${d+1} of ${i.length}</span>
    ...
    `;const w=h.querySelector("#play-pause-btn"),y=h.querySelector("#stop-btn"),S=h.querySelector("#speed-normal-btn"),C=h.querySelector("#speed-slow-btn"),j=h.querySelector("#dictation-seekbar"),v=h.querySelector("#dictation-input"),E=h.querySelector("#check-btn"),$=h.querySelector("#next-dictation-btn"),q=h.querySelector("#dictation-feedback");function z(T,G){if(!e){const H=T/G*100;j.value=Math.round(H),l=T}}function F(){p=!1,u=!1,l=0,w.innerText="▶ Play",j.value=0}function P(){oe(L,m,z,F,l)}w.addEventListener("click",()=>{e||(p?u?(u=!1,w.innerText="⏸ Pause",ue()):(u=!0,w.innerText="▶ Play",de()):(p=!0,u=!1,w.innerText="⏸ Pause",P()))}),y.addEventListener("click",()=>{K(),F()}),j.addEventListener("change",T=>{if(e)return;const G=parseInt(T.target.value),H=L.replace(/["'➔]/g,"").trim();l=Math.floor(G/100*H.length);const I=H.indexOf(" ",l);I!==-1&&I-l<5&&(l=I+1),p=!0,u=!1,w.innerText="⏸ Pause",P()}),S.addEventListener("click",()=>{m!==1&&(m=1,S.classList.add("active"),S.style.backgroundColor="var(--color-primary)",S.style.color="#FFFFFF",C.classList.remove("active"),C.style.backgroundColor="transparent",C.style.color="var(--color-accent)",p&&!u&&P())}),C.addEventListener("click",()=>{m!==.75&&(m=.75,C.classList.add("active"),C.style.backgroundColor="var(--color-accent)",C.style.color="#FFFFFF",S.classList.remove("active"),S.style.backgroundColor="transparent",S.style.color="var(--color-accent)",p&&!u&&P())}),E.addEventListener("click",()=>{if(e)return;K(),F();const T=v.value,G=g(T),H=g(L),I=b(G),le=b(H);let U=!1,Y=!1;G===H?U=!0:I===le&&(Y=!0),q.style.display="block",v.disabled=!0,E.style.display="none",$.style.display="block",e=!0,U?(q.style.backgroundColor="#E8F5E9",q.style.borderLeft="4px solid var(--color-success)",q.style.color="var(--color-success)",q.innerHTML=`
        ...
        `)}),$.addEventListener("click",()=>{d++,_()}),o.appendChild(h),v.focus()}_()}const f={meta:null,db:{knowledge:[],quizzes:[]},loaded:{knowledge:new Set,quizzes:!1},favorites:new Set(JSON.parse(localStorage.getItem("cba_favorites")||"[]")),wrongAnswers:JSON.parse(localStorage.getItem("cba_wrong")||"[]"),streak:parseInt(localStorage.getItem("cba_streak")||"0"),lastStudyDate:localStorage.getItem("cba_last_study")||"",srs:JSON.parse(localStorage.getItem("cba_srs")||"{}"),settings:JSON.parse(localStorage.getItem("cba_settings")||JSON.stringify({targetLevel:"ALL",newCardsPerDay:5,maxReviewsPerDay:20,includeGeneral:!1}))};function D(t){f.favorites.has(t)?f.favorites.delete(t):f.favorites.add(t),localStorage.setItem("cba_favorites",JSON.stringify(Array.from(f.favorites)))}function N(t){return f.favorites.has(t)}function ae(t){f.wrongAnswers.includes(t)||(f.wrongAnswers.push(t),localStorage.setItem("cba_wrong",JSON.stringify(f.wrongAnswers)))}function ot(t){f.wrongAnswers=f.wrongAnswers.filter(c=>c!==t),localStorage.setItem("cba_wrong",JSON.stringify(f.wrongAnswers))}function st(){const t=new Date().toISOString().split("T")[0];if(f.lastStudyDate!==t){if(f.lastStudyDate){const c=new Date(f.lastStudyDate),r=new Date(t),n=Math.abs(r-c),a=Math.ceil(n/(1e3*60*60*24));a===1?f.streak+=1:a>1&&(f.streak=1)}else f.streak=1;f.lastStudyDate=t,localStorage.setItem("cba_streak",f.streak.toString()),localStorage.setItem("cba_last_study",t)}}function O(t,c){const r=new Date().toISOString().split("T")[0],n=f.srs[t]||{easiness:2.5,interval:0,repetitions:0,dueDate:r};let a=Math.max(0,Math.min(5,c));a>=3?(n.repetitions===0?n.interval=1:n.repetitions===1?n.interval=6:n.interval=Math.round(n.interval*n.easiness),n.repetitions++):(n.repetitions=0,n.interval=1),n.easiness=n.easiness+(.1-(5-a)*(.08+(5-a)*.02)),n.easiness<1.3&&(n.easiness=1.3);const i=new Date;i.setDate(i.getDate()+n.interval),n.dueDate=i.toISOString().split("T")[0],n.lastRated=r,f.srs[t]=n,localStorage.setItem("cba_srs",JSON.stringify(f.srs))}function ct(t){f.settings={...f.settings,...t},localStorage.setItem("cba_settings",JSON.stringify(f.settings))}async function M(t,c){const r=c==="ALL"?["A1","A2","B1","B2","C1","C2"]:[c];for(const n of r)if(!f.loaded.knowledge.has(n))try{const a=await fetch(`data/knowledge_${n}.json`);if(!a.ok){console.warn(`Could not load data/knowledge_${n}.json`);continue}const i=await a.json(),d=new Set(f.db.knowledge.map(e=>e.id));for(const e of i)d.has(e.id)||f.db.knowledge.push(e);f.loaded.knowledge.add(n)}catch(a){console.error(`Failed to load knowledge level ${n}:`,a)}}async function ce(){if(!f.loaded.quizzes)try{const t=await fetch("data/quizzes.json");if(!t.ok)throw new Error("Network response was not ok");f.db.quizzes=await t.json(),f.loaded.quizzes=!0}catch(t){console.error("Failed to load quizzes:",t)}}async function Q(){await Promise.all([M("vocabulary","ALL"),M("grammar","ALL"),M("cuisine","ALL"),ce()])}const re={home:me,vocabulary:pe,grammar:fe,cuisine:Ke,quiz:Ye,favorites:V,review:et,search:nt,settings:at,dictation:rt};function ie(t){const c=document.getElementById("main-content");if(re[t]){st(),c.innerHTML="";const r=re[t]();r instanceof HTMLElement?c.appendChild(r):c.innerHTML=r,document.querySelectorAll(".nav-link").forEach(n=>{n.getAttribute("data-tab")===t?n.classList.add("active"):n.classList.remove("active")})}}async function lt(){try{const t=await fetch("data/meta.json");f.meta=await t.json(),document.querySelectorAll(".nav-link").forEach(c=>{c.addEventListener("click",r=>{const n=r.target.closest(".nav-link").getAttribute("data-tab");ie(n)})}),ie("home"),dt()}catch(t){console.error("Failed to load database metadata:",t),document.getElementById("main-content").innerHTML=`
    ...
    `}}function dt(){const t=document.getElementById("youglish-sidebar"),c=document.getElementById("youglish-sidebar-toggle"),r=document.getElementById("youglish-search-input"),n=document.getElementById("youglish-search-btn"),a=document.getElementById("yg-widget-container");if(!t||!c||!r||!n||!a)return;c.addEventListener("click",e=>{e.stopPropagation(),t.classList.toggle("open"),t.classList.contains("open")&&r.focus()}),document.addEventListener("click",e=>{t.classList.contains("open")&&!t.contains(e.target)&&t.classList.remove("open")});let i=null;function d(){const e=r.value.trim();if(e)if(a.innerHTML='<div id="yg-widget" style="width: 100%;"></div>',window.YG)try{i=new YG("yg-widget",{width:230,components:9,accentColor:"#C5A880",autoStart:1,search:e,language:"french"})}catch(p){console.error("Failed to load YouGlish widget:",p),a.innerHTML=`
          <div style="font-size: 0.75rem; text-align: center; color: var(--color-error); padding: 1rem;">
            ウィジェットの読み込みに失敗しました。<br>
            <a href="https://youglish.com/pronounce/${encodeURIComponent(e)}/french" target="_blank" style="color: var(--color-accent); font-weight: 600; text-decoration: underline;">YouGlishサイトで直接開く</a>
          </div>
        `}else window.open(`https://youglish.com/pronounce/${encodeURIComponent(e)}/french`,"_blank"),a.innerHTML=`
        <div style="font-size: 0.75rem; text-align: center; color: rgba(255,255,255,0.7); padding: 1rem;">
          発音ページを別タブで開きました。<br>
          <a href="https://youglish.com/pronounce/${encodeURIComponent(e)}/french" target="_blank" style="color: var(--color-accent); font-weight: 600; text-decoration: underline;">開かない場合はこちら</a>
        </div>
      `}n.addEventListener("click",d),r.addEventListener("keypress",e=>{e.key==="Enter"&&d()})}document.addEventListener("DOMContentLoaded",lt);
