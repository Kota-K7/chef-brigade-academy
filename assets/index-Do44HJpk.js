(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const u of o.addedNodes)u.tagName==="LINK"&&u.rel==="modulepreload"&&n(u)}).observe(document,{childList:!0,subtree:!0});function t(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(a){if(a.ep)return;a.ep=!0;const o=t(a);fetch(a.href,o)}})();let Se=null,je="",Te=0,fe=null,Y=null;function N(e,i=1){Fe(e,i)}function Fe(e,i=1,t=null,n=null,a=0){if(!("speechSynthesis"in window)){console.warn("Speech synthesis not supported on this browser.");return}window.speechSynthesis.cancel();const o=e.replace(/["'➔]/g,"").trim();je=o,Te=a,fe=t,Y=n;const u=o.substring(a);if(!u.trim()){Y&&Y();return}const r=new SpeechSynthesisUtterance(u);r.lang="fr-FR",r.rate=i;const c=window.speechSynthesis.getVoices().find(m=>m.lang==="fr-FR"||m.lang.startsWith("fr"));c&&(r.voice=c),r.onboundary=m=>{if(m.name==="word"&&fe){const v=Te+m.charIndex;fe(v,je.length)}},r.onend=()=>{Se===r&&Y&&Y()},r.onerror=m=>{console.warn("SpeechSynthesis error:",m),Y&&Y()},Se=r,window.speechSynthesis.speak(r)}function rt(){"speechSynthesis"in window&&window.speechSynthesis.pause()}function ot(){"speechSynthesis"in window&&window.speechSynthesis.resume()}function he(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}"speechSynthesis"in window&&(window.speechSynthesis.getVoices(),window.speechSynthesis.onvoiceschanged=()=>{window.speechSynthesis.getVoices()});function st(){var S,w,s,h,y,L,x;const e=document.createElement("div"),i=document.createElement("h2");i.className="section-title",i.innerText="Tableau de Bord",e.appendChild(i);const t=document.createElement("p");t.className="section-subtitle",t.innerText="Welcome back, Commis. Here is your kitchen curriculum status.",e.appendChild(t);const n=new Date().toISOString().split("T")[0],a=Object.values(C.srs),o=a.filter(k=>k.dueDate<=n).length,u=a.length,r=document.createElement("div");r.className="dashboard-grid";const b=document.createElement("div");b.style.display="flex",b.style.flexDirection="column",b.style.gap="1.5rem";const c=document.createElement("div");c.className="card",c.style.padding="1.5rem",c.innerHTML=`
  ...
  `,l.appendChild(p);const f=document.createElement("div");f.className="card",f.style.padding="1.5rem";const d=((s=(w=C.meta)==null?void 0:w.counts)==null?void 0:s.vocabulary)||0,g=((y=(h=C.meta)==null?void 0:h.counts)==null?void 0:y.grammar)||0,T=((x=(L=C.meta)==null?void 0:L.counts)==null?void 0:x.cuisine)||0;return f.innerHTML=`
  ...
  `,l.appendChild(f),r.appendChild(l),e.appendChild(r),e}function be(e){const i=lt(e);return H[i]||""}function lt(e){return e?e.trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]/g," ").replace(/\s+/g," ").trim():""}const I=(e,i,t)=>`
...
`,H={sur:I("sur",`
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
      <div>
        ...
        <select id="vocab-category-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${o.map(g=>`<option value="${g}" ${g===b?"selected":""}>${g}</option>`).join("")}
        </select>
      </div>
      <div>
        ...
        <select id="vocab-tag-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${u.map(g=>`<option value="${g}" ${g===c?"selected":""}>${g==="ALL"?"ALL TAGS":"#"+g}</option>`).join("")}
        </select>
      </div>
      <div>
        ...
        <select id="vocab-level-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${r.map(g=>`<option value="${g}" ${g===m?"selected":""}>${g==="ALL"?"ALL LEVELS":g}</option>`).join("")}
        </select>
      </div>
    </div>
  ...
  `,e.appendChild(n),i.sections.forEach(a=>{const o=document.createElement("div");if(o.className="ref-section-container",a.type==="table"){const u=document.createElement("h4");u.className="ref-sec-title",u.innerText=a.title,o.appendChild(u);const r=document.createElement("div");r.className="ref-table-wrapper";const b=document.createElement("table");b.className="ref-table";const c=document.createElement("tr");if(a.headers.forEach(m=>{const v=document.createElement("th");v.innerText=m,c.appendChild(v)}),b.appendChild(c),a.rows.forEach(m=>{const v=document.createElement("tr");m.forEach((l,p)=>{const f=document.createElement("td");f.innerText=l,p===1&&(a.title.includes("Conjugaison")||a.title.includes("Présent")||a.title.includes("Exemple"))&&(f.style.position="relative",f.innerHTML=`
            ...
            `),v.appendChild(f)}),b.appendChild(v)}),r.appendChild(b),o.appendChild(r),a.example){const m=document.createElement("div");m.className="ref-table-attached-ex",m.innerHTML=`
  ...
  `,e.appendChild(i);let t=document.getElementById("map-floating-tooltip");t||(t=document.createElement("div"),t.id="map-floating-tooltip",t.style.position="fixed",t.style.pointerEvents="none",t.style.zIndex="9999",t.style.padding="0.5rem 0.8rem",t.style.background="rgba(10, 25, 49, 0.95)",t.style.border="1px solid var(--color-accent)",t.style.borderRadius="var(--radius-sm)",t.style.color="#fff",t.style.fontSize="0.75rem",t.style.boxShadow="0 4px 12px rgba(0, 0, 0, 0.15)",t.style.display="none",t.style.fontFamily="var(--font-main)",t.style.lineHeight="1.3",document.body.appendChild(t));const n=i.querySelectorAll(".segment-btn");n.forEach(c=>{c.addEventListener("click",m=>{n.forEach(v=>v.classList.remove("active")),m.target.classList.add("active"),F.activeMap=m.target.getAttribute("data-map"),F.selectedRegionId=null,F.selectedItemId=null,u()})});const a=i.querySelectorAll(".theme-tab-btn");a.forEach(c=>{c.addEventListener("click",m=>{a.forEach(v=>v.classList.remove("active")),m.target.classList.add("active"),F.activeTheme=m.target.getAttribute("data-theme"),F.selectedItemId=null,u()})}),o().then(()=>{u()});async function o(){try{const c=["data/regions_administrative.json","data/regions_cultural.json","data/dishes.json","data/cheeses.json","data/wines.json"],m=await Promise.all(c.map(v=>fetch(v)));for(const v of m)if(!v.ok)throw new Error(`Failed to load ${v.url}`);ee=await m[0].json(),te=await m[1].json(),se=await m[2].json(),le=await m[3].json(),ce=await m[4].json(),console.log("Map engine data loaded successfully.")}catch(c){console.error("Failed to load map JSON datasets:",c),ee=[],te=[]}}function u(){if(!(ee.length>0&&te.length>0)){i.querySelector("#regions-list-group").innerHTML='<p style="font-size:0.8rem;color:var(--color-text-muted);padding:1rem;">読み込み中...</p>';return}const m=i.querySelector("#legacy-map-illustration"),v=i.querySelector("#classic-map-decorations"),l=i.querySelector("#interactive-france-svg");F.activeMap==="cultural"?(m.style.display="block",v.style.display="none",l.setAttribute("viewBox","0 0 600 600")):(m.style.display="none",v.style.display="block",l.setAttribute("viewBox","0 0 1000 1000"));const p=F.activeMap==="admin"?ee:te,f=i.querySelector("#map-paths-group"),d=F.activeMap==="cultural";f.innerHTML=p.map(s=>{const h=F.selectedRegionId===s.region_id,y=Lt[s.region_id]||"rgba(197, 168, 128, 0.1)",L=d?"none":y,x=d?"none":"rgba(92, 64, 51, 0.65)",k=d?0:1,_=h?"rgba(197, 168, 128, 0.45)":L,j=h?"var(--color-primary)":x,$=h?3.2:d?0:2.2;return`
        ...
        />
      `}).join("");const g=i.querySelector("#map-labels-group");d?g.innerHTML="":g.innerHTML=p.map(s=>{const h=s.french_name,y=h.length>20?11.5:h.length>12?12.5:14.5,L=y*.65,x=Math.max(90,h.length*L+20),k=y+8,_=s.label_y-k/2-1,j=s.label_y+y/3+.5;return`
        ...
        `}).join("")}b(),r()}function r(){i.querySelectorAll(".region-path").forEach(l=>{l.addEventListener("mouseenter",p=>{const f=p.target.getAttribute("data-id"),g=(F.activeMap==="admin"?ee:te).find(T=>T.region_id===f);g&&(t.innerHTML=`
            <div style="font-weight:700; border-bottom:1px solid rgba(255,255,255,0.3); padding-bottom:2px; margin-bottom:2px;">
              ${g.french_name}
            </div>
            <div style="font-size:0.7rem; opacity:0.85;">${g.japanese_name}</div>
            <div style="font-size:0.65rem; opacity:0.7; font-style:italic;">${g.english_name}</div>
          `,t.style.display="block")}),l.addEventListener("mousemove",p=>{t.style.left=p.clientX+15+"px",t.style.top=p.clientY+15+"px"}),l.addEventListener("mouseleave",()=>{t.style.display="none"}),l.addEventListener("click",p=>{const f=p.target.getAttribute("data-id");F.selectedRegionId=f,F.selectedItemId=null,u()})}),i.querySelectorAll(".list-item-btn").forEach(l=>{l.addEventListener("click",p=>{const f=p.target.closest(".list-item-btn");f.classList.contains("item-btn")?(F.selectedItemId=f.getAttribute("data-item-id"),F.selectedRegionId=f.getAttribute("data-region-id")):(F.selectedRegionId=f.getAttribute("data-id"),F.selectedItemId=null),u()})}),i.querySelectorAll(".map-pin-marker").forEach(l=>{l.addEventListener("click",p=>{const f=p.target.closest(".map-pin-marker");F.selectedItemId=f.getAttribute("data-item-id"),F.selectedRegionId=f.getAttribute("data-region-id"),u()})})}function b(){const c=i.querySelector("#map-detail-drawer"),m=i.querySelector("#map-region-title"),v=i.querySelector("#map-region-sub"),l=i.querySelector("#drawer-main-content"),p=i.querySelector("#map-audio-title-btn");if(p.style.display="none",F.selectedItemId){let f=null,d="";if(F.activeTheme==="cheeses"?(f=le.find(g=>g.id===F.selectedItemId),d="特産チーズ (Fromage AOP)"):F.activeTheme==="wines"?(f=ce.find(g=>g.id===F.selectedItemId),d="地方ワイン (AOC Wine)"):F.activeTheme==="dishes"&&(f=se.find(g=>g.id===F.selectedItemId),d="郷土料理 (Plat Régional)"),f){m.innerText=`${f.name_fr} (${f.name_ja})`,v.innerText=`${d} • ${f.region}`;let g="";if(f.properties){g+='<div style="display:flex; flex-wrap:wrap; gap:0.6rem; margin-bottom:0.5rem;">';for(const[S,w]of Object.entries(f.properties)){let s=S;S==="sweetness"?s="甘み":S==="alcohol"?s="アルコール":S==="body"?s="ボディ":S==="saltiness"?s="塩気":S==="aroma"?s="香り":S==="rarity"?s="希少性":S==="type"&&(s="タイプ"),g+=`
          ...
          `:""}
        `,p.style.display="inline-block",p.onclick=()=>N(f.name_fr);const T=i.querySelector("#map-audio-desc-btn");T&&(T.onclick=()=>N(f.science||f.desc||"")),c.style.display="block";return}}if(F.selectedRegionId){const d=(F.activeMap==="admin"?ee:te).find(g=>g.region_id===F.selectedRegionId);if(d){m.innerText=`${d.french_name} (${d.japanese_name})`,v.innerText=`${d.english_name} Region`;let g=jt(d.region_id);l.innerHTML=`
          <div>
            <div style="display:flex; align-items:center; gap:0.5rem; margin-bottom:0.3rem;">
              <h4 style="font-size:0.75rem; text-transform:uppercase; color:var(--color-accent); font-weight:600;">Français</h4>
              <button class="audio-btn" id="map-audio-desc-btn" style="background:none; border:none; font-size:0.95rem; cursor:pointer; color:var(--color-accent);">🔊</button>
            </div>
            <p style="font-size:0.95rem; color:var(--color-primary); font-style:italic; line-height:1.5; text-align:justify;" id="map-desc-fr">${g.desc_fr}</p>
          </div>
          <div>
            ...
            <p style="font-size:0.9rem; color:var(--color-text-main); line-height:1.5; text-align:justify;">${g.desc_en}</p>
          </div>
          <div>
            <h4 style="font-size:0.75rem; text-transform:uppercase; color:var(--color-accent); font-weight:600; margin-bottom:0.3rem;">日本語解説</h4>
            <p style="font-size:0.9rem; color:var(--color-text-main); line-height:1.5; text-align:justify;">${g.desc_ja}</p>
          </div>
  ...
  `,e.appendChild(a);const o=a.querySelector("#quiz-cat-select");o.addEventListener("change",p=>{t=p.target.value,r()});const u=document.createElement("div");u.className="quiz-game-wrapper",e.appendChild(u),n.querySelectorAll(".mode-tab-btn").forEach(p=>{p.addEventListener("click",f=>{n.querySelectorAll(".mode-tab-btn").forEach(d=>d.classList.remove("active")),f.target.classList.add("active"),i=f.target.getAttribute("data-mode"),i==="matching_taking"?(o.disabled=!0,o.style.opacity="0.5"):(o.disabled=!1,o.style.opacity="1.0"),r()})});function r(){u.innerHTML="",i==="multiple"?b():i==="matching_vocab"?c():i==="matching_taking"?m():i==="spelling"&&l()}function b(){var h;const p=((h=C.db)==null?void 0:h.quizzes)||[],f=$t();let d=[...p,...f];if(t!=="ALL"&&(d=d.filter(y=>y.category===t)),d.length===0){u.innerHTML=`
      ...
      `;return}const g=W(d).slice(0,10);let T=0,S=0,w=!1;function s(){if(u.innerHTML="",w=!1,T>=g.length){const $=Math.round(S/g.length*100);let E="Apprenti (Apprentice)";$>=90?E="Chef de Partie (Station Chef)":$>=70&&(E="Commis de Cuisine (Line Cook)"),u.innerHTML=`
          <div class="quiz-card" style="text-align: center;">
            ...
            <p style="font-size: 1.1rem; margin-bottom: 1.5rem;">Your Score: <strong>${S} / ${g.length}</strong> (${$}%)</p>
        ...
        `,u.querySelector("#restart-choice-btn").addEventListener("click",()=>{b()});return}const y=g[T],L=document.createElement("div");L.className="quiz-card",L.innerHTML=`
        <div class="quiz-meta">
          <span>Question ${T+1} of ${g.length}</span>
      ...
      `;return}const g=W(d).slice(0,4),T=W(g),S=W(g),w=document.createElement("div");w.className="quiz-card",w.innerHTML=`
    ...
    `,u.appendChild(w),v(w,4,c)}function m(){const p=Et(),f=document.createElement("div");f.className="quiz-card",f.innerHTML=`
      <div class="quiz-meta" style="margin-bottom: 1rem;">
    ...
    `,u.appendChild(f),v(f,4,m)}function v(p,f,d){let g=null,T=null,S=0;const w=p.querySelectorAll(".drag-card"),s=p.querySelectorAll(".drop-zone"),h=p.querySelector("#matching-completion-panel");w.forEach(x=>{x.addEventListener("dragstart",k=>{g=k.target.closest(".drag-card").getAttribute("data-id"),k.target.closest(".drag-card").classList.add("dragging")}),x.addEventListener("dragend",k=>{k.target.closest(".drag-card").classList.remove("dragging")}),x.addEventListener("click",k=>{const _=k.target.closest(".drag-card");_.classList.contains("matched")||(w.forEach(j=>j.style.borderColor="rgba(10,25,49,0.08)"),T=_.getAttribute("data-id"),_.style.borderColor="var(--color-accent)")})}),s.forEach(x=>{x.addEventListener("dragover",k=>{k.preventDefault(),x.classList.contains("matched")||x.classList.add("hovered")}),x.addEventListener("dragleave",()=>{x.classList.remove("hovered")}),x.addEventListener("drop",k=>{k.preventDefault(),x.classList.remove("hovered");const _=x.getAttribute("data-id");g===_?L(g,x):y(g)}),x.addEventListener("click",()=>{if(x.classList.contains("matched")||!T)return;const k=x.getAttribute("data-id");T===k?(L(T,x),T=null):(y(T),T=null,w.forEach(_=>_.style.borderColor="rgba(10,25,49,0.08)"))})});function y(x){const k=p.querySelector(`#drag-${x}`);k&&(k.style.animation="shake-anim 0.4s ease-in-out",setTimeout(()=>k.style.animation="",400))}function L(x,k){const _=p.querySelector(`#drag-${x}`);_.classList.add("matched"),_.style.borderColor="var(--color-success)",k.classList.add("matched"),S++,S===f&&(h.style.display="block")}p.querySelector("#restart-match-btn").addEventListener("click",()=>{d()})}function l(){var x,k;const p=((x=C.settings)==null?void 0:x.includeGeneral)||!1;let d=(((k=C.db)==null?void 0:k.vocabulary)||[]).filter(_=>p||_.is_professional);if(t!=="ALL"&&(t==="meat"?d=d.filter(_=>{var j,$,E,A;return((j=_.tags)==null?void 0:j.includes("meat"))||(($=_.tags)==null?void 0:$.includes("beef"))||((E=_.tags)==null?void 0:E.includes("pork"))||((A=_.tags)==null?void 0:A.includes("poultry"))||/viande|boeuf|porc|poulet/i.test(_.french)}):t==="sauces"?d=d.filter(_=>{var j,$,E;return((j=_.tags)==null?void 0:j.includes("sauce"))||(($=_.tags)==null?void 0:$.includes("sauces"))||((E=_.tags)==null?void 0:E.includes("stocks"))||/sauce|fond|jus|bouillon/i.test(_.french)}):t==="cuts"?d=d.filter(_=>{var j,$;return((j=_.tags)==null?void 0:j.includes("cutting"))||(($=_.tags)==null?void 0:$.includes("vegetables"))||/coupe|tailler|ciseler|mincer|brunoise|julienne/i.test(_.french)}):t==="science"?d=d.filter(_=>{var j;return((j=_.tags)==null?void 0:j.includes("science"))||/réaction|émulsion|liaison/i.test(_.french)}):t==="grammar"&&(d=[])),d.length===0){u.innerHTML=`
      ...
      `;return}let g=W(d)[0];const T=document.createElement("div");T.className="quiz-card",T.innerHTML=`
      <div class="quiz-meta" style="margin-bottom: 1.2rem;">
        <span>📖 Orthographe de Cuisine</span>
        <span class="grammar-badge" style="background-color: var(--color-secondary);">${g.category}</span>
      </div>
      ...
      <div class="spelling-box" style="margin-bottom: 1.5rem; background-color: rgba(10,25,49,0.02); padding: 1.2rem; border-radius: var(--radius-sm); border-left: 3px solid var(--color-accent);">
        ...
        <p style="font-size: 1.05rem; font-style: italic; color: var(--color-primary); line-height: 1.4; font-family: var(--font-serif);">${g.definition_fr}</p>
        ...
        <div style="margin-top: 1rem; border-top: 1px solid rgba(0,0,0,0.05); padding-top: 0.8rem; font-size: 0.85rem; color: var(--color-text-muted);">
          <strong>Hint (Japanese):</strong> ${g.japanese}
        </div>
      </div>
    ...
    `,e;const a=document.createElement("div");return a.className="loading-placeholder",a.innerText="Chargement des favoris... (Loading favorites...)",e.appendChild(a),Le().then(()=>{a.remove(),Pt(e,n)}),e}function Pt(e,i){var r,b,c;const t=(((r=C.db)==null?void 0:r.vocabulary)||[]).filter(m=>i.includes(m.id)),n=(((b=C.db)==null?void 0:b.grammar)||[]).filter(m=>i.includes(m.id)),a=(((c=C.db)==null?void 0:c.cuisine)||[]).filter(m=>i.includes(m.id)),o=document.createElement("div");o.className="card-grid",e.appendChild(o);function u(){o.innerHTML="",t.forEach(m=>{const v=document.createElement("div");v.className="card",v.innerHTML=`
      ...
      `,v.querySelector(".fav-btn").addEventListener("click",f=>{X(m.id),e.innerHTML="",e.appendChild(ie())}),o.appendChild(v)}),a.forEach(m=>{const v=document.createElement("div");v.className="card",v.innerHTML=`
        <div>
      ...
      `;return}const l=n[o];C.srs[l.id];const p=C.wrongAnswers.includes(l.id),f=document.createElement("div");f.className="card srs-flip-card",f.style.padding="2rem",f.style.minHeight="300px",f.style.display="flex",f.style.flexDirection="column",f.style.justifyContent="space-between";let d=l.category||l.level||"Theory";l.type==="quiz"&&(d="Quiz Mistake");let g=`
      <div>
    ...
    `;f.innerHTML=g;let T="";l.type==="vocabulary"?T=`
  ...
  `,Le().then(()=>{a.disabled=!1,a.placeholder="Rechercher... (e.g. sauce, roux, cut, culer, 刻む)";function u(r){var f;if(o.innerHTML="",!r.trim()){o.innerHTML=`
        ...
        `;return}p.forEach(d=>{const g=document.createElement("div");g.className="card search-result-card",g.style.marginBottom="1.2rem",g.style.borderLeft=`4px solid ${d.type==="vocabulary"?"var(--color-primary)":d.type==="grammar"?"var(--color-secondary)":"var(--color-accent)"}`;const T=Z(d.id);let S="";d.type==="vocabulary"?S=`
...
`).join("<br>"),j.appendChild(document.createRange().createContextualFragment($)),j.appendChild(E),a)j.appendChild(A);else{const q=document.createElement("a");q.href="#",q.className="toggle-translation-link",q.style.fontSize="0.78rem",q.style.color="var(--color-accent)",q.style.textDecoration="underline",q.style.display="block",q.style.marginTop="0.5rem",q.innerText="Afficher la traduction (日本語訳を表示)",A.style.display="none",q.addEventListener("click",B=>{B.preventDefault(),A.style.display==="none"?(A.style.display="block",q.innerText="Masquer la traduction (翻訳を非表示)"):(A.style.display="none",q.innerText="Afficher la traduction (日本語訳を表示)")}),j.appendChild(q),j.appendChild(A)}y.appendChild(j)}),y.querySelectorAll(".section-audio-btn").forEach(k=>{k.addEventListener("click",_=>{_.stopPropagation(),N(k.getAttribute("data-text"))})}),(()=>{c.querySelector("#helper-toggle-outlines").addEventListener("change",k=>{C.settings.readingHelper.outlines=k.target.checked,k.target.checked&&(C.settings.readingHelper.pureText=!1,c.querySelector("#helper-toggle-pure").checked=!1),U(y)}),c.querySelector("#helper-toggle-translations").addEventListener("change",k=>{C.settings.readingHelper.translations=k.target.checked,k.target.checked&&(C.settings.readingHelper.pureText=!1,c.querySelector("#helper-toggle-pure").checked=!1),U(y)}),c.querySelector("#helper-toggle-target").addEventListener("change",k=>{C.settings.readingHelper.targetOnly=k.target.checked,U(y)}),c.querySelector("#helper-toggle-pure").addEventListener("change",k=>{C.settings.readingHelper.pureText=k.target.checked,k.target.checked&&(C.settings.readingHelper.outlines=!1,C.settings.readingHelper.translations=!1,c.querySelector("#helper-toggle-outlines").checked=!1,c.querySelector("#helper-toggle-translations").checked=!1),U(y)})})(),U(y),e.appendChild(y)}function Zt(e,i){var l;const t=((l=C.settings)==null?void 0:l.targetLevel)||"ALL";let n="débutant";["B1","B2","intermédiaire"].includes(t)?n="intermédiaire":["C1","C2","avancé"].includes(t)&&(n="avancé");const a=n==="débutant",o=n==="intermédiaire",u=n==="avancé",r=document.createElement("button");r.className="next-btn",r.style.backgroundColor="transparent",r.style.border="1px solid var(--color-secondary)",r.style.color="var(--color-secondary)",r.style.padding="0.4rem 1rem",r.style.marginBottom="1.5rem",r.innerText="← Quitter l'entraînement (戻る)",r.addEventListener("click",()=>{J="preview",K(e,i)}),e.appendChild(r);const b=document.createElement("div");b.className="magazine-header",b.style.textAlign="center",b.style.marginBottom="2.5rem",b.innerHTML=`
  ...
  `,e.appendChild(c);const m=document.createElement("div");m.className="magazine-body-container",m.style.maxWidth="720px",m.style.margin="0 auto",m.style.display="flex",m.style.flexDirection="column",m.style.gap="2rem",i.sessions.forEach((p,f)=>{const d=document.createElement("div");d.id=`session-card-${f}`;let g="ref-locked-session";f===O?g="ref-active-session":f<O&&(g="ref-completed-session"),d.className=`ref-session-card ${g}`;const T=document.createElement("div");T.style.display="flex",T.style.justifyContent="space-between",T.style.alignItems="center",T.style.marginBottom="0.8rem",T.innerHTML=`
      <div style="display: flex; align-items: center; gap: 0.6rem;">
        ...
        ${f<=O?`<button class="section-audio-btn" data-text="${p.text_fr.replace(/"/g,"&quot;")}" style="background: none; border: none; font-size: 1rem; cursor: pointer; color: var(--color-accent); padding: 0.1rem; line-height: 1;" title="Lire la section">🔊</button>`:""}
      </div>
      ${f<O?'<span style="color: var(--color-success); font-size: 0.8rem; font-weight: 700;">✓ Complétée</span>':""}
    `,d.appendChild(T);const S=document.createElement("div");if(S.className="ref-text-block",f<O)S.innerHTML=`
...
`).join("<br>")}
      ...
      `;else if(f===O){const w=document.createElement("div");w.className="ref-para-fr",w.style.fontSize="1.15rem",w.style.lineHeight="1.8",w.style.fontFamily="var(--font-serif)",w.style.marginBottom="1.5rem",f===0&&w.classList.add("editorial-dropcap"),p.tokens.forEach((y,L)=>{const x=document.createElement("span");x.className=`anatomy-token token-pos-${y.pos} has-definition`,x.innerText=y.word+" ";const k=p.tokens.slice(L).map(_=>_.word).join(" ");(y.vocab_id||y.gram_id)&&(x.setAttribute("data-has-db","true"),a?y.pos==="noun"?x.style.borderBottom="2px solid var(--color-primary)":y.pos==="verb"?x.style.borderBottom="2px solid var(--color-success)":y.pos==="article_partitive"?x.style.borderBottom="2px dashed var(--color-secondary)":y.pos==="preposition"&&(x.style.borderBottom="2px dotted var(--color-accent)"):o?x.style.borderBottom="1px solid rgba(0,0,0,0.12)":u&&(x.style.borderBottom="none")),x.addEventListener("click",_=>{_.stopPropagation(),_e(x,y,k)}),w.appendChild(x)}),S.appendChild(w);const s=document.createElement("div");s.className="ref-quiz-container",s.style.marginTop="1.5rem",s.style.padding="1.2rem",s.style.backgroundColor="#fdfcf7",s.style.border="1px solid var(--color-accent)",s.style.borderRadius="8px";const h=p.quizzes.find(y=>y.target_level===n)||p.quizzes[0];if(h){let y="";if(h.type==="choice"||h.type==="structure_analysis"){const L=u&&h.options_fr,x=L?h.options_fr:h.options,k=L?h.answer_fr:h.answer;y=`
          ...
          `;const _=s.querySelector(".quiz-submit-btn"),j=s.querySelector(".quiz-feedback-msg");_.addEventListener("click",()=>{const $=s.querySelector(`input[name="quiz-${h.id}"]:checked`);$&&$.value===k?(j.style.color="var(--color-success)",j.innerText="✓ Correct ! Excellent !",_.style.display="none",setTimeout(()=>{ne(e,i,f)},1e3)):(j.style.color="var(--color-error)",j.innerText="❌ Incorrect. Réessayez !")})}else if(h.type==="preposition"){y=`
          ...
          `;const L=s.querySelector(".quiz-submit-btn"),x=s.querySelector(".quiz-feedback-msg");L.addEventListener("click",()=>{s.querySelector(`#preposition-select-${h.id}`).value===h.answer?(x.style.color="var(--color-success)",x.innerText="✓ Correct ! Très bien !",L.style.display="none",setTimeout(()=>{ne(e,i,f)},1e3)):(x.style.color="var(--color-error)",x.innerText="❌ Incorrect. Réessayez !")})}else if(h.type==="word_order"){s.innerHTML=`
          ...
          `;const L=s.querySelector(".quiz-submit-btn"),x=s.querySelector(".quiz-feedback-msg");L.addEventListener("click",()=>{s.querySelector(`#quiz-input-${h.id}`).value.trim().toLowerCase()===h.blank_word.toLowerCase()?(x.style.color="var(--color-success)",x.innerText="✓ Oui, Chef ! Correct !",L.style.display="none",setTimeout(()=>{ne(e,i,f)},1e3)):(x.style.color="var(--color-error)",x.innerText="❌ Ce n'est pas tout à fait ça. Réessayez !")})}else if(h.type==="input"||h.type==="conjugation"){y=`
            <div style="margin-top: 0.8rem;">
          ...
          `;const L=s.querySelector(".quiz-submit-btn"),x=s.querySelector(".quiz-feedback-msg");let k=0;L.addEventListener("click",()=>{if(s.querySelector(`#quiz-input-${h.id}`).value.trim().toLowerCase()===h.blank_word.toLowerCase())x.style.color="var(--color-success)",x.innerText="✓ Correct ! Excellent !",L.style.display="none",setTimeout(()=>{ne(e,i,f)},1e3);else if(k++,x.style.color="var(--color-error)",a&&k>=2){const j=h.blank_word[0].toUpperCase();x.innerText=`❌ Incorrect. Indice : commence par "${j}"`}else x.innerText="❌ Incorrect. Réessayez !"})}}else s.innerHTML=`
        ...
        `,s.querySelector(".validate-session-btn").addEventListener("click",()=>{ne(e,i,f)});S.appendChild(s)}else S.innerHTML=`
...
`).join("<br>")}
    ...
    `),o){const f=!!o.french,d=`<span style="font-size: 0.85rem; font-weight: bold; color: var(--color-secondary);">${o.level}</span>`;if(f){const g=o.gender?o.gender==="m"?"Masculin":o.gender==="f"?"Féminin":"M/F":"",T=g?`<span style="font-size: 0.7rem; font-weight: 600; padding: 0.05rem 0.3rem; border-radius: 3px; background-color: #f0f4f8; color: var(--color-primary); border: 1px solid rgba(0,0,91,0.1);">${g}</span>`:"",S=o.verb_group?o.verb_group===1?"1er groupe":o.verb_group===2?"2e groupe":"3e groupe":"",w=S?`<span style="font-size: 0.7rem; font-weight: 600; padding: 0.05rem 0.3rem; border-radius: 3px; background-color: rgba(107,156,104,0.08); color: var(--color-success); border: 1px solid rgba(107,156,104,0.15);">${S}</span>`:"";n.innerHTML=`
