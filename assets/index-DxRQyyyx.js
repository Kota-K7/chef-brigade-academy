(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&a(l)}).observe(document,{childList:!0,subtree:!0});function r(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=r(n);fetch(n.href,i)}})();let re=null,ie="",oe=0,Z=null,O=null;function G(e,o=1){pe(e,o)}function pe(e,o=1,r=null,a=null,n=0){if(!("speechSynthesis"in window)){console.warn("Speech synthesis not supported on this browser.");return}window.speechSynthesis.cancel();const i=e.replace(/["'➔]/g,"").trim();ie=i,oe=n,Z=r,O=a;const l=i.substring(n);if(!l.trim()){O&&O();return}const t=new SpeechSynthesisUtterance(l);t.lang="fr-FR",t.rate=o;const u=window.speechSynthesis.getVoices().find(d=>d.lang==="fr-FR"||d.lang.startsWith("fr"));u&&(t.voice=u),t.onboundary=d=>{if(d.name==="word"&&Z){const m=oe+d.charIndex;Z(m,ie.length)}},t.onend=()=>{re===t&&O&&O()},t.onerror=d=>{console.warn("SpeechSynthesis error:",d),O&&O()},re=t,window.speechSynthesis.speak(t)}function he(){"speechSynthesis"in window&&window.speechSynthesis.pause()}function be(){"speechSynthesis"in window&&window.speechSynthesis.resume()}function ee(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}"speechSynthesis"in window&&(window.speechSynthesis.getVoices(),window.speechSynthesis.onvoiceschanged=()=>{window.speechSynthesis.getVoices()});function ye(){var S,h,f,k,x,L,C;const e=document.createElement("div"),o=document.createElement("h2");o.className="section-title",o.innerText="Tableau de Bord",e.appendChild(o);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Welcome back, Commis. Here is your kitchen curriculum status.",e.appendChild(r);const a=new Date().toISOString().split("T")[0],n=Object.values(b.srs),i=n.filter(q=>q.dueDate<=a).length,l=n.length,t=document.createElement("div");t.className="dashboard-grid";const p=document.createElement("div");p.style.display="flex",p.style.flexDirection="column",p.style.gap="1.5rem";const u=document.createElement("div");u.className="card",u.style.padding="1.5rem",u.innerHTML=`
  ...
  `,p.appendChild(u);const d=document.createElement("div");d.className="card",d.style.padding="1.5rem";const m=((S=b.meta)==null?void 0:S.featured)||null;d.innerHTML=`
    ...
    `:"<p>No data loaded.</p>"}
  `,m&&d.querySelectorAll(".audio-btn").forEach(q=>{q.addEventListener("click",y=>{y.stopPropagation();const T=y.target.closest(".audio-btn").getAttribute("data-text");G(T)})}),p.appendChild(d),t.appendChild(p);const s=document.createElement("div");s.style.display="flex",s.style.flexDirection="column",s.style.gap="1.5rem";const g=document.createElement("div");g.className="streak-card",g.innerHTML=`
  ...
  `,s.appendChild(g);const v=document.createElement("div");v.className="card",v.style.padding="1.5rem";const _=((f=(h=b.meta)==null?void 0:h.counts)==null?void 0:f.vocabulary)||0,c=((x=(k=b.meta)==null?void 0:k.counts)==null?void 0:x.grammar)||0,w=((C=(L=b.meta)==null?void 0:L.counts)==null?void 0:C.cuisine)||0;return v.innerHTML=`
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
      <div>
        ...
        <select id="vocab-category-select" style="padding: 0.5rem; font-size: 0.9rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1); width: 100%; background: var(--color-bg);">
          ${i.map(c=>`<option value="${c}" ${c===p?"selected":""}>${c}</option>`).join("")}
        </select>
      </div>
      <div>
  ...
  `,e.appendChild(m);const s=document.createElement("div");s.className="card-grid",e.appendChild(s);function g(){var h;s.innerHTML="";const S=(((h=b.db)==null?void 0:h.knowledge)||[]).filter(f=>f.french&&f.japanese).filter(f=>r||f.is_professional).filter(f=>{const k=p==="ALL"||f.category===p,x=u==="ALL"||f.tags&&f.tags.includes(u),L=d==="ALL"||f.level===d;return k&&x&&L});if(S.length===0){s.innerHTML='<p style="color: var(--color-text-muted); grid-column: 1 / -1; text-align: center; padding: 2rem;">Aucun terme trouvé correspondant à vos critères de filtrage.</p>';return}S.forEach(f=>{const k=document.createElement("div");k.className="card";const x=V(f.id),L=b.srs[f.id],C=!!L,q=f.examples&&f.examples[0]?f.examples[0].fr:"",y=f.examples&&f.examples[0]?f.examples[0].ja:"";k.innerHTML=`
        <div style="display: flex; flex-direction: column; height: 100%; justify-content: space-between; gap: 1.5rem;">
          <div>
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
              <span class="card-category" style="margin:0;">${f.category}</span>
              <span class="grammar-badge" style="background-color: var(--color-primary);">${f.level}</span>
            </div>
            
            <div class="term-header">
              <div style="display: flex; align-items: center; gap: 0.5rem;">
                <h3 class="term-title">${f.french}</h3>
                <button class="audio-btn" data-text="${f.french}" title="Listen pronunciation" style="background: none; border: none; font-size: 1.15rem; cursor: pointer; color: var(--color-accent); transition: var(--transition); padding: 0.2rem;">🔊</button>
              </div>
              <button class="fav-btn ${x?"active":""}" data-id="${f.id}">
                ${x?"★":"☆"}
              </button>
            </div>
            ...
            <div class="term-translations" style="margin-top: 0.6rem; display: flex; flex-direction: column; gap: 0.4rem;">
              <div class="flip-translation-container">
                <div class="flip-translation-card">
                  <div class="flip-front" style="display: flex; align-items: center; gap: 0.4rem; justify-content: flex-start; padding: 0.4rem 0.6rem; text-align: left;">
                    <button class="audio-btn" data-text="${f.definition_fr}" title="Listen definition" style="background: none; border: none; font-size: 0.9rem; cursor: pointer; color: var(--color-accent); padding: 0.1rem; line-height: 1; flex-shrink: 0;">🔊</button>
                    <span style="font-size: 0.76rem; line-height: 1.25; color: var(--color-text-main); font-weight: 500;">${f.definition_fr||"No definition loaded."}</span>
                  </div>
              ...
              <div class="trans-ja" style="margin-top: 0.2rem; font-weight: 500;">${f.japanese}</div>
            </div>
            ...
            ${f.tags&&f.tags.length>0?`
              <div style="margin-top: 1rem; display: flex; flex-wrap: wrap; gap: 0.3rem;">
                ${f.tags.map($=>`<span class="tag-badge" style="background-color: rgba(197, 168, 128, 0.08); color: var(--color-accent); font-size: 0.75rem; padding: 0.1rem 0.5rem; border-radius: 12px;">#${$}</span>`).join("")}
              </div>
            `:""}
          </div>
          ...
          <div style="border-top: 1px solid rgba(0,0,0,0.05); padding-top: 1rem; font-size: 0.8rem; display: flex; flex-direction: column; gap: 0.5rem;">
            ${C?`
              ...
              <div style="display: flex; gap: 0.5rem;">
                <button class="next-btn srs-action-btn" data-id="${f.id}" data-action="memorized" style="background-color: var(--color-success); font-size: 0.75rem; padding: 0.3rem 0.6rem; flex: 1;">
                  ✓ Memorized
                </button>
                <button class="next-btn srs-action-btn" data-id="${f.id}" data-action="reset" style="background-color: transparent; border: 1px solid var(--color-error); color: var(--color-error); font-size: 0.75rem; padding: 0.3rem 0.6rem;">
                  Forget
                </button>
              </div>
            `:`
              <div style="color: var(--color-text-muted); font-style: italic; margin-bottom: 0.2rem;">Not yet added to SRS memory deck.</div>
              <button class="next-btn srs-action-btn" data-id="${f.id}" data-action="start" style="font-size: 0.75rem; padding: 0.4rem 0.8rem; width: 100%;">
                Start Memorizing (Add to SRS)
              </button>
            `}
          </div>
        </div>
      `;const T=k.querySelector(".flip-translation-container");T.addEventListener("click",$=>{$.stopPropagation(),T.querySelector(".flip-translation-card").classList.toggle("flipped")}),k.querySelector(".fav-btn").addEventListener("click",$=>{$.stopPropagation(),W(f.id);const j=$.target,A=V(f.id);j.classList.toggle("active",A),j.innerText=A?"★":"☆"}),k.querySelectorAll(".audio-btn").forEach($=>{$.addEventListener("click",j=>{j.stopPropagation();const A=j.target.closest(".audio-btn").getAttribute("data-text");G(A)})}),k.querySelectorAll(".srs-action-btn").forEach($=>{$.addEventListener("click",j=>{j.stopPropagation();const A=j.target.getAttribute("data-action"),F=j.target.getAttribute("data-id");A==="start"?U(F,4):A==="memorized"?U(F,5):A==="reset"&&U(F,0),g()})}),s.appendChild(k)})}m.querySelector("#vocab-category-select").addEventListener("change",c=>{p=c.target.value,g()}),m.querySelector("#vocab-tag-select").addEventListener("change",c=>{u=c.target.value,g()}),m.querySelector("#vocab-level-select").addEventListener("change",async c=>{var q;d=c.target.value,s.innerHTML=`<div style="grid-column: 1 / -1; text-align: center; padding: 2rem; color: var(--color-text-muted);">Chargement (Loading level ${d})...</div>`,await R("knowledge",d);const S=(((q=b.db)==null?void 0:q.knowledge)||[]).filter(y=>y.french&&y.japanese).filter(y=>r||y.is_professional),h=["ALL",...new Set(S.map(y=>y.category))],f=["ALL",...new Set(S.flatMap(y=>y.tags||[]))],k=m.querySelector("#vocab-category-select"),x=k.value;k.innerHTML=h.map(y=>`<option value="${y}" ${y===x?"selected":""}>${y}</option>`).join(""),p=k.value;const L=m.querySelector("#vocab-tag-select"),C=L.value;L.innerHTML=f.map(y=>`<option value="${y}" ${y===C?"selected":""}>${y==="ALL"?"ALL TAGS":"#"+y}</option>`).join(""),u=L.value,g()}),g()}function xe(){var i;const e=document.createElement("div"),o=document.createElement("h2");o.className="section-title",o.innerText="Grammaire de la Cuisine",e.appendChild(o);const r=document.createElement("p");r.className="section-subtitle",r.innerText="French grammar concepts framed through recipe instructions and professional dialogues.",e.appendChild(r);const a=document.createElement("div");a.className="loading-placeholder",a.innerText="Chargement de la grammaire... (Loading grammar...)",e.appendChild(a);const n=((i=b.settings)==null?void 0:i.targetLevel)||"ALL";return R("knowledge",n).then(()=>{a.remove(),ke(e,n)}),e}function ke(e,o){var n;const a=(((n=b.db)==null?void 0:n.knowledge)||[]).filter(i=>i.grammar).map(i=>({id:i.id,level:i.level,topic:i.grammar.topic,explanation_en:i.grammar.explanation_en,explanation_ja:i.grammar.explanation_ja,examples:i.examples||[]})).filter(i=>o==="ALL"||i.level===o);if(a.length===0){const i=document.createElement("p");i.style.color="var(--color-text-muted)",i.innerText="Aucune leçon de grammaire chargée pour ce niveau.",e.appendChild(i);return}a.forEach(i=>{const l=document.createElement("div");l.className="grammar-card";const t=V(i.id),p=i.topic.match(/\(([^)]+)\)/),u=p?p[1]:i.topic;l.innerHTML=`
    ...
    `;const d=l.querySelector(".grammar-header"),m=l.querySelector(".grammar-body"),s=l.querySelector(".toggle-icon");d.addEventListener("click",g=>{if(g.target.classList.contains("fav-btn")||g.target.closest(".audio-btn"))return;const v=m.classList.toggle("open");s.innerText=v?"▲":"▼"}),l.querySelector(".fav-btn").addEventListener("click",g=>{g.stopPropagation(),W(i.id);const v=g.target,_=V(i.id);v.classList.toggle("active",_),v.innerText=_?"★":"☆"}),l.querySelectorAll(".audio-btn").forEach(g=>{g.addEventListener("click",v=>{v.stopPropagation();const _=v.target.closest(".audio-btn").getAttribute("data-text");G(_)})}),e.appendChild(l)})}function Ce(e){var r;e.innerHTML='<div style="text-align: center; padding: 2rem; color: var(--color-text-muted);">Chargement de la théorie... (Loading theory...)</div>';const o=((r=b.settings)==null?void 0:r.targetLevel)||"ALL";R("knowledge",o).then(()=>{var l;e.innerHTML="";const n=(((l=b.db)==null?void 0:l.knowledge)||[]).filter(t=>t.cuisine).map(t=>({id:t.id,level:t.level,category:t.category||"Theory",topic:t.cuisine.topic,content_fr:t.cuisine.content_fr,content_en:t.cuisine.content_en,content_ja:t.cuisine.content_ja})).filter(t=>o==="ALL"||t.level===o);if(n.length===0){e.innerHTML='<p style="color: var(--color-text-muted);">Aucun document de théorie culinaire disponible pour ce niveau.</p>';return}const i=document.createElement("div");i.style.display="flex",i.style.flexDirection="column",i.style.gap="2rem",n.forEach(t=>{const p=document.createElement("div");p.className="card",p.style.display="block",p.style.padding="2rem";const u=V(t.id),d=t.topic.match(/^([^(]+)/),m=d?d[1].trim():t.topic;p.innerHTML=`
  ...
  `,o.appendChild(r);const a=document.createElement("div");a.className="meat-type-tabs",a.style.marginTop="0.5rem",a.style.borderBottom="none",o.appendChild(a);const n=document.createElement("div");n.className="meat-display-area",o.appendChild(n),e.appendChild(o);const i={primary:{title:"主要食肉 (Primary Meats)",subCategories:[{key:"beef",label:"🐂 Bœuf (牛)",cuts:Ee,img:"assets/beef_cuts.png",placeholder:"Coupe+de+Boeuf",pdf:"beef_maff_guide.pdf",pdfText:"📄 日本農水省 牛肉部位基準 (PDF)"},{key:"porc",label:"🐖 Porc (豚)",cuts:ze,img:"assets/porc_cuts.png",placeholder:"Coupe+de+Porc",pdf:"pork_maff_guide.pdf",pdfText:"📄 日本農水省 豚肉部位基準 (PDF)",quote:'"Tout est bon dans le cochon" (豚はすべてが使える食材である)'},{key:"volaille",label:"🐓 Volaille (鶏)",cuts:Te,img:"assets/poultry_cuts.png",placeholder:"Coupe+de+Volaille",pdf:"poultry_maff_guide.pdf",pdfText:"📄 日本農水省 鶏肉部位基準 (PDF)",quote:'"La volaille est la reine des cuisines et la directrice des banquets" (鶏肉は厨房の女王であり、宴の演出家である)'}]},livestock:{title:"その他の家畜肉 (Other Livestock)",subCategories:[{key:"agneau",label:"🐏 Agneau (羊)",cuts:Ae,img:"assets/agneau_cuts.png",placeholder:"Agneau"},{key:"veau",label:"🐂 Veau (仔牛)",cuts:Me,img:"assets/veal_cuts.png",placeholder:"Veau"},{key:"canard",label:"🦆 Canard (鴨)",cuts:Pe,img:"assets/canard_cuts.png",placeholder:"Canard"}]},game:{title:"ジビエ (Game Meats)",subCategories:[{key:"chevreuil",label:"🦌 Chevreuil (鹿)",cuts:Be,img:"assets/game_cuts.png",placeholder:"Chevreuil"},{key:"sanglier",label:"🐗 Sanglier (猪)",cuts:Fe,img:"assets/game_cuts.png",placeholder:"Sanglier"},{key:"pigeon",label:"🕊️ Pigeon (鳩)",cuts:He,img:"assets/game_cuts.png",placeholder:"Pigeon"},{key:"lievre",label:"🐇 Lièvre (野ウサギ)",cuts:Ne,img:"assets/game_cuts.png",placeholder:"Lievre"}]},seafood:{title:"魚介類 (Seafood)",subCategories:[{key:"poisson",label:"🐟 Poisson (魚)",cuts:Re,img:"assets/seafood_cuts.png",placeholder:"Poisson"},{key:"crustaces",label:"🦞 Crustacés (甲殻類)",cuts:De,img:"assets/seafood_cuts.png",placeholder:"Crustaces"},{key:"coquillages",label:"🐚 Coquillages (貝類)",cuts:Ie,img:"assets/seafood_cuts.png",placeholder:"Coquillages"},{key:"mollusques",label:"🐙 Mollusques (軟体類)",cuts:Ge,img:"assets/seafood_cuts.png",placeholder:"Mollusques"}]},cheese_wine:{title:"チーズ・ワイン (Cheese & Wine)",subCategories:[{key:"fromages",label:"🧀 Fromages (チーズ)",cuts:Ve,img:"assets/cheese_wine.png",placeholder:"Fromages"},{key:"vins",label:"🍷 Vins (ワイン)",cuts:Oe,img:"assets/cheese_wine.png",placeholder:"Vins"}]},vegetables:{title:"野菜 (Vegetables)",subCategories:[{key:"legumes",label:"🥦 Légumes (野菜全般)",cuts:We,img:"assets/vegetables.png",placeholder:"Legumes"}]},herbs_spices:{title:"ハーブ・スパイス (Herbs & Spices)",subCategories:[{key:"herbes",label:"🌿 Herbes (香草)",cuts:Ke,img:"assets/herbs_spices.png",placeholder:"Herbes"},{key:"epices",label:"🌶️ Épices (香辛料)",cuts:Qe,img:"assets/herbs_spices.png",placeholder:"Epices"}]},fruits_mushrooms:{title:"果物・きのこ (Fruits & Mushrooms)",subCategories:[{key:"fruits",label:"🍎 Fruits (果物)",cuts:Je,img:"assets/fruits_mushrooms.png",placeholder:"Fruits"},{key:"champignons",label:"🍄 Champignons (きのこ)",cuts:Ue,img:"assets/fruits_mushrooms.png",placeholder:"Champignons"}]}};function l(t){const p=i[t];a.innerHTML="",p.subCategories.forEach((u,d)=>{const m=document.createElement("button");m.className=`meat-tab-btn ${d===0?"active":""}`,m.setAttribute("data-sub",u.key),m.innerText=u.label,m.style.fontSize="0.8rem",m.style.padding="0.3rem 0.8rem",m.addEventListener("click",s=>{a.querySelectorAll(".meat-tab-btn").forEach(g=>g.classList.remove("active")),m.classList.add("active"),se(u,n)}),a.appendChild(m)}),p.subCategories.length<=1?a.style.display="none":a.style.display="flex",se(p.subCategories[0],n)}r.querySelectorAll(".meat-tab-btn").forEach(t=>{t.addEventListener("click",p=>{r.querySelectorAll(".meat-tab-btn").forEach(d=>d.classList.remove("active")),t.classList.add("active");const u=t.getAttribute("data-category");l(u)})}),l("primary")}function se(e,o){o.innerHTML="";const r=document.createElement("div");r.style.display="flex",r.style.flexDirection="column",r.style.gap="1.5rem";let a="";e.quote&&(a+=`
  ...
  `,e.appendChild(o);const r=o.querySelector("#map-detail-drawer"),a=o.querySelectorAll(".interactive-area"),n=o.querySelectorAll(".list-item-btn");function i(l,t){a.forEach(m=>m.classList.remove("active")),n.forEach(m=>m.classList.remove("active"));const p=o.querySelector(`.interactive-area[data-id="${l}"]`);p&&p.classList.add("active");const u=o.querySelector(`.list-item-btn[data-id="${l}"]`);u&&u.classList.add("active");const d=te.find(m=>m.id===l);if(d){o.querySelector("#map-region-title").innerText=`${d.name_fr} (${d.name_ja})`,o.querySelector("#map-region-sub").innerText=`${d.name_en} Region`,o.querySelector("#map-desc-fr").innerText=d.desc_fr,o.querySelector("#map-desc-en").innerText=d.desc_en,o.querySelector("#map-desc-ja").innerText=d.desc_ja;const m=o.querySelector("#map-relations-container"),s=o.querySelector("#map-relations-content"),g=ge(l,"region");g?(s.innerHTML=g,m.style.display="block"):m.style.display="none";const v=o.querySelector("#map-audio-title-btn");v.style.display="inline-block",v.onclick=()=>G(d.name_fr);const _=o.querySelector("#map-audio-desc-btn");_.style.display="inline-block",_.onclick=()=>G(d.desc_fr),r.style.display="block",window.innerWidth<=600&&r.scrollIntoView({behavior:"smooth"})}}a.forEach(l=>{l.addEventListener("click",t=>{const p=t.target.getAttribute("data-id");i(p,t.target)})}),n.forEach(l=>{l.addEventListener("click",t=>{const p=t.target.closest(".list-item-btn").getAttribute("data-id");i(p,t.target.closest(".list-item-btn"))})})}function Ze(){const e=document.createElement("div"),o=document.createElement("h2");o.className="section-title",o.innerText="Théorie de l'Art Culinaire",e.appendChild(o);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Classical French culinary theory, stock making, classical sauces, and interactive gastronomy diagrams.",e.appendChild(r);const a=document.createElement("div");a.className="cuisine-tabs",a.innerHTML=`
  ...
  `,document.head.appendChild(e)}function tt(){var l;const e=[],o=((l=b.db)==null?void 0:l.knowledge)||[],r=o.filter(t=>t.french&&t.japanese),a=o.filter(t=>t.grammar),n=o.filter(t=>t.cuisine);function i(t,p,u,d){const s=t.filter(v=>v.id!==u).map(d).sort(()=>.5-Math.random()),g=[...new Set(s)].filter(Boolean).slice(0,p);for(;g.length<p;)g.push("Autre option "+(g.length+1));return g}return r.forEach(t=>{var s,g,v,_,c;let p="vocabulary";(s=t.tags)!=null&&s.includes("meat")?p="meat":(g=t.tags)!=null&&g.includes("sauces")||(v=t.tags)!=null&&v.includes("sauce")?p="sauces":(_=t.tags)!=null&&_.includes("cutting")?p="cuts":(c=t.tags)!=null&&c.includes("science")&&(p="science");const u=i(r,3,t.id,w=>w.japanese),d=t.examples&&t.examples[0]?t.examples[0].fr:"";e.push({id:`dyn_vocab_fr_ja_${t.id}`,type:"choice",category:p,question:`Que signifie le mot français "${t.french}" ? / What does the French word "${t.french}" mean?`,question_fr:`Que signifie le mot français "${t.french}" ?`,question_en:`What does the French word "${t.french}" mean?`,context:t.definition_fr||`Usage: ${d}`,options:N([t.japanese,...u]),answer:t.japanese});const m=i(r,3,t.id,w=>w.french);e.push({id:`dyn_vocab_ja_fr_${t.id}`,type:"choice",category:p,question:`Quel est le mot français pour "${t.japanese}" ? / What is the French word for "${t.japanese}"?`,question_fr:`Quel est le mot français pour "${t.japanese}" ?`,question_en:`What is the French word for "${t.japanese}"?`,context:t.definition_fr||`Usage: ${d}`,options:N([t.french,...m]),answer:t.french})}),a.forEach(t=>{if(!t.grammar.topic||!t.examples||t.examples.length===0)return;const p=i(a,3,t.id,u=>u.grammar.topic);e.push({id:`dyn_gram_topic_${t.id}`,type:"choice",category:"grammar",question:`De quel concept de grammaire s'agit-il : "${t.grammar.explanation_ja}" ? / Which grammar concept is this: "${t.grammar.explanation_en}"?`,question_fr:"De quel concept de grammaire s'agit-il ?",question_en:`Which grammar concept is this: "${t.grammar.explanation_en}"?`,context:`Niveau : ${t.level}. Indispensable pour la cuisine.`,options:N([t.grammar.topic,...p]),answer:t.grammar.topic}),t.examples.forEach((u,d)=>{const s=a.flatMap(g=>g.examples||[]).filter(g=>g.fr!==u.fr).sort(()=>.5-Math.random()).map(g=>g.ja).slice(0,3);for(;s.length<3;)s.push("Option de traduction "+(s.length+1));e.push({id:`dyn_gram_ex_${t.id}_${d}`,type:"choice",category:"grammar",question:`Traduisez la phrase : "${u.fr}" / Translate the sentence: "${u.fr}"`,question_fr:`Traduisez la phrase : "${u.fr}"`,question_en:`Translate the sentence: "${u.fr}"`,context:`Grammaire: ${t.grammar.topic} (${t.level})`,options:N([u.ja,...s]),answer:u.ja})})}),n.forEach(t=>{var d,m,s,g,v,_;if(!t.cuisine.topic||!t.cuisine.content_ja)return;const p=i(n,3,t.id,c=>c.cuisine.topic);let u="sauces";(d=t.tags)!=null&&d.includes("knife-cuts")||(m=t.tags)!=null&&m.includes("cuts")?u="cuts":(s=t.tags)!=null&&s.includes("meat")?u="meat":((g=t.tags)!=null&&g.includes("molecular")||(v=t.tags)!=null&&v.includes("chemistry")||(_=t.tags)!=null&&_.includes("science"))&&(u="science"),e.push({id:`dyn_cuis_topic_${t.id}`,type:"choice",category:u,question:`De quel concept culinaire s'agit-il : "${t.cuisine.content_ja.substring(0,120)}..." ? / Which culinary concept is described: "${t.cuisine.content_en.substring(0,120)}..."?`,question_fr:"De quel concept culinaire s'agit-il ?",question_en:"Which culinary concept is described here?",context:`Niveau : ${t.level}. Mots-clés : ${t.tags.join(", ")}`,options:N([t.cuisine.topic,...p]),answer:t.cuisine.topic})}),e}function nt(){var i;const e=[],o=((i=b.db)==null?void 0:i.knowledge)||[],r=[];o.forEach(l=>{(l.examples||[]).forEach(t=>{r.push({fr:t.fr,ja:t.ja})})});const a=r.sort(()=>.5-Math.random());for(const l of a){if(e.length>=4)break;const t=l.fr.replace(/^➔\s*/,"").trim();if(t.includes(" - ")){const p=t.split(" - ");e.push({id:`pair_${e.length}`,left:`🗣️ ${p[0].trim()}`,right:`💬 ${p[1].trim()}`,context:l.ja})}else if(t.includes(",")){const p=t.split(",");e.push({id:`pair_${e.length}`,left:`${p[0].trim()} ,`,right:`... ${p[1].trim()}`,context:l.ja})}else{const p=t.split(" ");if(p.length>=4){const u=Math.floor(p.length/2),d=p.slice(0,u).join(" "),m=p.slice(u).join(" ");e.push({id:`pair_${e.length}`,left:`${d} ...`,right:`... ${m}`,context:l.ja})}}}const n=[{left:"🗣️ Chaud devant !",right:"💬 Oui, chef !",context:"お通りです！ / はい、シェフ！"},{left:"🗣️ Entrée prête ?",right:"💬 Oui, j'envoie.",context:"前菜はできていますか？ / はい、送ります。"},{left:"🗣️ Combien de couverts ?",right:"💬 Vingt couverts.",context:"何名様（何席）ですか？ / 20席です。"},{left:"🗣️ Envoyez la sauce !",right:"💬 Tout de suite !",context:"ソースを出して！ / ただちに！"}];for(;e.length<4;){const l=n[e.length%n.length];e.push({id:`default_${e.length}`,left:l.left,right:l.right,context:l.context})}return e}function at(){var n;const e=document.createElement("div"),o=document.createElement("h2");o.className="section-title",o.innerText="Vérification des Connaissances",e.appendChild(o);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Test your professional French vocabulary, kitchen commands, and classical cuisine theory.",e.appendChild(r);const a=document.createElement("div");return a.className="loading-placeholder",a.innerText="Chargement du quiz... (Loading quiz...)",e.appendChild(a),et(),(n=b.settings)!=null&&n.targetLevel,Promise.all([ne(),R("vocabulary","ALL"),R("grammar","ALL"),R("cuisine","ALL")]).then(()=>{a.remove(),rt(e)}),e}function rt(e){let o="multiple",r="ALL";const a=document.createElement("div");a.className="quiz-mode-selector",a.innerHTML=`
  ...
  `,e.appendChild(n);const i=n.querySelector("#quiz-cat-select");i.addEventListener("change",g=>{r=g.target.value,t()});const l=document.createElement("div");l.className="quiz-game-wrapper",e.appendChild(l),a.querySelectorAll(".mode-tab-btn").forEach(g=>{g.addEventListener("click",v=>{a.querySelectorAll(".mode-tab-btn").forEach(_=>_.classList.remove("active")),v.target.classList.add("active"),o=v.target.getAttribute("data-mode"),o==="matching_taking"?(i.disabled=!0,i.style.opacity="0.5"):(i.disabled=!1,i.style.opacity="1.0"),t()})});function t(){l.innerHTML="",o==="multiple"?p():o==="matching_vocab"?u():o==="matching_taking"?d():o==="spelling"&&s()}function p(){var k;const g=((k=b.db)==null?void 0:k.quizzes)||[],v=tt();let _=[...g,...v];if(r!=="ALL"&&(_=_.filter(x=>x.category===r)),_.length===0){l.innerHTML=`
      ...
      `;return}const c=N(_).slice(0,10);let w=0,S=0,h=!1;function f(){if(l.innerHTML="",h=!1,w>=c.length){const $=Math.round(S/c.length*100);let j="Apprenti (Apprentice)";$>=90?j="Chef de Partie (Station Chef)":$>=70&&(j="Commis de Cuisine (Line Cook)"),l.innerHTML=`
          <div class="quiz-card" style="text-align: center;">
            ...
            <p style="font-size: 1.1rem; margin-bottom: 1.5rem;">Your Score: <strong>${S} / ${c.length}</strong> (${$}%)</p>
        ...
        `,l.querySelector("#restart-choice-btn").addEventListener("click",()=>{p()});return}const x=c[w],L=document.createElement("div");L.className="quiz-card",L.innerHTML=`
        <div class="quiz-meta">
      ...
      `;return}const c=N(_).slice(0,4),w=N(c),S=N(c),h=document.createElement("div");h.className="quiz-card",h.innerHTML=`
    ...
    `,l.appendChild(h),m(h,4,u)}function d(){const g=nt(),v=document.createElement("div");v.className="quiz-card",v.innerHTML=`
      <div class="quiz-meta" style="margin-bottom: 1rem;">
    ...
    `,l.appendChild(v),m(v,4,d)}function m(g,v,_){let c=null,w=null,S=0;const h=g.querySelectorAll(".drag-card"),f=g.querySelectorAll(".drop-zone"),k=g.querySelector("#matching-completion-panel");h.forEach(C=>{C.addEventListener("dragstart",q=>{c=q.target.closest(".drag-card").getAttribute("data-id"),q.target.closest(".drag-card").classList.add("dragging")}),C.addEventListener("dragend",q=>{q.target.closest(".drag-card").classList.remove("dragging")}),C.addEventListener("click",q=>{const y=q.target.closest(".drag-card");y.classList.contains("matched")||(h.forEach(T=>T.style.borderColor="rgba(10,25,49,0.08)"),w=y.getAttribute("data-id"),y.style.borderColor="var(--color-accent)")})}),f.forEach(C=>{C.addEventListener("dragover",q=>{q.preventDefault(),C.classList.contains("matched")||C.classList.add("hovered")}),C.addEventListener("dragleave",()=>{C.classList.remove("hovered")}),C.addEventListener("drop",q=>{q.preventDefault(),C.classList.remove("hovered");const y=C.getAttribute("data-id");c===y?L(c,C):x(c)}),C.addEventListener("click",()=>{if(C.classList.contains("matched")||!w)return;const q=C.getAttribute("data-id");w===q?(L(w,C),w=null):(x(w),w=null,h.forEach(y=>y.style.borderColor="rgba(10,25,49,0.08)"))})});function x(C){const q=g.querySelector(`#drag-${C}`);q&&(q.style.animation="shake-anim 0.4s ease-in-out",setTimeout(()=>q.style.animation="",400))}function L(C,q){const y=g.querySelector(`#drag-${C}`);y.classList.add("matched"),y.style.borderColor="var(--color-success)",q.classList.add("matched"),S++,S===v&&(k.style.display="block")}g.querySelector("#restart-match-btn").addEventListener("click",()=>{_()})}function s(){var C,q;const g=((C=b.settings)==null?void 0:C.includeGeneral)||!1;let _=(((q=b.db)==null?void 0:q.vocabulary)||[]).filter(y=>g||y.is_professional);if(r!=="ALL"&&(r==="meat"?_=_.filter(y=>{var T,$,j,A;return((T=y.tags)==null?void 0:T.includes("meat"))||(($=y.tags)==null?void 0:$.includes("beef"))||((j=y.tags)==null?void 0:j.includes("pork"))||((A=y.tags)==null?void 0:A.includes("poultry"))||/viande|boeuf|porc|poulet/i.test(y.french)}):r==="sauces"?_=_.filter(y=>{var T,$,j;return((T=y.tags)==null?void 0:T.includes("sauce"))||(($=y.tags)==null?void 0:$.includes("sauces"))||((j=y.tags)==null?void 0:j.includes("stocks"))||/sauce|fond|jus|bouillon/i.test(y.french)}):r==="cuts"?_=_.filter(y=>{var T,$;return((T=y.tags)==null?void 0:T.includes("cutting"))||(($=y.tags)==null?void 0:$.includes("vegetables"))||/coupe|tailler|ciseler|mincer|brunoise|julienne/i.test(y.french)}):r==="science"?_=_.filter(y=>{var T;return((T=y.tags)==null?void 0:T.includes("science"))||/réaction|émulsion|liaison/i.test(y.french)}):r==="grammar"&&(_=[])),_.length===0){l.innerHTML=`
        <div class="quiz-card" style="text-align: center; padding: 2rem;">
    ...
    `,l.appendChild(w);const S=w.querySelector("#spelling-input-field"),h=w.querySelector("#spelling-submit-btn"),f=w.querySelector("#spelling-next-btn"),k=w.querySelector("#spelling-feedback-panel"),x=w.querySelector("#spelling-feedback-title"),L=w.querySelector("#spelling-feedback-msg");S.addEventListener("keydown",y=>{y.key==="Enter"&&h.click()}),h.addEventListener("click",()=>{const y=S.value,T=c.french,$=ce(y),j=ce(T),A=$===j;S.disabled=!0,h.style.display="none",f.style.display="block",k.style.display="block",A?(S.style.borderColor="var(--color-success)",S.style.backgroundColor="#E8F5E9",k.style.backgroundColor="#E8F5E9",k.style.color="var(--color-success)",x.innerText="✓ Félicitations ! (Correct)",L.innerText=`You correctly spelled: "${T}"`):(S.style.borderColor="var(--color-error)",S.style.backgroundColor="#FFEBEE",k.style.backgroundColor="#FFEBEE",k.style.color="var(--color-error)",x.innerText="✗ Incorrect",L.innerHTML=`Correct spelling is: <strong>${T}</strong>.<br><em style="font-size:0.85rem;">You typed: "${y}"</em>`,ue(c.id))}),f.addEventListener("click",()=>{s()})}t()}function K(){const e=document.createElement("div"),o=document.createElement("h2");o.className="section-title",o.innerText="Coups de Cœur (Favorites)",e.appendChild(o);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Your bookmarked vocabulary terms, grammar guides, and culinary theories.",e.appendChild(r);const a=Array.from(b.favorites);if(a.length===0)return e.innerHTML+=`
      <div class="alert alert-info">
        <p>No favorites saved yet. Browse Vocabulary, Grammar, or Cuisine and click the star (☆) button to save items here.</p>
      </div>
    `,e;const n=document.createElement("div");return n.className="loading-placeholder",n.innerText="Chargement des favoris... (Loading favorites...)",e.appendChild(n),ae().then(()=>{n.remove(),it(e,a)}),e}function it(e,o){var t,p,u;const r=(((t=b.db)==null?void 0:t.vocabulary)||[]).filter(d=>o.includes(d.id)),a=(((p=b.db)==null?void 0:p.grammar)||[]).filter(d=>o.includes(d.id)),n=(((u=b.db)==null?void 0:u.cuisine)||[]).filter(d=>o.includes(d.id)),i=document.createElement("div");i.className="card-grid",e.appendChild(i);function l(){i.innerHTML="",r.forEach(d=>{const m=document.createElement("div");m.className="card",m.innerHTML=`
        <div>
      ...
      `,m.querySelector(".fav-btn").addEventListener("click",s=>{W(d.id),K(),e.innerHTML="",e.appendChild(K())}),i.appendChild(m)}),a.forEach(d=>{var s,g;const m=document.createElement("div");m.className="card",m.innerHTML=`
        <div>
        ...
        <div class="term-context" style="margin-top: 0.5rem;">
          ...
          <div class="context-ja">${((g=d.examples[0])==null?void 0:g.ja)||""}</div>
        </div>
      ...
      `,m.querySelector(".fav-btn").addEventListener("click",s=>{W(d.id),e.innerHTML="",e.appendChild(K())}),i.appendChild(m)})}l()}function ot(){const e=document.createElement("div"),o=document.createElement("h2");o.className="section-title",o.innerText="Pont d'Études (SRS Review Deck)",e.appendChild(o);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Review and grade yourself on items scheduled for active recall today.",e.appendChild(r);const a=document.createElement("div");return a.className="loading-placeholder",a.innerText="Chargement des révisions... (Loading reviews...)",e.appendChild(a),ae().then(()=>{a.remove(),st(e)}),e}function st(e){var p,u,d,m;const o=new Date().toISOString().split("T")[0],a=[...(((p=b.db)==null?void 0:p.vocabulary)||[]).map(s=>({...s,type:"vocabulary",front:s.french})),...(((u=b.db)==null?void 0:u.grammar)||[]).map(s=>({...s,type:"grammar",front:s.topic})),...(((d=b.db)==null?void 0:d.cuisine)||[]).map(s=>({...s,type:"cuisine",front:s.topic})),...(((m=b.db)==null?void 0:m.quizzes)||[]).map(s=>({...s,type:"quiz",front:s.question_fr||s.question||""}))].filter(s=>{const g=b.srs[s.id];return b.wrongAnswers.includes(s.id)||g&&g.dueDate<=o}),n=a.length;let i=0;const l=document.createElement("div");l.className="srs-review-container",e.appendChild(l);function t(){if(l.innerHTML="",a.length===0||i>=a.length){l.innerHTML=`
    ...
    `;v.innerHTML=c;let w="";s.type==="vocabulary"?w=`
      ...
      `;const S=v.querySelector(".flip-translation-container");S&&S.addEventListener("click",h=>{h.stopPropagation(),S.querySelector(".flip-translation-card").classList.toggle("flipped")}),v.querySelectorAll(".audio-btn").forEach(h=>{h.addEventListener("click",f=>{f.stopPropagation();const k=f.target.closest(".audio-btn").getAttribute("data-text");G(k)})}),v.querySelectorAll(".srs-score-btn").forEach(h=>{h.addEventListener("click",f=>{const k=parseInt(f.target.getAttribute("data-score"));U(s.id,k),k>=4&&g&&vt(s.id),i++,t()})})}),l.appendChild(v)}t()}function ct(){const e=document.createElement("div"),o=document.createElement("h2");o.className="section-title",o.innerText="Recherche Globale",e.appendChild(o);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Search terms across the entire curriculum: Vocabulary, Grammar, and Culinary Theory.",e.appendChild(r);const a=document.createElement("div");a.className="search-box-wrapper",a.style.marginBottom="2rem",a.style.position="relative";const n=document.createElement("input");n.type="text",n.placeholder="Chargement de la base de données... (Loading database...)",n.className="search-input",n.style.width="100%",n.style.padding="1rem 1.5rem",n.style.fontSize="1.1rem",n.style.borderRadius="var(--radius-md)",n.style.border="2px solid rgba(197, 168, 128, 0.2)",n.style.backgroundColor="var(--color-bg)",n.style.fontFamily="var(--font-sans)",n.style.transition="var(--transition)",n.style.outline="none",n.disabled=!0,n.addEventListener("focus",()=>{n.style.borderColor="var(--color-accent)",n.style.boxShadow="0 0 10px rgba(212, 175, 55, 0.15)"}),n.addEventListener("blur",()=>{n.style.borderColor="rgba(197, 168, 128, 0.2)",n.style.boxShadow="none"}),a.appendChild(n),e.appendChild(a);const i=document.createElement("div");return i.className="search-results",e.appendChild(i),i.innerHTML=`
    <div style="text-align: center; padding: 3rem; color: var(--color-text-muted);">
      <p>Préparation de la recherche... (Preparing search database...)</p>
    </div>
  `,ae().then(()=>{n.disabled=!1,n.placeholder="Rechercher... (e.g. sauce, roux, cut, culer, 刻む)";function l(t){var g,v,_;if(i.innerHTML="",!t.trim()){i.innerHTML=`
          <div style="text-align: center; padding: 3rem; color: var(--color-text-muted);">
            <span style="font-size: 3rem;">🔍</span>
            <p style="margin-top: 1rem;">Tapez un mot-clé pour commencer votre recherche.</p>
          </div>
        `;return}const p=t.toLowerCase().trim(),u=((g=b.db)==null?void 0:g.vocabulary)||[],d=((v=b.db)==null?void 0:v.grammar)||[],m=((_=b.db)==null?void 0:_.cuisine)||[],s=[];if(u.forEach(c=>{`${c.french} ${c.english} ${c.japanese} ${c.category} ${c.context_fr} ${c.context_en} ${c.context_ja} ${(c.tags||[]).join(" ")}`.toLowerCase().includes(p)&&s.push({...c,type:"vocabulary",title:c.french,subtitle:`${c.category} • Vocabulary`})}),d.forEach(c=>{`${c.topic} ${c.explanation_en} ${c.explanation_ja} ${c.level} ${(c.tags||[]).join(" ")} ${c.examples.map(S=>`${S.fr} ${S.en} ${S.ja}`).join(" ")}`.toLowerCase().includes(p)&&s.push({...c,type:"grammar",title:c.topic,subtitle:`${c.level} • Grammar Lesson`})}),m.forEach(c=>{`${c.topic} ${c.category} ${c.content_fr} ${c.content_en} ${c.content_ja} ${(c.tags||[]).join(" ")}`.toLowerCase().includes(p)&&s.push({...c,type:"cuisine",title:c.topic,subtitle:`${c.category} • Culinary Theory`})}),s.length===0){i.innerHTML=`
          <div style="text-align: center; padding: 3rem; color: var(--color-text-muted);">
        ...
        `;return}s.forEach(c=>{const w=document.createElement("div");w.className="card search-result-card",w.style.marginBottom="1.2rem",w.style.borderLeft=`4px solid ${c.type==="vocabulary"?"var(--color-primary)":c.type==="grammar"?"var(--color-secondary)":"var(--color-accent)"}`;const S=V(c.id);let h="";c.type==="vocabulary"?h=`
          ...
          `);const f=(c.tags||[]).map(k=>`<span class="tag-badge" style="background-color: rgba(197, 168, 128, 0.12); color: var(--color-accent); font-size: 0.75rem; padding: 0.2rem 0.6rem; border-radius: 20px; font-weight: 500;">#${k}</span>`).join(" ");w.innerHTML=`
          <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem;">
            ...
            <button class="fav-btn ${S?"active":""}" data-id="${c.id}">
              ${S?"★":"☆"}
            </button>
          </div>
          ${h}
          ${c.tags&&c.tags.length>0?`<div style="margin-top: 1rem; display: flex; flex-wrap: wrap; gap: 0.4rem;">${f}</div>`:""}
        `,w.querySelector(".fav-btn").addEventListener("click",k=>{k.stopPropagation(),W(c.id);const x=k.target,L=V(c.id);x.classList.toggle("active",L),x.innerText=L?"★":"☆"}),i.appendChild(w)})}n.addEventListener("input",t=>{l(t.target.value)}),l("")}),e}function lt(){const e=document.createElement("div"),o=document.createElement("h2");o.className="section-title",o.innerText="Configuration de l'Académie",e.appendChild(o);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Customize your learning goals, target CEFR levels, and database options.",e.appendChild(r);const a=document.createElement("div");a.className="card",a.style.padding="2rem";const n=b.settings;a.innerHTML=`
  ...
  `;const i=a.querySelector("#target-level-select"),l=a.querySelector("#new-cards-goal"),t=a.querySelector("#max-reviews-goal"),p=a.querySelector("#include-general-checkbox"),u=a.querySelector("#settings-status");function d(){const m=i.value,s=parseInt(l.value)||5,g=parseInt(t.value)||20,v=p.checked;xt({targetLevel:m,newCardsPerDay:s,maxReviewsPerDay:g,includeGeneral:v}),u.style.display="block",setTimeout(()=>{u.style.display="none"},3e3)}return i.addEventListener("change",d),l.addEventListener("input",d),t.addEventListener("input",d),p.addEventListener("change",d),a.querySelector("#reset-srs-btn").addEventListener("click",()=>{confirm("Voulez-vous vraiment réinitialiser toutes vos données de progression SRS ? Cette action est irréversible.")&&(localStorage.removeItem("cba_srs"),b.srs={},alert("Spaced Repetition System progress has been reset."),window.location.reload())}),a.querySelector("#clear-favs-btn").addEventListener("click",()=>{confirm("Voulez-vous vraiment supprimer tous vos favoris ?")&&(localStorage.removeItem("cba_favorites"),b.favorites=new Set,alert("All favorites have been cleared."),window.location.reload())}),a.querySelector("#share-app-btn").addEventListener("click",()=>{const m={title:"Académie de la Brigade",text:"フランス料理・厨房フランス語の統合学習PWAアプリ「Académie de la Brigade」で一緒に料理と語学を学びましょう！",url:window.location.origin+window.location.pathname};navigator.share?navigator.share(m).then(()=>console.log("Shared successfully")).catch(s=>console.log("Error sharing:",s)):navigator.clipboard.writeText(m.url).then(()=>{alert("App link copied to clipboard! Share it with your friends.")}).catch(s=>{console.error("Failed to copy link:",s)})}),e.appendChild(a),e}function dt(){var i;const e=document.createElement("div"),o=document.createElement("h2");o.className="section-title",o.innerText="Dictée de Cuisine (Culinary Dictations)",e.appendChild(o);const r=document.createElement("p");r.className="section-subtitle",r.innerText="Listen to French kitchen instruction sentences, type what you hear, and master French spelling.",e.appendChild(r);const a=document.createElement("div");a.className="loading-placeholder",a.innerText="Chargement de la dictée... (Loading dictation...)",e.appendChild(a);const n=((i=b.settings)==null?void 0:i.targetLevel)||"ALL";return R("vocabulary",n).then(()=>{a.remove(),ut(e)}),e}function ut(e){var c,w;const o=((c=b.settings)==null?void 0:c.includeGeneral)||!1,n=(((w=b.db)==null?void 0:w.vocabulary)||[]).filter(S=>o||S.is_professional).filter(S=>S.context_fr);if(n.length===0){e.innerHTML+='<p style="color: var(--color-text-muted);">Aucun exercice disponible.</p>';return}const i=[...n].sort(()=>.5-Math.random()).slice(0,5);let l=0,t=!1,p=!1,u=!1,d=0,m=1;const s=document.createElement("div");s.className="dictation-container",s.style.maxWidth="600px",s.style.margin="1.5rem auto",e.appendChild(s);function g(S){return S.toLowerCase().trim().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()'"?]/g,"").replace(/\s+/g," ")}function v(S){return S.normalize("NFD").replace(/[\u0300-\u036f]/g,"")}function _(){if(s.innerHTML="",t=!1,ee(),p=!1,u=!1,d=0,l>=i.length){const E=document.createElement("div");E.className="card",E.style.padding="2.5rem",E.style.textAlign="center",E.innerHTML=`
        <h3 style="font-family: var(--font-serif); font-size: 2rem; color: var(--color-primary); margin-bottom: 1rem;">Session Terminée !</h3>
      ...
      `,E.querySelector("#restart-dictation-btn").addEventListener("click",()=>{l=0,i.length=0,i.push(...[...n].sort(()=>.5-Math.random()).slice(0,5)),_()}),s.appendChild(E);return}const S=i[l],h=S.context_fr,f=document.createElement("div");f.className="card",f.style.padding="2rem",f.innerHTML=`
    ...
    `;const k=f.querySelector("#play-pause-btn"),x=f.querySelector("#stop-btn"),L=f.querySelector("#speed-normal-btn"),C=f.querySelector("#speed-slow-btn"),q=f.querySelector("#dictation-seekbar"),y=f.querySelector("#dictation-input"),T=f.querySelector("#check-btn"),$=f.querySelector("#next-dictation-btn"),j=f.querySelector("#dictation-feedback");function A(E,z){if(!t){const M=E/z*100;q.value=Math.round(M),d=E}}function F(){p=!1,u=!1,d=0,k.innerText="▶ Play",q.value=0}function B(){pe(h,m,A,F,d)}k.addEventListener("click",()=>{t||(p?u?(u=!1,k.innerText="⏸ Pause",be()):(u=!0,k.innerText="▶ Play",he()):(p=!0,u=!1,k.innerText="⏸ Pause",B()))}),x.addEventListener("click",()=>{ee(),F()}),q.addEventListener("change",E=>{if(t)return;const z=parseInt(E.target.value),M=h.replace(/["'➔]/g,"").trim();d=Math.floor(z/100*M.length);const P=M.indexOf(" ",d);P!==-1&&P-d<5&&(d=P+1),p=!0,u=!1,k.innerText="⏸ Pause",B()}),L.addEventListener("click",()=>{m!==1&&(m=1,L.classList.add("active"),L.style.backgroundColor="var(--color-primary)",L.style.color="#FFFFFF",C.classList.remove("active"),C.style.backgroundColor="transparent",C.style.color="var(--color-accent)",p&&!u&&B())}),C.addEventListener("click",()=>{m!==.75&&(m=.75,C.classList.add("active"),C.style.backgroundColor="var(--color-accent)",C.style.color="#FFFFFF",L.classList.remove("active"),L.style.backgroundColor="transparent",L.style.color="var(--color-accent)",p&&!u&&B())}),T.addEventListener("click",()=>{if(t)return;ee(),F();const E=y.value,z=g(E),M=g(h),P=v(z),I=v(M);let D=!1,X=!1;z===M?D=!0:P===I&&(X=!0),j.style.display="block",y.disabled=!0,T.style.display="none",$.style.display="block",t=!0,D?(j.style.backgroundColor="#E8F5E9",j.style.borderLeft="4px solid var(--color-success)",j.style.color="var(--color-success)",j.innerHTML=`
        ...
        `):X?(j.style.backgroundColor="#FFF3E0",j.style.borderLeft="4px solid var(--color-accent)",j.style.color="#E65100",j.innerHTML=`
          ...
          <div style="margin: 0.8rem 0; padding: 0.6rem; background: rgba(0,0,0,0.02); font-size: 0.9rem; border-radius: var(--radius-sm);">
            <div><strong>Expected:</strong> <span style="font-family: monospace; font-weight: 600; color: var(--color-success);">${h}</span></div>
        ...
        `)}),$.addEventListener("click",()=>{l++,_()}),s.appendChild(f),y.focus()}_()}const mt={proprietress:1.5};let H=null;function pt(){H||(H=new(window.AudioContext||window.webkitAudioContext))}function Q(e,o,r,a=.06){try{pt(),(!H||H.state==="suspended")&&H.resume();const n=H.createOscillator(),i=H.createGain();n.type=o||"sine",n.frequency.setValueAtTime(e,H.currentTime),i.gain.setValueAtTime(a,H.currentTime),i.gain.exponentialRampToValueAtTime(.001,H.currentTime+r),n.connect(i),i.connect(H.destination),n.start(),n.stop(H.currentTime+r)}catch{}}function gt(){Q(780,"sine",.03,.04)}function le(){Q(523.25,"sine",.08,.08),setTimeout(()=>Q(659.25,"sine",.15,.08),80)}function de(){Q(180,"triangle",.3,.12)}function ft(){Q(110,"sawtooth",.2,.1)}function ht(){const e=document.createElement("div");return e.className="story-mode-container",fe(e),e}async function fe(e){e.innerHTML=`
  ...
  `,e.querySelectorAll(".play-episode-btn").forEach(o=>{o.addEventListener("click",r=>{const a=r.target.getAttribute("data-chapter"),n=r.target.getAttribute("data-episode");bt(e,a,n)})})}async function bt(e,o,r){try{e.innerHTML='<div class="story-loader"><div class="spinner"></div><p>物語を読み込んでいます...</p></div>';const a=await fetch(`data/story/chapter_${o}.json`);if(!a.ok)throw new Error("Story file could not be loaded");const i=(await a.json()).episodes.find(l=>l.episodeId===r);if(!i)throw new Error("Episode data not found in chapter file");yt(e,i)}catch(a){e.innerHTML=`
    ...
    `}}function yt(e,o){let r=0;const a=o.sequence;e.innerHTML=`
  ...
  `;const n=e.querySelector("#rpg-viewport"),i=e.querySelector("#rpg-dialog-pane"),l=e.querySelector("#rpg-battle-pane"),t=e.querySelector("#rpg-reward-pane"),p=e.querySelector("#rpg-tutorial-overlay"),u=e.querySelector("#rpg-sidebar");let d=null,m=!1,s="";function g(){if(m){clearInterval(d);const f=e.querySelector("#dialog-text");f.innerText=s,m=!1;return}if(r>=a.length){fe(e);return}const h=a[r];h.type==="tutorial"?_(h):h.type==="dialog"?c(h):h.type==="fixedBattle"||h.type==="randomBattle"?w(h):h.type==="reward"&&S(h),v(h)}function v(h){let f=n.querySelector(".ai-policy-notice");h&&h.background==="bg_thief_caught.png"||n.style.background&&n.style.background.includes("bg_thief_caught.png")?f||(f=document.createElement("div"),f.className="ai-policy-notice",f.innerText="※AIポリシーの都合により、カミーユはうさぎの縫いぐるみに差し替えられました",n.appendChild(f)):f&&f.remove()}n.addEventListener("click",h=>{h.target.closest("button")||h.target.closest(".battle-question-box")||i.style.display==="none"||g()}),g();function _(h){i.style.display="none",l.style.display="none",t.style.display="none",u.style.opacity="0",u.style.pointerEvents="none";const f=e.querySelector("#rpg-character-layer");f&&(f.innerHTML=""),p.innerHTML=`
      <div class="tutorial-card">
        <h3>📖 ${h.title}</h3>
        <p style="white-space: pre-line; line-height: 1.6; text-align: justify; margin: 1.2rem 0; font-size: 0.9rem;">${h.text}</p>
        <button class="action-btn start-tut-btn" style="width: 100%; padding: 0.8rem; font-weight: 700;">冒険を開始する</button>
      </div>
    `,p.style.display="flex",p.querySelector(".start-tut-btn").addEventListener("click",()=>{p.style.display="none",r++,g()})}function c(h){p.style.display="none",l.style.display="none",t.style.display="none",i.style.display="block";const f=o.backgrounds[h.background]||"#000000";n.style.background=f,h.shake&&(n.classList.add("shake-vfx"),setTimeout(()=>n.classList.remove("shake-vfx"),400)),f.includes("url(")?(n.style.backgroundSize="cover",n.style.backgroundPosition="center",n.style.backgroundRepeat="no-repeat"):(n.style.backgroundSize="",n.style.backgroundPosition="",n.style.backgroundRepeat="");const k=e.querySelector("#rpg-character-layer");if(k){k.innerHTML="";let $=[];h.characters&&Array.isArray(h.characters)?$=h.characters:h.character&&($=[{id:h.character,expression:h.expression||"default",position:h.position||"center"}]),$.forEach(j=>{const A=o.characters[j.id],F=j.expression||"default";let B=j.position||"center";$.length===1&&j.id==="proprietress"&&(B="center");let E=null;if(A&&A.images)if(A.images[F])E=A.images[F];else if(F==="default"&&A.images.normal)E=A.images.normal;else{const z=Object.keys(A.images);z.length>0&&(E=A.images[z[0]])}if(E){const z=document.createElement("img");z.src=E,z.className=`rpg-character-sprite pos-${B} sprite-${j.id}`;const M=mt[j.id]||1;z.style.setProperty("--char-scale",M),k.appendChild(z),setTimeout(()=>{z.classList.add("active")},50)}})}const x=h.character?o.characters[h.character]:null,L=e.querySelector("#dialog-avatar"),C=e.querySelector("#dialog-name"),q=e.querySelector("#dialog-text");L.style.display="none",C.style.display="none",h.learningPoint?(e.querySelector("#lp-title").innerText=h.learningPoint.title,e.querySelector("#lp-content").innerText=h.learningPoint.text,u.style.opacity="1",u.style.pointerEvents="auto"):(u.style.opacity="0",u.style.pointerEvents="none");const y=x?`${x.name}：${h.text}`:h.text;q.innerText="",s=y,m=!0;let T=0;clearInterval(d),d=setInterval(()=>{T<y.length?(q.innerText+=y[T],T++,T%2===0&&gt()):(clearInterval(d),m=!1)},30),r++}async function w(h){var A,F;i.style.display="none",p.style.display="none",t.style.display="none",u.style.opacity="0",u.style.pointerEvents="none",l.style.display="flex";const f=e.querySelector("#rpg-character-layer");f&&(f.innerHTML=""),l.innerHTML=`
      <div style="margin: auto; text-align: center;">
        <div class="spinner" style="margin: 0 auto 1rem;"></div>
        <p>試練を読み込んでいます...</p>
      </div>
    `;let k=[];if(h.type==="randomBattle")try{await ne();const B=((A=h.conditions)==null?void 0:A.level)||"ALL";B!=="ALL"&&await R("vocabulary",B);let E=[...b.db.quizzes];const z=(F=h.conditions)==null?void 0:F.category;z&&(E=E.filter(P=>P.category&&P.category.toLowerCase()===z.toLowerCase())),E=T(E);const M=h.enemyHp||3;if(k=E.slice(0,M),k.length<M){const P=T(b.db.quizzes.filter(I=>!k.some(D=>D.id===I.id)));k=k.concat(P.slice(0,M-k.length))}k=k.map(P=>{const I=P.options.indexOf(P.answer);return{questionId:P.id,text:P.question,options:P.options,answerIndex:I!==-1?I:0,explanation:P.context||`正解は「${P.answer}」です。`}})}catch(B){console.error("Failed to generate random questions:",B),k=[{questionId:"fb_err_1",text:"プロヴァンス地方の代表的なスープ料理は？",options:["Bouillabaisse","Bœuf bourguignon","Cassoulet","Choucroute"],answerIndex:0,explanation:"ブイヤベースはプロヴァンス地方（マルセイユ）の名物です。"}]}else k=h.questions;let x=h.enemyHp||k.length;const L=x;let C=10;const q=10;let y=0;function T(B){const E=[...B];for(let z=E.length-1;z>0;z--){const M=Math.floor(Math.random()*(z+1));[E[z],E[M]]=[E[M],E[z]]}return E}function $(){if(x<=0){le(),l.innerHTML=`
        ...
        `,l.querySelector(".proceed-battle-btn").addEventListener("click",()=>{r++,g()});return}if(C<=0){de(),l.innerHTML=`
        ...
        `,l.querySelector(".retry-battle-btn").addEventListener("click",()=>{w(h)}),l.querySelector(".exit-battle-btn").addEventListener("click",()=>{var M;const z=(M=h.conditions)==null?void 0:M.category;Y(z==="grammar"?"grammar":"vocabulary")});return}if(y>=k.length){x>0&&(x=0,$());return}const B=k[y];l.innerHTML=`
        <div class="battle-hud">
          <!-- Player HP -->
          <div class="hud-bar-container">
            <span class="hud-label">VOUS (あなた)</span>
            <div class="hud-hp-track">
              <div class="hud-hp-fill player-hp" style="width: ${C/q*100}%"></div>
            </div>
            <span class="hud-value">${C} / ${q}</span>
          </div>
          
          <!-- Enemy HP -->
          <div class="hud-bar-container">
            <span class="hud-label">${h.enemyName}</span>
            <div class="hud-hp-track">
              <div class="hud-hp-fill enemy-hp" style="width: ${x/L*100}%"></div>
            </div>
      ...
      `;const E=l.querySelectorAll(".battle-opt-btn");E.forEach(z=>{z.addEventListener("click",M=>{const P=parseInt(M.target.getAttribute("data-idx"));j(P,B,E)})})}function j(B,E,z){z.forEach(J=>J.disabled=!0);const M=l.querySelector("#battle-feedback"),P=l.querySelector("#fb-title"),I=l.querySelector("#fb-desc"),D=l.querySelector("#next-q-btn");if(B===E.answerIndex){le(),x=Math.max(0,x-1),P.innerText="✅ 正解！ (Très bien)",P.className="feedback-title text-success",D.innerText="次の試練へ";const J=l.querySelector(".enemy-hp");J&&J.classList.add("flash-white"),D.onclick=()=>{y++,$()}}else de(),ft(),C=Math.max(0,C-h.enemyDamage),P.innerText="❌ 不正解！",P.className="feedback-title text-error",D.innerText="もう一度挑戦する",n.classList.add("shake-vfx"),setTimeout(()=>n.classList.remove("shake-vfx"),400),D.onclick=()=>{$()};I.innerText=E.explanation,M.style.display="block"}$()}function S(h){i.style.display="none",l.style.display="none",p.style.display="none",u.style.opacity="0",u.style.pointerEvents="none",t.style.display="block";const f=e.querySelector("#rpg-character-layer");f&&(f.innerHTML=""),localStorage.setItem(`cba_story_${o.episodeId}_cleared`,"true"),t.innerHTML=`
      <div class="reward-card">
        ...
        <div style="background: rgba(197, 168, 128, 0.1); border: 1px solid rgba(197, 168, 128, 0.3); border-radius: var(--radius-sm); padding: 1rem; margin: 1.2rem 0; display: flex; align-items: center; justify-content: center; gap: 1rem;">
          ...
          <div style="text-align: left;">
            <div style="font-weight: 700; color: var(--color-primary); font-size: 1rem;">+${h.xp} XP</div>
    ...
    `,t.querySelector(".claim-reward-btn").addEventListener("click",()=>{r++,g()})}}const b={meta:null,db:{knowledge:[],quizzes:[]},loaded:{knowledge:new Set,quizzes:!1},favorites:new Set(JSON.parse(localStorage.getItem("cba_favorites")||"[]")),wrongAnswers:JSON.parse(localStorage.getItem("cba_wrong")||"[]"),streak:parseInt(localStorage.getItem("cba_streak")||"0"),lastStudyDate:localStorage.getItem("cba_last_study")||"",srs:JSON.parse(localStorage.getItem("cba_srs")||"{}"),settings:JSON.parse(localStorage.getItem("cba_settings")||JSON.stringify({targetLevel:"ALL",newCardsPerDay:5,maxReviewsPerDay:20,includeGeneral:!1}))};function W(e){b.favorites.has(e)?b.favorites.delete(e):b.favorites.add(e),localStorage.setItem("cba_favorites",JSON.stringify(Array.from(b.favorites)))}function V(e){return b.favorites.has(e)}function ue(e){b.wrongAnswers.includes(e)||(b.wrongAnswers.push(e),localStorage.setItem("cba_wrong",JSON.stringify(b.wrongAnswers)))}function vt(e){b.wrongAnswers=b.wrongAnswers.filter(o=>o!==e),localStorage.setItem("cba_wrong",JSON.stringify(b.wrongAnswers))}function _t(){const e=new Date().toISOString().split("T")[0];if(b.lastStudyDate!==e){if(b.lastStudyDate){const o=new Date(b.lastStudyDate),r=new Date(e),a=Math.abs(r-o),n=Math.ceil(a/(1e3*60*60*24));n===1?b.streak+=1:n>1&&(b.streak=1)}else b.streak=1;b.lastStudyDate=e,localStorage.setItem("cba_streak",b.streak.toString()),localStorage.setItem("cba_last_study",e)}}function U(e,o){const r=new Date().toISOString().split("T")[0],a=b.srs[e]||{easiness:2.5,interval:0,repetitions:0,dueDate:r};let n=Math.max(0,Math.min(5,o));n>=3?(a.repetitions===0?a.interval=1:a.repetitions===1?a.interval=6:a.interval=Math.round(a.interval*a.easiness),a.repetitions++):(a.repetitions=0,a.interval=1),a.easiness=a.easiness+(.1-(5-n)*(.08+(5-n)*.02)),a.easiness<1.3&&(a.easiness=1.3);const i=new Date;i.setDate(i.getDate()+a.interval),a.dueDate=i.toISOString().split("T")[0],a.lastRated=r,b.srs[e]=a,localStorage.setItem("cba_srs",JSON.stringify(b.srs))}function xt(e){b.settings={...b.settings,...e},localStorage.setItem("cba_settings",JSON.stringify(b.settings))}async function R(e,o){const r=o==="ALL"?["A1","A2","B1","B2","C1","C2"]:[o];for(const a of r)if(!b.loaded.knowledge.has(a))try{const n=await fetch(`data/knowledge_${a}.json`);if(!n.ok){console.warn(`Could not load data/knowledge_${a}.json`);continue}const i=await n.json(),l=new Set(b.db.knowledge.map(t=>t.id));for(const t of i)l.has(t.id)||b.db.knowledge.push(t);b.loaded.knowledge.add(a)}catch(n){console.error(`Failed to load knowledge level ${a}:`,n)}}async function ne(){if(!b.loaded.quizzes)try{const e=await fetch("data/quizzes.json");if(!e.ok)throw new Error("Network response was not ok");b.db.quizzes=await e.json(),b.loaded.quizzes=!0}catch(e){console.error("Failed to load quizzes:",e)}}async function ae(){await Promise.all([R("vocabulary","ALL"),R("grammar","ALL"),R("cuisine","ALL"),ne()])}const me={home:ye,vocabulary:ve,grammar:xe,cuisine:Ze,quiz:at,favorites:K,review:ot,search:ct,settings:lt,dictation:dt,story:ht};function Y(e){const o=document.getElementById("main-content");if(me[e]){_t(),o.innerHTML="";const r=me[e]();r instanceof HTMLElement?o.appendChild(r):o.innerHTML=r,document.querySelectorAll(".nav-link").forEach(a=>{a.getAttribute("data-tab")===e?a.classList.add("active"):a.classList.remove("active")})}}async function kt(){try{const e=await fetch("data/meta.json");b.meta=await e.json(),document.querySelectorAll(".nav-link").forEach(o=>{o.addEventListener("click",r=>{const a=r.target.closest(".nav-link").getAttribute("data-tab");Y(a)})}),Y("home"),Ct()}catch(e){console.error("Failed to load database metadata:",e),document.getElementById("main-content").innerHTML=`
      <div class="alert alert-info" style="border-left-color: var(--color-error); background-color: #FFEBEE; color: var(--color-error)">
        <h3>Error: Métadonnées inaccessibles</h3>
        <p>Could not load curriculum metadata. Please reload or check your local setup.</p>
      </div>
    `}}function Ct(){const e=document.getElementById("youglish-sidebar"),o=document.getElementById("youglish-sidebar-toggle"),r=document.getElementById("youglish-search-input"),a=document.getElementById("youglish-search-btn"),n=document.getElementById("yg-widget-container");if(!e||!o||!r||!a||!n)return;o.addEventListener("click",t=>{t.stopPropagation(),e.classList.toggle("open"),e.classList.contains("open")&&r.focus()}),document.addEventListener("click",t=>{e.classList.contains("open")&&!e.contains(t.target)&&e.classList.remove("open")});let i=null;function l(){const t=r.value.trim();if(t)if(n.innerHTML='<div id="yg-widget" style="width: 100%;"></div>',window.YG)try{i=new YG("yg-widget",{width:230,components:9,accentColor:"#C5A880",autoStart:1,search:t,language:"french"})}catch(p){console.error("Failed to load YouGlish widget:",p),n.innerHTML=`
      ...
      `}a.addEventListener("click",l),r.addEventListener("keypress",t=>{t.key==="Enter"&&l()})}document.addEventListener("DOMContentLoaded",kt);
