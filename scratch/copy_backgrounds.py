...
src_base = r"c:\Users\kotya\.gemini\antigravity-ide\scratch\chef-brigade-academy"
bg_dir = os.path.join(src_base, "assets", "story", "backgrounds")
os.makedirs(bg_dir, exist_ok=True)

# Complete Mapping from relative src to target filename
mapping = {
    # Career Story
    ...
    "assets/story/chapter_2/森.webp": "forest.webp", # Shared
...
copied = 0
for src_rel, dst_fn in mapping.items():
    src_path = os.path.join(src_base, src_rel.replace('/', os.sep))
    dst_path = os.path.join(bg_dir, dst_fn)
...
print(f"Successfully copied {copied} background files to {bg_dir}")
print(f"Total unique files in {bg_dir}: {len(os.listdir(bg_dir))}")
